...
namespace Game.Chat
{
	...
	{
		private static uint[] s_hashtable =
		{
		...
		private MultiMap<uint, LanguageDesc> _langsMap = new();
		private MultiMap<Tuple<uint, byte>, string> _wordsMap = new();
		...
		private void ReplaceUntranslatableCharactersWithSpace(ref string text)
		{
			...
			for (var i = 0; i < text.Length; ++i)
			{
				...
				if (!Extensions.isExtendedLatinCharacter(w) &&
				    !char.IsNumber(w) &&
				    w <= 0xFF &&
				    w != '\\')
					chars[i] = ' ';
