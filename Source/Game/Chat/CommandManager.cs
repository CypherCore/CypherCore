...
namespace Game.Chat
{
	...
	{
		private static SortedDictionary<string, ChatCommandNode> _commands = new();
	...
	public class ChatCommandNode
	{
		...
		{
			if (_methodInfo != null &&
			    (_helpText.IsEmpty() && _helpString == 0))
		...
		public void SendCommandHelp(CommandHandler handler)
		{
			...
			foreach (var (_, command) in _subCommands)
			{
				...
				if (!subCommandHasSubCommand &&
				    !command.IsInvokerVisible(handler))
					continue;
