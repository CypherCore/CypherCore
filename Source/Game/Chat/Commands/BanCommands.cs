    ...
    internal class BanInfoCommands
    {
        ...
        private static bool HandleBanInfoCharacterCommand(CommandHandler handler, string name)
        {
            ...
            do
            {
                ...
                if (result.Read<bool>(2) &&
                    (result.Read<long>(1) == 0L || unbanDate >= GameTime.GetGameTime()))
                    active = true;
        ...
        private static bool HandleBanInfoHelper(uint accountId, string accountName, CommandHandler handler)
        {
            ...
            do
            {
                ...
                if (result.Read<bool>(2) &&
                    (result.Read<ulong>(1) == 0 || unbanDate >= GameTime.GetGameTime()))
                    active = true;
                ...
                do
                {
                    ...
                    if (!banInfo.IsEmpty())
                        do
                        {
                            ...
                            string bannedby = banInfo.Read<string>(2)[..15];
                            string banreason = banInfo.Read<string>(3)[..15];
        ...
        private static bool HandleBanListIPCommand(CommandHandler handler, [OptionalArg] string filter)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    DateTime tmBan = Time.UnixTimeToDateTime(timeBan);
                    string bannedby = result.Read<string>(3)[..15];
                    string banreason = result.Read<string>(4)[..15];
