...
namespace Game.Chat.Commands
{
	...
	internal class TicketCommands
	{
		...
		private static bool HandleUnAssignCommand<T>(CommandHandler handler, uint ticketId) where T : Ticket
		{
			...
			// Check security
			//! If no _session present it means we're issuing this command from the console
		...
		private class TicketResetCommands
		{
			...
			{
				if (Global.SupportMgr.GetOpenTicketCount<BugTicket>() != 0 ||
				    Global.SupportMgr.GetOpenTicketCount<ComplaintTicket>() != 0 ||
				    Global.SupportMgr.GetOpenTicketCount<SuggestionTicket>() != 0)
				{
					handler.SendSysMessage(CypherStrings.CommandTicketpending);
