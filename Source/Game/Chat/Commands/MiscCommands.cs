...
namespace Game.Chat
{
    internal class MiscCommands
    {
        ...
        private static bool HandleAddItemCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
            ...
            var msg = playerTarget.CanStoreNewItem(ItemConst.NullBag, ItemConst.NullSlot, dest, itemId, (uint)count, out noSpaceForCount);
            ...
            var item = playerTarget.StoreNewItem(dest, itemId, true, ItemEnchantmentManager.GenerateItemRandomBonusListId(itemId), null, ItemContext.None, bonusListIDs);
        ...
        private static bool HandleAddItemSetCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;

            var idStr = handler.ExtractKeyFromLink(args, "Hitemset"); // number or [name] Shift-click form |color|Hitemset:itemset_id|h[name]|h|r
            if (string.IsNullOrEmpty(idStr))
                return false;

            // prevent generation all items with itemset field value '0'
            if (!uint.TryParse(idStr, out var itemSetId) || itemSetId == 0)
            {
            ...
            foreach (var template in its)
            {
                if (template.Value.GetItemSet() == itemSetId)
                {
                    ...
                    {
                        var item = playerTarget.StoreNewItem(dest, template.Value.GetId(), true, 0, null, ItemContext.None, bonusListIDs);
        ...
        private static bool HandleAppearCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            ...
            if (target)
            {
                ...
                else if (map.IsDungeon())
                {
                    ...
                    var bind = _player.GetBoundInstance(target.GetMapId(), target.GetDifficultyID(map.GetEntry()));
                    if (bind == null)
                    {
                        var group = _player.GetGroup();
                        // if no bind exists, create a solo bind
                        var gBind = group ? group.GetBoundInstance(target) : null;                // if no bind exists, create a solo bind
                        if (gBind == null)
                        {
                            var save = Global.InstanceSaveMgr.GetInstanceSave(target.GetInstanceId());
                            if (save != null)
        ...
        private static bool HandleBankCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleBindSightCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleCombatStopCommand(StringArguments args, CommandHandler handler)
        {
            Player target = null;
        ...
        private static bool HandleComeToMeCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleCommandsCommand(StringArguments args, CommandHandler handler)
        {
            var list = "";
        ...
        private static bool HandleDamageCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
            ...
            if (!int.TryParse(str, out var damage_int))
                return false;

            if (damage_int <= 0)
                return true;

            var damage_ = (uint)damage_int;
            ...
            {
                var dmgInfo = new DamageInfo(attacker, target, damage_, null, schoolmask, DamageEffectType.SpellDirect, WeaponAttackType.BaseAttack);
            ...
            var spellInfo = Global.SpellMgr.GetSpellInfo(spellid, attacker.GetMap().GetDifficultyID());
            if (spellInfo == null)
                return false;

            var damageInfo = new SpellNonMeleeDamage(attacker, target, spellInfo, new Networking.Packets.SpellCastVisual(spellInfo.GetSpellXSpellVisualId(attacker), 0), spellInfo.SchoolMask);
        ...
        private static bool HandleDevCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
            {
        ...
        private static bool HandleDieCommand(StringArguments args, CommandHandler handler)
        {
            var target = handler.GetSelectedUnit();
        ...
        private static bool HandleDismountCommand(StringArguments args, CommandHandler handler)
        {
            var player = handler.GetSession().GetPlayer();
        ...
        private static bool HandleGetDistanceCommand(StringArguments args, CommandHandler handler)
        {
            WorldObject obj;
        ...
        private static bool HandleFreezeCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleGPSCommand(StringArguments args, CommandHandler handler)
        {
            WorldObject obj;
            if (!args.Empty())
            {
            ...
            var zoneX = obj.GetPositionX();
            var zoneY = obj.GetPositionY();

            Global.DB2Mgr.Map2ZoneCoordinates((int)zoneId, ref zoneX, ref zoneY);

            var map = obj.GetMap();
            var groundZ = map.GetHeight(obj.GetPhaseShift(), obj.GetPositionX(), obj.GetPositionY(), MapConst.MaxHeight);
            var floorZ = map.GetHeight(obj.GetPhaseShift(), obj.GetPositionX(), obj.GetPositionY(), obj.GetPositionZ());

            var gridCoord = GridDefines.ComputeGridCoord(obj.GetPositionX(), obj.GetPositionY());

            // 63? WHY?
            var gridX = (MapConst.MaxGrids - 1) - gridCoord.X_coord;
            var gridY = (MapConst.MaxGrids - 1) - gridCoord.Y_coord;

            var haveMap = Map.ExistMap(mapId, gridX, gridY);
            var haveVMap = Map.ExistVMap(mapId, gridX, gridY);
            var haveMMap = (Global.DisableMgr.IsPathfindingEnabled(mapId) && Global.MMapMgr.GetNavMesh(handler.GetSession().GetPlayer().GetMapId()) != null);
            ...
            var transport = obj.GetTransport();
            if (transport)
            {
            ...
            var status = map.GetLiquidStatus(obj.GetPhaseShift(), obj.GetPositionX(), obj.GetPositionY(), obj.GetPositionZ(), MapConst.MapAllLiquidTypes, out liquidStatus);
        ...
        private static bool HandleGUIDCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleHelpCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleHideAreaCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
            ...
            var area = CliDB.AreaTableStorage.LookupByKey(args.NextUInt32());
            if (area == null)
            {
                handler.SendSysMessage(CypherStrings.BadValue);
                return false;
            ...
            var offset = (uint)area.AreaBit / 64;
        ...
        private static bool HandleItemMoveCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;

            var srcSlot = args.NextByte();
            var dstSlot = args.NextByte();
        ...
        private static bool HandleKickPlayerCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            string playerName;
        ...
        private static bool HandleLinkGraveCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
        ...
        private static bool HandleListFreezeCommand(StringArguments args, CommandHandler handler)
        {
            // Get names from DB
        ...
        private static bool HandleMailBoxCommand(StringArguments args, CommandHandler handler)
        {
            var player = handler.GetSession().GetPlayer();
        ...
        private static bool HandleMovegensCommand(StringArguments args, CommandHandler handler)
        {
            var unit = handler.GetSelectedUnit();
            if (!unit)
            {
            ...
            var motionMaster = unit.GetMotionMaster();
            float x, y, z;
            motionMaster.GetDestination(out x, out y, out z);

            for (byte i = 0; i < (int)MovementSlot.Max; ++i)
            {
                var movementGenerator = motionMaster.GetMotionSlot(i);
        ...
        private static bool HandleMuteCommand(StringArguments args, CommandHandler handler)
        {
            string nameStr;
            string delayStr;
            ...
            var muteReason = args.NextString();
            var muteReasonStr = "No reason";
            if (muteReason != null)
            ...
            var accountId = target ? target.GetSession().GetAccountId() : Global.CharacterCacheStorage.GetCharacterAccountIdByGuid(targetGuid);
            ...
            if (!uint.TryParse(delayStr, out var notSpeakTime))
                return false;
            ...
            if (target)
            {
                ...
                var muteTime = Time.UnixTime + notSpeakTime * Time.Minute;
            ...
            else
            {
                ...
                var muteTime = -(int)(notSpeakTime * Time.Minute);
                stmt.AddValue(0, muteTime);
            }
            ...
            var nameLink_ = handler.PlayerLink(targetName);
        ...
        private static bool HandleMuteInfoCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
        ...
        private static bool HandleNearGraveCommand(StringArguments args, CommandHandler handler)
        {
            var px2 = args.NextString();
            Team team;
            if (string.IsNullOrEmpty(px2))
            ...
            var player = handler.GetSession().GetPlayer();
            var zone_id = player.GetZoneId();

            var graveyard = Global.ObjectMgr.GetClosestGraveYard(player, team, null);
            if (graveyard != null)
            {
                var graveyardId = graveyard.Id;

                var data = Global.ObjectMgr.FindGraveYardData(graveyardId, zone_id);
                if (data == null)
                {
        ...
        private static bool HandlePInfoCommand(StringArguments args, CommandHandler handler)
        {            
            ...
            uint level;
            var alive = handler.GetCypherString(CypherStrings.Error);
            ulong money;
            uint xp = 0;
            uint xptotal = 0;
            ...
            uint mapId;
            uint areaId;
            var areaName = handler.GetCypherString(CypherStrings.Unknown);
            var zoneName = handler.GetCypherString(CypherStrings.Unknown);
        ...
        private static bool HandlePlayAllCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
        ...
        private static bool HandlePossessCommand(StringArguments args, CommandHandler handler)
        {
            var unit = handler.GetSelectedUnit();
            if (!unit)
                return false;
        ...
        private static bool HandlePvPstatsCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleRecallCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            if (!handler.ExtractPlayerTarget(args, out target))
                return false;
        ...
        private static bool HandleRepairitemsCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            if (!handler.ExtractPlayerTarget(args, out target))
                return false;
        ...
        private static bool HandleRespawnCommand(StringArguments args, CommandHandler handler)
        {
            var player = handler.GetSession().GetPlayer();
            ...
            var data = new List<RespawnInfo>();
            player.GetMap().GetRespawnInfo(data, SpawnObjectTypeMask.All, 0);
            if (!data.Empty())
            {
                var gridId = GridDefines.ComputeGridCoord(player.GetPositionX(), player.GetPositionY()).GetId();
        ...
        private static bool HandleReviveCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            ObjectGuid targetGuid;
        ...
        private static bool HandleSaveAllCommand(StringArguments args, CommandHandler handler)
        {
        ...
        private static bool HandleSaveCommand(StringArguments args, CommandHandler handler)
        {
            var player = handler.GetSession().GetPlayer();
        ...
        private static bool HandleShowAreaCommand(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
        ...
        private static bool HandleSummonCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            ObjectGuid targetGuid;
            string targetName;
        ...
        private static bool HandleUnbindSightCommand(StringArguments args, CommandHandler handler)
        {
            var player = handler.GetSession().GetPlayer();
        ...
        private static bool HandleUnFreezeCommand(StringArguments args, CommandHandler handler)
        {
            var name = "";
            Player player;
            var targetName = args.NextString(); // Get entered name
        ...
        private static bool HandleUnmuteCommand(StringArguments args, CommandHandler handler)
        {
            Player target;
            ObjectGuid targetGuid;
            string targetName;
        ...
        private static bool HandleUnPossessCommand(StringArguments args, CommandHandler handler)
        {
            var unit = handler.GetSelectedUnit();
            if (!unit)
        ...
        private static bool HandleUnstuckCommand(StringArguments args, CommandHandler handler)
        {
            ...
            if (player.IsInFlight() || player.IsInCombat())
            {
                ...
                if (caster)
                {
                    var castId = ObjectGuid.Create(HighGuid.Cast, SpellCastSource.Normal, player.GetMapId(), SPELL_UNSTUCK_ID, player.GetMap().GenerateLowGuid(HighGuid.Cast));
        ...
        private static bool HandleChangeWeather(StringArguments args, CommandHandler handler)
        {
            if (args.Empty())
                return false;
            ...
            if (!uint.TryParse(args.NextString(), out var type))
                return false; 
            
            //0 to 1, sending -1 is instand good weather
            if (!float.TryParse(args.NextString(), out var grade))
                return false;                       
    ...
    internal class AchievementCommand
    {
        ...
        private static bool Add(StringArguments args, CommandHandler handler)
        {
            ...
            var idStr = handler.ExtractKeyFromLink(args, "Hachievement");
            if (string.IsNullOrEmpty(idStr))
                return false;

            if (!uint.TryParse(idStr, out var achievementId) || achievementId == 0)
                return false;
            ...
            var achievementEntry = CliDB.AchievementStorage.LookupByKey(achievementId);
