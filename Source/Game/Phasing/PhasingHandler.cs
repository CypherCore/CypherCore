...
namespace Game
{
	public class PhasingHandler
	{
		...
		public static void OnConditionChange(WorldObject obj)
		{
			...
			foreach (var pair in phaseShift.Phases.ToList())
				if (pair.Value.AreaConditions != null &&
				    !Global.ConditionMgr.IsObjectMeetToConditions(srcInfo, pair.Value.AreaConditions))
				{
	...
	{
		private HashSet<WorldObject> _visited = new();
