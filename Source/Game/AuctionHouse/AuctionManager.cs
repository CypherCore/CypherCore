...
namespace Game
{
	...
	public class AuctionHouseObject
	{
		...
		private SortedDictionary<AuctionsBucketKey, AuctionsBucketData> _buckets = new(); // ordered for search by itemid only
		private Dictionary<ObjectGuid, CommodityQuote> _commodityQuotes = new();

		private SortedList<uint, AuctionPosting> _itemsByAuctionId = new(); // ordered for replicate
		private MultiMap<ObjectGuid, uint> _playerBidderAuctions = new();
