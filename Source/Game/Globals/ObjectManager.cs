...
namespace Game
{
    public sealed class ObjectManager : Singleton<ObjectManager>
    {
        ...
        public static ExtendedPlayerName ExtractExtendedPlayerName(string name)
        {
            ...
            if (pos != -1)
                return new ExtendedPlayerName(name[..pos], name[(pos + 1)..]);
            else
        ...
        //General
        private readonly Dictionary<uint, StringArray> CypherStringStorage = new();
        ...
        //Quest
        private readonly Dictionary<uint, Quest> _questTemplates = new();
        private readonly List<Quest> _questTemplatesAutoPush = new();
        ...
        private readonly MultiMap<uint, uint> spellScriptsStorage = new();
        ...
        private readonly Dictionary<uint, uint> areaTriggerScriptStorage = new();

        //Maps
        public Dictionary<uint, GameTele> gameTeleStorage = new();
        private readonly Dictionary<(uint mapId, Difficulty difficulty), Dictionary<uint, CellObjectGuids>> mapObjectGuidsStore = new();
        private readonly Dictionary<(uint mapId, Difficulty diffuculty, uint phaseId), Dictionary<uint, CellObjectGuids>> mapPersonalObjectGuidsStore = new();
        private readonly Dictionary<uint, InstanceTemplate> instanceTemplateStorage = new();
        ...
        //Gossip
        private readonly MultiMap<uint, GossipMenus> gossipMenusStorage = new();
        private readonly MultiMap<uint, GossipMenuItems> gossipMenuItemsStorage = new();
        private readonly Dictionary<uint, GossipMenuAddon> _gossipMenuAddonStorage = new();
        private readonly Dictionary<uint, PointOfInterest> pointsOfInterestStorage = new();

        //Creature
        private readonly Dictionary<uint, CreatureTemplate> creatureTemplateStorage = new();
        private readonly Dictionary<uint, CreatureModelInfo> creatureModelStorage = new();
        private readonly Dictionary<uint, CreatureSummonedData> creatureSummonedDataStorage = new();
        private readonly Dictionary<ulong, CreatureData> creatureDataStorage = new();
        private readonly Dictionary<ulong, CreatureAddon> creatureAddonStorage = new();
        private readonly MultiMap<uint, uint> creatureQuestItemStorage = new();
        private readonly Dictionary<uint, CreatureAddon> creatureTemplateAddonStorage = new();
        private readonly Dictionary<ulong, CreatureMovementData> creatureMovementOverrides = new();
        private readonly MultiMap<uint, Tuple<uint, EquipmentInfo>> equipmentInfoStorage = new();
        private readonly Dictionary<ObjectGuid, ObjectGuid> linkedRespawnStorage = new();
        private readonly Dictionary<uint, CreatureBaseStats> creatureBaseStatsStorage = new();
        private readonly Dictionary<uint, VendorItemData> cacheVendorItemStorage = new();
        private readonly Dictionary<uint, Trainer> trainers = new();
        private readonly Dictionary<(uint creatureId, uint gossipMenuId, uint gossipOptionIndex), uint> _creatureDefaultTrainers = new();
        private readonly List<uint>[] difficultyEntries = new List<uint>[SharedConst.MaxCreatureDifficulties];    // already loaded difficulty 1 value in creatures, used in CheckCreatureTemplate
        private readonly List<uint>[] hasDifficultyEntries = new List<uint>[SharedConst.MaxCreatureDifficulties]; // already loaded creatures with difficulty 1 values, used in CheckCreatureTemplate
        private readonly Dictionary<uint, NpcText> npcTextStorage = new();

        //GameObject
        private readonly Dictionary<uint, GameObjectTemplate> _gameObjectTemplateStorage = new();
        private readonly Dictionary<ulong, GameObjectData> gameObjectDataStorage = new();
        ...
        //Item
        private readonly Dictionary<uint, ItemTemplate> ItemTemplateStorage = new();

        //Player
        private readonly Dictionary<Tuple<Race, Class>, PlayerInfo> _playerInfo = new();
        ...
        //Pets
        private readonly Dictionary<uint, PetLevelInfo[]> petInfoStore = new();
