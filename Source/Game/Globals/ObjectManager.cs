...
namespace Game
{
	...
	{
		private static float[] qualityMultipliers = new float[]
		                                            {
			                                            0.92f, 0.92f, 0.92f, 1.11f, 1.32f, 1.61f, 0.0f, 0.0f
		                                            };

		private static float[] armorMultipliers = new float[]
		                                          {
		...
		private static float[] weaponMultipliers = new float[]
		                                           {
		...
		{
			if (data.Length > (int)locale &&
			    !string.IsNullOrEmpty(data[(int)locale]))
			...
			foreach (var wchar in str)
			{
				...
				switch (culture)
				{
					...
					case LanguageType.EastAsia:
						if (wchar >= 0x1100 &&
						    wchar <= 0x11F9) // Hangul Jamo
							return true;

						if (wchar >= 0x3041 &&
						    wchar <= 0x30FF) // Hiragana + Katakana
							return true;

						if (wchar >= 0x3131 &&
						    wchar <= 0x318E) // Hangul Compatibility Jamo
							return true;

						if (wchar >= 0x31F0 &&
						    wchar <= 0x31FF) // Katakana Phonetic Ext.
							return true;

						if (wchar >= 0x3400 &&
						    wchar <= 0x4DB5) // CJK Ideographs Ext. A
							return true;

						if (wchar >= 0x4E00 &&
						    wchar <= 0x9FC3) // Unified CJK Ideographs
							return true;

						if (wchar >= 0xAC00 &&
						    wchar <= 0xD7A3) // Hangul Syllables
							return true;

						if (wchar >= 0xFF01 &&
						    wchar <= 0xFFEE) // Halfwidth forms
							return true;
		...
		public void LoadRaceAndClassExpansionRequirements()
		{
			...
			if (!result.IsEmpty())
			{
				...
				do
				{
					...
					if (achievementId != 0 &&
					    !CliDB.AchievementStorage.ContainsKey(achievementId))
					{
		...
		public string GetCypherString(uint entry, Locale locale = Locale.enUS)
		{
			...
			if (cs.Length > (int)locale &&
			    !string.IsNullOrEmpty(cs[(int)locale]))
		...
		public void LoadGossipMenuItems()
		{
			...
			do
			{
				...
				if (gMenuItem.Language != 0 &&
				    !CliDB.LanguagesStorage.ContainsKey(gMenuItem.Language))
				{
		...
		public WorldSafeLocsEntry GetClosestGraveYard(WorldLocation location, Team team, WorldObject conditionObject)
		{
			...
			// not need to check validity of map object; _mapId _MUST_ be valid here
			if (range.Empty() &&
			...
			foreach (var data in range)
			{
				...
				if (MapId != entry.Loc.GetMapId() &&
				    entry.Loc.GetMapId() != mapEntry.ParentMapID)
				{
					// if find graveyard at different map from where entrance placed (or no entrance _data), use any first
					if (mapEntry == null ||
					...
					float dist2 = (entry.Loc.GetPositionX() - mapEntry.Corpse.X) * (entry.Loc.GetPositionX() - mapEntry.Corpse.X) + (entry.Loc.GetPositionY() - mapEntry.Corpse.Y) * (entry.Loc.GetPositionY() - mapEntry.Corpse.Y);
			...
			do
			{
				...
				switch (tmp.command)
				{
					...
					case ScriptCommands.QuestExplored:
					{
						...
						if (tmp.QuestExplored.Distance != 0 &&
						    tmp.QuestExplored.Distance > SharedConst.DefaultVisibilityDistance)
						{
							Log.outError(LogFilter.Sql,
						...
						if (tmp.QuestExplored.Distance != 0 &&
						    tmp.QuestExplored.Distance < SharedConst.InteractionDistance)
						{
							Log.outError(LogFilter.Sql,
					...
					case ScriptCommands.CastSpell:
					{
						...
						if ((int)tmp.CastSpell.Flags != 4 &&
						    Convert.ToBoolean(tmp.CastSpell.CreatureEntry & ~0x1)) // 1 bit (0, 1)
						{
							Log.outError(LogFilter.Sql,
						...
						else if ((int)tmp.CastSpell.Flags == 4 &&
						         GetCreatureTemplate((uint)tmp.CastSpell.CreatureEntry) == null)
						{
							Log.outError(LogFilter.Sql,
		...
		public void LoadSpellScripts()
		{
			...
			foreach (var script in sSpellScripts)
			{
				...
				if (spellInfo.GetEffect(spellEffIndex).Effect == 0 ||
		...
		public void LoadEventScripts()
		{
			...
			foreach (var script in sEventScripts)
			{
				...
				if (id == 0)
					Log.outError(LogFilter.Sql,
					             "Table `event_scripts` has script (Id: {0}) not referring to any gameobject_template Type 10 data2 field, Type 3 data6 field, Type 13 _data 2 field or any spell effect {1}",
					             script.Key,
		...
		private void LoadCreatureTemplateModels()
		{
			...
			do
			{
				...
				if (modelInfo == null)
					Log.outDebug(LogFilter.Sql, $"No model _data exist for `CreatureDisplayID` = {creatureDisplayId} listed by creature (Entry: {creatureId}).");
		...
		private void LoadCreatureSummonedData()
		{
			...
			if (result.IsEmpty())
			{
				Log.outInfo(LogFilter.ServerLoading, "Loaded 0 creature summoned _data definitions. DB table `creature_summoned_data` is empty.");
			...
			do
			{
				...
				if (!creatureSummonedDataStorage.ContainsKey(creatureId))
					creatureSummonedDataStorage[creatureId] = new CreatureSummonedData();
			...
			Log.outInfo(LogFilter.ServerLoading, $"Loaded {creatureSummonedDataStorage.Count} creature summoned _data definitions in {Time.GetMSTimeDiffToNow(oldMSTime)} ms");
		}
		...
		public void LoadCreatureTemplateAddons()
		{
			...
			do
			{
				...
				if (creatureAddon.AiAnimKit != 0 &&
				    !CliDB.AnimKitStorage.ContainsKey(creatureAddon.AiAnimKit))
				{
				...
				if (creatureAddon.MovementAnimKit != 0 &&
				    !CliDB.AnimKitStorage.ContainsKey(creatureAddon.MovementAnimKit))
				{
				...
				if (creatureAddon.MeleeAnimKit != 0 &&
				    !CliDB.AnimKitStorage.ContainsKey(creatureAddon.MeleeAnimKit))
				{
		...
		public void LoadCreatureAddons()
		{
			...
			do
			{
				...
				if (creData.MovementType == (byte)MovementGeneratorType.Waypoint &&
				    creatureAddon.PathId == 0)
				{
		...
		public void LoadCreatureQuestItems()
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, "Table `creature_questitem` has _data for nonexistent creature (entry: {0}, idx: {1}), skipped", entry, idx);
		...
		public void LoadCreatureMovementOverrides()
		{
			...
			// Load the _data from creature_movement_override and if NULL fallback to creature_template_movement
		...
		public void LoadCreatureClassLevelStats()
		{
			...
			do
			{
				...
				if (_class == 0 ||
				    ((1 << (_class - 1)) & (int)Class.ClassMaskAllCreatures) == 0)
		...
		public void LoadCreatureModelInfo()
		{
			...
			do
			{
				...
				if (modelInfo.DisplayIdOtherGender != 0 &&
				    !CliDB.CreatureDisplayInfoStorage.ContainsKey(modelInfo.DisplayIdOtherGender))
				{
		...
		public void LoadCreatureScalingData()
		{
			...
			Log.outInfo(LogFilter.ServerLoading, $"Loaded {count} creature template scaling _data in {Time.GetMSTimeDiffToNow(oldMSTime)} ms");
		}
		...
		public void CheckCreatureTemplate(CreatureTemplate cInfo)
		{
			...
			for (uint diff = 0; diff < SharedConst.MaxCreatureDifficulties && ok; ++diff)
			{
				...
				{
					Log.outError(LogFilter.Sql,
					             "Creature (Entry: {0}, `UnitFlags2`: {1}) has different `UnitFlags2` in difficulty {2} mode (Entry: {3}, `UnitFlags2`: {4}).",
					             cInfo.Entry,
					             cInfo.UnitFlags2,
					             diff + 1,
					             cInfo.DifficultyEntry[diff],
					...
					Log.outError(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `UnitFlags2`=`UnitFlags2`^{0} WHERE `entry`={1};", cInfo.UnitFlags2 ^ difficultyInfo.UnitFlags2, cInfo.DifficultyEntry[diff]);
				}
			...
			if (cInfo.UnitClass == 0 ||
			    ((1 << ((int)cInfo.UnitClass - 1)) & (int)Class.ClassMaskAllCreatures) == 0)
			{
			...
			if (cInfo.CreatureType != 0 &&
			    !CliDB.CreatureTypeStorage.ContainsKey((uint)cInfo.CreatureType))
			{
			...
			if (cInfo.Family != 0 &&
			    !CliDB.CreatureFamilyStorage.ContainsKey(cInfo.Family))
			{
			...
			for (byte j = 0; j < SharedConst.MaxCreatureSpells; ++j)
				if (cInfo.Spells[j] != 0 &&
				    !Global.SpellMgr.HasSpellInfo(cInfo.Spells[j], Difficulty.None))
				{
			...
			if (cInfo.HealthScalingExpansion < (int)Expansion.LevelCurrent ||
			    cInfo.HealthScalingExpansion >= (int)Expansion.Max)
			{
			...
			{
				Log.outTrace(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags` {disallowedUnitFlags}, removing incorrect flag.");
			...
			{
				Log.outTrace(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags2` {disallowedUnitFlags2}, removing incorrect flag.");
			...
			{
				Log.outTrace(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags2` {disallowedUnitFlags3}, removing incorrect flag.");
			...
			if (levels[0] < 1 ||
			    levels[0] > SharedConst.StrongMaxLevel)
			{
			...
			if (levels[1] < 1 ||
			    levels[1] > SharedConst.StrongMaxLevel)
			{
			...
			if (cInfo.GossipMenuId != 0 &&
			    !cInfo.Npcflag.HasAnyFlag((ulong)NPCFlags.Gossip))
			...
			do
			{
				...
				switch ((CreatureLinkedRespawnType)linkType)
				{
					case CreatureLinkedRespawnType.CreatureToCreature:
					{
						...
						if (slave == null)
						{
							Log.outError(LogFilter.Sql, "Couldn't get creature _data for GUIDLow {0}", guidLow);
							error = true;
						...
						if (master == null)
						{
							Log.outError(LogFilter.Sql, "Couldn't get creature _data for GUIDLow {0}", linkedGuidLow);
							error = true;
					...
					case CreatureLinkedRespawnType.CreatureToGO:
					{
						...
						if (master == null)
						{
							Log.outError(LogFilter.Sql, "Couldn't get gameobject _data for GUIDLow {0}", linkedGuidLow);
							error = true;
					...
					case CreatureLinkedRespawnType.GOToGO:
					{
						...
						if (slave == null)
						{
							Log.outError(LogFilter.Sql, "Couldn't get gameobject _data for GUIDLow {0}", guidLow);
							error = true;
		...
		public void LoadNPCText()
		{
			...
			do
			{
				...
				for (byte i = 0; i < SharedConst.MaxNpcTextOptions; i++)
					if (npcText.Data[i].Probability > 0 &&
					    npcText.Data[i].BroadcastTextID == 0)
					{
		...
		public void LoadTrainers()
		{
			...
			if (!trainerSpellsResult.IsEmpty())
				do
				{
					...
					if (spell.ReqSkillLine != 0 &&
					    !CliDB.SkillLineStorage.ContainsKey(spell.ReqSkillLine))
					{
					...
					for (var i = 0; i < spell.ReqAbility.Count; ++i)
					{
						...
						if (requiredSpell != 0 &&
						    !Global.SpellMgr.HasSpellInfo(requiredSpell, Difficulty.None))
						{
		...
		public void LoadCreatures()
		{
			...
			do
			{
				...
				if (data.PhaseUseFlags.HasAnyFlag(PhaseUseFlagsValues.AlwaysVisible) &&
				    data.PhaseUseFlags.HasAnyFlag(PhaseUseFlagsValues.Inverse))
				{
					Log.outError(LogFilter.Sql,
				...
				if (disallowedUnitFlags != 0)
				{
					Log.outError(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags` {disallowedUnitFlags}, removing incorrect flag.");
					cInfo.UnitFlags &= UnitFlags.Allowed;
				}
				...
				if (disallowedUnitFlags2 != 0)
				{
					Log.outError(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags2` {disallowedUnitFlags2}, removing incorrect flag.");
					cInfo.UnitFlags2 &= (uint)UnitFlags2.Allowed;
				}
				...
				if (disallowedUnitFlags3 != 0)
				{
					Log.outError(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `UnitFlags2` {disallowedUnitFlags3}, removing incorrect flag.");
					cInfo.UnitFlags3 &= (uint)UnitFlags3.Allowed;
				}
			...
			if (!isPersonalPhase)
				foreach (Difficulty difficulty in data.SpawnDifficulties)
				{
					...
					if (!mapObjectGuidsStore.ContainsKey(key))
						mapObjectGuidsStore[key] = new Dictionary<uint, CellObjectGuids>();

					if (!mapObjectGuidsStore[key].ContainsKey(cellId))
						mapObjectGuidsStore[key][cellId] = new CellObjectGuids();
				...
				foreach (Difficulty difficulty in data.SpawnDifficulties)
				{
					...
					if (!mapPersonalObjectGuidsStore.ContainsKey(key))
						mapPersonalObjectGuidsStore[key] = new Dictionary<uint, CellObjectGuids>();

					if (!mapPersonalObjectGuidsStore[key].ContainsKey(cellId))
						mapPersonalObjectGuidsStore[key][cellId] = new CellObjectGuids();
		...
		private void RemoveSpawnDataFromGrid(SpawnData data)
		{
			...
			if (!isPersonalPhase)
				foreach (Difficulty difficulty in data.SpawnDifficulties)
				{
					...
					if (!mapObjectGuidsStore.ContainsKey(key) ||
					    !mapObjectGuidsStore[key].ContainsKey(cellId))
						continue;
				...
				foreach (Difficulty difficulty in data.SpawnDifficulties)
				{
					...
					if (!mapPersonalObjectGuidsStore.ContainsKey(key) ||
					    !mapPersonalObjectGuidsStore[key].ContainsKey(cellId))
						continue;
			...
			do
			{
				...
				if (gostate >= (uint)GameObjectState.Max)
					if (gInfo.type != GameObjectTypes.Transport ||
					    gostate > (int)GameObjectState.TransportActive + SharedConst.MaxTransportStopFrames)
					{
		...
		public void LoadGameObjectQuestItems()
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, "Table `gameobject_questitem` has _data for nonexistent gameobject (entry: {0}, idx: {1}), skipped", entry, idx);
		...
		private void CheckGOLockId(GameObjectTemplate goInfo, uint dataN, uint N)
		{
			...
			Log.outError(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but lock (Id: {4}) not found.", goInfo.entry, goInfo.type, N, goInfo.Door.open, goInfo.Door.open);
		}
		...
		private void CheckGOLinkedTrapId(GameObjectTemplate goInfo, uint dataN, uint N)
		{
			...
			if (trapInfo != null)
				if (trapInfo.type != GameObjectTypes.Trap)
					Log.outError(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but GO (Entry {4}) have not GAMEOBJECT_TYPE_TRAP Type.", goInfo.entry, goInfo.type, N, dataN, dataN);
		}
		...
		private void CheckGOSpellId(GameObjectTemplate goInfo, uint dataN, uint N)
		{
			...
			Log.outError(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but Spell (Entry {4}) not exist.", goInfo.entry, goInfo.type, N, dataN, dataN);
		}
		...
		private void CheckAndFixGOChairHeightId(GameObjectTemplate goInfo, ref uint dataN, uint N)
		{
			...
			Log.outError(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but correct chair height in range 0..{4}.", goInfo.entry, goInfo.type, N, dataN, UnitStandStateType.SitHighChair - UnitStandStateType.SitLowChair);
		...
		private void CheckGONoDamageImmuneId(GameObjectTemplate goTemplate, uint dataN, uint N)
		{
			...
			Log.outError(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but expected boolean (0/1) noDamageImmune field value.", goTemplate.entry, goTemplate.type, N, dataN);
		}
		...
		private void CheckGOConsumable(GameObjectTemplate goInfo, uint dataN, uint N)
		{
			...
			Log.outError(LogFilter.Sql,
			             "Gameobject (Entry: {0} GoType: {1}) have _data{2}={3} but expected boolean (0/1) consumable field value.",
			             goInfo.entry,
			             goInfo.type,
			             N,
			             dataN);
		...
		private List<Difficulty> ParseSpawnDifficulties(string difficultyString, string table, ulong spawnId, uint mapId, List<Difficulty> mapDifficulties)
		{
			...
			foreach (string token in tokens)
			{
				...
				if (difficultyId != 0 &&
				    !CliDB.DifficultyStorage.ContainsKey(difficultyId))
				{
				...
				if (!isTransportMap &&
				    !mapDifficulties.Contains(difficultyId))
				{
		...
		public bool IsVendorItemValid(uint vendorentry, VendorItem vItem, Player player = null, List<uint> skipvendors = null, ulong ORnpcflag = 0)
		{
			...
			if (cInfo == null)
			{
				if (player != null)
					...
					Log.outError(LogFilter.Sql, "Table `(gameevent)npcvendor` have _data for not existed creature template (Entry: {0}), ignore", vendorentry);
			...
			if (!Convert.ToBoolean(((ulong)cInfo.Npcflag | ORnpcflag) & (ulong)NPCFlags.Vendor))
			{
				    ...
				    skipvendors.Count == 0)
				{
					if (player != null)
						...
						Log.outError(LogFilter.Sql, "Table `(gameevent)npcvendor` have _data for not creature template (Entry: {0}) without vendor flag, ignore", vendorentry);
			...
			if (vItem.PlayerConditionId != 0 &&
			    !CliDB.PlayerConditionStorage.ContainsKey(vItem.PlayerConditionId))
			{
			...
			if (vItem.ExtendedCost != 0 &&
			    !CliDB.ItemExtendedCostStorage.ContainsKey(vItem.ExtendedCost))
			{
				if (player != null)
			...
			if (vItem.Type == ItemVendorType.Currency &&
			    vItem.Maxcount == 0)
			{
		...
		public void LoadInstanceEncounters()
		{
			...
			do
			{
				...
				if (lastEncounterDungeon != 0 &&
				    Global.LFGMgr.GetLFGDungeonEntry(lastEncounterDungeon) == 0)
				{
					Log.outError(LogFilter.Sql,
					             "Table `instance_encounters` has an encounter {0} ({1}) marked as final for invalid dungeon Id {2}, skipped!",
					             entry,
					             dungeonEncounter.Name[Global.WorldMgr.GetDefaultDbcLocale()],
				...
				if (lastEncounterDungeon != 0)
				{
					if (pair != null)
					{
						Log.outError(LogFilter.Sql,
						             ...
						             dungeonEncounter.Name[Global.WorldMgr.GetDefaultDbcLocale()],
						             pair.Item1,
						             pair.Item2.Name[Global.WorldMgr.GetDefaultDbcLocale()]);
		...
		public void LoadSpawnGroupTemplates()
		{
			...
			if (!result.IsEmpty())
				do
				{
					...
					if (flags.HasAnyFlag(SpawnGroupFlags.System) &&
					    flags.HasAnyFlag(SpawnGroupFlags.ManualSpawn))
					{
		...
		public void LoadSpawnGroups()
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, $"Spawn _data with invalid Type {spawnType} listed for spawn group {groupId}. Skipped.");
				...
				if (data == null)
				{
					Log.outError(LogFilter.Sql, $"Spawn _data with ID ({spawnType},{spawnId}) not found, but is listed as a member of spawn group {groupId}!");
				...
				else
				{
					...
					else if (groupTemplate.mapId != data.MapId &&
					         !groupTemplate.flags.HasAnyFlag(SpawnGroupFlags.System))
					{
		...
		public void LoadInstanceSpawnGroups()
		{
			...
			do
			{
				...
				if (spawnGroupTemplate == null ||
				    spawnGroupTemplate.flags.HasAnyFlag(SpawnGroupFlags.System))
				{
				...
				if (flags.HasFlag(InstanceSpawnGroupFlags.AllianceOnly) &&
				    flags.HasFlag(InstanceSpawnGroupFlags.HordeOnly))
				{
		...
		private void OnDeleteSpawnData(SpawnData data)
		{
			...
			Cypher.Assert(templateIt != null, $"Creature _data for ({data.type},{data.SpawnId}) is being deleted and has invalid spawn group index {data.spawnGroupData.groupId}!");
			...
			Cypher.Assert(false, $"Spawn _data ({data.type},{data.SpawnId}) being removed is member of spawn group {data.spawnGroupData.groupId}, but not actually listed in the lookup table for that group!");
		}
		...
		{
			return _spawnGroupDataStorage.ElementAt(0).Value;
		}

		public SpawnGroupTemplateData GetLegacySpawnGroup()
		{
			return _spawnGroupDataStorage.ElementAt(1).Value;
		}
		...
		public void LoadPlayerInfo()
		{
			...
			// Load playercreate
			{
				...
				do
				{
					...
					// accept DB _data only for valid position (and non instanceable)
					...
					PlayerInfo info = new();
					info.CreatePositionInfo.Loc = new WorldLocation(mapId, positionX, positionY, positionZ, orientation);
					...
					if (!result.IsNull(7))
					{
						    ...
						    Global.TransportMgr.GetTransportSpawn(info.CreatePositionNPE.Value.TransportGuid.Value) == null)
						{
							...
							info.CreatePositionNPE = null; // remove entire NPE _data - assume user put Transport offsets into npe_position fields
						}
					}
						...
						if (playerInfo != null)
						{
							...
							foreach (ItemTemplate itemTemplate in items)
							{
								...
								if (itemTemplate.GetClass() == ItemClass.Consumable &&
								    (ItemSubClassConsumable)itemTemplate.GetSubClass() == ItemSubClassConsumable.FoodDrink)
								{
				...
				foreach (SkillRaceClassInfoRecord rcInfo in CliDB.SkillRaceClassInfoStorage.Values)
					if (rcInfo.Availability == 1)
						for (Race raceIndex = Race.Human; raceIndex < Race.Max; ++raceIndex)
							if (rcInfo.RaceMask == -1 ||
							    Convert.ToBoolean(SharedConst.GetMaskForRace(raceIndex) & rcInfo.RaceMask))
								for (Class classIndex = Class.Warrior; classIndex < Class.Max; ++classIndex)
									if (rcInfo.ClassMask == -1 ||
									    Convert.ToBoolean((1 << ((int)classIndex - 1)) & rcInfo.ClassMask))
									{
...

			{
				...
				else
				{
					...
					do
					{
						...
						if (raceMask != 0 &&
						    !Convert.ToBoolean(raceMask & SharedConst.RaceMaskAllPlayable))
						{
						...
						if (classMask != 0 &&
						    !Convert.ToBoolean(classMask & (int)Class.ClassMaskAllPlayable))
						{
						...
						for (Race raceIndex = Race.Human; raceIndex < Race.Max; ++raceIndex)
							if (raceMask == 0 ||
							    Convert.ToBoolean((ulong)SharedConst.GetMaskForRace(raceIndex) & raceMask))
								for (Class classIndex = Class.Warrior; classIndex < Class.Max; ++classIndex)
									if (classMask == 0 ||
									    Convert.ToBoolean((1 << ((int)classIndex - 1)) & classMask))
									{
...

			{
				...
				else
				{
					...
					do
					{
						...
						if (raceMask != 0 &&
						    (raceMask & SharedConst.RaceMaskAllPlayable) == 0)
						{
						...
						if (classMask != 0 &&
						    !classMask.HasAnyFlag((uint)Class.ClassMaskAllPlayable))
						{
						...
						if (playerCreateMode < 0 ||
						    playerCreateMode >= (sbyte)PlayerCreateMode.Max)
						{
			...
			time = Time.GetMSTime();
			// Loading levels _data (class/race dependent)
				...
				for (Race race = 0; race < Race.Max; ++race)
				{
					...
					for (Class _class = 0; _class < Class.Max; ++_class)
					{
						...
						if (ConfigMgr.GetDefaultValue("character.EnforceRaceAndClassExpations", true))
						{
							...
							if (WorldConfig.GetIntValue(WorldCfg.Expansion) < (int)Expansion.BurningCrusade &&
							    (race == Race.BloodElf || race == Race.Draenei))
								continue;

							// skip expansion classes if not playing with expansion
							if (WorldConfig.GetIntValue(WorldCfg.Expansion) < (int)Expansion.WrathOfTheLichKing &&
							    _class == Class.Deathknight)
								continue;

							if (WorldConfig.GetIntValue(WorldCfg.Expansion) < (int)Expansion.MistsOfPandaria &&
							    (race == Race.PandarenNeutral || race == Race.PandarenHorde || race == Race.PandarenAlliance))
								continue;

							if (WorldConfig.GetIntValue(WorldCfg.Expansion) < (int)Expansion.Legion &&
							    _class == Class.DemonHunter)
								continue;

							if (WorldConfig.GetIntValue(WorldCfg.Expansion) < (int)Expansion.Dragonflight &&
							    _class == Class.Evoker)
								continue;
						}

						// fatal error if no level 1 _data
						if (playerInfo.LevelInfo == null ||
						    playerInfo.LevelInfo[0].Stats[0] == 0)
						{
							Log.outError(LogFilter.Sql, "Race {0} Class {1} Level 1 does not have Stats _data!", race, _class);
							Global.WorldMgr.StopNow();
						...
						for (var level = 1; level < WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel); ++level)
							...
							{
								Log.outError(LogFilter.Sql, "Race {0} Class {1} Level {2} does not have Stats _data. Using Stats _data of level {3}.", race, _class, level + 1, level);
...

			{
				...
				if (!result.IsEmpty())
				{
					...
					for (var level = 1; level < WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel); ++level)
						...
						{
							Log.outError(LogFilter.Sql, "Level {0} does not have XP for level _data. Using _data of level [{1}] + 12000.", level + 1, level);
		...
		private void PlayerCreateInfoAddItemHelper(uint race, uint class_, uint itemId, int count)
		{
			...
			else
			{
				...
				playerInfo.Item.RemoveAll(item => item.ItemId == itemId);
			}
		}
		...
		public void GetPlayerClassLevelInfo(Class _class, uint level, out uint baseMana)
		{
			...
			if (mp == null)
			{
				Log.outError(LogFilter.Sql, "Tried to get non-existant Class-Level combination _data for base mp. Class {0} Level {1}", _class, level);
		...
		private PlayerLevelInfo BuildPlayerLevelInfo(Race race, Class _class, uint level)
		{
			// base _data (last known level)
		...
		public PetLevelInfo GetPetLevelInfo(uint creatureid, uint level)
		{
			...
			return petinfo[level - 1]; // _data for level 1 stored in [0] array element, ...
		}
		...
		public void LoadQuests()
		{
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, $"Table `quest_reward_choice_items` has _data for quest {questId} but such quest does not exist");
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, $"Table `quest_reward_display_spell` has _data for quest {questId} but such quest does not exist");
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, "Table `quest_details` has _data for quest {0} but such quest does not exist", questId);
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, "Table `quest_request_items` has _data for quest {0} but such quest does not exist", questId);
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, "Table `quest_offer_reward` has _data for quest {0} but such quest does not exist", questId);
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, "Table `quest_template_addon` has _data for quest {0} but such quest does not exist", questId);
				} while (result.NextRow());
			...
			if (result.IsEmpty())
				...
				do
				{
					...
					if (quest != null)
						...
						Log.outError(LogFilter.Sql, "Table `quest_mail_sender` has _data for quest {0} but such quest does not exist", questId);
				} while (result.NextRow());
			...
			foreach (var qinfo in _questTemplates.Values)
			{
				...
				if (qinfo.Flags.HasAnyFlag(QuestFlags.Daily) &&
				    qinfo.Flags.HasAnyFlag(QuestFlags.Weekly))
				{
					...
					for (int j = 1; j < qinfo.RewardChoiceItemId.Length; ++j)
					{
						...
						if (id != 0)
							Log.outError(LogFilter.Sql,
							             ...
							             id,
							             j + 1);
						// no changes, quest ignore this _data
					}

				if (qinfo.ContentTuningId != 0 &&
				    !CliDB.ContentTuningStorage.ContainsKey(qinfo.ContentTuningId))
				...
				if (qinfo.AllowableRaces != -1)
					if (qinfo.AllowableRaces > 0 &&
					    !Convert.ToBoolean(qinfo.AllowableRaces & (long)SharedConst.RaceMaskAllPlayable))
					{
				...
				if (qinfo.RequiredSkillPoints != 0)
					if (qinfo.RequiredSkillPoints > Global.WorldMgr.GetConfigMaxSkillValue())
						Log.outError(LogFilter.Sql,
						             ...
						             Global.WorldMgr.GetConfigMaxSkillValue());
				// no changes, quest can't be done for this requirement
				// else Skill quests can have 0 skill level, this is ok

				if (qinfo.RequiredMinRepFaction != 0 &&
				    !CliDB.FactionStorage.ContainsKey(qinfo.RequiredMinRepFaction))
					Log.outError(LogFilter.Sql,
				...
				// no changes, quest can't be done for this requirement
				if (qinfo.RequiredMaxRepFaction != 0 &&
				    !CliDB.FactionStorage.ContainsKey(qinfo.RequiredMaxRepFaction))
					Log.outError(LogFilter.Sql,
				...
				// no changes, quest can't be done for this requirement
				if (qinfo.RequiredMinRepValue != 0 &&
				    qinfo.RequiredMinRepValue > SharedConst.ReputationCap)
					Log.outError(LogFilter.Sql,
				...
				// warning
				if (qinfo.RewardTitleId != 0 &&
				    !CliDB.CharTitlesStorage.ContainsKey(qinfo.RewardTitleId))
				{
					Log.outError(LogFilter.Sql,
				...
				foreach (QuestObjective obj in qinfo.Objectives)
				{
					...
					// Check storage index for objectives which store _data
					...
					switch (obj.Type)
					{
						...
						case QuestObjectiveType.WinPetBattleAgainstNpc:
							if (obj.ObjectID != 0 &&
							    GetCreatureTemplate((uint)obj.ObjectID) == null)
						...
						case QuestObjectiveType.CriteriaTree:
							...
							break;
						case QuestObjectiveType.AreaTrigger:
							if (!CliDB.AreaTriggerStorage.ContainsKey((uint)obj.ObjectID) &&
							    obj.ObjectID != -1)
						...
						case QuestObjectiveType.AreaTriggerExit:
							if (Global.AreaTriggerDataStorage.GetAreaTriggerTemplate(new AreaTriggerId((uint)obj.ObjectID, false)) == null &&
							    Global.AreaTriggerDataStorage.GetAreaTriggerTemplate(new AreaTriggerId((uint)obj.ObjectID, true)) != null)
		...
		public void LoadGameobjectQuestStarters()
		{
			...
			foreach (var pair in _goQuestRelations)
			{
				...
				if (goInfo == null)
					Log.outError(LogFilter.Sql, "Table `gameobject_queststarter` have _data for not existed gameobject entry ({0}) and existed quest {1}", pair.Key, pair.Value);
				else if (goInfo.type != GameObjectTypes.QuestGiver)
					Log.outError(LogFilter.Sql, "Table `gameobject_queststarter` have _data gameobject entry ({0}) for quest {1}, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", pair.Key, pair.Value);
			}
		}
		...
		public void LoadGameobjectQuestEnders()
		{
			...
			foreach (var pair in _goQuestInvolvedRelations)
			{
				...
				if (goInfo == null)
					Log.outError(LogFilter.Sql, "Table `gameobject_questender` have _data for not existed gameobject entry ({0}) and existed quest {1}", pair.Key, pair.Value);
				else if (goInfo.type != GameObjectTypes.QuestGiver)
					Log.outError(LogFilter.Sql, "Table `gameobject_questender` have _data gameobject entry ({0}) for quest {1}, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", pair.Key, pair.Value);
			}
		}
		...
		public void LoadCreatureQuestStarters()
		{
			...
			foreach (var pair in _creatureQuestRelations)
			{
				...
				if (cInfo == null)
				{
					Log.outDebug(LogFilter.Sql, "Table `creature_queststarter` have _data for not existed creature entry ({0}) and existed quest {1}", pair.Key, pair.Value);
				}
		...
		public void LoadCreatureQuestEnders()
		{
			...
			foreach (var pair in _creatureQuestInvolvedRelations)
			{
				...
				if (cInfo == null)
				{
					Log.outError(LogFilter.Sql, "Table `creature_questender` have _data for not existed creature entry ({0}) and existed quest {1}", pair.Key, pair.Value);
				}
				else if (!Convert.ToBoolean(cInfo.Npcflag & (uint)NPCFlags.QuestGiver))
				{
		...
		public void LoadNPCSpellClickSpells()
		{
			...
			// all spellclick _data loaded, now we check if there are creatures with NPC_FLAG_SPELLCLICK but with no _data
			// NOTE: It *CAN* be the other way around: no spellclick flag but with spellclick _data, in case of creature-only vehicle accessories
			var ctc = GetCreatureTemplates();

			foreach (var creature in ctc.Values)
				if (creature.Npcflag.HasAnyFlag((uint)NPCFlags.SpellClick) &&
				    !_spellClickInfoStorage.ContainsKey(creature.Entry))
				{
					Log.outError(LogFilter.Sql, "npc_spellclick_spells: Creature template {0} has UNIT_NPC_FLAG_SPELLCLICK but no _data in spellclick table! Removing flag", creature.Entry);
		...
		public void LoadReputationOnKill()
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, "Table `creature_onkill_reputation` have _data for not existed creature entry ({0}), skipped", creature_id);
		...
		public void LoadMailLevelRewards()
		{
			...
			do
			{
				...
				if (level > WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel))
				{
					Log.outError(LogFilter.Sql, "Table `mail_level_reward` have _data for level {0} that more supported by client ({1}), ignoring.", level, WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel));
		...
		public void LoadPlayerChoices()
		{
			...
			if (!rewards.IsEmpty())
				do
				{
					...
					if (reward.TitleId != 0 &&
					    !CliDB.CharTitlesStorage.ContainsKey(reward.TitleId))
					{
					...
					if (reward.PackageId != 0 &&
					    Global.DB2Mgr.GetQuestPackageItems((uint)reward.PackageId) == null)
					{
					...
					if (reward.SkillLineId != 0 &&
					    !CliDB.SkillLineStorage.ContainsKey(reward.SkillLineId))
					{
		...
		public void InitializeQueriesData(QueryDataGroup mask)
		{
			...
			{
				Log.outInfo(LogFilter.ServerLoading, "Query _data caching is disabled. Skipped initialization.");

				return;
			}

			// Initialize Query _data for creatures
			...
			// Initialize Quest POI _data
			if (mask.HasAnyFlag(QueryDataGroup.POIs))
				foreach (var poiPair in _questPOIStorage)
					poiPair.Value.InitializeQueryData();

			Log.outInfo(LogFilter.ServerLoading, $"Initialized query cache _data in {Time.GetMSTimeDiffToNow(oldMSTime)} ms");
		}
		...
		public uint GetNearestTaxiNode(float x, float y, float z, uint mapid, Team team)
		{
			...
			foreach (var node in CliDB.TaxiNodesStorage.Values)
			{
				...
				if (node.ContinentID != mapid ||
				    !node.Flags.HasAnyFlag(requireFlag))
					continue;
		...
		public void LoadVehicleTemplateAccessories()
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, "Table `vehicle_template_accessory`: creature template entry {0} has no _data in npc_spellclick_spells", entry);
		...
		//General
		private Dictionary<uint, StringArray> CypherStringStorage = new();
		private Dictionary<uint, RepRewardRate> _repRewardRateStorage = new();
		private Dictionary<uint, ReputationOnKillEntry> _repOnKillStorage = new();
		private Dictionary<uint, RepSpilloverTemplate> _repSpilloverTemplateStorage = new();
		private MultiMap<byte, MailLevelReward> _mailLevelRewardStorage = new();
		private MultiMap<Tuple<uint, SummonerType, byte>, TempSummonData> _tempSummonDataStorage = new();
		private Dictionary<int /*choiceId*/, PlayerChoice> _playerChoices = new();
		private Dictionary<uint, PageText> _pageTextStorage = new();
		private List<string> _reservedNamesStorage = new();
		private Dictionary<uint, SceneTemplate> _sceneTemplateStorage = new();
		private Dictionary<int, JumpChargeParams> _jumpChargeParams = new();
		private Dictionary<uint, string> _phaseNameStorage = new();

		private Dictionary<byte, RaceUnlockRequirement> _raceUnlockRequirementStorage = new();
		private List<RaceClassAvailability> _classExpansionRequirementStorage = new();
		private Dictionary<uint, string> _realmNameStorage = new();

		//Quest
		private Dictionary<uint, Quest> _questTemplates = new();
		private List<Quest> _questTemplatesAutoPush = new();
		private MultiMap<uint, uint> _goQuestRelations = new();
		private MultiMap<uint, uint> _goQuestInvolvedRelations = new();
		private MultiMap<uint, uint> _goQuestInvolvedRelationsReverse = new();
		private MultiMap<uint, uint> _creatureQuestRelations = new();
		private MultiMap<uint, uint> _creatureQuestInvolvedRelations = new();
		private MultiMap<uint, uint> _creatureQuestInvolvedRelationsReverse = new();
		private MultiMap<int, uint> _exclusiveQuestGroups = new();
		private Dictionary<uint, QuestPOIData> _questPOIStorage = new();
		private MultiMap<uint, uint> _questAreaTriggerStorage = new();
		private Dictionary<uint, QuestObjective> _questObjectives = new();
		private Dictionary<uint, QuestGreeting>[] _questGreetingStorage = new Dictionary<uint, QuestGreeting>[2];
		private Dictionary<uint, QuestGreetingLocale>[] _questGreetingLocaleStorage = new Dictionary<uint, QuestGreetingLocale>[2];

		//Scripts
		private ScriptNameContainer _scriptNamesStorage = new();
		private MultiMap<uint, uint> spellScriptsStorage = new();
		...
		private Dictionary<uint, uint> areaTriggerScriptStorage = new();

		//Maps
		public Dictionary<uint, GameTele> gameTeleStorage = new();
		private Dictionary<(uint mapId, Difficulty difficulty), Dictionary<uint, CellObjectGuids>> mapObjectGuidsStore = new();
		private Dictionary<(uint mapId, Difficulty diffuculty, uint phaseId), Dictionary<uint, CellObjectGuids>> mapPersonalObjectGuidsStore = new();
		private Dictionary<uint, InstanceTemplate> instanceTemplateStorage = new();
		public MultiMap<uint, GraveYardData> GraveYardStorage = new();
		private List<ushort> _transportMaps = new();
		private Dictionary<uint, SpawnGroupTemplateData> _spawnGroupDataStorage = new();
		private MultiMap<uint, SpawnMetadata> _spawnGroupMapStorage = new();
		private MultiMap<uint, uint> _spawnGroupsByMap = new();
		private MultiMap<ushort, InstanceSpawnGroupInfo> _instanceSpawnGroupStorage = new();

		//Spells /Skills / Phases
		private Dictionary<uint, PhaseInfoStruct> _phaseInfoById = new();
		private Dictionary<uint, TerrainSwapInfo> _terrainSwapInfoById = new();
		private MultiMap<uint, PhaseAreaInfo> _phaseInfoByArea = new();
		private MultiMap<uint, TerrainSwapInfo> _terrainSwapInfoByMap = new();
		private MultiMap<uint, SpellClickInfo> _spellClickInfoStorage = new();
		private Dictionary<uint, int> _fishingBaseForAreaStorage = new();
		private Dictionary<uint, SkillTiersEntry> _skillTiers = new();

		//Gossip
		private MultiMap<uint, GossipMenus> gossipMenusStorage = new();
		private MultiMap<uint, GossipMenuItems> gossipMenuItemsStorage = new();
		private Dictionary<uint, GossipMenuAddon> _gossipMenuAddonStorage = new();
		private Dictionary<uint, PointOfInterest> pointsOfInterestStorage = new();

		//Creature
		private Dictionary<uint, CreatureTemplate> creatureTemplateStorage = new();
		private Dictionary<uint, CreatureModelInfo> creatureModelStorage = new();
		private Dictionary<uint, CreatureSummonedData> creatureSummonedDataStorage = new();
		private Dictionary<ulong, CreatureData> creatureDataStorage = new();
		private Dictionary<ulong, CreatureAddon> creatureAddonStorage = new();
		private MultiMap<uint, uint> creatureQuestItemStorage = new();
		private Dictionary<uint, CreatureAddon> creatureTemplateAddonStorage = new();
		private Dictionary<ulong, CreatureMovementData> creatureMovementOverrides = new();
		private MultiMap<uint, Tuple<uint, EquipmentInfo>> equipmentInfoStorage = new();
		private Dictionary<ObjectGuid, ObjectGuid> linkedRespawnStorage = new();
		private Dictionary<uint, CreatureBaseStats> creatureBaseStatsStorage = new();
		private Dictionary<uint, VendorItemData> cacheVendorItemStorage = new();
		private Dictionary<uint, Trainer> trainers = new();
		private Dictionary<(uint creatureId, uint gossipMenuId, uint gossipOptionIndex), uint> _creatureDefaultTrainers = new();
		private List<uint>[] difficultyEntries = new List<uint>[SharedConst.MaxCreatureDifficulties];    // already loaded difficulty 1 value in creatures, used in CheckCreatureTemplate
		private List<uint>[] hasDifficultyEntries = new List<uint>[SharedConst.MaxCreatureDifficulties]; // already loaded creatures with difficulty 1 values, used in CheckCreatureTemplate
		private Dictionary<uint, NpcText> npcTextStorage = new();

		//GameObject
		private Dictionary<uint, GameObjectTemplate> _gameObjectTemplateStorage = new();
		private Dictionary<ulong, GameObjectData> gameObjectDataStorage = new();
		private Dictionary<ulong, GameObjectTemplateAddon> _gameObjectTemplateAddonStorage = new();
		private Dictionary<ulong, GameObjectOverride> _gameObjectOverrideStorage = new();
		private Dictionary<ulong, GameObjectAddon> _gameObjectAddonStorage = new();
		private MultiMap<uint, uint> _gameObjectQuestItemStorage = new();
		private List<uint> _gameObjectForQuestStorage = new();

		//Item
		private Dictionary<uint, ItemTemplate> ItemTemplateStorage = new();

		//Player
		private Dictionary<Tuple<Race, Class>, PlayerInfo> _playerInfo = new();
		...
		//Pets
		private Dictionary<uint, PetLevelInfo[]> petInfoStore = new();
		private MultiMap<uint, string> _petHalfName0 = new();
		private MultiMap<uint, string> _petHalfName1 = new();

		//Vehicles
		private Dictionary<uint, VehicleTemplate> _vehicleTemplateStore = new();
		private MultiMap<uint, VehicleAccessory> _vehicleTemplateAccessoryStore = new();
		private MultiMap<ulong, VehicleAccessory> _vehicleAccessoryStore = new();
		private Dictionary<uint, VehicleSeatAddon> _vehicleSeatAddonStore = new();

		//Locales
		private Dictionary<uint, CreatureLocale> _creatureLocaleStorage = new();
		private Dictionary<uint, GameObjectLocale> _gameObjectLocaleStorage = new();
		private Dictionary<uint, QuestTemplateLocale> _questTemplateLocaleStorage = new();
		private Dictionary<uint, QuestObjectivesLocale> _questObjectivesLocaleStorage = new();
		private Dictionary<uint, QuestOfferRewardLocale> _questOfferRewardLocaleStorage = new();
		private Dictionary<uint, QuestRequestItemsLocale> _questRequestItemsLocaleStorage = new();
		private Dictionary<Tuple<uint, uint>, GossipMenuItemsLocale> _gossipMenuItemsLocaleStorage = new();
		private Dictionary<uint, PageTextLocale> _pageTextLocaleStorage = new();
		private Dictionary<uint, PointOfInterestLocale> _pointOfInterestLocaleStorage = new();
		private Dictionary<int, PlayerChoiceLocale> _playerChoiceLocales = new();

		private List<uint> _tavernAreaTriggerStorage = new();
		private Dictionary<uint, AreaTriggerStruct> _areaTriggerStorage = new();
		private Dictionary<ulong, AccessRequirement> _accessRequirementStorage = new();
		private MultiMap<ulong, DungeonEncounter> _dungeonEncounterStorage = new();
		private Dictionary<uint, WorldSafeLocsEntry> _worldSafeLocs = new();
		...
		private Dictionary<uint, uint> _baseXPTable = new();
	...
	{
		private List<Entry> IndexToName = new();
		private Dictionary<string, Entry> NameToIndex = new();
