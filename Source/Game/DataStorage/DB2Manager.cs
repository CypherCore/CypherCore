...
namespace Game.DataStorage
{
	...
	{
		private MultiMap<uint, Tuple<uint, AllowedHotfixOptionalData>> _allowedHotfixOptionalData = new();

		private MultiMap<uint, uint> _areaGroupMembers = new();
		private MultiMap<uint, uint> _artifactPowerLinks = new();
		private Dictionary<Tuple<uint, byte>, ArtifactPowerRankRecord> _artifactPowerRanks = new();
		private MultiMap<uint, ArtifactPowerRecord> _artifactPowers = new();
		private Dictionary<uint, AzeriteEmpoweredItemRecord> _azeriteEmpoweredItems = new();
		private Dictionary<(uint azeriteEssenceId, uint rank), AzeriteEssencePowerRecord> _azeriteEssencePowersByIdAndRank = new();
		...
		private MultiMap<uint, AzeritePowerSetMemberRecord> _azeritePowers = new();
		private Dictionary<(uint azeriteUnlockSetId, ItemContext itemContext), byte[]> _azeriteTierUnlockLevels = new();
		private Dictionary<(uint itemId, ItemContext itemContext), AzeriteUnlockMappingRecord> _azeriteUnlockMappings = new();
		private Dictionary<(int broadcastTextId, CascLocaleBit cascLocaleBit), int> _broadcastTextDurations = new();
		private MultiMap<uint, ChrCustomizationChoiceRecord> _chrCustomizationChoicesByOption = new();
		private Dictionary<Tuple<byte, byte, byte>, ShapeshiftFormModelData> _chrCustomizationChoicesForShapeshifts = new();
		private MultiMap<Tuple<byte, byte>, ChrCustomizationOptionRecord> _chrCustomizationOptionsByRaceAndGender = new();
		private Dictionary<uint, MultiMap<uint, uint>> _chrCustomizationRequiredChoices = new();
		private Dictionary<Tuple<byte, byte>, ChrModelRecord> _chrModelsByRaceAndGender = new();
		private ChrSpecializationRecord[][] _chrSpecializationsByIndex = new ChrSpecializationRecord[(int)Class.Max + 1][];
		private MultiMap<uint, CurrencyContainerRecord> _currencyContainers = new();
		private MultiMap<uint, CurvePointRecord> _curvePoints = new();
		private Dictionary<Tuple<uint, byte, byte, byte>, EmotesTextSoundRecord> _emoteTextSounds = new();
		private MultiMap<uint, ContentTuningXExpectedRecord> _expectedStatModsByContentTuning = new();
		private Dictionary<Tuple<uint, int>, ExpectedStatRecord> _expectedStatsByLevel = new();
		private MultiMap<uint, uint> _factionTeams = new();
		private MultiMap<uint, FriendshipRepReactionRecord> _friendshipRepReactions = new();
		private MultiMap<uint, uint> _glyphBindableSpells = new();
		private MultiMap<uint, uint> _glyphRequiredSpecs = new();
		private Dictionary<uint, HeirloomRecord> _heirlooms = new();
		...
		private MultiMap<int, HotfixRecord> _hotfixData = new();
		...
		private MultiMap<uint, ItemBonusRecord> _itemBonusLists = new();
		private MultiMap<uint, ItemBonusTreeNodeRecord> _itemBonusTrees = new();
		private MultiMap<uint, ItemLimitCategoryConditionRecord> _itemCategoryConditions = new();
		private Dictionary<uint, ItemChildEquipmentRecord> _itemChildEquipment = new();
		private ItemClassRecord[] _itemClassByOldEnum = new ItemClassRecord[20];
		private Dictionary<short, uint> _itemLevelDeltaToBonusListContainer = new();
		private MultiMap<uint, ItemLevelSelectorQualityRecord> _itemLevelQualitySelectorQualities = new();
		private Dictionary<uint, ItemModifiedAppearanceRecord> _itemModifiedAppearancesByItem = new();
		private MultiMap<uint, ItemSetSpellRecord> _itemSetSpells = new();
		private MultiMap<uint, ItemSpecOverrideRecord> _itemSpecOverrides = new();
		private List<uint> _itemsWithCurrencyCost = new();
		private MultiMap<uint, uint> _itemToBonusTree = new();
		private List<JournalTierRecord> _journalTiersByIndex = new();
		private Dictionary<uint, Dictionary<uint, MapDifficultyRecord>> _mapDifficulties = new();
		private MultiMap<uint, Tuple<uint, PlayerConditionRecord>> _mapDifficultyConditions = new();
		private MultiMap<uint, MountTypeXCapabilityRecord> _mountCapabilitiesByType = new();
		private MultiMap<uint, MountXDisplayRecord> _mountDisplays = new();
		private Dictionary<uint, MountRecord> _mountsBySpellId = new();
		private Dictionary<uint, List<NameGenRecord>[]> _nameGenData = new();
		private List<string>[] _nameValidators = new List<string>[(int)Locale.Total + 1];
		private Dictionary<uint, ParagonReputationRecord> _paragonReputations = new();
		private MultiMap<uint, uint> _phasesByGroup = new();
		private uint[][] _powersByClass = new uint[(int)Class.Max][];
		private Dictionary<PowerType, PowerTypeRecord> _powerTypes = new();
		private Dictionary<uint, byte> _pvpItemBonus = new();
		private PvpTalentSlotUnlockRecord[] _pvpTalentSlotUnlock = new PvpTalentSlotUnlockRecord[PlayerConst.MaxPvpTalentSlots];
		private Dictionary<uint, Tuple<List<QuestPackageItemRecord>, List<QuestPackageItemRecord>>> _questPackages = new();
		private MultiMap<uint, QuestLineXQuestRecord> _questsByQuestLine = new();
		private MultiMap<uint, RewardPackXCurrencyTypeRecord> _rewardPackCurrencyTypes = new();
		private MultiMap<uint, RewardPackXItemRecord> _rewardPackItems = new();
		private MultiMap<uint, SkillLineAbilityRecord> _skillLineAbilitiesBySkillupSkill = new();
		private MultiMap<uint, SkillLineRecord> _skillLinesByParentSkillLine = new();
		private MultiMap<uint, SkillRaceClassInfoRecord> _skillRaceClassInfoBySkill = new();
		private Dictionary<Tuple<int, int>, SoulbindConduitRankRecord> _soulbindConduitRanks = new();
		private MultiMap<uint, SpecializationSpellsRecord> _specializationSpellsBySpec = new();
		private List<Tuple<int, uint>> _specsBySpecSet = new();
		private List<byte> _spellFamilyNames = new();
		private MultiMap<uint, SpellProcsPerMinuteModRecord> _spellProcsPerMinuteMods = new();
		private MultiMap<uint, SpellVisualMissileRecord> _spellVisualMissilesBySet = new();

		private Dictionary<uint, IDB2Storage> _storage = new();
		private List<TalentRecord>[][][] _talentsByPosition = new List<TalentRecord>[(int)Class.Max][][];
		private List<uint> _toys = new();
		private Dictionary<uint, TransmogIllusionRecord> _transmogIllusionsByEnchantmentId = new();
		private MultiMap<uint, TransmogSetItemRecord> _transmogSetItemsByTransmogSet = new();
		private MultiMap<uint, TransmogSetRecord> _transmogSetsByItemModifiedAppearance = new();
		private ChrClassUIDisplayRecord[] _uiDisplayByClass = new ChrClassUIDisplayRecord[(int)Class.Max];
		...
		private Dictionary<int, UiMapBounds> _uiMapBounds = new();
		private List<int> _uiMapPhases = new();
		private Dictionary<Tuple<short, sbyte, int>, WMOAreaTableRecord> _wmoAreaTableLookup = new();
