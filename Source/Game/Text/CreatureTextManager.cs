...
namespace Game
{
	...
	{
		private Dictionary<CreatureTextId, CreatureTextLocale> mLocaleTextMap = new();

		private Dictionary<uint, MultiMap<byte, CreatureTextEntry>> mTextMap = new();
		...
		public void LoadCreatureTexts()
		{
			...
			do
			{
				...
				if (temp.lang != Language.Universal &&
				    !Global.LanguageMgr.IsLanguageExist(temp.lang))
				{
	...
	public class CreatureTextLocalizer : IDoWork<Player>
	{
		...
		private Dictionary<Locale, ChatPacketSender> _packetCache = new();
