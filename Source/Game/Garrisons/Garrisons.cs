...
namespace Game.Garrisons
{
	public class Garrison
	{
		...
		private List<uint> _followerIds = new();
		private Dictionary<ulong, Follower> _followers = new();
		private List<uint> _knownBuildings = new();

		private Player _owner;

		private Dictionary<uint, Plot> _plots = new();
		...
		public void AddFollower(uint garrFollowerId)
		{
			...
			if (_followerIds.Contains(garrFollowerId) ||
			    followerEntry == null)
			{
		...
		public void SendRemoteInfo()
		{
			...
			if (garrisonMap == null ||
			    _owner.GetMapId() != garrisonMap.ParentMapID)
				return;
			...
			foreach (var p in _plots)
				if (p.Value.BuildingInfo.PacketInfo != null)
				{
					...
					if (existingBuilding.BuildingType == building.BuildingType)
						if (p.Key != garrPlotInstanceId ||
						    existingBuilding.UpgradeLevel + 1 != building.UpgradeLevel) // check if its an upgrade in same plot
