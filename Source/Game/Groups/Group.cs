...
namespace Game.Groups
{
	public class Group
	{
		...
		// Raid markers
		private RaidMarker[] _markers = new RaidMarker[MapConst.RaidMarkersCount];
		private ObjectGuid _masterLooterGuid;
		private GroupRefManager _memberMgr = new();

		private List<MemberSlot> _memberSlots = new();
		private GroupInstanceRefManager _ownedInstancesMgr = new();
		private Difficulty _raidDifficulty;

		// Ready Check
		private bool _readyCheckStarted;
		private TimeSpan _readyCheckTimer;
		private Dictionary<uint, Tuple<ObjectGuid, uint>> _recentInstances = new();
		private byte[] _subGroupsCounts;
		private ObjectGuid[] _targetIcons = new ObjectGuid[MapConst.TargetIconsCount];
