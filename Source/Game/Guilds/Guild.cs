...
namespace Game.Guilds
{
	public class Guild
	{
		...
		private List<RankInfo> _ranks = new();
		private Dictionary<ObjectGuid, Member> _members = new();
		private List<BankTab> _bankTabs = new();

		// These are actually ordered lists. The first element is the oldest entry.
		private LogHolder<EventLogEntry> _eventLog = new();
		private LogHolder<BankEventLogEntry>[] _bankEventLog = new LogHolder<BankEventLogEntry>[GuildConst.MaxBankTabs + 1];
		private LogHolder<NewsLogEntry> _newsLog = new();
		private GuildAchievementMgr _achievementSys;
		...
		public class Member
		{
			...
			private uint[] _bankWithdraw = new uint[GuildConst.MaxBankTabs];
		...
		{
			private GuildEventLogTypes _eventType;
			private byte _newRank;
			private ulong _playerGuid1;
			private ulong _playerGuid2;
		...
		{
			private byte _bankTabId;
			private byte _destTabId;

			private GuildBankEventLogTypes _eventType;
			private ulong _itemOrMoney;
			private ushort _itemStackCount;
		...
		public class RankInfo
		{
			...
			private GuildBankRightsAndSlots[] _bankTabRightsAndSlots = new GuildBankRightsAndSlots[GuildConst.MaxBankTabs];
		...
		public class BankTab
		{
			...
			private Item[] _items = new Item[GuildConst.MaxBankSlots];
			private string _name;
			private byte _tabId;
