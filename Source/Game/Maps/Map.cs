...
namespace Game.Maps
{
	public class Map : IDisposable
	{
			...
			if (obj.GetExpectedMapId() == GetId())
			{
				...
				foreach (var player in GetPlayers())
					    ...
					    player.InSamePhase(obj))
					{
						...
						player._visibleTransports.Add(obj.GetGUID());
						UpdateObject packet;
		...
		public void RemoveFromMap(Transport obj, bool remove)
		{
			if (obj.IsInWorld)
			{
				...
				foreach (var player in GetPlayers())
					if (player.GetTransport() != obj &&
					    player._visibleTransports.Contains(obj.GetGUID()))
					{
						player.SendPacket(packet);
						player._visibleTransports.Remove(obj.GetGUID());
					}
			}
		...
		public void SendInitSelf(Player player)
		{
			...
			if (transport != null)
			{
				...
				player._visibleTransports.Add(transport.GetGUID());
			}
		...
		private void SendRemoveTransports(Player player)
		{
			...
			foreach (Transport transport in _transports)
				if (player._visibleTransports.Contains(transport.GetGUID()) &&
				    transport != player.GetTransport())
				{
					transport.BuildOutOfRangeUpdateBlock(transData);
					player._visibleTransports.Remove(transport.GetGUID());
				}
		...
		public void SendUpdateTransportVisibility(Player player)
		{
			...
			foreach (var transport in _transports)
			{
				...
				var hasTransport = player._visibleTransports.Contains(transport.GetGUID());
		...
		private List<Creature> creaturesToMove = new();

		private bool _gameObjectsToMoveLock;
		private List<GameObject> _gameObjectsToMove = new();

		private bool _dynamicObjectsToMoveLock;
		private List<DynamicObject> _dynamicObjectsToMove = new();

		private bool _areaTriggersToMoveLock;
		private List<AreaTrigger> _areaTriggersToMove = new();

		private DynamicMapTree _dynamicTree = new();

		private SortedSet<RespawnInfo> _respawnTimes = new(new CompareRespawnInfo());
		private Dictionary<ulong, RespawnInfo> _creatureRespawnTimesBySpawnId = new();
		private Dictionary<ulong, RespawnInfo> _gameObjectRespawnTimesBySpawnId = new();
		private List<uint> _toggledSpawnGroupIds = new();
		private uint _respawnCheckTimer;
		private Dictionary<uint, uint> _zonePlayerCountMap = new();

		private List<Transport> _transports = new();
		private Grid[][] i_grids = new Grid[MapConst.MaxGrids][];
		private MapRecord i_mapRecord;
		private List<WorldObject> i_objectsToRemove = new();
		private Dictionary<WorldObject, bool> i_objectsToSwitch = new();
		private Difficulty i_spawnMode;
		private List<WorldObject> i_worldObjects = new();
		protected List<WorldObject> _activeNonPlayers = new();
		protected List<Player> _activePlayers = new();
		private TerrainInfo _terrain;

		private MultiMap<long, ScriptAction> _scriptSchedule = new();

		private BitSet marked_cells = new(MapConst.TotalCellsPerMap * MapConst.TotalCellsPerMap);
		...
		private Dictionary<uint, ZoneDynamicInfo> _zoneDynamicInfo = new();
		private IntervalTimer _weatherUpdateTimer;
		private Dictionary<HighGuid, ObjectGuidGenerator> _guidGenerators = new();
		private SpawnedPoolData _poolData;
		private Dictionary<ObjectGuid, WorldObject> _objectsStore = new();
		private MultiMap<ulong, Creature> _creatureBySpawnIdStore = new();
		private MultiMap<ulong, GameObject> _gameobjectBySpawnIdStore = new();
		private MultiMap<ulong, AreaTrigger> _areaTriggerBySpawnIdStore = new();
		private MultiMap<uint, Corpse> _corpsesByCell = new();
		private Dictionary<ObjectGuid, Corpse> _corpsesByPlayer = new();
		private List<Corpse> _corpseBones = new();

		private List<WorldObject> _updateObjects = new();

		public delegate void FarSpellCallback(Map map);

		private Queue<FarSpellCallback> _farSpellCallbacks = new();

		private MultiPersonalPhaseTracker _multiPersonalPhaseTracker = new();

		private Dictionary<int, int> _worldStateValues = new();
	...
	public class InstanceMap : Map
	{
		...
		private InstanceLock i_instanceLock;
		private GroupInstanceReference i_owningGroupRef = new();
		...
		public override bool AddPlayerToMap(Player player, bool initPlayer = true)
		{
			...
			if (entries.MapDifficulty.HasResetSchedule() &&
			    i_instanceLock != null &&
			    i_instanceLock.GetData().CompletedEncountersMask != 0)
		...
		public override void Update(uint diff)
		{
			...
			if (i_instanceExpireEvent.HasValue &&
			    i_instanceExpireEvent.Value < GameTime.GetSystemTime())
			{
		...
		public override void RemovePlayerFromMap(Player player, bool remove)
		{
			...
			if (_unloadTimer == 0 &&
			    GetPlayers().Count == 1)
				_unloadTimer = (i_instanceLock != null && i_instanceLock.IsExpired()) ? 1 : (uint)Math.Max(WorldConfig.GetIntValue(WorldCfg.InstanceUnloadDelay), 1);
		...
		public void CreateInstanceData()
		{
			...
			if (i_instanceLock == null ||
			    i_instanceLock.GetInstanceId() == 0)
			{
				i_data.Create();
		...
		{
			return i_owningGroupRef.GetTarget();
		}
			...
			if (i_instanceLock != null)
			{
				...
				if (entries.IsInstanceIdBound())
					Global.InstanceLockMgr.UpdateSharedInstanceLock(trans,
					                                                new InstanceLockUpdateEvent(GetInstanceId(),
					                                                                            ...
					                                                                            i_data.GetEntranceLocationForCompletedEncounters(instanceCompletedEncounters)));
				...
				foreach (var player in GetPlayers())
				{
					...
					InstanceLock newLock = Global.InstanceLockMgr.UpdateInstanceLockForPlayer(trans,
					                                                                          ...
					                                                                          entries,
					                                                                          new InstanceLockUpdateEvent(GetInstanceId(),
					                                                                                                      i_data.UpdateBossStateSaveData(oldData, updateSaveDataEvent),
					                                                                                                      instanceCompletedEncounters,
					                                                                                                      updateSaveDataEvent.DungeonEncounter,
					                                                                                                      i_data.GetEntranceLocationForCompletedEncounters(playerCompletedEncounters)));
			...
			if (i_instanceLock != null)
			{
				...
				foreach (var player in GetPlayers())
				{
					...
					InstanceLock newLock = Global.InstanceLockMgr.UpdateInstanceLockForPlayer(trans,
					                                                                          ...
					                                                                          entries,
					                                                                          new InstanceLockUpdateEvent(GetInstanceId(),
					                                                                                                      i_data.UpdateAdditionalSaveData(oldData, updateSaveDataEvent),
					                                                                                                      instanceCompletedEncounters,
					                                                                                                      null,
					                                                                                                      null));
