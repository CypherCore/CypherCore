...
namespace Game.Maps
{
	public class TerrainManager : Singleton<TerrainManager>
	{
		...
		private Dictionary<uint, TerrainInfo> _terrainMaps = new();
	...
	{
		private static TimeSpan CleanupInterval = TimeSpan.FromMinutes(1);
		private List<TerrainInfo> _childTerrain = new();

		// global garbage collection timer
		private TimeTracker _cleanupTimer;
		private BitSet _gridFileExists = new(MapConst.MaxGrids * MapConst.MaxGrids); // cache what grids are available for this map (not including parent/child maps)
		private GridMap[][] _gridMap = new GridMap[MapConst.MaxGrids][];

		private BitSet _loadedGrids = new(MapConst.MaxGrids * MapConst.MaxGrids);

		private object _loadLock = new();
		private uint _mapId;

		private TerrainInfo _parentTerrain;
		private ushort[][] _referenceCountFromMap = new ushort[MapConst.MaxGrids][];
		...
		public void DiscoverGridMapFiles()
		{
			...
			if (File.Exists(tileListName))
			{
				...
				if (mapMagic == MapConst.MapMagic &&
				    versionMagic == MapConst.MapVersionMagic)
				{
		...
		public void CleanUpGrids(uint diff)
		{
			...
			for (int x = 0; x < MapConst.MaxGrids; ++x)
				for (int y = 0; y < MapConst.MaxGrids; ++y)
					if (_loadedGrids[GetBitsetIndex(x, y)] &&
					    _referenceCountFromMap[x][y] == 0)
						UnloadMapImpl(x, y);
		...
		public void GetFullTerrainStatusForPosition(PhaseShift phaseShift, uint mapId, float x, float y, float z, PositionFullTerrainStatus data, LiquidHeaderTypeFlags reqLiquidType = LiquidHeaderTypeFlags.AllLiquids, float collisionHeight = MapConst.DefaultCollesionHeight, DynamicMapTree dynamicMapTree = null)
		{
			...
			if (gridMapHeight > MapConst.InvalidHeight &&
		...
		public uint GetAreaId(PhaseShift phaseShift, uint mapId, float x, float y, float z, DynamicMapTree dynamicMapTree = null)
		{
			...
			// floor is the height we are closer to (but only if above)
			if (hasVmapArea &&
			    MathFunctions.fuzzyGe(z, vmapZ - MapConst.GroundHeightTolerance) &&
			    (MathFunctions.fuzzyLt(z, gridMapHeight - MapConst.GroundHeightTolerance) || vmapZ > gridMapHeight))
			{
				// wmo found
			...
			if (vmapHeight > MapConst.InvalidHeight)
			{
				if (mapHeight > MapConst.InvalidHeight)
				{
					...
					if (vmapHeight > mapHeight ||
					    Math.Abs(mapHeight - z) > Math.Abs(vmapHeight - z))
						return vmapHeight;
