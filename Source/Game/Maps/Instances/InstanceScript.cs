...
namespace Game.Maps
{
	...
	{
		private List<uint> _activatedAreaTriggers = new();
		...
		private Dictionary<uint, uint> _creatureInfo = new();
		private uint _entranceId;
		private Dictionary<uint, uint> _gameObjectInfo = new();
		private List<InstanceSpawnGroupInfo> _instanceSpawnGroups = new();
		private Dictionary<uint, ObjectGuid> _objectGuids = new();
		private List<PersistentInstanceScriptValueBase> _persistentScriptValues = new();
		private uint _temporaryEntranceId;
		private Dictionary<uint, BossInfo> bosses = new();
		private uint completedEncounters; // DEPRECATED, REMOVE
		private MultiMap<uint, DoorInfo> doors = new();
		private string headers;

		public InstanceMap instance;
		private Dictionary<uint, MinionInfo> minions = new();
		...
		public void Load(string data)
		{
			...
			if (reader.Load(data) == InstanceScriptDataReader.Result.Ok)
			{
				...
				for (uint i = 0; i < bosses.Count; ++i)
					if (bosses[i].state == EncounterState.Done &&
		...
		public bool IsEncounterCompleted(uint dungeonEncounterId)
		{
			for (uint i = 0; i < bosses.Count; ++i)
				for (var j = 0; j < bosses[i].DungeonEncounters.Length; ++j)
					if (bosses[i].DungeonEncounters[j] != null &&
		...
		{
			return id < bosses.Count ? bosses[id].state : EncounterState.ToBeDecided;
		}

		public List<AreaBoundary> GetBossBoundary(uint id)
		{
			return id < bosses.Count ? bosses[id].boundary : null;
		}
