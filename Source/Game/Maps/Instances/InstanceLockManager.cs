...
namespace Game.Maps
{
	...
	{
		private Dictionary<uint, SharedInstanceLockData> _instanceLockDataById = new();
		private Dictionary<ObjectGuid, Dictionary<InstanceLockKey, InstanceLock>> _instanceLocksByPlayer = new();
		private object _lockObject = new();
		private Dictionary<ObjectGuid, Dictionary<InstanceLockKey, InstanceLock>> _temporaryInstanceLocksByPlayer = new(); // locks stored here before any boss gets killed
		...
		public TransferAbortReason CanJoinInstanceLock(ObjectGuid playerGuid, MapDb2Entries entries, InstanceLock instanceLock)
		{
			...
			if (!entries.MapDifficulty.IsUsingEncounterLocks() &&
			    playerInstanceLock.GetInstanceId() != 0 &&
			    playerInstanceLock.GetInstanceId() != instanceLock.GetInstanceId())
		...
		public InstanceLock FindActiveInstanceLock(ObjectGuid playerGuid, MapDb2Entries entries, bool ignoreTemporary, bool ignoreExpired)
		{
			...
			if (instanceLock != null &&
			    (!instanceLock.IsExpired() || instanceLock.IsExtended() || !ignoreExpired))
				return instanceLock;
        ...
        public void ResetInstanceLocksForPlayer(ObjectGuid playerGuid, uint? mapId, Difficulty? difficulty, List<InstanceLock> locksReset, List<InstanceLock> locksFailedToReset)
		{
			...
			foreach (var playerLockPair in playerLocks)
			{
				...
				if (mapId.HasValue &&
				    mapId.Value != playerLockPair.Value.GetMapId())
					continue;

				if (difficulty.HasValue &&
				    difficulty.Value != playerLockPair.Value.GetDifficultyId())
					continue;
	...
	{
		private InstanceLockData _data = new();
		private Difficulty _difficultyId;
