...
namespace Game.Maps
{
	...
	{
		private Dictionary<uint, TransportAnimation> _transportAnimations = new();
		private MultiMap<uint, TransportSpawn> _transportsByMap = new();
		private Dictionary<ulong, TransportSpawn> _transportSpawns = new();

		private Dictionary<uint, TransportTemplate> _transportTemplates = new();
	...
	public class TransportTemplate
	{
		...
		private double CalculateDistanceMoved(double timePassedInSegment, double segmentDuration, bool isFirstSegment, bool isLastSegment)
		{
			if (isFirstSegment)
			{
				if (!isLastSegment)
