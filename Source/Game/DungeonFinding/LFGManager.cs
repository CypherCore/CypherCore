...
namespace Game.DungeonFinding
{
	public class LFGManager : Singleton<LFGManager>
	{
		...
		// Reward System
		private MultiMap<uint, LfgReward> RewardMapStore = new(); //< Stores rewards for random dungeons

		// Rolecheck - Proposal - Vote Kicks
		private Dictionary<ObjectGuid, LfgRoleCheck> RoleChecksStore = new(); //< Current Role checks
		...
		public void Update(uint diff)
		{
			...
			if (lastProposalId != _lfgProposalId)
				...
				foreach (var itProposal in ProposalsStore.SkipWhile(p => p.Key == _lfgProposalId))
				{
		...
		public uint AddProposal(LfgProposal proposal)
		{
			...
			ProposalsStore[_lfgProposalId] = proposal;
