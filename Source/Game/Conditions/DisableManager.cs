...
namespace Game
{
	...
	{
		private Dictionary<DisableType, Dictionary<uint, DisableData>> _DisableMap = new();
				...
				switch (type)
				{
					...
					case DisableType.LFGMap:
					{
						...
						switch (mapEntry.InstanceType)
						{
							...
							case MapTypes.Raid:
								if (flags.HasFlag(DisableFlags.DungeonStatusHeroic) &&
								    Global.DB2Mgr.GetMapDifficultyData(entry, Difficulty.Heroic) == null)
									flags &= ~DisableFlags.DungeonStatusHeroic;

								if (flags.HasFlag(DisableFlags.DungeonStatusHeroic10Man) &&
								    Global.DB2Mgr.GetMapDifficultyData(entry, Difficulty.Raid10HC) == null)
									flags &= ~DisableFlags.DungeonStatusHeroic10Man;

								if (flags.HasFlag(DisableFlags.DungeonStatusHeroic25Man) &&
								    Global.DB2Mgr.GetMapDifficultyData(entry, Difficulty.Raid25HC) == null)
					...
					if (refe != null)
					{
						    ...
						    (refe.IsGameObject() && spellFlags.HasFlag(DisableFlags.SpellGameobject)))
						{
							if (spellFlags.HasAnyFlag(DisableFlags.SpellArenas | DisableFlags.SpellBattleGrounds))
							{
								...
								if (map != null)
								{
									if (spellFlags.HasFlag(DisableFlags.SpellArenas) &&
									    map.IsBattleArena())
										return true; // Current map is Arena and this spell is disabled here

									if (spellFlags.HasFlag(DisableFlags.SpellBattleGrounds) &&
									    map.IsBattleground())
