...
namespace Game
{
	...
	{
		private MultiMap<Tuple<uint, bool>, Condition> areaTriggerConditionContainerStorage = new();
		private MultiMap<uint, Condition> conditionReferenceStorage = new();

		private Dictionary<ConditionSourceType, MultiMap<uint, Condition>> conditionStorage = new();
		private Dictionary<uint, MultiMap<uint, Condition>> npcVendorConditionContainerStorage = new();
		private MultiMap<(uint objectType, uint objectId), Condition> objectVisibilityConditionStorage = new();
		private Dictionary<Tuple<int, uint>, MultiMap<uint, Condition>> smartEventConditionStorage = new();
		private Dictionary<uint, MultiMap<uint, Condition>> spellClickEventConditionStorage = new();
		private List<uint> spellsUsedInSpellClickConditions = new();
		...
		private Dictionary<uint, MultiMap<uint, Condition>> trainerSpellConditionContainerStorage = new();
		private Dictionary<uint, MultiMap<uint, Condition>> vehicleSpellConditionStorage = new();
