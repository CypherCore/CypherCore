...
namespace Game
{
    ...
    {
        private readonly MultiMap<Tuple<uint, bool>, Condition> areaTriggerConditionContainerStorage = new();
        private readonly MultiMap<uint, Condition> conditionReferenceStorage = new();

        private readonly Dictionary<ConditionSourceType, MultiMap<uint, Condition>> conditionStorage = new();
        private readonly Dictionary<uint, MultiMap<uint, Condition>> npcVendorConditionContainerStorage = new();
        private readonly MultiMap<(uint objectType, uint objectId), Condition> objectVisibilityConditionStorage = new();
        private readonly Dictionary<Tuple<int, uint>, MultiMap<uint, Condition>> smartEventConditionStorage = new();
        private readonly Dictionary<uint, MultiMap<uint, Condition>> spellClickEventConditionStorage = new();
        private readonly List<uint> spellsUsedInSpellClickConditions = new();
        ...
        private readonly Dictionary<uint, MultiMap<uint, Condition>> trainerSpellConditionContainerStorage = new();
        private readonly Dictionary<uint, MultiMap<uint, Condition>> vehicleSpellConditionStorage = new();
        ...
        private bool CanHaveConditionType(ConditionSourceType sourceType, ConditionTypes conditionType)
        {
            switch (sourceType)
            {
        ...
        public bool IsObjectMeetingNotGroupedConditions(ConditionSourceType sourceType, uint entry, ConditionSourceInfo sourceInfo)
        {
            if (sourceType > ConditionSourceType.None &&
                sourceType < ConditionSourceType.Max)
            {
        ...
        public void LoadConditions(bool isReload = false)
        {
            ...
            do
            {
                ...
                if (iConditionTypeOrReference >= 0 &&
                    !IsSourceTypeValid(cond))
                    continue;

                //Grouping is only allowed for some types (loot templates, gossip menus, gossip items)
                if (cond.SourceGroup != 0 &&
                    !CanHaveSourceGroupSet(cond.SourceType))
                {
                ...
                if (cond.SourceId != 0 &&
                    !CanHaveSourceIdSet(cond.SourceType))
                {
                        ...
                        case ConditionSourceType.SpellClickEvent:
                            {
                                ...
                                continue; // do not add to _AllocatedMemory to avoid double deleting
                            }
                ...
                if (cond.SourceType == ConditionSourceType.SpellClickEvent &&
                    cond.ConditionType == ConditionTypes.Aura)
                    spellsUsedInSpellClickConditions.Add(cond.ConditionValue1);
        ...
        private bool AddToGossipMenus(Condition cond)
        {
            ...
            foreach (var menu in pMenuBounds)
                if (menu.MenuId == cond.SourceGroup &&
                    menu.TextId == cond.SourceEntry)
                {
        ...
        private bool AddToGossipMenuItems(Condition cond)
        {
            ...
            foreach (var gossipMenuItem in pMenuItemBounds)
                if (gossipMenuItem.MenuID == cond.SourceGroup &&
                    gossipMenuItem.OrderIndex == cond.SourceEntry)
                {
                                                                       ...
                                                                       case ConditionTypes.ObjectEntryGuid:
                                                                           {
                                                                               ...
                                                                               if (implicitTargetMask.HasFlag(SpellCastTargetFlags.UnitMask) &&
        ...
        private bool IsSourceTypeValid(Condition cond)
        {
            switch (cond.SourceType)
            {
                ...
                case ConditionSourceType.SpellImplicitTarget:
                    {
                        ...
                        if ((cond.SourceGroup > SpellConst.MaxEffectMask) ||
                            cond.SourceGroup == 0)
                        {
                ...
                case ConditionSourceType.Phase:
                    if (cond.SourceEntry != 0 &&
                        !CliDB.AreaTableStorage.ContainsKey(cond.SourceEntry))
                    {
        ...
        private bool IsConditionTypeValid(Condition cond)
        {
            switch (cond.ConditionType)
            {
                ...
                case ConditionTypes.Team:
                    {
                        if (cond.ConditionValue1 != (uint)Team.Alliance &&
                            cond.ConditionValue1 != (uint)Team.Horde)
                        {
                ...
                case ConditionTypes.Skill:
                    {
                        ...
                        if (cond.ConditionValue2 < 1 ||
                            cond.ConditionValue2 > Global.WorldMgr.GetConfigMaxSkillValue())
                        {
                ...
                case ConditionTypes.ObjectEntryGuid:
                    {
                        switch ((TypeId)cond.ConditionValue1)
                        {
                            case TypeId.Unit:
                                if (cond.ConditionValue2 != 0 &&
                                    Global.ObjectMgr.GetCreatureTemplate(cond.ConditionValue2) == null)
                                {
                            ...
                            case TypeId.GameObject:
                                if (cond.ConditionValue2 != 0 &&
                                    Global.ObjectMgr.GetGameObjectTemplate(cond.ConditionValue2) == null)
                                {
                ...
                case ConditionTypes.TypeMask:
                    {
                        if (cond.ConditionValue1 == 0 ||
                            Convert.ToBoolean(cond.ConditionValue1 & ~(uint)(TypeMask.Unit | TypeMask.Player | TypeMask.GameObject | TypeMask.Corpse)))
                        {
                ...
                case ConditionTypes.CreatureType:
                    {
                        if (cond.ConditionValue1 == 0 ||
                            cond.ConditionValue1 > (uint)CreatureType.GasCloud)
                        {
            ...
            if (cond.ConditionValue1 != 0 &&
                !StaticConditionTypeData[(int)cond.ConditionType].HasConditionValue1)
                LogUselessConditionValue(cond, 1, cond.ConditionValue1);

            if (cond.ConditionValue2 != 0 &&
                !StaticConditionTypeData[(int)cond.ConditionType].HasConditionValue2)
                LogUselessConditionValue(cond, 2, cond.ConditionValue2);

            if (cond.ConditionValue3 != 0 &&
                !StaticConditionTypeData[(int)cond.ConditionType].HasConditionValue3)
                LogUselessConditionValue(cond, 3, cond.ConditionValue3);
            ...
            switch (status)
            {
                ...
                case PlayerConditionLfgStatus.InLFGRandomDungeon:
                    return Global.LFGMgr.InLfgDungeonMap(player.GetGUID(), player.GetMapId(), player.GetMap().GetDifficultyID()) &&
                           Global.LFGMgr.SelectedRandomLfgDungeon(player.GetGUID())
                               ? 1
                               : 0u;
                case PlayerConditionLfgStatus.InLFGFirstRandomDungeon:
                    {
                        ...
                        LfgReward reward = Global.LFGMgr.GetRandomDungeonReward(selectedRandomDungeon, player.GetLevel());
        ...
        public static bool IsPlayerMeetingCondition(Player player, PlayerConditionRecord condition)
        {
            ...
            if (levels.HasValue)
            {
                ...
                if (condition.Flags.HasAnyFlag(0x80))
                {
                    if (minLevel != 0 &&
                        ...
                        (maxLevel == 0 || player.GetLevel() <= maxLevel))
                        return false;

                    if (maxLevel != 0 &&
                        player.GetLevel() <= maxLevel &&
                        (minLevel == 0 || player.GetLevel() >= minLevel))
                        return false;
                }
                else
                {
            ...
            if (condition.RaceMask != 0 &&
                !Convert.ToBoolean(SharedConst.GetMaskForRace(player.GetRace()) & condition.RaceMask))
                return false;

            if (condition.ClassMask != 0 &&
                !Convert.ToBoolean(player.GetClassMask() & condition.ClassMask))
                return false;

            if (condition.Gender >= 0 &&
                (int)player.GetGender() != condition.Gender)
                return false;

            if (condition.NativeGender >= 0 &&
                player.GetNativeGender() != (Gender)condition.NativeGender)
                return false;

            if (condition.PowerType != -1 &&
                condition.PowerTypeComp != 0)
            {
            ...
            if (condition.ChrSpecializationIndex >= 0 ||
                condition.ChrSpecializationRole >= 0)
            {
                ...
                {
                    if (condition.ChrSpecializationIndex >= 0 &&
                        spec.OrderIndex != condition.ChrSpecializationIndex)
                        return false;

                    if (condition.ChrSpecializationRole >= 0 &&
                        spec.Role != condition.ChrSpecializationRole)
                        return false;
                }
            }
            ...
            if (condition.SkillID[0] != 0 ||
                condition.SkillID[1] != 0 ||
                condition.SkillID[2] != 0 ||
                condition.SkillID[3] != 0)
            {
            ...
            if (condition.LanguageID != 0)
            {
                ...
                if (condition.MinLanguage != 0 &&
                    languageSkill < condition.MinLanguage)
                    return false;

                if (condition.MaxLanguage != 0 &&
                    languageSkill > condition.MaxLanguage)
                    return false;
            }

            if (condition.MinFactionID[0] != 0 &&
                condition.MinFactionID[1] != 0 &&
                condition.MinFactionID[2] != 0 &&
                condition.MaxFactionID != 0)
            {
                if (condition.MinFactionID[0] == 0 &&
                    condition.MinFactionID[1] == 0 &&
                    condition.MinFactionID[2] == 0)
                    ...
                    if (forcedRank != 0)
                    {
                        if ((uint)forcedRank > condition.MaxReputation)
                            return false;
                             ...
                             (uint)player.GetReputationRank(condition.MaxFactionID) > condition.MaxReputation)
                    {
                        return false;
                    }
                }
            ...
            if (condition.MovementFlags[0] != 0 &&
                !Convert.ToBoolean((uint)player.GetUnitMovementFlags() & condition.MovementFlags[0]))
                return false;

            if (condition.MovementFlags[1] != 0 &&
                !Convert.ToBoolean((uint)player.GetUnitMovementFlags2() & condition.MovementFlags[1]))
                return false;
            ...
            if (condition.WeaponSubclassMask != 0)
            {
                ...
                if (!mainHand ||
                    !Convert.ToBoolean((1 << (int)mainHand.GetTemplate().GetSubClass()) & condition.WeaponSubclassMask))
                    return false;
            }
            ...
            if (condition.Explored[0] != 0 ||
                condition.Explored[1] != 0)
            ...
            if (condition.MinExpansionLevel != -1 &&
                condition.MinExpansionTier != -1 &&
                !player.IsGameMaster() &&
                ((condition.MinExpansionLevel == WorldConfig.GetIntValue(WorldCfg.Expansion) && condition.MinExpansionTier > 0) /*TODO: implement tier*/ || condition.MinExpansionLevel > WorldConfig.GetIntValue(WorldCfg.Expansion)))
                return false;

            if (condition.PhaseID != 0 ||
                condition.PhaseGroupID != 0 ||
                condition.PhaseUseFlags != 0)
            ...
            if (condition.QuestKillID != 0)
            {
                ...
                if (quest != null &&
                    player.GetQuestStatus(condition.QuestKillID) != QuestStatus.Complete &&
                    questSlot < SharedConst.MaxQuestLogSize)
                {
            ...
            if (condition.ModifierTreeID != 0 &&
                !player.ModifierTreeSatisfied(condition.ModifierTreeID))
                return false;
            ...
            if (condition.TraitNodeEntryID.Any(traitNodeEntryId => traitNodeEntryId != 0))
            {
                ...
                for (var i = 0; i < condition.TraitNodeEntryID.Count(); ++i)
                {
                    ...
                    if (!rank.HasValue)
                        results[i] = false;
                    else if (condition.TraitNodeEntryMinRank[i] != 0 &&
                             rank < condition.TraitNodeEntryMinRank[i])
                        results[i] = false;
                    else if (condition.TraitNodeEntryMaxRank[i] != 0 &&
                             rank > condition.TraitNodeEntryMaxRank[i])
                        results[i] = false;
                }
            ...
            switch (variable)
            {
                               ...
                               .Count(attacker =>
                                      {
                                          ...
                                          if (unit.HasUnitFlag(UnitFlags.PlayerControlled) ||
                                              attacker.HasUnitFlag(UnitFlags.PlayerControlled))
                                              distance += 1.0f;
                ...
                case UnitConditionVariable.InMeleeRange:
                    if (otherUnit)
                    {
                        ...
                        if (unit.HasUnitFlag(UnitFlags.PlayerControlled) ||
                            otherUnit.HasUnitFlag(UnitFlags.PlayerControlled))
                            distance += 1.0f;
                ...
                case UnitConditionVariable.IsMeleeAttacking:
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            if (unit.HasUnitFlag(UnitFlags.PlayerControlled) ||
                                target.HasUnitFlag(UnitFlags.PlayerControlled))
                                distance += 1.0f;
                ...
                case UnitConditionVariable.HasHelpfulAuraSchool:
                    return unit.GetAuraApplication(aurApp => { return !aurApp.GetFlags().HasFlag(AuraFlags.Negative) && ((int)aurApp.GetBase().GetSpellInfo().GetSchoolMask() & (1 << value)) != 0; }) != null ? value : 0;
                default:
                    break;
            }
