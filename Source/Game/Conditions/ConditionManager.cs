                ...
                foreach (var effectMask in sharedMasks)
                {
                    ...
                    if (commonMask != 0)
                    {
                        ...
                        if (sharedList != null)
                        {
                            ...
                            {
                                Log.outDebug(LogFilter.Sql, "{0} in `condition` table, has incorrect SourceGroup {1} (spell effectMask) set - " +
        ...
        bool IsConditionTypeValid(Condition cond)
        {
            switch (cond.ConditionType)
            {
                ...
                case ConditionTypes.Class:
                {
                    ...
                    {
                        Log.outDebug(LogFilter.Sql, "{0} has non existing classmask ({1}), skipped.", cond.ToString(true), cond.ConditionValue1 & ~(uint)Class.ClassMaskAllPlayable);
                        return false;
                    }
                    break;
                }
