    ...
    public class SmartAI : CreatureAI
    {
        ...
        public void StopPath(uint despawnTime = 0, uint quest = 0, bool fail = false)
        {
            if (!HasEscortState(SmartEscortState.Escorting))
            {
                ...
                if (waypointInfo.nodeId != 0)
                    _script.ProcessEventsFor(SmartEvents.WaypointStopped, null, waypointInfo.nodeId, waypointInfo.pathId);

                if (!fail)
                {
                    if (waypointInfo.nodeId != 0)
                        _script.ProcessEventsFor(SmartEvents.WaypointEnded, null, waypointInfo.nodeId, waypointInfo.pathId);
