...
namespace Game.AI
{
	public class SmartScript
	{
		...
		private Dictionary<uint, uint> _counterList = new();
		private uint _currentPriority;
		private uint _eventPhase;

		private List<SmartScriptHolder> _events = new();
		private bool _eventSortingRequired;
		private GameObject _go;
		private ObjectGuid _goOrigGUID;
		private List<SmartScriptHolder> _installEvents = new();
		...
		private List<uint> _remIDs = new();
		private SceneTemplate _sceneTemplate;
		private SmartScriptType _scriptType;
		private List<SmartScriptHolder> _storedEvents = new();

		private Dictionary<uint, ObjectGuidList> _storedTargets = new();
		...
		private void ProcessAction(SmartScriptHolder e, Unit unit = null, uint var0 = 0, uint var1 = 0, bool bvar = false, SpellInfo spell = null, GameObject gob = null, string varString = "")
		{
			...
			if (e.GetEventType() != SmartEvents.Link &&
			    e.Event.event_chance < 100 &&
			    e.Event.event_chance != 0 &&
			    !e.Event.event_flags.HasFlag(SmartEventFlags.TempIgnoreChanceRoll))
			...
			switch (e.GetActionType())
			{
				case SmartActions.Talk:
				{
					...
					foreach (var target in targets)
						if (IsCreature(target) &&
						    !target.ToCreature().IsPet()) // Prevented sending text to pets.
						{
				...
				case SmartActions.Cast:
				{
					if (e.Action.cast.targetsLimit > 0 &&
					    targets.Count > e.Action.cast.targetsLimit)
					...
					foreach (var target in targets)
					{
						...
						if (!e.Action.cast.castFlags.HasAnyFlag((uint)SmartCastFlags.AuraNotPresent) ||
				...
				case SmartActions.Evade:
				{
					...
					{
						var pos = _me.GetRespawnPosition();
						_me.SetHomePosition(pos);
					}
				...
				{
					if (e.Target.type == SmartTargets.None ||
					    e.Target.type == SmartTargets.Self) // Loot recipient and his group members
					{
						if (_me == null)
							break;
				...
				case SmartActions.SetInstData:
				{
					...
					if (instance == null)
					{
						Log.outError(LogFilter.Sql, "SmartScript: Event {0} attempt to set instance _data without instance script. EntryOrGuid {1}", e.GetEventType(), e.EntryOrGuid);
					...
					switch (e.Action.setInstanceData.type)
					{
						case 0:
							...
							Log.outDebug(LogFilter.ScriptsAi,
							             "SmartScript.ProcessAction: SMART_ACTION_SET_INST_DATA: SetData Field: {0}, _data: {1}",
				...
				case SmartActions.SetInstData64:
				{
					...
					Log.outDebug(LogFilter.ScriptsAi,
					             "SmartScript.ProcessAction: SMART_ACTION_SET_INST_DATA64: Field: {0}, _data: {1}",
					             e.Action.setInstanceData64.field,
					             targets.First().GetGUID());
				...
				case SmartActions.MoveOffset:
				{
					foreach (var target in targets)
					{
						...
						if (!e.Event.event_flags.HasAnyFlag(SmartEventFlags.WhileCharmed) &&
						    IsCharmedCreature(target))
							continue;
				...
				case SmartActions.SummonCreature:
				{
					...
					for (uint counter = 0; counter < spawnsCount; counter++)
					{
						...
						if (summon != null)
							if (unit != null &&
							    e.Action.summonCreature.attackInvoker != 0)
				...
				case SmartActions.MoveToPos:
				{
					...
					if (target == null)
					{
						...
						if (e.Action.moveToPos.transport != 0)
						{
							...
							if (trans != null)
								trans.CalculatePassengerPosition(ref dest.X, ref dest.Y, ref dest.Z, ref dest.Orientation);
						}
					...
					foreach (var target in targets)
					{
						...
						if (npc != null)
						{
							...
							for (uint i = 0; i < SharedConst.MaxEquipmentItems; ++i)
								if (e.Action.equip.mask == 0 ||
								    (e.Action.equip.mask & (1 << (int)i)) != 0)
				...
				case SmartActions.CreateTimedEvent:
				{
					...
					if (ne.minMaxRepeat.repeatMin == 0 &&
				...
				case SmartActions.CrossCast:
				{
					...
					foreach (var caster in casters)
					{
						...
						foreach (var target in targets)
						{
							...
							if (!(e.Action.crossCast.castFlags.HasAnyFlag((uint)SmartCastFlags.AuraNotPresent)) ||
							    !target.ToUnit().HasAura(e.Action.crossCast.spell))
							{
								if (!interruptedSpell &&
								    e.Action.crossCast.castFlags.HasAnyFlag((uint)SmartCastFlags.InterruptPrevious))
								{
				...
				case SmartActions.SummonCreatureGroup:
				{
					...
					foreach (var summon in summonList)
						if (unit == null &&
						    e.Action.creatureGroup.attackInvoker != 0)
							summon.GetAI().AttackStart(unit);
				...
				{
					if (e.Action.groupSpawn.minDelay == 0 &&
					    e.Action.groupSpawn.maxDelay == 0)
					{
				...
				case SmartActions.OverrideLight:
				{
					...
					if (obj != null)
					{
						...
						Log.outDebug(LogFilter.ScriptsAi,
						             $"SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_LIGHT: {obj.GetGUID()} sets zone override light (zoneId: {e.Action.overrideLight.zoneId}, " +
				...
				case SmartActions.OverrideWeather:
				{
					...
					if (obj != null)
					{
						...
						Log.outDebug(LogFilter.ScriptsAi,
						             $"SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_WEATHER: {obj.GetGUID()} sets zone weather (zoneId: {e.Action.overrideWeather.zoneId}, " +
				...
				case SmartActions.CreateConversation:
				{
					...
					foreach (WorldObject target in targets)
					{
						...
						{
							Conversation conversation = Conversation.CreateConversation(e.Action.conversation.id,
							                                                            playerTarget,
							                                                            playerTarget,
							                                                            playerTarget.GetGUID(),
							                                                            null);
		...
		private List<WorldObject> GetTargets(SmartScriptHolder e, WorldObject invoker = null)
		{
			...
			switch (e.GetTargetType())
			{
				...
				case SmartTargets.ActionInvokerVehicle:
					if (scriptTrigger != null &&
					    scriptTrigger.ToUnit()?.GetVehicle() != null &&
					    scriptTrigger.ToUnit().GetVehicle().GetBase() != null)
				...
				case SmartTargets.CreatureDistance:
				{
					...
					foreach (var obj in units)
					{
						...
						if (e.Target.unitDistance.creature == 0 ||
						    obj.ToCreature().GetEntry() == e.Target.unitDistance.creature)
							targets.Add(obj);
					}
				...
				case SmartTargets.GameobjectDistance:
				{
					...
					foreach (var obj in units)
					{
						...
						if (e.Target.goDistance.entry == 0 ||
						    obj.ToGameObject().GetEntry() == e.Target.goDistance.entry)
							targets.Add(obj);
					}
				...
				case SmartTargets.CreatureGuid:
				{
					...
					if (target)
						if (target != null &&
						    (e.Target.unitGUID.entry == 0 || target.GetEntry() == e.Target.unitGUID.entry))
							targets.Add(target);
				...
				case SmartTargets.GameobjectGuid:
				{
					...
					if (target)
						if (target != null &&
						    (e.Target.goGUID.entry == 0 || target.GetEntry() == e.Target.goGUID.entry))
							targets.Add(target);
				...
				case SmartTargets.PlayerRange:
				{
					...
					if (!units.Empty() &&
						...
						foreach (var obj in units)
							if (IsPlayer(obj) &&
							    baseObject.IsInRange(obj, e.Target.playerRange.minDist, e.Target.playerRange.maxDist))
								targets.Add(obj);
				...
				case SmartTargets.OwnerOrSummoner:
				{
					...
					// Get owner of owner
					if (e.Target.owner.useCharmerOrOwner != 0 &&
					    !targets.Empty())
					{
				...
				case SmartTargets.VehiclePassenger:
				{
					if (_me && _me.IsVehicle())
						foreach (var pair in _me.GetVehicleKit().Seats)
							if (e.Target.vehicle.seatMask == 0 ||
							    (e.Target.vehicle.seatMask & (1 << pair.Key)) != 0)
							{
		...
		private void ProcessEvent(SmartScriptHolder e, Unit unit = null, uint var0 = 0, uint var1 = 0, bool bvar = false, SpellInfo spell = null, GameObject gob = null, string varString = "")
		{
			...
			switch (e.GetEventType())
			{
				...
				case SmartEvents.HealthPct:
				{
					...
					if (perc > e.Event.minMaxRepeat.max ||
					    perc < e.Event.minMaxRepeat.min)
						return;
				...
				case SmartEvents.ManaPct:
				{
					if (_me == null ||
					    !_me.IsEngaged() ||
					    _me.GetMaxPower(PowerType.Mana) == 0)
						return;
				...
				case SmartEvents.HasAura:
				{
					...
					if ((e.Event.aura.count == 0 && count == 0) ||
					    (e.Event.aura.count != 0 && count >= e.Event.aura.count))
				...
				case SmartEvents.Kill:
				{
					...
					if (e.Event.kill.playerOnly != 0 &&
					    !unit.IsTypeId(TypeId.Player))
						return;

					if (e.Event.kill.creature != 0 &&
					    unit.GetEntry() != e.Event.kill.creature)
						return;
				...
				case SmartEvents.OocLos:
				{
					...
					if (_me.IsWithinDistInMap(unit, range) &&
					    _me.IsWithinLOSInMap(unit))
					{
				...
				{
					if (var0 > e.Event.minMaxRepeat.max ||
				...
				case SmartEvents.Movementinform:
				{
					if ((e.Event.movementInform.type != 0 && var0 != e.Event.movementInform.type) ||
					    (e.Event.movementInform.id != 0xFFFFFFFF && var1 != e.Event.movementInform.id))
						return;
				...
				{
					if (e.Event.transportRelocate.pointID != 0 &&
					    var0 != e.Event.transportRelocate.pointID)
						return;
				...
				{
					if (_me == null ||
					    (e.Event.waypoint.pointID != 0 && var0 != e.Event.waypoint.pointID) ||
				...
				{
					if (e.Event.summoned.creature != 0 &&
					    e.Event.summoned.creature != var0)
						return;
				...
				{
					if (e.Event.instancePlayerEnter.team != 0 &&
				...
				{
					if (e.Event.dataSet.id != var0 ||
					    e.Event.dataSet.value != var1)
						return;
				...
				case SmartEvents.FriendlyHealthPCT:
				{
					...
					switch (e.GetTargetType())
					{
						...
						case SmartTargets.PlayerDistance:
						{
							...
							foreach (WorldObject target in targets)
								if (IsUnit(target) &&
								    _me.IsFriendlyTo(target.ToUnit()) &&
								    target.ToUnit().IsAlive() &&
								    target.ToUnit().IsInCombat())
								{
									uint healthPct = (uint)target.ToUnit().GetHealthPct();

									if (healthPct > e.Event.friendlyHealthPct.maxHpPct ||
									    healthPct < e.Event.friendlyHealthPct.minHpPct)
										continue;
		...
		private void UpdateTimer(SmartScriptHolder e, uint diff)
		{
			...
			if (e.Event.event_phase_mask != 0 &&
			    !IsInPhase(e.Event.event_phase_mask))
				return;

			if (e.GetEventType() == SmartEvents.UpdateIc &&
			    (_me == null || !_me.IsEngaged()))
				return;

			if (e.GetEventType() == SmartEvents.UpdateOoc &&
			    (_me != null && _me.IsEngaged())) //can be used with me=NULL (go script)
				return;
			...
			if (e.Timer < diff)
			{
				...
				if (e.GetActionType() == SmartActions.Cast)
					if (!Convert.ToBoolean(e.Action.cast.castFlags & (uint)SmartCastFlags.InterruptPrevious))
						if (_me != null &&
						    _me.HasUnitState(UnitState.Casting))
						{
							RaisePriority(e);
				...
				switch (e.GetEventType()) //process ONLY timed events
				{
					...
					case SmartEvents.DistanceGameobject:
					{
						if (e.GetScriptType() == SmartScriptType.TimedActionlist)
						{
							...
							if (_me != null &&
							    !mTimedActionListInvoker.IsEmpty())
	...
	{
		private List<ObjectGuid> _guidList = new();
		private List<WorldObject> _objectList = new();
