...
namespace Game
{
    internal class TraitMgr
    {
        ...
        private static readonly Dictionary<int, TraitNode> _traitNodes = new();
        private static readonly Dictionary<int, TraitTree> _traitTrees = new();
        ...
        private static readonly MultiMap<int, TraitTreeLoadoutEntryRecord> _traitTreeLoadoutsByChrSpecialization = new();
        ...
        public static void Load()
        {
            ...
            foreach (TraitNodeRecord traitNode in CliDB.TraitNodeStorage.Values)
            {
                ...
                TraitTree tree = _traitTrees.LookupByKey(traitNode.TraitTreeID);
            ...
            {
                TraitNode left = _traitNodes.LookupByKey(traitEdgeEntry.LeftTraitNodeID);
                TraitNode right = _traitNodes.LookupByKey(traitEdgeEntry.RightTraitNodeID);
            ...
            {
                TraitTree tree = _traitTrees.LookupByKey(skillLineXTraitTreeEntry.TraitTreeID);
            ...
            {
                TraitTree tree = _traitTrees.LookupByKey(traitTreeId);
		 ...
		 */
        public static List<TraitTree> GetTreesForConfig(TraitConfigPacket traitConfig)
        {
        ...
        public static bool HasEnoughCurrency(TraitEntryPacket entry, Dictionary<int, int> currencies)
        {
            ...
            TraitNode node = _traitNodes.LookupByKey(entry.TraitNodeID);
            ...
            TraitTree tree = _traitTrees.LookupByKey(node.Data.TraitTreeID);
        ...
        {
            List<TraitTree> trees = GetTreesForConfig(traitConfig);
            ...
            foreach (TraitTree tree in trees)
            {
                    ...
                    switch (currency.GetCurrencyType())
                    {
                        ...
                        case TraitCurrencyType.TraitSourced:
                            ...
                            if (currencySources != null)
                                ...
                                {
                                    if (currencySource.QuestID != 0 &&
                                        !player.IsQuestRewarded(currencySource.QuestID))
                                        continue;

                                    if (currencySource.AchievementID != 0 &&
                                        !player.HasAchieved(currencySource.AchievementID))
                                        continue;

                                    if (currencySource.PlayerLevel != 0 &&
                                        player.GetLevel() < currencySource.PlayerLevel)
                                        continue;

                                    if (currencySource.TraitNodeEntryID != 0 &&
                                        !hasTraitNodeEntry(currencySource.TraitNodeEntryID))
                                        continue;
        ...
        {
            if (condition.QuestID != 0 &&
                !player.IsQuestRewarded(condition.QuestID))
                return false;

            if (condition.AchievementID != 0 &&
                !player.HasAchieved(condition.AchievementID))
                return false;
            ...
            if (condition.TraitCurrencyID != 0 &&
                condition.SpentAmountRequired != 0)
            {
                if (cachedCurrencies == null)
                {
                    cachedCurrencies = new Dictionary<int, int>();
            ...
            if (condition.RequiredLevel != 0 &&
                player.GetLevel() < condition.RequiredLevel)
                return false;
                ...
                foreach (TraitNode node in tree.Nodes)
                {
                    foreach (TraitNodeEntry entry in node.Entries)
                        foreach (TraitCondRecord condition in entry.Conditions)
                            if (condition.GetCondType() == TraitConditionType.Granted &&
                                MeetsTraitCondition(traitConfig, player, condition, ref cachedCurrencies))
        ...
        {
            TraitNode node = _traitNodes.LookupByKey(traitEntry.TraitNodeID);
            ...
            bool meetsConditions(List<TraitCondRecord> conditions)
            {
                ...
                foreach (TraitCondRecord condition in conditions)
                    if (condition.GetCondType() == TraitConditionType.Available ||
                        condition.GetCondType() == TraitConditionType.Visible)
                    {
        ...
        public static void InitializeStarterBuildTraitConfig(TraitConfigPacket traitConfig, Player player)
        {
            ...
            if (loadoutEntries != null)
            {
                ...
                foreach (TraitTreeLoadoutEntryRecord loadoutEntry in loadoutEntries)
                {
                    ...
                    TraitNode node = _traitNodes.LookupByKey(loadoutEntry.SelectedTraitNodeID);
