...
namespace Game.Entities
{
	public sealed class SpellManager : Singleton<SpellManager>
	{
		...
		private Dictionary<uint, SpellChainNode> mSpellChains = new();
		private MultiMap<uint, uint> mSpellsReqSpell = new();
		private MultiMap<uint, uint> mSpellReq = new();
		private Dictionary<uint, SpellLearnSkillNode> mSpellLearnSkills = new();
		private MultiMap<uint, SpellLearnSpellNode> mSpellLearnSpells = new();
		private Dictionary<KeyValuePair<uint, uint>, SpellTargetPosition> mSpellTargetPositions = new();
		private MultiMap<uint, SpellGroup> mSpellSpellGroup = new();
		private MultiMap<SpellGroup, int> mSpellGroupSpell = new();
		private Dictionary<SpellGroup, SpellGroupStackRule> mSpellGroupStack = new();
		private MultiMap<SpellGroup, AuraType> mSpellSameEffectStack = new();
		private List<ServersideSpellName> mServersideSpellNames = new();
		private Dictionary<(uint id, Difficulty difficulty), SpellProcEntry> mSpellProcMap = new();
		private Dictionary<uint, SpellThreatEntry> mSpellThreatMap = new();
		private Dictionary<uint, PetAura> mSpellPetAuraMap = new();
		private MultiMap<(SpellLinkedType, uint), int> mSpellLinkedMap = new();
		private Dictionary<uint, SpellEnchantProcEntry> mSpellEnchantProcEventMap = new();
		private MultiMap<uint, SpellArea> mSpellAreaMap = new();
		private MultiMap<uint, SpellArea> mSpellAreaForQuestMap = new();
		private MultiMap<uint, SpellArea> mSpellAreaForQuestEndMap = new();
		private MultiMap<uint, SpellArea> mSpellAreaForAuraMap = new();
		private MultiMap<uint, SpellArea> mSpellAreaForAreaMap = new();
		private MultiMap<uint, SkillLineAbilityRecord> mSkillLineAbilityMap = new();
		private Dictionary<uint, MultiMap<uint, uint>> mPetLevelupSpellMap = new();
		private Dictionary<uint, PetDefaultSpellsEntry> mPetDefaultSpellsMap = new(); // only spells not listed in related mPetLevelupSpellMap entry
		private MultiMap<uint, SpellInfo> mSpellInfoMap = new();
		private Dictionary<Tuple<uint, byte>, uint> mSpellTotemModel = new();

		public delegate void AuraEffectHandler(AuraEffect effect, AuraApplication aurApp, AuraEffectHandleModes mode, bool apply);

		private Dictionary<AuraType, AuraEffectHandler> AuraEffectHandlers = new();

		public delegate void SpellEffectHandler(Spell spell);

		private Dictionary<SpellEffectName, SpellEffectHandler> SpellEffectsHandlers = new();
	...
	{
		private Dictionary<uint, uint> auras = new();
		private int damage;
