...
namespace Game.Spells
{
	public class SpellInfo
	{
		...
		public bool IsPrimaryProfession()
		{
			foreach (var effectInfo in _effects)
				if (effectInfo.IsEffect(SpellEffectName.Skill) &&
				    Global.SpellMgr.IsPrimaryProfessionSkill((uint)effectInfo.MiscValue))
					return true;
		...
		public bool CanPierceImmuneAura(SpellInfo auraSpellInfo)
		{
			...
			if (auraSpellInfo == null ||
			    auraSpellInfo.HasAttribute(SpellAttr0.NoImmunities))
				return false;
			...
			*/
			...
			if (actAsShifted)
			{
				if (HasAttribute(SpellAttr0.NotShapeshifted) ||
				    (shapeInfo != null && shapeInfo.Flags.HasAnyFlag(SpellShapeshiftFormFlags.CanOnlyCastShapeshiftSpells))) // not while shapeshifted
					return SpellCastResult.NotShapeshift;
			...
			{
				// needs shapeshift
				if (!HasAttribute(SpellAttr2.AllowWhileNotShapeshiftedCasterForm) &&
				    Stances != 0)
					return SpellCastResult.OnlyShapeshift;
			}
		...
		public SpellCastResult CheckLocation(uint map_id, uint zone_id, uint area_id, Player player)
		{
			...
			if (HasAttribute(SpellAttr4.OnlyFlyingAreas))
			{
				...
				if (player)
				{
					...
					if ((mapToCheck == 1116 || mapToCheck == 1464) &&
					    !player.HasSpell(191645)) // Draenor Pathfinder
						return SpellCastResult.IncorrectArea;
					else if (mapToCheck == 1220 &&
					         !player.HasSpell(233368)) // Broken Isles Pathfinder
						return SpellCastResult.IncorrectArea;
					else if ((mapToCheck == 1642 || mapToCheck == 1643) &&
					         !player.HasSpell(278833)) // Battle for Azeroth Pathfinder
						return SpellCastResult.IncorrectArea;
				}
			}
				...
				foreach (var effectInfo in _effects)
				{
					...
					switch (effectInfo.ApplyAuraName)
					{
						...
						case AuraType.Mounted:
						{
							...
							if (mountType != 0 &&
							    player.GetMountCapability(mountType) == null)
								return SpellCastResult.NotHere;
		...
		{
			if (HasAttribute(SpellAttr1.ExcludeCaster) &&
			    caster == target)
				return SpellCastResult.BadTargets;

			// check visibility - ignore stealth for implicit (area) targets
			if (!HasAttribute(SpellAttr6.IgnorePhaseShift) &&
				...
				if (caster != unitTarget)
					if (caster.IsTypeId(TypeId.Player))
					{
						...
						if (HasAttribute(SpellCustomAttributes.PickPocket))
						{
							...
							if (!LootStorage.Pickpocketing.HaveLootFor(targetCreature.GetCreatureTemplate().PickPocketId))
						...
						if (Mechanic == Mechanics.Disarm)
						{
							if (unitTarget.IsTypeId(TypeId.Player))
							{
								...
								if (player.GetWeaponForAttack(WeaponAttackType.BaseAttack) == null ||
								    !player.IsUseEquipedWeapon(true))
			...
			if (!unitTarget.IsPlayer())
			{
				...
				if (HasAttribute(SpellAttr5.NotOnPlayerControlledNpc) &&
				    unitTarget.IsControlledByPlayer())
			...
			if (unitTarget != caster &&
			    (caster.GetAffectingPlayer() != null || !IsPositive()) &&
			...
			if (unitTarget.HasUnitState(UnitState.InFlight) &&
			...
			if (unitCaster != null)
				    ...
				    unitCaster.GetCharmerOrOwner() != target)
				{
					if (TargetAuraState != 0 &&
					    !unitTarget.HasAuraState(TargetAuraState, this, unitCaster))
						return SpellCastResult.TargetAurastate;

					if (ExcludeTargetAuraState != 0 &&
					    unitTarget.HasAuraState(ExcludeTargetAuraState, this, unitCaster))
						return SpellCastResult.TargetAurastate;
				}

			if (TargetAuraSpell != 0 &&
			    !unitTarget.HasAura(TargetAuraSpell))
				return SpellCastResult.TargetAurastate;

			if (ExcludeTargetAuraSpell != 0 &&
			    unitTarget.HasAura(ExcludeTargetAuraSpell))
				return SpellCastResult.TargetAurastate;

			if (unitTarget.HasAuraType(AuraType.PreventResurrection) &&
			    !HasAttribute(SpellAttr7.BypassNoResurrectAura))
				if (HasEffect(SpellEffectName.SelfResurrect) ||
				    HasEffect(SpellEffectName.Resurrect))
		...
		public SpellCastResult CheckExplicitTarget(WorldObject caster, WorldObject target, Item itemTarget = null)
		{
			...
			if (target == null)
			{
				if (Convert.ToBoolean(neededTargets & (SpellCastTargetFlags.UnitMask | SpellCastTargetFlags.GameobjectMask | SpellCastTargetFlags.CorpseMask)))
					if (!Convert.ToBoolean(neededTargets & SpellCastTargetFlags.GameobjectItem) ||
					    itemTarget == null)
						return SpellCastResult.BadTargets;
			...
			if (unitTarget != null)
				if (neededTargets.HasAnyFlag(SpellCastTargetFlags.UnitEnemy | SpellCastTargetFlags.UnitAlly | SpellCastTargetFlags.UnitRaid | SpellCastTargetFlags.UnitParty | SpellCastTargetFlags.UnitMinipet | SpellCastTargetFlags.UnitPassenger))
				{
					...
					if (neededTargets.HasFlag(SpellCastTargetFlags.UnitMinipet) &&
					    unitCaster != null)
						if (unitTarget.GetGUID() == unitCaster.GetCritterGUID())
							return SpellCastResult.SpellCastOk;

					if (neededTargets.HasFlag(SpellCastTargetFlags.UnitPassenger) &&
					    unitCaster != null)
			...
			if (vehicle)
			{
				...
				foreach (var effectInfo in _effects)
					if (effectInfo.IsAura(AuraType.ModShapeshift))
					{
						...
						if (shapeShiftFromEntry != null &&
						    !shapeShiftFromEntry.Flags.HasAnyFlag(SpellShapeshiftFormFlags.Stance))
				...
				if (!HasAttribute(SpellAttr6.AllowWhileRidingVehicle) &&
				    !HasAttribute(SpellAttr0.AllowWhileMounted) &&
				...
				if (vehicleSeat.HasFlag(VehicleSeatFlags.CanControl | VehicleSeatFlags.Unk2))
					foreach (var effectInfo in _effects)
					{
						...
						if (props != null &&
						    props.Control != SummonCategory.Wild)
							return SpellCastResult.CantDoThatRightNow;
					}
			}
		...
		public ulong GetEffectMechanicMask(uint effIndex)
		{
			...
			if (GetEffect(effIndex).IsEffect() &&
			    GetEffect(effIndex).Mechanic != 0)
		...
		public ulong GetSpellMechanicMaskByEffectMask(uint effectMask)
		{
			...
			foreach (var effectInfo in _effects)
				if ((effectMask & (1 << (int)effectInfo.EffectIndex)) != 0 &&
				    effectInfo.Mechanic != 0)
					mask |= 1ul << (int)effectInfo.Mechanic;
			...
			switch (SpellFamilyName)
			{
				...
				case SpellFamilyNames.Mage:
				{
					...
					if (SpellFamilyFlags[0].HasAnyFlag(0x1000000u) &&
					    GetEffect(0).IsAura(AuraType.ModConfuse))
				...
				case SpellFamilyNames.Warlock:
				{
					...
					if (SpellFamilyFlags[1].HasAnyFlag(0x20000020u) ||
					    SpellFamilyFlags[2].HasAnyFlag(0x00000010u))
						_spellSpecific = SpellSpecificType.WarlockArmor;

					//seed of corruption and corruption
					if (SpellFamilyFlags[1].HasAnyFlag(0x10u) ||
					    SpellFamilyFlags[0].HasAnyFlag(0x2u))
				...
				switch (effectInfo.ApplyAuraName)
				{
					...
					case AuraType.ModImmuneAuraApplySchool:
						if (aurEff.GetSpellInfo().HasAttribute(SpellAttr2.NoSchoolImmunities) ||
						    !Convert.ToBoolean((uint)aurEff.GetSpellInfo().SchoolMask & miscValue))
							continue;
		...
		public int CalcCastTime(Spell spell = null)
		{
			...
			if (HasAttribute(SpellAttr0.UsesRangedSlot) &&
			    (!IsAutoRepeatRangedSpell()) &&
			    !HasAttribute(SpellAttr9.AimedShot))
				castTime += 500;
			...
			foreach (var effectInfo in GetEffects())
			{
				...
				switch (effectInfo.ApplyAuraName)
				{
					...
					case AuraType.PeriodicHealthFunnel:
						...
						if (effectInfo.ApplyAuraPeriod > 0 &&
						    DotDuration > 0)
						{
		...
		public SpellPowerCost CalcPowerCost(SpellPowerRecord power, bool optionalCost, WorldObject caster, SpellSchoolMask schoolMask, Spell spell = null)
		{
			...
			if (power.RequiredAuraSpellID != 0 &&
			    !unitCaster.HasAura(power.RequiredAuraSpellID))
				return null;
			...
			else
			{
				...
				powerCost += unitCaster.GetTotalAuraModifier(AuraType.ModAdditionalPowerCost, aurEff => { return aurEff.GetMiscValue() == (int)power.PowerType && aurEff.IsAffectingSpell(this); });
			}
			...
			if (HasAttribute(SpellAttr4.WeaponSpeedCostScaling))
			{
				...
				else
				{
					...
					if (!HasAttribute(SpellAttr3.RequiresMainHandWeapon) &&
					    HasAttribute(SpellAttr3.RequiresOffHandWeapon))
			...
			if (!unitCaster.IsControlledByPlayer() &&
			    ...
			    SpellLevel != 0 &&
			    power.PowerType == PowerType.Mana)
		...
		public SpellInfo GetAuraRankForLevel(uint level)
		{
			...
			if (HasAttribute(SpellAttr0.AuraIsDebuff) ||
			    HasAttribute(SpellAttr2.AllowLowLevelBuff) ||
			    HasAttribute(SpellAttr3.OnlyProcOnCaster))
				return this;
			...
			if (effect.IsAura())
				...
				switch (effect.ApplyAuraName)
				{
					...
					case AuraType.ModSpeedAlways:
						if (bp < 0 ||
						    bpScalePerLevel < 0) //TODO: What if both are 0? Should it be a buff or debuff?
							return false;
						...
						switch ((SpellModOp)effect.MiscValue)
						{
							...
							case SpellModOp.PowerCost2:
								if (!spellInfo.IsPositive() &&
								    bp > 0) // dependent on prev effects too (ex Arcane Power)
									return false;
							...
							case SpellModOp.ChainTargets:
								if (!spellInfo.IsPositive() &&
								    bp < 0) // dependent on prev effects too
									return false;
		...
		public BitSet NegativeEffects { get; set; } = new(SpellConst.MaxEffects);
		...
		private List<SpellProcsPerMinuteModRecord> ProcPPMMods = new();
		...
		private List<SpellEffectInfo> _effects = new();
		private List<SpellXSpellVisualRecord> _visuals = new();
