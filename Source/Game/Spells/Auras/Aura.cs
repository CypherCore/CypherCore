...
namespace Game.Spells
{
	...
	public class Aura
	{
		...
		private static List<IAuraScript> _dummy = new();
		private static List<(IAuraScript, IAuraEffectHandler)> _dummyAuraEffects = new();
		private readonly Dictionary<Type, List<IAuraScript>> _auraScriptsByType = new();
		private Dictionary<uint, Dictionary<AuraScriptHookType, List<(IAuraScript, IAuraEffectHandler)>>> _effectHandlers = new();
		...
		private Difficulty _castDifficulty;
		...
		private uint _casterLevel; // Aura level (store caster level for correct show level dep amount)
		...
		private List<AuraApplication> _removedApplications = new();
