...
namespace Game.BattleGrounds.Zones
{
	internal class BgEyeofStorm : Battleground
	{
		...
		public BgEyeofStorm(BattlegroundTemplate battlegroundTemplate) : base(battlegroundTemplate)
		{
			...
			_Points_Trigger[EotSPoints.FelReaver]    = EotSPointsTrigger.FelReaverBuff;
			_Points_Trigger[EotSPoints.BloodElf]     = EotSPointsTrigger.BloodElfBuff;
			_Points_Trigger[EotSPoints.DraeneiRuins] = EotSPointsTrigger.DraeneiRuinsBuff;
			_Points_Trigger[EotSPoints.MageTower]    = EotSPointsTrigger.MageTowerBuff;
		...
		public override void PostUpdateImpl(uint diff)
		{
			if (GetStatus() == BattlegroundStatus.InProgress)
			{
				...
				{
					_PointAddingTimer = EotSMisc.FPointsTickTime;
				...
				if (_TowerCapCheckTimer <= 0)
				{
					...
					*/
					...
					CheckSomeoneLeftPo();
					UpdatePointStatuses();
					_TowerCapCheckTimer = EotSMisc.FPointsTickTime;
				}
			}
		}
		...
		private void CheckSomeoneJoinedPoint()
		{
			...
			for (byte i = 0; i < EotSPoints.PointsMax; ++i)
			{
				...
				if (obj)
				{
					...
					while (j < _PlayersNearPoint[EotSPoints.PointsMax].Count)
					{
						Player player = Global.ObjAccessor.FindPlayer(_PlayersNearPoint[EotSPoints.PointsMax][j]);

						if (!player)
						{
							Log.outError(LogFilter.Battleground, "BattlegroundEY:CheckSomeoneJoinedPoint: Player ({0}) could not be found!", _PlayersNearPoint[EotSPoints.PointsMax][j].ToString());
							++j;
						    ...
						    player.IsWithinDistInMap(obj, (float)EotSProgressBarConsts.PointRadius))
						{
							...
							player.SendUpdateWorldState(EotSWorldStateIds.ProgressBarStatus, (uint)_PointBarStatus[i]);
							player.SendUpdateWorldState(EotSWorldStateIds.ProgressBarShow, (uint)EotSProgressBarConsts.ProgressBarShow);
							//add player to point
							_PlayersNearPoint[i].Add(_PlayersNearPoint[EotSPoints.PointsMax][j]);
							//remove player from "free space"
							_PlayersNearPoint[EotSPoints.PointsMax].RemoveAt(j);
						}
						else
						{
							++j;
				...
				if (obj)
				{
					...
					while (j < _PlayersNearPoint[i].Count)
					{
						...
						if (!player)
						{
							...
							_PlayersNearPoint[EotSPoints.PointsMax].Add(_PlayersNearPoint[i][j]);
			...
			for (byte point = 0; point < EotSPoints.PointsMax; ++point)
			{
				if (!_PlayersNearPoint[point].Empty())
				{
					...
					for (byte i = 0; i < _PlayersNearPoint[point].Count; ++i)
					{
						...
						if (player)
						{
							player.SendUpdateWorldState(EotSWorldStateIds.ProgressBarStatus, (uint)_PointBarStatus[point]);
				...
				{
					UpdateWorldState(EotSMisc._PointsIconStruct[point].WorldStateAllianceStatusBarIcon, (int)(captureStatus == BattlegroundPointCaptureStatus.AllianceControlled ? 2 : (captureStatus == BattlegroundPointCaptureStatus.AllianceCapturing ? 1 : 0)));
					UpdateWorldState(EotSMisc._PointsIconStruct[point].WorldStateHordeStatusBarIcon, (int)(captureStatus == BattlegroundPointCaptureStatus.HordeControlled ? 2 : (captureStatus == BattlegroundPointCaptureStatus.HordeCapturing ? 1 : 0)));
		...
		private void UpdatePointsCount(Team team)
		{
			if (team == Team.Alliance)
				UpdateWorldState(EotSWorldStateIds.AllianceBase, (int)_TeamPointsCount[TeamId.Alliance]);
			else
				UpdateWorldState(EotSWorldStateIds.HordeBase, (int)_TeamPointsCount[TeamId.Horde]);
		}
		...
		private void UpdatePointsIcons(Team team, int Point)
		{
			...
			if (_PointState[Point] == EotSPointState.UnderControl)
			{
				UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateControlIndex, 0);

				if (team == Team.Alliance)
					UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateAllianceControlledIndex, 1);
				else
					UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateHordeControlledIndex, 1);
			}
			else
			{
				if (team == Team.Alliance)
					UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateAllianceControlledIndex, 0);
				else
					UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateHordeControlledIndex, 0);

				UpdateWorldState(EotSMisc._PointsIconStruct[Point].WorldStateControlIndex, 1);
			}
		}
		...
		public override void AddPlayer(Player player)
		{
			...
			_PlayersNearPoint[EotSPoints.PointsMax].Add(player.GetGUID());
		}
		...
		public override void HandleAreaTrigger(Player player, uint trigger, bool entered)
		{
			...
			switch (trigger)
			{
				...
				case EotSPointsTrigger.BloodElfPoint:
					if (_PointState[EotSPoints.BloodElf] == EotSPointState.UnderControl &&
					    _PointOwnedByTeam[EotSPoints.BloodElf] == GetPlayerTeam(player.GetGUID()))
						if (_FlagState != 0 &&
						    GetFlagPickerGUID() == player.GetGUID())
				...
				case EotSPointsTrigger.FelReaverPoint:
					if (_PointState[EotSPoints.FelReaver] == EotSPointState.UnderControl &&
					    _PointOwnedByTeam[EotSPoints.FelReaver] == GetPlayerTeam(player.GetGUID()))
						if (_FlagState != 0 &&
						    GetFlagPickerGUID() == player.GetGUID())
							EventPlayerCapturedFlag(player, EotSObjectTypes.FlagFelReaver);
				...
				case EotSPointsTrigger.MageTowerPoint:
					if (_PointState[EotSPoints.MageTower] == EotSPointState.UnderControl &&
					    _PointOwnedByTeam[EotSPoints.MageTower] == GetPlayerTeam(player.GetGUID()))
						if (_FlagState != 0 &&
						    GetFlagPickerGUID() == player.GetGUID())
				...
				case EotSPointsTrigger.DraeneiRuinsPoint:
					if (_PointState[EotSPoints.DraeneiRuins] == EotSPointState.UnderControl &&
					    _PointOwnedByTeam[EotSPoints.DraeneiRuins] == GetPlayerTeam(player.GetGUID()))
						if (_FlagState != 0 &&
						    GetFlagPickerGUID() == player.GetGUID())
		...
		public override bool SetupBattleground()
		{
			...
			if (sg == null ||
			    !AddSpiritGuide(EotSCreaturesTypes.SpiritMainAlliance, sg.Loc.GetPositionX(), sg.Loc.GetPositionY(), sg.Loc.GetPositionZ(), 3.124139f, TeamId.Alliance))
			{
			...
			if (sg == null ||
			    !AddSpiritGuide(EotSCreaturesTypes.SpiritMainHorde, sg.Loc.GetPositionX(), sg.Loc.GetPositionY(), sg.Loc.GetPositionZ(), 3.193953f, TeamId.Horde))
			{
				Log.outError(LogFilter.Sql, "BatteGroundEY: Failed to spawn spirit guide. The battleground was not created.");
		...
		public override void Reset()
		{
			...
			_HonorTics = (isBGWeekend) ? EotSMisc.EYWeekendHonorTicks : EotSMisc.NotEYWeekendHonorTicks;
			...
			_PlayersNearPoint[EotSPoints.PlayersOutOfPoints].Clear();
		}
		...
		private void EventTeamLostPoint(Player player, int Point)
		{
			...
			if (Team == Team.Alliance)
			{
				...
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeAlliance, BattlegroundConst.RespawnOneDay);
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeAlliance + 1, BattlegroundConst.RespawnOneDay);
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeAlliance + 2, BattlegroundConst.RespawnOneDay);
			}
			else
			{
				_TeamPointsCount[TeamId.Horde]--;
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeHorde, BattlegroundConst.RespawnOneDay);
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeHorde + 1, BattlegroundConst.RespawnOneDay);
				SpawnBGObject(EotSMisc._LosingPointTypes[Point].DespawnObjectTypeHorde + 2, BattlegroundConst.RespawnOneDay);
			}

			SpawnBGObject(EotSMisc._LosingPointTypes[Point].SpawnNeutralObjectType, BattlegroundConst.RespawnImmediately);
			SpawnBGObject(EotSMisc._LosingPointTypes[Point].SpawnNeutralObjectType + 1, BattlegroundConst.RespawnImmediately);
			SpawnBGObject(EotSMisc._LosingPointTypes[Point].SpawnNeutralObjectType + 2, BattlegroundConst.RespawnImmediately);
			...
			if (Team == Team.Alliance)
				SendBroadcastText(EotSMisc._LosingPointTypes[Point].MessageIdAlliance, ChatMsg.BgSystemAlliance, player);
			else
				SendBroadcastText(EotSMisc._LosingPointTypes[Point].MessageIdHorde, ChatMsg.BgSystemHorde, player);
		...
		private void EventTeamCapturedPoint(Player player, int Point)
		{
			...
			SpawnBGObject(EotSMisc._CapturingPointTypes[Point].DespawnNeutralObjectType, BattlegroundConst.RespawnOneDay);
			SpawnBGObject(EotSMisc._CapturingPointTypes[Point].DespawnNeutralObjectType + 1, BattlegroundConst.RespawnOneDay);
			SpawnBGObject(EotSMisc._CapturingPointTypes[Point].DespawnNeutralObjectType + 2, BattlegroundConst.RespawnOneDay);

			if (Team == Team.Alliance)
			{
				_TeamPointsCount[TeamId.Alliance]++;
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeAlliance, BattlegroundConst.RespawnImmediately);
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeAlliance + 1, BattlegroundConst.RespawnImmediately);
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeAlliance + 2, BattlegroundConst.RespawnImmediately);
			}
			else
			{
				_TeamPointsCount[TeamId.Horde]++;
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeHorde, BattlegroundConst.RespawnImmediately);
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeHorde + 1, BattlegroundConst.RespawnImmediately);
				SpawnBGObject(EotSMisc._CapturingPointTypes[Point].SpawnObjectTypeHorde + 2, BattlegroundConst.RespawnImmediately);
			}
			...
			if (Team == Team.Alliance)
				SendBroadcastText(EotSMisc._CapturingPointTypes[Point].MessageIdAlliance, ChatMsg.BgSystemAlliance, player);
			else
				SendBroadcastText(EotSMisc._CapturingPointTypes[Point].MessageIdHorde, ChatMsg.BgSystemHorde, player);

			if (!BgCreatures[Point].IsEmpty())
				DelCreature(Point);

			WorldSafeLocsEntry sg = Global.ObjectMgr.GetWorldSafeLoc(EotSMisc._CapturingPointTypes[Point].GraveYardId);
			...
			if (sg == null ||
				...
				Log.outError(LogFilter.Battleground,
				             ...
				             Point,
				             Team,
				             EotSMisc._CapturingPointTypes[Point].GraveYardId);
		...
		public override WorldSafeLocsEntry GetClosestGraveYard(Player player)
		{
			...
			for (byte i = 0; i < EotSPoints.PointsMax; ++i)
				...
				{
					entry = Global.ObjectMgr.GetWorldSafeLoc(EotSMisc._CapturingPointTypes[i].GraveYardId);

					if (entry == null)
					{
						Log.outError(LogFilter.Battleground, "BattlegroundEY: Graveyard {0} could not be found.", EotSMisc._CapturingPointTypes[i].GraveYardId);
					}
					else
					{
	...
	internal struct EotSMisc
	{
		...
		public static BattlegroundEYPointIconsStruct[] _PointsIconStruct =
		{
		...
		public static BattlegroundEYLosingPointStruct[] _LosingPointTypes =
		{
		...
		public static BattlegroundEYCapturingPointStruct[] _CapturingPointTypes =
		{
