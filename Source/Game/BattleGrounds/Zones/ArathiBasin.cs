...
namespace Game.BattleGrounds.Zones
{
	internal class BgArathiBasin : Battleground
	{
		...
		private BannerTimer[] _BannerTimers = new BannerTimer[ABBattlegroundNodes.DynamicNodesCount];
		private uint[] _HonorScoreTics = new uint[SharedConst.PvpTeamsCount];
		private uint _HonorTics;
		private bool _IsInformedNearVictory;
		private uint[] _lastTick = new uint[SharedConst.PvpTeamsCount];
        ...
        ///  4: horde occupied
        /// </summary>
        private ABNodeStatus[] _Nodes = new ABNodeStatus[ABBattlegroundNodes.DynamicNodesCount];

		private uint[] _NodeTimers = new uint[ABBattlegroundNodes.DynamicNodesCount];
		private ABNodeStatus[] _prevNodes = new ABNodeStatus[ABBattlegroundNodes.DynamicNodesCount];
		private uint[] _ReputationScoreTics = new uint[SharedConst.PvpTeamsCount];
		...
		public override void PostUpdateImpl(uint diff)
		{
			if (GetStatus() == BattlegroundStatus.InProgress)
			{
				...
				for (int team = 0; team < SharedConst.PvpTeamsCount; ++team)
				{
					...
					if (_lastTick[team] > TickIntervals[points])
					{
						...
						if (!_IsInformedNearVictory &&
						    _TeamScores[team] > WarningNearVictoryScore)
						{
							if (team == TeamId.Alliance)
							{
						...
						if (team == TeamId.Alliance)
							UpdateWorldState(ABWorldStates.ResourcesAlly, (int)_TeamScores[team]);
						else
				...
				// Test win condition
				if (_TeamScores[TeamId.Alliance] >= MaxTeamScore)
					EndBattleground(Team.Alliance);
				else if (_TeamScores[TeamId.Horde] >= MaxTeamScore)
		...
		private void _SendNodeUpdate(byte node)
		{
			...
			switch (node)
			{
				case ABBattlegroundNodes.NodeStables:
					UpdateWorldState(ABWorldStates.StablesIconNew, (int)_Nodes[node] + statePlusArray[(int)_Nodes[node]]);
					UpdateWorldState(ABWorldStates.StablesHordeControlState, _Nodes[node] == ABNodeStatus.HordeOccupied ? 2 : (_Nodes[node] == ABNodeStatus.HordeContested ? 1 : 0));
					UpdateWorldState(ABWorldStates.StablesAllianceControlState, _Nodes[node] == ABNodeStatus.AllyOccupied ? 2 : (_Nodes[node] == ABNodeStatus.AllyContested ? 1 : 0));

					break;
				case ABBattlegroundNodes.NodeBlacksmith:
					UpdateWorldState(ABWorldStates.BlacksmithIconNew, (int)_Nodes[node] + statePlusArray[(int)_Nodes[node]]);
					UpdateWorldState(ABWorldStates.BlacksmithHordeControlState, _Nodes[node] == ABNodeStatus.HordeOccupied ? 2 : (_Nodes[node] == ABNodeStatus.HordeContested ? 1 : 0));
					UpdateWorldState(ABWorldStates.BlacksmithAllianceControlState, _Nodes[node] == ABNodeStatus.AllyOccupied ? 2 : (_Nodes[node] == ABNodeStatus.AllyContested ? 1 : 0));

					break;
				case ABBattlegroundNodes.NodeFarm:
					UpdateWorldState(ABWorldStates.FarmIconNew, (int)_Nodes[node] + statePlusArray[(int)_Nodes[node]]);
					UpdateWorldState(ABWorldStates.FarmHordeControlState, _Nodes[node] == ABNodeStatus.HordeOccupied ? 2 : (_Nodes[node] == ABNodeStatus.HordeContested ? 1 : 0));
					UpdateWorldState(ABWorldStates.FarmAllianceControlState, _Nodes[node] == ABNodeStatus.AllyOccupied ? 2 : (_Nodes[node] == ABNodeStatus.AllyContested ? 1 : 0));

					break;
				case ABBattlegroundNodes.NodeLumberMill:
					UpdateWorldState(ABWorldStates.LumberMillIconNew, (int)_Nodes[node] + statePlusArray[(int)_Nodes[node]]);
					UpdateWorldState(ABWorldStates.LumberMillHordeControlState, _Nodes[node] == ABNodeStatus.HordeOccupied ? 2 : (_Nodes[node] == ABNodeStatus.HordeContested ? 1 : 0));
					UpdateWorldState(ABWorldStates.LumberMillAllianceControlState, _Nodes[node] == ABNodeStatus.AllyOccupied ? 2 : (_Nodes[node] == ABNodeStatus.AllyContested ? 1 : 0));

					break;
				case ABBattlegroundNodes.NodeGoldMine:
					UpdateWorldState(ABWorldStates.GoldMineIconNew, (int)_Nodes[node] + statePlusArray[(int)_Nodes[node]]);
					UpdateWorldState(ABWorldStates.GoldMineHordeControlState, _Nodes[node] == ABNodeStatus.HordeOccupied ? 2 : (_Nodes[node] == ABNodeStatus.HordeContested ? 1 : 0));
					UpdateWorldState(ABWorldStates.GoldMineAllianceControlState, _Nodes[node] == ABNodeStatus.AllyOccupied ? 2 : (_Nodes[node] == ABNodeStatus.AllyContested ? 1 : 0));
		...
		public override void EventPlayerClickedOnFlag(Player source, GameObject target_obj)
		{
			...
			if (_Nodes[node] == ABNodeStatus.Neutral)
			{
				...
				_SendNodeUpdate(node);
				_NodeTimers[node] = FlagCapturingTime;
		...
		public override void Reset()
		{
			...
			_HonorTics      = (isBGWeekend) ? ABBGWeekendHonorTicks : NotABBGWeekendHonorTicks;
			_ReputationTics = (isBGWeekend) ? ABBGWeekendReputationTicks : NotABBGWeekendReputationTicks;
