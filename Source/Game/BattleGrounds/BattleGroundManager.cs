...
using Game.BattleGrounds.Zones.ArathiBasin;
using Game.BattleGrounds.Zones.EyeOfTheStorm;
...
namespace Game.BattleGrounds
{
	public class BattlegroundManager : Singleton<BattlegroundManager>
	{
		...
		private Dictionary<uint, BattlegroundTemplate> _battlegroundMapTemplates = new();
		private Dictionary<BattlegroundQueueTypeId, BattlegroundQueue> _BattlegroundQueues = new();
		private Dictionary<BattlegroundTypeId, BattlegroundTemplate> _battlegroundTemplates = new();
		private MultiMap<BattlegroundQueueTypeId, Battleground> _BGFreeSlotQueue = new();
		...
		private Dictionary<BattlegroundTypeId, BattlegroundData> bgDataStore = new();
		private Dictionary<uint, BattlegroundTypeId> mBattleMastersMap = new();

		private BattlegroundManager()
		{
			_NextRatedArenaUpdate = WorldConfig.GetUIntValue(WorldCfg.ArenaRatedUpdateTimer);
		}
		...
		public void DeleteAllBattlegrounds()
		{
			foreach (var data in bgDataStore.Values.ToList())
				while (!data._Battlegrounds.Empty())
					data._Battlegrounds.First().Value.Dispose();
		...
		public void Update(uint diff)
		{
			...
			if (_UpdateTimer > 1000)
			{
				foreach (var data in bgDataStore.Values)
				{
					...
					foreach (var pair in bgs.ToList())
					{
						...
						{
							bgs.Remove(pair.Key);
							var clients = data._ClientBattlegroundIds[(int)bg.GetBracketId()];
			    ...
			    WorldConfig.GetIntValue(WorldCfg.ArenaRatedUpdateTimer) != 0)
			{
				...
				if (_NextRatedArenaUpdate < diff)
				{
					...
					foreach (ArenaTypes teamSize in new[]
					                                {
		...
		public void BuildBattlegroundStatusFailed(out BattlefieldStatusFailed battlefieldStatus, BattlegroundQueueTypeId queueId, Player pPlayer, uint ticketId, GroupJoinBattlegroundResult result, ObjectGuid errorGuid = default)
		{
			...
			if (!errorGuid.IsEmpty() &&
			    (result == GroupJoinBattlegroundResult.NotInBattleground || result == GroupJoinBattlegroundResult.JoinTimedOut))
		...
		public Battleground GetBattleground(uint instanceId, BattlegroundTypeId bgTypeId)
		{
			...
			if (bgTypeId != BattlegroundTypeId.None ||
			    bgTypeId == BattlegroundTypeId.RB ||
			    bgTypeId == BattlegroundTypeId.RandomEpic)
			{
				var data = bgDataStore.LookupByKey(bgTypeId);

				return data._Battlegrounds.LookupByKey(instanceId);
			}

			foreach (var it in bgDataStore)
			{
				var bgs = it.Value._Battlegrounds;
		...
		private uint CreateClientVisibleInstanceId(BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id)
		{
			...
			var  clientIds = bgDataStore[bgTypeId]._ClientBattlegroundIds[(int)bracket_id];
			uint lastId    = 0;
		...
		public Battleground CreateNewBattleground(BattlegroundQueueTypeId queueId, PvpDifficultyRecord bracketEntry)
		{
			...
			if (bgTypeId == BattlegroundTypeId.RB ||
			    bgTypeId == BattlegroundTypeId.AA ||
			    bgTypeId == BattlegroundTypeId.RandomEpic)
				return null;
		...
		public void LoadBattlegroundTemplates()
		{
			...
			do
			{
				...
				if (bgTemplate.Id != BattlegroundTypeId.AA &&
				    bgTemplate.Id != BattlegroundTypeId.RB &&
				    bgTemplate.Id != BattlegroundTypeId.RandomEpic)
				{
		...
		{
			if (bg)
				bgDataStore[bg.GetTypeID()]._Battlegrounds[bg.GetInstanceID()] = bg;
		}

		public void RemoveBattleground(BattlegroundTypeId bgTypeId, uint instanceId)
		{
			bgDataStore[bgTypeId]._Battlegrounds.Remove(instanceId);
		}
	...
	{
		public Dictionary<uint, Battleground> _Battlegrounds = new();
		public List<uint>[] _ClientBattlegroundIds = new List<uint>[(int)BattlegroundBracketId.Max];
		public Battleground Template;

		public BattlegroundData()
		{
			for (var i = 0; i < (int)BattlegroundBracketId.Max; ++i)
				_ClientBattlegroundIds[i] = new List<uint>();
		}
	}
