...
namespace Game.BattleGrounds
{
	...
	{
		// Event handler
		private EventSystem _events = new();
        ...
        /// </summary>
        private List<GroupQueueInfo>[][] _QueuedGroups = new List<GroupQueueInfo>[(int)BattlegroundBracketId.Max][];

		private Dictionary<ObjectGuid, PlayerQueueInfo> _QueuedPlayers = new();

		private BattlegroundQueueTypeId _queueId;

		private SelectionPool[] _SelectionPools = new SelectionPool[SharedConst.PvpTeamsCount];
		private uint[][] _SumOfWaitTimes = new uint[SharedConst.PvpTeamsCount][];
		private uint[][] _WaitTimeLastPlayer = new uint[SharedConst.PvpTeamsCount][];

		private uint[][][] _WaitTimes = new uint[SharedConst.PvpTeamsCount][][];
    ...
    internal class BGQueueInviteEvent : BasicEvent
	{
		...
		private uint _BgInstanceGUID;
		private BattlegroundTypeId _BgTypeId;

		private ObjectGuid _PlayerGuid;
		private uint _RemoveTime;
