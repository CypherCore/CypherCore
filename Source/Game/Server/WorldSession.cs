...
namespace Game
{
	public partial class WorldSession : IDisposable
	{
		...
		public bool Update(uint diff, PacketFilter updater)
		{
			...
			if (IsConnectionIdle() &&
			    !HasPermission(RBACPermissions.IgnoreIdleConnection))
		...
		public void SendPacket(ServerPacket packet)
		{
			...
			if (packet.GetOpcode() == ServerOpcodes.Unknown ||
			    packet.GetOpcode() == ServerOpcodes.Max)
			{
			...
			if (conIdx != ConnectionType.Instance &&
			    PacketManager.IsInstanceOnlyOpcode(packet.GetOpcode()))
			{
		...
		{
			return !_playerLoading.IsEmpty();
		}

		public bool PlayerLogout()
		{
			return _playerLogout;
		}

		public bool PlayerLogoutWithSave()
		{
			return _playerLogout && _playerSave;
		}

		public bool PlayerRecentlyLoggedOut()
		{
			return _playerRecentlyLogout;
		}

		public bool PlayerDisconnected()
		{
			return !(_Socket[(int)ConnectionType.Realm] != null &&
			         _Socket[(int)ConnectionType.Realm].IsOpen() &&
			         _Socket[(int)ConnectionType.Instance] != null &&
			         _Socket[(int)ConnectionType.Instance].IsOpen());
		}

		public AccountTypes GetSecurity()
		{
			return _security;
		}
		...
		public ObjectGuid GetAccountGUID()
		{
		...
		private void SetSecurity(AccountTypes security)
		{
			_security = security;
		}
		...
		public Expansion GetAccountExpansion()
		{
			return _accountExpansion;
		}
		...
		{
			return _logoutTime != 0 || _playerLogout;
		}

		public ulong GetConnectToInstanceKey()
		{
			return _instanceConnectKey.Raw;
		}

		public AsyncCallbackProcessor<QueryCallback> GetQueryProcessor()
		{
		...
		{
			return _sessionDbcLocale;
		}

		public Locale GetSessionDbLocaleIndex()
		{
			return _sessionDbLocaleIndex;
		}
		...
		public uint GetRecruiterId()
		{
			return recruiterId;
		}

		public bool IsARecruiter()
		{
			return isRecruiter;
		}

		// Packets cooldown
		public long GetCalendarEventCreationCooldown()
		{
			return _calendarEventCreationCooldown;
		}

		public void SetCalendarEventCreationCooldown(long cooldown)
		{
			_calendarEventCreationCooldown = cooldown;
		}

		// Battle Pets
		public BattlePetMgr GetBattlePetMgr()
		{
			return _battlePetMgr;
		}

		public CollectionMgr GetCollectionMgr()
		{
			return _collectionMgr;
		}

		// Battlenet
		public Array<byte> GetRealmListSecret()
		{
			return _realmListSecret;
		}

		private void SetRealmListSecret(Array<byte> secret)
		{
			_realmListSecret = secret;
		}

		public Dictionary<uint, byte> GetRealmCharacterCounts()
		{
			return _realmCharacterCounts;
		}
		...
		private List<ObjectGuid> _legitCharacters = new();
		private ulong _GUIDLow;
		private Player _player;
		private WorldSocket[] _Socket = new WorldSocket[(int)ConnectionType.Max];
		private string _Address;

		private AccountTypes _security;
		private uint _accountId;
		private string _accountName;
		private uint _battlenetAccountId;
		private Expansion _accountExpansion;
		...
		private AccountData[] _accountData = new AccountData[(int)AccountDataTypes.Max];
		private uint[] tutorials = new uint[SharedConst.MaxAccountTutorialValues];
		private TutorialsFlag tutorialsChanged;

		private Array<byte> _realmListSecret = new(32);
		private Dictionary<uint /*realmAddress*/, byte> _realmCharacterCounts = new();
		private Dictionary<uint, Action<CodedInputStream>> _battlenetResponseCallbacks = new();
		private uint _battlenetRequestToken;

		private List<string> _registeredAddonPrefixes = new();
		private bool _filterAddonMessages;
		private uint recruiterId;
		private bool isRecruiter;

		public long _muteTime;
		private long _timeOutTime;

		private ConcurrentQueue<WorldPacket> _recvQueue = new();
		private RBACData _RBACData;

		private CircularBuffer<Tuple<long, uint>> _timeSyncClockDeltaQueue = new(6); // first member: clockDelta. Second member: latency of the packet exchange that was used to compute that clockDelta.
		private long _timeSyncClockDelta;

		private Dictionary<uint, uint> _pendingTimeSyncRequests = new(); // key: counter. value: server time when packet with that counter was sent.
		private uint _timeSyncNextCounter;
		private uint _timeSyncTimer;

		private CollectionMgr _collectionMgr;
		...
		private BattlePetMgr _battlePetMgr;

		private AsyncCallbackProcessor<QueryCallback> _queryProcessor = new();
		private AsyncCallbackProcessor<TransactionCallback> _transactionCallbacks = new();
		private AsyncCallbackProcessor<ISqlCallback> _queryHolderProcessor = new();
	...
	public struct ConnectToKey
	{
		public ulong Raw
		{
			get => ((ulong)AccountId | ((ulong)connectionType << 32) | (Key << 33));
			set
			{
	...
	{
		private Dictionary<uint, PacketCounter> _PacketThrottlingMap = new();
