        ...
        bool ModifierSatisfied(ModifierTreeRecord modifier, ulong miscValue1, ulong miscValue2, WorldObject refe, Player referencePlayer)
        {
            ...
            switch ((ModifierTreeType)modifier.Type)
            {
                ...
                case ModifierTreeType.PlayerLevelWithinContentTuning: // 268
                    {
                        ...
                        var levels = Global.DB2Mgr.GetContentTuningData(reqValue, []);
                        if (levels.HasValue)
                        {
                            if (secondaryAsset != 0)
                ...
                case ModifierTreeType.PlayerInChromieTimeForScaling: // 304
                    if (referencePlayer.m_playerData.CtrOptions.GetValue().ConditionalFlags.Empty() || (referencePlayer.m_playerData.CtrOptions.GetValue().ConditionalFlags[0] & 1) == 0)
                        return false;
                    break;
                ...
                case ModifierTreeType.PlayerMoneyIsRelOp: // 417                    
                    ...
                    {
                        case 1:
                            if (referencePlayer.GetMoney() <= (uint)secondaryAsset)
                                return false;
                            break;
                        case 2:
                            if (referencePlayer.GetMoney() >= (uint)secondaryAsset)
