...
namespace Game.Combat
{
	...
	{
		private Unit _owner;
		private Dictionary<ObjectGuid, CombatReference> _pveRefs = new();
		private Dictionary<ObjectGuid, PvPCombatReference> _pvpRefs = new();
		...
		public static bool CanBeginCombat(Unit a, Unit b)
		{
			...
			if (a.HasUnitState(UnitState.Evade) ||
			    b.HasUnitState(UnitState.Evade))
				return false;

			if (a.HasUnitState(UnitState.InFlight) ||
			    b.HasUnitState(UnitState.InFlight))
				return false;

			// ... both units must be allowed to enter combat
			if (a.IsCombatDisallowed() ||
			    b.IsCombatDisallowed())
				return false;
			...
			if ((playerA && playerA.IsGameMaster()) ||
			    (playerB && playerB.IsGameMaster()))
				return false;
		...
		public bool HasPvECombatWithPlayers()
		{
			foreach (var reference in _pveRefs)
				if (!reference.Value.IsSuppressedFor(_owner) &&
				    reference.Value.GetOther(_owner).IsPlayer())
					return true;
		...
		public bool SetInCombatWith(Unit who, bool addSecondUnitSuppressed = false)
		{
			...
			if (_owner.IsControlledByPlayer() &&
			    who.IsControlledByPlayer())
		...
		public Dictionary<ObjectGuid, CombatReference> GetPvECombatRefs()
		{
			return _pveRefs;
		}

		public Dictionary<ObjectGuid, PvPCombatReference> GetPvPCombatRefs()
		{
			return _pvpRefs;
		}
