...
namespace Game.Movement
{
	public class FlightPathMovementGenerator : MovementGeneratorMedium<Player>
	{
		...
		private List<TaxiPathNodeRecord> _path = new();
		private List<TaxiNodeChangeInfo> _pointsForPathSwitch = new(); //! node indexes and costs where TaxiPath changes
			...
			for (int src = 0, dst = 1; dst < taxi.Count; src = dst++)
			{
				...
				if (!nodes.Empty())
				{
					...
					for (uint i = 0; i < nodes.Length; ++i)
						if (passedPreviousSegmentProximityCheck ||
						    src == 0 ||
						    _path.Empty() ||
						    IsNodeIncludedInShortenedPath(_path.Last(), nodes[i]))
						{
