...
namespace Game.Movement
{
	public class PathGenerator
	{
		...
		private Detour.dtQueryFilter _filter = new();
		...
		public bool CalculatePath(float destX, float destY, float destZ, bool forceDest = false)
		{
			...
			if (!GridDefines.IsValidMapCoord(destX, destY, destZ) ||
			    !GridDefines.IsValidMapCoord(x, y, z))
				return false;
			...
			if (_navMesh == null ||
			    _navMeshQuery == null ||
			    (_sourceUnit != null && _sourceUnit.HasUnitState(UnitState.IgnorePathfinding)) ||
			    !HaveTile(start) ||
			    !HaveTile(dest))
			{
				BuildShortcut();
		...
		private ulong GetPolyByLocation(float[] point, ref float distance)
		{
			...
			if (Detour.dtStatusSucceed(_navMeshQuery.findNearestPoly(point, extents, _filter, ref polyRef, ref closestPoint)) &&
			    polyRef != 0)
			{
		...
		private void BuildPolyPath(Vector3 startPos, Vector3 endPos)
		{
			...
			float[] startPoint =
			{
				startPos.Y, startPos.Z, startPos.X
			};

			float[] endPoint =
			{
				endPos.Y, endPos.Z, endPos.X
			};
		...
		private void UpdateFilter()
		{
			...
			if (_sourceUnit != null)
			{
				if (_sourceUnit.IsInWater() ||
				    _sourceUnit.IsUnderWater())
				{
