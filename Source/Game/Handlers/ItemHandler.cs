...
namespace Game
{
	public partial class WorldSession
	{
		...
		private void HandleSwapInvenotryItem(SwapInvItem swapInvItem)
		{
			...
			if (Player.IsBankPos(InventorySlots.Bag0, swapInvItem.Slot1) &&
			    !CanUseBank())
			{
			...
			if (Player.IsBankPos(InventorySlots.Bag0, swapInvItem.Slot2) &&
			    !CanUseBank())
			{
				Log.outDebug(LogFilter.Network, $"WORLD: HandleSwapInvItemOpcode - {_player.PlayerTalkClass.GetInteractionData().SourceGuid} not found or you can't interact with him.");
		...
		private void HandleAutoEquipItemSlot(AutoEquipItemSlot packet)
		{
			...
			if (packet.Inv.Items.Count != 1 ||
			    !Player.IsEquipmentPos(InventorySlots.Bag0, packet.ItemDstSlot))
				return;
		...
		private void HandleSwapItem(SwapItem swapItem)
		{
			...
			if (Player.IsBankPos(swapItem.ContainerSlotA, swapItem.SlotA) &&
			    !CanUseBank())
			{
				Log.outDebug(LogFilter.Network, $"WORLD: HandleSwapInvItemOpcode - {_player.PlayerTalkClass.GetInteractionData().SourceGuid} not found or you can't interact with him.");
			...
			if (Player.IsBankPos(swapItem.ContainerSlotB, swapItem.SlotB) &&
			    !CanUseBank())
			{
				Log.outDebug(LogFilter.Network, $"WORLD: HandleSwapInvItemOpcode - {_player.PlayerTalkClass.GetInteractionData().SourceGuid} not found or you can't interact with him.");
		...
		private void HandleAutoEquipItem(AutoEquipItem autoEquipItem)
		{
			...
			else // have currently equipped item, not simple case
			{
				...
				if (!dstItem.HasItemFlag(ItemFieldFlags.Child))
				{
					...
					if (msg == InventoryResult.Ok &&
					    Player.IsEquipmentPos(dest) &&
					    !srcItem.GetChildItem().IsEmpty())
		...
		private void HandleReadItem(ReadItem readItem)
		{
			...
			if (item != null &&
			    item.GetTemplate().GetPageText() != 0)
			{
		...
		private void HandleBuybackItem(BuyBackItem packet)
		{
			...
			if (pItem != null)
			{
				uint price = _player._activePlayerData.BuybackPrice[(int)(packet.Slot - InventorySlots.BuyBackStart)];
		...
		private void HandleWrapItem(WrapItem packet)
		{
			...
			stmt.AddValue(3, (uint)item._itemData.DynamicFlags);
			trans.Append(stmt);
		...
		private void HandleSocketGems(SocketGems socketGems)
		{
			...
			for (uint i = 0; i < ItemConst.MaxGemSockets; ++i) //check for hack maybe
			{
				...
				if (ItemConst.SocketColorToGemTypeMask[(int)itemTarget.GetSocketColor(i)] != gemProperties[i].Type)
					...
					if (!ItemConst.SocketColorToGemTypeMask[(int)itemTarget.GetSocketColor(i)].HasAnyFlag(SocketColor.Prismatic) ||
					    !gemProperties[i].Type.HasAnyFlag(SocketColor.Prismatic))
						return;
			}
			...
			for (int i = 0; i < ItemConst.MaxGemSockets; ++i)
			{
				...
				if (iGemProto.GetItemLimitCategory() != 0)
				{
					...
					if (limitEntry != null)
					{
						...
						if (limit_newcount > 0 &&
						    limit_newcount > _player.GetItemLimitCategoryQuantity(limitEntry))
						{
							GetPlayer().SendEquipError(InventoryResult.ItemUniqueEquippableSocketed, itemTarget);
