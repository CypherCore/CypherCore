...
namespace Game
{
	public partial class WorldSession
	{
		...
		private void HandleCharEnum(EnumCharactersQueryHolder holder)
		{
			...
			foreach (var requirement in Global.ObjectMgr.GetRaceUnlockRequirements())
			{
				...
				raceUnlock.HasAchievement = requirement.Value.AchievementId != 0 &&
				                            (WorldConfig.GetBoolValue(WorldCfg.CharacterCreatingDisableAlliedRaceAchievementRequirement)
		...
		public bool MeetsChrCustomizationReq(ChrCustomizationReqRecord req, Class playerClass, bool checkRequiredDependentChoices, List<ChrCustomizationChoice> selectedChoices)
		{
			...
			if (req.ClassMask != 0 &&
			    (req.ClassMask & (1 << ((int)playerClass - 1))) == 0)
				return false;

			if (req.AchievementID != 0 /*&& !HasAchieved(req->AchievementID)*/)
				return false;

			if (req.ItemModifiedAppearanceID != 0 &&
			    !GetCollectionMgr().HasItemAppearance(req.ItemModifiedAppearanceID).PermAppearance)
				return false;
		...
		private void HandleCharCreate(CreateCharacter charCreate)
		{
			...
			{
				if (raceClassExpansionRequirement.ActiveExpansionLevel > (byte)GetExpansion() ||
				    raceClassExpansionRequirement.AccountExpansionLevel > (byte)GetAccountExpansion())
				{
					Log.outError(LogFilter.Cheat,
					             $"Account:[{GetAccountId()}] tried to create character with race/class {charCreate.CreateInfo.RaceId}/{charCreate.CreateInfo.ClassId} without required expansion " +
			...
			else
			{
				...
				{
					if (classExpansionRequirement.MinActiveExpansionLevel > (byte)GetExpansion() ||
					    classExpansionRequirement.AccountExpansionLevel > (byte)GetAccountExpansion())
					{
						Log.outError(LogFilter.Cheat,
						             $"Account:[{GetAccountId()}] tried to create character with race/class {charCreate.CreateInfo.RaceId}/{charCreate.CreateInfo.ClassId} without required expansion " +
			...
			if (!HasPermission(RBACPermissions.SkipCheckCharacterCreationReservedname) &&
			    Global.ObjectMgr.IsReservedName(charCreate.CreateInfo.Name))
			{
			                              ...
			                              .WithChainingCallback((queryCallback, result) =>
			                                                    {
				                                                    ...
				                                                    bool checkClassLevelReqs    = (createInfo.ClassId == Class.DemonHunter || createInfo.ClassId == Class.Evoker) && !HasPermission(RBACPermissions.SkipCheckCharacterCreationDemonHunter);
				                                                    ...
				                                                    void finalizeCharacterCreation(SQLResult result1)
				                                                    {
					                                                    ...
					                                                    if (createInfo.ClassId == Class.Evoker &&
					                                                        hasEvokerLimit &&
					                                                        evokerLimit < 1)
					                                                    {
						                                                    SendCharCreate(ResponseCodes.CharCreateNewPlayer);
				                                                    ...
				                                                    if (!allowTwoSideAccounts ||
				                                                        skipCinematics == 1 ||
				                                                        createInfo.ClassId == Class.DemonHunter)
				                                                    {
		...
		public void HandleContinuePlayerLogin()
		{
			...
			LoginQueryHolder holder = new(GetAccountId(), _playerLoading);
			holder.Initialize();
		...
		public void HandlePlayerLogin(LoginQueryHolder holder)
		{
				...
				if (playerInfo != null)
					switch (pCurrChar.GetCreateMode())
					{
						case PlayerCreateMode.Normal:
							...
							else if (CliDB.ChrRacesStorage.TryGetValue((uint)pCurrChar.GetRace(), out ChrRacesRecord chrRacesRecord) &&
							         chrRacesRecord.CinematicSequenceID != 0)
			...
			if (Global.WorldMgr.IsFFAPvPRealm() &&
			    !pCurrChar.IsGameMaster() &&
			    !pCurrChar.HasPlayerFlag(PlayerFlags.Resting))
			...
			if (!pCurrChar.IsStandState() &&
			    !pCurrChar.HasUnitState(UnitState.Stunned))
		...
		private void HandleCharRename(CharacterRenameRequest request)
		{
			...
			if (!HasPermission(RBACPermissions.SkipCheckCharacterCreationReservedname) &&
			    Global.ObjectMgr.IsReservedName(request.RenameInfo.NewName))
			{
		...
		private void HandleCharCustomizeCallback(CharCustomizeInfo customizeInfo, SQLResult result)
		{
			...
			if (WorldConfig.GetBoolValue(WorldCfg.PreventRenameCustomization) &&
			    (customizeInfo.CharName != oldName))
			{
				...
				var validateIllusion = new Func<uint, bool>(enchantId =>
				                                            {
					                                            ...
					                                            if (illusion.ItemVisual == 0 ||
					                                                !illusion.GetFlags().HasFlag(SpellItemEnchantmentFlags.AllowTransmog))
						                                            return false;
					                                            ...
					                                            if (illusion.ScalingClassRestricted > 0 &&
					                                                illusion.ScalingClassRestricted != (byte)_player.GetClass())
						                                            return false;

					                                            return true;
				                                            });

				if (saveEquipmentSet.Set.Enchants[0] != 0 &&
				    !validateIllusion((uint)saveEquipmentSet.Set.Enchants[0]))
					return;

				if (saveEquipmentSet.Set.Enchants[1] != 0 &&
				    !validateIllusion((uint)saveEquipmentSet.Set.Enchants[1]))
					return;
			}
		...
		private void HandleUseEquipmentSet(UseEquipmentSet useEquipmentSet)
		{
			...
			for (byte i = 0; i < EquipmentSlot.End; ++i)
			{
				...
				if (GetPlayer().IsInCombat() &&
				    i != EquipmentSlot.MainHand &&
				    i != EquipmentSlot.OffHand)
					continue;
		...
		private void HandleCharRaceOrFactionChangeCallback(CharRaceOrFactionChangeInfo factionChangeInfo, SQLResult result)
		{
			...
			// prevent character rename
			if (WorldConfig.GetBoolValue(WorldCfg.PreventRenameCustomization) &&
			    (factionChangeInfo.Name != oldName))
			{
			...
			if (oldRace != factionChangeInfo.RaceID)
			{
				...
				if (factionChangeInfo.RaceID != Race.Orc &&
				    factionChangeInfo.RaceID != Race.Human)
				{
					stmt = DB.Characters.GetPreparedStatement(CharStatements.INS_CHAR_SKILL_LANGUAGE);
					stmt.AddValue(0, lowGuid);
				...
				if (factionChangeInfo.FactionChange)
				{
					...
					// Disable all old-faction specific quests
					{
						...
						foreach (Quest quest in questTemplates.Values)
						{
							...
							if (quest.AllowableRaces != -1 &&
							    !Convert.ToBoolean(quest.AllowableRaces & newRaceMask))
							{
			                              ...
			                              .WithChainingCallback((queryCallback, result) =>
			                                                    {
				                                                    if (!result.IsEmpty())
				                                                    {
					                                                    ...
					                                                    if (lastUndelete != 0 &&
					                                                        (lastUndelete + maxCooldown > GameTime.GetGameTime()))
					                                                    {
		...
		private void HandleRepopRequest(RepopRequest packet)
		{
			if (GetPlayer().IsAlive() ||
			    GetPlayer().HasPlayerFlag(PlayerFlags.Ghost))
				return;
	...
	public class LoginQueryHolder : SQLQueryHolder<PlayerLoginQueryLoad>
	{
		...
		{
			ulong lowGuid = _guid.GetCounter();
