	...
	public partial class WorldSession
	{
		...
		private void HandleTraitsCommitConfig(TraitsCommitConfig traitsCommitConfig)
		{
			...
			if (_player.GetBattleground() &&
			    _player.GetBattleground().GetStatus() == BattlegroundStatus.InProgress)
			{
			...
			foreach (TraitEntryPacket newEntry in traitsCommitConfig.Config.Entries)
			{
				    ...
				    traitEntry.Rank > newEntry.Rank)
				{
					...
					if (traitDefinition.SpellID != 0 &&
					    _player.GetSpellHistory().HasCooldown(traitDefinition.SpellID))
					{
					...
					if (traitDefinition.VisibleSpellID != 0 &&
					    _player.GetSpellHistory().HasCooldown((uint)traitDefinition.VisibleSpellID))
					{
		...
		private void HandleClassTalentsRequestNewConfig(ClassTalentsRequestNewConfig classTalentsRequestNewConfig)
		{
			...
			long configCount = _player._activePlayerData.TraitConfigs._values.Count(traitConfig => { return (TraitConfigType)(int)traitConfig.Type == TraitConfigType.Combat && ((TraitCombatConfigFlags)(int)traitConfig.CombatConfigFlags & TraitCombatConfigFlags.ActiveForSpec) == TraitCombatConfigFlags.None; });
			...
			int findFreeLocalIdentifier()
			{
				...
				while (_player._activePlayerData.TraitConfigs.FindIndexIf(traitConfig => { return (TraitConfigType)(int)traitConfig.Type == TraitConfigType.Combat && traitConfig.ChrSpecializationID == _player.GetPrimarySpecialization() && traitConfig.LocalIdentifier == index; }) >= 0)
					++index;
		...
		private void HandleClassTalentsSetStarterBuildActive(ClassTalentsSetStarterBuildActive classTalentsSetStarterBuildActive)
		{
			...
			if (classTalentsSetStarterBuildActive.Active)
			{
				...
				while (_player._activePlayerData.TraitConfigs.FindIndexIf(traitConfig => { return (TraitConfigType)(int)traitConfig.Type == TraitConfigType.Combat && traitConfig.ChrSpecializationID == _player.GetPrimarySpecialization() && traitConfig.LocalIdentifier == freeLocalIdentifier; }) >= 0)
					++freeLocalIdentifier;
