...
namespace Game
{
    public partial class WorldSession
    {
        ...
        private void HandleDismissCritter(DismissCritter packet)
        {
            ...
            if (GetPlayer().GetCritterGUID() == pet.GetGUID())
                if (pet.IsCreature() &&
                    pet.IsSummon())
                {
                    if (!_player.GetSummonedBattlePetGUID().IsEmpty() &&
                        _player.GetSummonedBattlePetGUID() == pet.GetBattlePetCompanionGUID())
                        ...
                        case CommandStates.Attack: // spellid = 1792  //ATTACK
                            {
                                    ...
                                    !pet.GetCharmInfo().IsCommandAttack())
                                {
                                        ...
                                        pet.ToCreature().IsAIEnabled())
                                    {
                                        ...
                                        if (pet.IsPet() &&
                                            pet.ToPet().GetPetType() == PetType.Summon &&
                                            pet != TargetUnit &&
                                            RandomHelper.IRand(0, 100) < 10)
        ...
        private bool CheckStableMaster(ObjectGuid guid)
        {
            ...
            if (guid == GetPlayer().GetGUID())
            {
                if (!GetPlayer().IsGameMaster() &&
                    !GetPlayer().HasAuraType(AuraType.OpenStable))
                {
        ...
        private void HandlePetSetAction(PetSetAction packet)
        {
            ...
            if (!pet ||
                pet != GetPlayer().GetFirstControlled())
            {
            ...
            foreach (Unit petControlled in pets)
                ...
                if (!((act_state == ActiveStates.Enabled || act_state == ActiveStates.Disabled || act_state == ActiveStates.Passive) && spell_id != 0 && !petControlled.HasSpell(spell_id)))
                {
                    ...
                    if (spellInfo != null)
                    {
                        ...
                        {
                            if (petControlled.GetTypeId() == TypeId.Unit &&
                                petControlled.IsPet())
        ...
        private void HandlePetRename(PetRename packet)
        {
            ...
            if (!pet ||
                !pet.IsPet() ||
                pet.ToPet().GetPetType() != PetType.Hunter ||
        ...
        private void HandlePetSpellAutocast(PetSpellAutocast packet)
        {
            ...
            foreach (Unit petControlled in pets)
            {
                ...
                if (!petControlled.HasSpell(packet.SpellID) ||
        ...
        private void HandlePetCastSpell(PetCastSpell petCastSpell)
        {
            ...
            if (caster != GetPlayer().GetGuardianPet() &&
                caster != GetPlayer().GetCharmed())
            {
            ...
            if (result == SpellCastResult.SpellCastOk)
            {
                ...
                if (creature)
                {
                    ...
                    if (pet)
                    {
                        // 10% chance to play special pet attack talk, else growl
                        // actually this only seems to happen on special spells, fire shield for imp, torment for voidwalker, but it's stupid to check every spell
                        if (pet.GetPetType() == PetType.Summon &&
                            (RandomHelper.IRand(0, 100) < 10))
                            pet.SendPetTalk(PetTalk.SpecialSpell);
                        else
        ...
        private void HandleRequestPetInfo(RequestPetInfo requestPetInfo)
        {
            ...
            else
            {
                ...
                if (charm != null)
                {
                    ...
                    else if (charm.HasUnitFlag(UnitFlags.PlayerControlled) &&
                             charm.HasUnitFlag(UnitFlags.Possessed))
