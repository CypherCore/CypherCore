...
namespace Game.Entities
{
	public class Item : WorldObject
	{
		...
		{
			uint duration = _itemData.Expiration;
		...
		public virtual void SaveToDB(SQLTransaction trans)
		{
			...
			switch (uState)
			{
				...
				case ItemUpdateState.Changed:
				{
					...
					stmt.AddValue(++index, (uint)_itemData.Expiration);

					StringBuilder ss = new();

					for (byte i = 0; i < _itemData.SpellCharges.GetSize() && i < _bonusData.EffectCount; ++i)
						ss.AppendFormat("{0} ", GetSpellCharges(i));

					stmt.AddValue(++index, ss.ToString());
					stmt.AddValue(++index, (uint)_itemData.DynamicFlags);
					...
					for (EnchantmentSlot slot = 0; slot < EnchantmentSlot.Max; ++slot)
					{
						...
						if (enchantment != null &&
						    !enchantment.GetFlags().HasFlag(SpellItemEnchantmentFlags.DoNotSaveToDB))
					...
					stmt.AddValue(++index, (uint)_itemData.Durability);
					stmt.AddValue(++index, (uint)_itemData.CreatePlayedTime);
					...
					stmt.AddValue(++index, (byte)_itemData.Context);
					...
					if ((uState == ItemUpdateState.Changed) &&
					    IsWrapped())
					{
					...
					if (_itemData.Gems.Size() != 0)
					{
						...
						foreach (SocketedGem gemData in _itemData.Gems)
						{
							if (gemData.ItemId != 0)
							{
		...
		public virtual bool LoadFromDB(ulong guid, ObjectGuid ownerGuid, SQLFields fields, uint entry)
		{
			...
			if (IsSoulBound() &&
			    GetBonding() == ItemBondingType.None)
			{
								...
								case ItemEnchantmentType.ArtifactPowerBonusRankPicker:
									if (_bonusData.GemRelicType[e - EnchantmentSlot.Sock1] != -1)
									{
										...
										if (artifactPowerPicker != null)
										{
											...
											if (playerCondition == null ||
											    (owner != null && ConditionManager.IsPlayerMeetingCondition(owner, playerCondition)))
		...
		public bool CanBeTraded(bool mail = false, bool trade = false)
		{
			...
			if ((!mail || !IsBoundAccountWide()) &&
			    (IsSoulBound() && (!IsBOPTradeable() || !trade)))
				return false;

			if (IsBag() &&
			    (Player.IsBagPos(GetPos()) || !ToBag().IsEmpty()))
				return false;
		...
		{
			uint maxDurability = _itemData.MaxDurability;

			if (maxDurability == 0)
				return 0;

			uint curDurability = _itemData.Durability;
		...
		{
			var list = (List<SocketedGem>)_itemData.Gems;
		...
		public uint GetPlayedTime()
		{
			...
			return _itemData.CreatePlayedTime + elapsed;
		}
		...
		public bool CheckSoulboundTradeExpire()
		{
			...
			if (_itemData.CreatePlayedTime + 2 * Time.Hour < GetOwner().GetTotalPlayedTime())
			{
		...
		public static uint GetItemLevel(ItemTemplate itemTemplate, BonusData bonusData, uint level, uint fixedLevel, uint minItemLevel, uint minItemLevelCutoff, uint maxItemLevel, bool pvpBonus, uint azeriteLevel)
		{
			...
			{
				if (minItemLevel != 0 &&
				    (minItemLevelCutoff == 0 || itemLevelBeforeUpgrades >= minItemLevelCutoff) &&
		...
		{
			if (itemTemplate.HasFlag(ItemFlags.Conjured) ||
			    itemTemplate.HasFlag(ItemFlags.NoDisenchant) ||
			    itemTemplate.GetBonding() == ItemBondingType.Quest)
				return null;

			if (itemTemplate.GetArea(0) != 0 ||
			    itemTemplate.GetArea(1) != 0 ||
			    itemTemplate.GetMap() != 0 ||
			    itemTemplate.GetMaxStackSize() > 1)
				return null;

			if (GetSellPrice(itemTemplate, quality, itemLevel) == 0 &&
			    !Global.DB2Mgr.HasItemCurrencyCost(itemTemplate.GetId()))
				return null;
			...
			foreach (ItemDisenchantLootRecord disenchant in CliDB.ItemDisenchantLootStorage.Values)
			{
				...
				if (disenchant.MinLevel > itemLevel ||
				    disenchant.MaxLevel < itemLevel)
					continue;

				if (disenchant.ExpansionID != -2 &&
				    disenchant.ExpansionID != expansion)
					continue;
		...
		{
			return _itemData.ItemBonusKey.GetValue().BonusListIDs;
		}
		...
		public ArtifactPower GetArtifactPower(uint artifactPowerId)
		{
			...
			if (index != 0)
				return _itemData.ArtifactPowers[index];
		...
		public uint GetTotalUnlockedArtifactPowers()
		{
			...
			ulong artifactXp          = _itemData.ArtifactXP;
		...
		public uint GetTotalPurchasedArtifactPowers()
		{
			...
			foreach (ArtifactPower power in _itemData.ArtifactPowers)
		...
		public void GiveArtifactXp(ulong amount, Item sourceItem, ArtifactCategory artifactCategoryId)
		{
			...
			if (artifactCategoryId != 0)
			{
				...
				if (sourceItem != null &&
				    sourceItem.GetModifier(ItemModifier.ArtifactKnowledgeLevel) != 0)
			...
			SetArtifactXP(_itemData.ArtifactXP + amount);
		...
		{
			return (ItemContext)(int)_itemData.Context;
		}
		...
		{
			if (!_bonusData.HasFixedLevel ||
			    GetModifier(ItemModifier.TimewalkerLevel) != 0)
				return;
		...
		public int GetRequiredLevel()
		{
			...
			if (_bonusData.HasFixedLevel &&
			    _bonusData.PlayerLevelToItemLevelCurveId != 0)
				return fixedLevel;
		...
		public static void AddItemsSetItem(Player player, Item item)
		{
			...
			if (set.RequiredSkill != 0 &&
			    player.GetSkillValue((SkillType)set.RequiredSkill) < set.RequiredSkillRank)
				return;
			...
			foreach (var itemSetSpell in itemSetSpells)
			{
				...
				if (itemSetSpell.ChrSpecID == 0 ||
				    itemSetSpell.ChrSpecID == player.GetPrimarySpecialization())
		...
		public static void RemoveItemsSetItem(Player player, Item item)
		{
			...
			for (; setindex < player.ItemSetEff.Count; setindex++)
				if (player.ItemSetEff[setindex] != null &&
				    player.ItemSetEff[setindex].ItemSetID == setid)
				{
		...
		{
			return (_itemData.DynamicFlags & (uint)flag) != 0;
		}
		...
		{
			return (_itemData.DynamicFlags2 & (uint)flag) != 0;
		}
		...
		{
			return _itemData.MaxDurability > 0 && _itemData.Durability == 0;
		}
		...
		{
			return _itemData.Enchantment[(int)slot].ID;
		}

		public uint GetEnchantmentDuration(EnchantmentSlot slot)
		{
			return _itemData.Enchantment[(int)slot].Duration;
		}

		public int GetEnchantmentCharges(EnchantmentSlot slot)
		{
			return _itemData.Enchantment[(int)slot].Charges;
		}
		...
		{
			return _itemData.SpellCharges[index];
		}
		...
		{
			return _itemData.ItemAppearanceModID;
		}
		...
		{
			return _bonusData.ContentTuningId;
		}
		...
		{
			return _bonusData.Effects[0.._bonusData.EffectCount];
		}
		...
		private List<ObjectGuid> allowedGUIDs = new();
		...
		private Dictionary<uint, ushort> _artifactPowerIdToIndex = new();
		private Array<uint> _gemScalingLevels = new(ItemConst.MaxGemSockets);
