...
namespace Game.Entities
{
    public class Item : WorldObject
    {
                ...
                case ItemUpdateState.Changed:
                    {
                        ...
                        if (m_itemData.Gems.Size() != 0)
                        {
                            ...
                            foreach (var gemData in m_itemData.Gems)
                            {
                                if (gemData.ItemId != 0)
                                {
                                    ...
                                    var gemBonusListIDs = new StringBuilder();
                        ...
                        if (GetTemplate().GetArtifactID() != 0)
                        {
                            ...
                            foreach (var artifactPower in m_itemData.ArtifactPowers)
                            {
        ...
        public virtual bool LoadFromDB(ulong guid, ObjectGuid ownerGuid, SQLFields fields, uint entry)
        {
            ...
            if (enchantmentTokens.Length == (int)EnchantmentSlot.Max * 3)
            {
                ...
                {
                    var enchantmentField = m_values.ModifyValue(m_itemData).ModifyValue(m_itemData.Enchantment, i);
        ...
        public void SetEnchantment(EnchantmentSlot slot, uint id, uint duration, uint charges, ObjectGuid caster = default)
        {
            ...
            {
                var oldEnchant = GetEnchantmentId(slot);
                if (oldEnchant != 0)
            ...
            var enchantmentField = m_values.ModifyValue(m_itemData).ModifyValue(m_itemData.Enchantment, (int)slot);
        ...
        public void SetGem(ushort slot, ItemDynamicFieldGems gem, uint gemScalingLevel)
        {
            ...
            if (gemTemplate != null)
            {
                ...
                if (gemProperties != null)
                {
                    ...
                    if (gemEnchant != null)
                    {
                        var gemBonus = new BonusData(gemTemplate);
        ...
        {
            var socketGems = new SocketGemsSuccess();
        ...
        public void SendTimeUpdate(Player owner)
        {
            ...
            var itemTimeUpdate = new ItemTimeUpdate();
        ...
        public static Item CreateItem(uint item, uint count, ItemContext context, Player player = null)
        {
            ...
            if (pProto != null)
            {
                ...
                var pItem = Bag.NewItemOrBag(pProto);
        ...
        public void SetNotRefundable(Player owner, bool changestate = true, SQLTransaction trans = null, bool addToCollection = true)
        {
            ...
            var itemExpirePurchaseRefund = new ItemExpirePurchaseRefund();
        ...
        public void UpdatePlayedTime(Player owner)
        {
            ...
            var elapsed = (uint)(curtime - m_lastPlayedTimeUpdate);
        ...
        public static uint GetSellPrice(ItemTemplate proto, uint quality, uint itemLevel)
        {
            ...
            bool standardPrice;
            var cost = GetBuyPrice(proto, quality, itemLevel, out standardPrice);
        ...
        public uint GetItemLevel(Player owner)
        {
            ...
            var maxItemLevel = itemTemplate.GetFlags3().HasAnyFlag(ItemFlags3.IgnoreItemLevelCapInPvp) ? 0u : owner.m_unitData.MaxItemLevel;
        ...
        public int GetItemStatValue(uint index, Player owner)
        {
            ...
            {
                var statValue = (_bonusData.StatPercentEditor[index] * randomPropPoints) * 0.0001f;
        ...
        {
            var modifierIndex = m_itemData.Modifiers._value.Values.FindIndexIf(mod =>
            {
        ...
        void AddArtifactPower(ArtifactPowerData artifactPower)
        {
            ...
            var powerField = new ArtifactPower();
        ...
        public void InitArtifactPowers(byte artifactId, byte artifactTier)
        {
            foreach (var artifactPower in Global.DB2Mgr.GetArtifactPowers(artifactId))
            {
                ...
                var powerData = new ArtifactPowerData();
            ...
            if (enchant != null)
            {
                for (uint i = 0; i < ItemConst.MaxItemEnchantmentEffects; ++i)
                {
                    switch (enchant.Effect[i])
                    {
                        case ItemEnchantmentType.ArtifactPowerBonusRankByType:
                            {
                                for (var artifactPowerIndex = 0; artifactPowerIndex < m_itemData.ArtifactPowers.Size(); ++artifactPowerIndex)
                                {
                                    var artifactPower = m_itemData.ArtifactPowers[artifactPowerIndex];
                        ...
                        case ItemEnchantmentType.ArtifactPowerBonusRankByID:
                            {
                                var artifactPowerIndex = m_artifactPowerIdToIndex.LookupByKey(enchant.EffectArg[i]);
                                if (artifactPowerIndex != 0)
                                {
                                    var newRank = m_itemData.ArtifactPowers[artifactPowerIndex].CurrentRankWithBonus;
                                    if (apply)
                                        newRank += (byte)enchant.EffectPointsMin[i];
                                    else
                                        newRank -= (byte)enchant.EffectPointsMin[i];
                                    ...
                                    if (IsEquipped())
                                    {
                                        var artifactPowerRank = Global.DB2Mgr.GetArtifactPowerRank(m_itemData.ArtifactPowers[artifactPowerIndex].ArtifactPowerId, (byte)(newRank != 0 ? newRank - 1 : 0));
                                        if (artifactPowerRank != null)
                                            owner.ApplyArtifactPowerRank(this, artifactPowerRank, newRank != 0);
                                    }
                                }
        ...
        public void GiveArtifactXp(ulong amount, Item sourceItem, ArtifactCategory artifactCategoryId)
        {
            ...
            var artifactXpGain = new ArtifactXpGain();
        ...
        {
            var enchantmentField = m_values.ModifyValue(m_itemData).ModifyValue(m_itemData.Enchantment, 0);
    ...
    public class BonusData
    {
        ...
        public BonusData(ItemInstance itemInstance) : this(Global.ObjectMgr.GetItemTemplate(itemInstance.ItemID))
        {
            ...
            {
                foreach (var bonusListID in itemInstance.ItemBonus.Value.BonusListIDs)
            ...
            SQLResult azeriteItemUnlockedEssencesResult, SQLResult azeriteEmpoweredItemResult)
        {
            ...
            if (!artifactResult.IsEmpty())
            {
                do
                {
                    ...
                    var artifactPowerData = new ArtifactPowerData();
