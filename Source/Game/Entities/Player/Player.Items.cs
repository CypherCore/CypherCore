...
namespace Game.Entities
{
    public partial class Player
    {
        ...
        public void RefundItem(Item item)
        {
            ...
            if (iece == null)
            {
                Log.outDebug(LogFilter.Player, "Item refund: cannot find extendedcost _data.");
            ...
            // Save all relevant _data to DB to prevent desynchronisation exploits
        ...
        public void SendRefundInfo(Item item)
        {
            ...
            for (byte i = 0; i < ItemConst.MaxItemExtCostItems; ++i) // Item cost _data
            {
                setItemPurchaseData.Contents.Items[i].ItemCount = iece.ItemCount[i];
                setItemPurchaseData.Contents.Items[i].ItemID = iece.ItemID[i];
            }

            for (byte i = 0; i < ItemConst.MaxItemExtCostCurrencies; ++i) // currency cost _data
            {
                if (iece.Flags.HasAnyFlag((byte)((int)ItemExtendedCostFlags.RequireSeasonEarned1 << i)))
                    continue;
        ...
        {
            _itemSoulboundTradeable.Add(item.GetGUID());
        }

        public void RemoveTradeableItem(Item item)
        {
            _itemSoulboundTradeable.Remove(item.GetGUID());
        }

        private void UpdateSoulboundTradeItems()
        {
            // also checks for garbage _data
            foreach (var guid in _itemSoulboundTradeable.ToList())
            {
                Item item = GetItemByGuid(guid);

                if (!item ||
                    item.GetOwnerGUID() != GetGUID() ||
        ...
        public void DurabilityLoss(Item item, double percent)
        {
            ...
            uint pMaxDurability = item._itemData.MaxDurability;
        ...
        public void DurabilityPointsLoss(Item item, int points)
        {
            ...
            uint pMaxDurability = item._itemData.MaxDurability;
            uint pOldDurability = item._itemData.Durability;
        ...
        public void DurabilityRepairAll(bool takeCost, float discountMod, bool guildBank)
        {
            ...
            if (guildBank)
            {
                ...
                foreach (var (item, cost) in itemRepairCostStore)
                {
                    ...
                    if (newTotalCost > availableGuildMoney ||
                        newTotalCost > PlayerConst.MaxMoneyAmount)
                        break;
            ...
            for (int k = 0; k < count; ++k)
            {
                ...
                if (pProto.GetMaxStackSize() != 1)
                {
                    for (byte t = InventorySlots.ItemStart; t < inventoryEnd; ++t)
                    {
                        ...
                        if (item2 &&
                            item2.CanBeMergedPartlyWith(pProto) == InventoryResult.Ok &&
                            inventoryCounts[t - InventorySlots.ItemStart] < pProto.GetMaxStackSize())
                        {
                    ...
                    for (byte t = InventorySlots.BagStart; !b_found && t < InventorySlots.BagEnd; ++t)
                    {
                        ...
                        if (bag)
                        {
                            ...
                            for (byte j = 0; j < bag.GetBagSize(); j++)
                            {
                                ...
                                if (item2 &&
                                    item2.CanBeMergedPartlyWith(pProto) == InventoryResult.Ok &&
                                    bagCounts[t - InventorySlots.BagStart][j] < pProto.GetMaxStackSize())
                                {
        ...
        private Item _StoreItem(ushort pos, Item pItem, uint count, bool clone, bool update)
        {
            ...
            if (pItem2 == null)
            {
                ...
                if (bag == InventorySlots.Bag0 ||
                    (bag >= InventorySlots.BagStart && bag < InventorySlots.BagEnd))
        ...
        public Item StoreNewItem(List<ItemPosCount> pos, uint itemId, bool update, uint randomBonusListId = 0, List<ObjectGuid> allowedLooters = null, ItemContext context = 0, List<uint> bonusListIDs = null, bool addToCollection = true)
        {
            ...
            if (item != null)
            {
                ...
                if (allowedLooters != null &&
                    allowedLooters.Count > 1 &&
        ...
        public InventoryResult CanUseItem(ItemTemplate proto, bool skipRequiredLevelCheck = false)
        {
            ...
            if (proto.HasFlag(ItemFlags2.FactionHorde) &&
                GetTeam() != Team.Horde)
                return InventoryResult.CantEquipEver;

            if (proto.HasFlag(ItemFlags2.FactionAlliance) &&
                GetTeam() != Team.Alliance)
                return InventoryResult.CantEquipEver;

            if ((proto.GetAllowableClass() & GetClassMask()) == 0 ||
                (proto.GetAllowableRace() & (long)SharedConst.GetMaskForRace(GetRace())) == 0)
                return InventoryResult.CantEquipEver;
            ...
            if (proto.GetRequiredSpell() != 0 &&
                !HasSpell(proto.GetRequiredSpell()))
                return InventoryResult.ProficiencyNeeded;

            if (!skipRequiredLevelCheck &&
                GetLevel() < proto.GetBaseRequiredLevel())
                return InventoryResult.CantEquipLevelI;

            // If World Event is not active, prevent using event dependant items
            if (proto.GetHolidayID() != 0 &&
                !Global.GameEventMgr.IsHolidayActive(proto.GetHolidayID()))
                return InventoryResult.ClientLockedOut;

            if (proto.GetRequiredReputationFaction() != 0 &&
                (uint)GetReputationRank(proto.GetRequiredReputationFaction()) < proto.GetRequiredReputationRank())
                return InventoryResult.CantEquipReputation;

            // learning (recipes, mounts, pets, etc.)
            if (proto.Effects.Count >= 2)
                if (proto.Effects[0].SpellID == 483 ||
                    proto.Effects[0].SpellID == 55884)
        ...
        private InventoryResult CanUnequipItems(uint item, uint count)
        {
            ...
            bool result = ForEachItem(ItemSearchLocation.Equipment,
                                      pItem =>
                                      {
                        ...
                        if (slot < ProfessionSlots.End)
                        {
                            ...
                            if (slot == EquipmentSlot.MainHand)
                            {
                                ...
                                for (EnchantmentSlot enchantSlot = 0; enchantSlot < EnchantmentSlot.Max; ++enchantSlot)
                                {
                                    ...
                                    if (enchantment != null &&
                                        enchantment.GetFlags().HasFlag(SpellItemEnchantmentFlags.MainhandOnly))
        ...
        public void SwapItem(ushort src, ushort dst)
        {
            ...
            {
                Item parentItem = GetItemByGuid(pSrcItem._itemData.Creator);
            ...
            {
                Item parentItem = GetItemByGuid(pDstItem._itemData.Creator);
                        ...
                        fullBag != null)
                    {
                        ...
                        for (byte i = 0; i < fullBag.GetBagSize(); ++i)
                        {
                            ...
                            if (bagItemProto == null ||
                                !Item.ItemCanGoIntoBag(bagItemProto, emptyProto))
                            {
        ...
        private bool _StoreOrEquipNewItem(uint vendorslot, uint item, byte count, byte bag, byte slot, long price, ItemTemplate pProto, Creature pVendor, VendorItem crItem, bool bStore)
        {
            ...
            if (it != null)
            {
                ...
                if (pProto.HasFlag(ItemFlags.ItemPurchaseRecord) &&
                    crItem.ExtendedCost != 0 &&
        ...
        public void SendNewItem(Item item, uint quantity, bool pushed, bool created, bool broadcast = false, uint dungeonEncounterId = 0)
        {
            ...
            if (broadcast &&
                GetGroup() &&
                !item.GetTemplate().HasFlag(ItemFlags3.DontReportLootLogToParty))
        ...
        {
            if (item._itemData.Expiration != 0)
            {
        ...
        private void UpdateItemDuration(uint time, bool realtimeonly = false)
        {
            ...
            foreach (var item in _itemDuration)
                if (!realtimeonly ||
                    item.GetTemplate().HasFlag(ItemFlags.RealDuration))
        ...
        public float GetAverageItemLevel()
        {
            ...
            for (int i = EquipmentSlot.Start; i < EquipmentSlot.End; ++i)
            {
                ...
                if (i == EquipmentSlot.Tabard ||
                    i == EquipmentSlot.Ranged ||
                    i == EquipmentSlot.OffHand ||
                    i == EquipmentSlot.Shirt)
                    continue;
        ...
        public bool IsValidPos(byte bag, byte slot, bool explicit_pos)
        {
            ...
            if (bag == InventorySlots.Bag0)
            {
                ...
                if (slot >= ProfessionSlots.Start &&
                    slot < ProfessionSlots.End)
                    return true;

                // bag equip slots
                if (slot >= InventorySlots.BagStart &&
                    slot < InventorySlots.BagEnd)
                    return true;

                // reagent bag equip slots
                if (slot >= InventorySlots.ReagentBagStart &&
                    slot < InventorySlots.ReagentBagEnd)
                    return true;

                // backpack slots
                if (slot >= InventorySlots.ItemStart &&
                    slot < InventorySlots.ItemStart + GetInventorySlotCount())
                    return true;

                // bank main slots
                if (slot >= InventorySlots.BankItemStart &&
                    slot < InventorySlots.BankItemEnd)
                    return true;

                // bank bag slots
                if (slot >= InventorySlots.BankBagStart &&
                    slot < InventorySlots.BankBagEnd)
                    return true;

                // reagent bank bag slots
                if (slot >= InventorySlots.ReagentStart &&
                    slot < InventorySlots.ReagentEnd)
                    return true;
        ...
        public byte GetItemLimitCategoryQuantity(ItemLimitCategoryRecord limitEntry)
        {
            ...
            foreach (ItemLimitCategoryConditionRecord limitCondition in limitConditions)
            {
                ...
                if (playerCondition == null ||
                    ConditionManager.IsPlayerMeetingCondition(this, playerCondition))
        ...
        public InventoryResult CanRollNeedForItem(ItemTemplate proto, Map map, bool restrictOnlyLfg)
        {
            ...
            if (proto.GetClass() == ItemClass.Weapon &&
                GetSkillValue(proto.GetSkill()) == 0)
                return InventoryResult.ProficiencyNeeded;

            if (proto.GetClass() == ItemClass.Armor &&
                proto.GetInventoryType() != InventoryType.Cloak)
            {
        ...
        public void AddItemToBuyBackSlot(Item pItem)
        {
            if (pItem != null)
            {
                ...
                uint etime = (uint)(time - _logintime + (30 * 3600));
        ...
        public bool BuyCurrencyFromVendorSlot(ObjectGuid vendorGuid, uint vendorSlot, uint currency, uint count)
        {
            ...
            if (crItem.ExtendedCost != 0)
            {
                ...
                for (byte i = 0; i < ItemConst.MaxItemExtCostItems; ++i)
                    if (iece.ItemID[i] != 0 &&
                        !HasItemCount(iece.ItemID[i], (iece.ItemCount[i] * stacks)))
                    {
                ...
                if (iece.MinFactionID != 0 &&
                    (uint)GetReputationRank(iece.MinFactionID) < iece.RequiredAchievement)
                {
                ...
                if (iece.Flags.HasAnyFlag((byte)ItemExtendedCostFlags.RequireGuild) &&
                    GetGuildId() == 0)
                {
                    SendEquipError(InventoryResult.VendorMissingTurnins); // Find correct error
                ...
                if (iece.RequiredAchievement != 0 &&
                    !HasAchieved(iece.RequiredAchievement))
                {
                    SendEquipError(InventoryResult.VendorMissingTurnins); // Find correct error
        ...
        public bool BuyItemFromVendorSlot(ObjectGuid vendorguid, uint vendorslot, uint item, byte count, byte bag, byte slot)
        {
            ...
            if (!Convert.ToBoolean(pProto.GetAllowableClass() & GetClassMask()) &&
                pProto.GetBonding() == ItemBondingType.OnAcquire &&
                !IsGameMaster())
            {
                SendBuyError(BuyResult.CantFindItem, null, item);
            ...
            if (!IsGameMaster() &&
                ((pProto.HasFlag(ItemFlags2.FactionHorde) && GetTeam() == Team.Alliance) || (pProto.HasFlag(ItemFlags2.FactionAlliance) && GetTeam() == Team.Horde)))
                return false;
            ...
            if (pProto.GetRequiredReputationFaction() != 0 &&
                ((uint)GetReputationRank(pProto.GetRequiredReputationFaction()) < pProto.GetRequiredReputationRank()))
            {
            ...
            if (crItem.ExtendedCost != 0)
            {
                ...
                if (iece.MinFactionID != 0 &&
                    (uint)GetReputationRank(iece.MinFactionID) < iece.MinReputation)
                {
                    SendBuyError(BuyResult.ReputationRequire, creature, item);
            ...
            {
                if (pProto.GetQuality() > ItemQuality.Epic ||
                    (pProto.GetQuality() == ItemQuality.Epic && pProto.GetBaseItemLevel() >= GuildConst.MinNewsItemLevel))
                {
                    Guild guild = GetGuild();
        ...
        public void _ApplyItemMods(Item item, byte slot, bool apply, bool updateItemAuras = true)
        {
            ...
            if (updateItemAuras)
            {
                ...
                WeaponAttackType attackType = GetAttackBySlot(slot, item.GetTemplate().GetInventoryType());
        ...
        {
            if (item == null ||
                item.GetTemplate().HasFlag(ItemFlags.Legacy))
                return;
            ...
            foreach (ItemEffectRecord effectData in item.GetEffects())
            {
                ...
                if (effectData.ChrSpecializationID != 0 &&
                    effectData.ChrSpecializationID != GetPrimarySpecialization())
                    continue;
            ...
            if (apply)
            {
                ...
                if (formChange) // check aura active State from other form
                {
                    ...
                    foreach (var pair in range)
                    {
                        ...
                        if (item == null ||
                            pair.Value.GetBase().GetCastItemGUID() == item.GetGUID())
                            return;
                    }
                }
        ...
        private void _RemoveAllItemMods()
        {
            ...
            for (byte i = 0; i < InventorySlots.BagEnd; ++i)
                if (_items[i] != null)
                {
                    ItemTemplate proto = _items[i].GetTemplate();
        ...
        private void _ApplyAllItemMods()
        {
            ...
            for (byte i = 0; i < InventorySlots.BagEnd; ++i)
                if (_items[i] != null)
                {
                    ...
                    WeaponAttackType attackType = GetAttackBySlot(i, _items[i].GetTemplate().GetInventoryType());
        ...
        private void ApplyAllAzeriteEmpoweredItemMods(bool apply)
        {
            for (byte i = 0; i < InventorySlots.BagEnd; ++i)
                if (_items[i])
                {
                    if (!_items[i].IsAzeriteEmpoweredItem() ||
                        _items[i].IsBroken() ||
                        !CanUseAttackType(GetAttackBySlot(i, _items[i].GetTemplate().GetInventoryType())))
                        continue;
        ...
        public static bool IsInventoryPos(byte bag, byte slot)
        {
            ...
            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.ItemStart && slot < InventorySlots.ItemEnd))
                return true;

            if (bag >= InventorySlots.BagStart &&
                bag < InventorySlots.BagEnd)
                return true;

            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.ReagentStart && slot < InventorySlots.ReagentEnd))
                return true;

            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.ChildEquipmentStart && slot < InventorySlots.ChildEquipmentEnd))
                return true;
        ...
        private InventoryResult CanStoreItem_InSpecificSlot(byte bag, byte slot, List<ItemPosCount> dest, ItemTemplate pProto, ref uint count, bool swap, Item pSrcItem)
        {
            ...
            if (pSrcItem)
            {
                if (pSrcItem.IsNotEmptyBag() &&
                    !IsBagPos((ushort)(((ushort)bag << 8) | slot)))
                    return InventoryResult.DestroyNonemptyBag;

                if (pSrcItem.HasItemFlag(ItemFieldFlags.Child) &&
                    !IsEquipmentPos(bag, slot) &&
                    !IsChildEquipmentPos(bag, slot))
            ...
            if (pItem2 == null || swap)
            {
                if (bag == InventorySlots.Bag0)
                {
                    ...
                    if ((slot >= InventorySlots.BuyBackStart && slot < InventorySlots.BuyBackEnd) ||
                        slot >= (byte)PlayerSlots.End)
                        return InventoryResult.WrongBagType;

                    // can't store anything else than crafting reagents in Reagent Bank
                    if (IsReagentBankPos(bag, slot) &&
                        (!IsReagentBankUnlocked() || !pProto.IsCraftingReagent()))
                        return InventoryResult.WrongBagType;
                }
                else
                {
        ...
        {
            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.BankItemStart && slot < InventorySlots.BankItemEnd))
                return true;

            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.BankBagStart && slot < InventorySlots.BankBagEnd))
                return true;

            if (bag >= InventorySlots.BankBagStart &&
        ...
        public InventoryResult CanBankItem(byte bag, byte slot, List<ItemPosCount> dest, Item pItem, bool swap, bool not_loading = true, bool reagentBankOnly = false)
        {
            ...
            if (!reagentBankOnly &&
                pProto.GetBagFamily() != BagFamilyMask.None)
                for (byte i = InventorySlots.BankBagStart; i < InventorySlots.BankBagEnd; i++)
                {
                    res = CanStoreItem_InBag(i, dest, pProto, ref count, false, false, pItem, bag, slot);
        ...
        public static bool IsBagPos(ushort pos)
        {
            ...
            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.BagStart && slot < InventorySlots.BagEnd))
                return true;
            ...
            if (bag == InventorySlots.Bag0 &&
                (slot >= InventorySlots.ReagentBagStart && slot < InventorySlots.ReagentBagEnd))
                return true;
        ...
        public static bool IsEquipmentPos(byte bag, byte slot)
        {
            ...
            if (bag == InventorySlots.Bag0 &&
                (slot >= ProfessionSlots.Start && slot < ProfessionSlots.End))
                return true;
            ...
            if (pItem != null)
            {
                ...
                if (pProto != null)
                {
                    ...
                    if (requiredLevels.HasValue &&
                        requiredLevels.Value.MaxLevel < SharedConst.DefaultMaxLevel &&
                    ...
                    if (eslot == EquipmentSlot.OffHand)
                    {
                        ...
                        if (type == InventoryType.Weapon &&
                            pProto.GetSubClass() == (uint)ItemSubClassWeapon.Polearm)
                        {
                        ...
                        {
                            if (!CanDualWield() &&
                                !pProto.HasFlag(ItemFlags3.AlwaysAllowDualWield))
                                return InventoryResult.TwoHandSkillNotFound;
                        }
        ...
        public InventoryResult CanEquipUniqueItem(Item pItem, byte eslot = ItemConst.NullSlot, uint limit_count = 1)
        {
            ...
            foreach (SocketedGem gemData in pItem._itemData.Gems)
            {
        ...
        private void ApplyArtifactPowers(Item item, bool apply)
        {
            ...
            foreach (ArtifactPower artifactPower in item._itemData.ArtifactPowers)
            {
            ...
            if (artifactAppearance != null)
                if (artifactAppearance.OverrideShapeshiftDisplayID != 0 &&
                    GetShapeshiftForm() == (ShapeShiftForm)artifactAppearance.OverrideShapeshiftFormID)
                    RestoreDisplayId();
        }
        ...
        private void ApplyAzeritePowers(Item item, bool apply)
        {
            ...
            else
            {
                ...
                if (azeriteEmpoweredItem)
                    if (!apply ||
                        GetItemByEntry(PlayerConst.ItemIdHeartOfAzeroth, ItemSearchLocation.Equipment))
        ...
        public void ApplyAzeritePower(AzeriteEmpoweredItem item, AzeritePowerRecord azeritePower, bool apply)
        {
            if (apply)
            {
                if (azeritePower.SpecSetID == 0 ||
                    Global.DB2Mgr.IsSpecSetMember(azeritePower.SpecSetID, GetPrimarySpecialization()))
                    CastSpell(this, azeritePower.SpellID, item);
            }
            else
            {
            ...
            for (byte i = EquipmentSlot.Start; i < InventorySlots.BagEnd; i++)
            {
                ...
                if (item != null)
                        ...
                        !item.IsInTrade())
                    {
                        ...
                        {
                            if (!unequip_check ||
                                CanUnequipItem((ushort)((InventorySlots.Bag0 << 8) | i), false) == InventoryResult.Ok)
                            {
                                remcount += item.GetCount();
                                DestroyItem(InventorySlots.Bag0, i, update);
        ...
        public void StoreLootItem(ObjectGuid lootWorldObjectGuid, byte lootSlot, Loot loot, AELootResult aeResult = null)
        {
            ...
            if (msg == InventoryResult.Ok)
            {
                ...
                if (Global.ObjectMgr.GetItemTemplate(item.itemid) != null)
                    if (newitem.GetQuality() > ItemQuality.Epic ||
                        (newitem.GetQuality() == ItemQuality.Epic && newitem.GetItemLevel(this) >= GuildConst.MinNewsItemLevel))
                    {
                        Guild guild = GetGuild();
        ...
        public void SetEquipmentSet(EquipmentSetInfo.EquipmentSetData newEqSet)
        {
            if (newEqSet.Guid != 0)
            {
                ...
                if (equipmentSetInfo == null ||
                    equipmentSetInfo.Data.Guid != newEqSet.Guid)
                {
        ...
        {
            if (oldSlot >= SharedConst.VoidStorageMaxSlot ||
                newSlot >= SharedConst.VoidStorageMaxSlot ||
                oldSlot == newSlot)
                return false;
        ...
        public void UpdateAverageItemLevelTotal()
        {
            ...
            ForEachItem(ItemSearchLocation.Everywhere,
                        item =>
                        {
                            ...
                            if (itemTemplate != null &&
                                itemTemplate.GetInventoryType() < InventoryType.ProfessionTool)
                            {
                                ushort dest;
            ...
            if (!_canTitanGrip &&
                mainHand.inventoryType == InventoryType.Weapon2Hand)
