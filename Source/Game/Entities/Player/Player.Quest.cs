...
namespace Game.Entities
{
	public partial class Player
	{
		...
		{
			var questLevels = Global.DB2Mgr.GetContentTuningData(contentTuningId, _playerData.CtrOptions.GetValue().ContentTuningConditionMask);
		...
		{
			foreach (uint questId in _activePlayerData.DailyQuestsCompleted)
			{
			...
			dailyQuestsReset.Count = _activePlayerData.DailyQuestsCompleted.Size();
			SendPacket(dailyQuestsReset);

			ClearDynamicUpdateFieldValues(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.DailyQuestsCompleted));
		...
		public void ResetWeeklyQuestStatus()
		{
			...
			foreach (uint questId in _weeklyquests)
			{
				uint questBit = Global.DB2Mgr.GetQuestUniqueBitFlag(questId);
		...
		public void ResetMonthlyQuestStatus()
		{
			...
			foreach (uint questId in _monthlyquests)
			{
				uint questBit = Global.DB2Mgr.GetQuestUniqueBitFlag(questId);
		...
		public void PrepareQuestMenu(ObjectGuid guid)
		{
			...
			foreach (var questId in questRelations)
			{
				...
				if (quest.IsAutoComplete() &&
				    (!quest.IsRepeatable() || quest.IsDaily() || quest.IsWeekly() || quest.IsMonthly()))
		...
		public void SendPreparedQuest(WorldObject source)
		{
			...
			if (questMenu.GetMenuItemCount() == 1)
			{
				...
				if (quest != null)
				{
					...
					else if (!source.HasQuest(questId) &&
					         !source.HasInvolvedQuest(questId))
					{
		...
		{
			return _QuestStatus.ContainsKey(quest_id);
		}
		...
		public bool CanCompleteQuest(uint questId, uint ignoredQuestObjectiveId = 0)
		{
			if (questId != 0)
			{
				...
				var q_status = _QuestStatus.LookupByKey(questId);
				...
				if (q_status.Status == QuestStatus.Incomplete)
				{
					...
					{
						if (ignoredQuestObjectiveId != 0 &&
						    obj.Id == ignoredQuestObjectiveId)
							continue;
					...
					if (qInfo.HasSpecialFlag(QuestSpecialFlags.ExplorationOrEvent) &&
					    !q_status.Explored)
						return false;
			...
			if (quest.HasQuestObjectiveType(QuestObjectiveType.Item))
				foreach (QuestObjective obj in quest.Objectives)
					if (obj.Type == QuestObjectiveType.Item &&
					    !HasItemCount((uint)obj.ObjectID, (uint)obj.Amount))
						return false;
		...
		public bool CanRewardQuest(Quest quest, bool msg)
		{
			...
			if (!quest.IsDFQuest() &&
			    ...
			    GetQuestStatus(quest.Id) != QuestStatus.Complete)
				return false;

			// daily quest can't be rewarded (25 daily quest already completed)
			if (!SatisfyQuestDay(quest, msg) ||
			    !SatisfyQuestWeek(quest, msg) ||
			    !SatisfyQuestMonth(quest, msg) ||
			    !SatisfyQuestSeasonal(quest, msg))
				return false;

			// player no longer satisfies the quest's requirements (skill level etc.)
			if (!SatisfyQuestLevel(quest, msg) ||
			    !SatisfyQuestSkill(quest, msg) ||
					...
					case LootItemType.Item:
						for (uint i = 0; i < SharedConst.QuestRewardChoicesCount; ++i)
							if (quest.RewardChoiceItemId[i] != 0 &&
							    ...
							    quest.RewardChoiceItemId[i] == rewardId)
							{
		...
		public void AddQuestAndCheckCompletion(Quest quest, WorldObject questGiver)
		{
			...
			foreach (QuestObjective obj in quest.Objectives)
				if (obj.Type == QuestObjectiveType.CriteriaTree)
					if (_questObjectiveCriteriaMgr.HasCompletedObjective(obj))
		...
		public void AddQuest(Quest quest, WorldObject questGiver)
		{
			...
			if (!_QuestStatus.ContainsKey(questId))
				_QuestStatus[questId] = new QuestStatusData();

			QuestStatusData questStatusData = _QuestStatus.LookupByKey(questId);
			...
			foreach (QuestObjective obj in quest.Objectives)
			{
				...
				switch (obj.Type)
				{
					...
					case QuestObjectiveType.CriteriaTree:
						_questObjectiveCriteriaMgr.ResetCriteriaTree((uint)obj.ObjectID);
			...
			if (limittime != 0)
			{
				// shared timed quest
				if (questGiver != null &&
				    questGiver.IsTypeId(TypeId.Player))
					limittime = questGiver.ToPlayer()._QuestStatus[questId].Timer / Time.InMilliseconds;
			...
			if (quest.SourceSpellID > 0)
			{
				...
				if (questGiver != null &&
				    questGiver.IsTypeMask(TypeMask.Unit) &&
				    !quest.HasFlag(QuestFlags.PlayerCastOnAccept) &&
				    !spellInfo.HasTargetType(Targets.UnitCaster) &&
				    !spellInfo.HasTargetType(Targets.DestCasterSummon))
				{
		...
		public void CompleteQuest(uint quest_id)
		{
			if (quest_id != 0)
			{
				...
				QuestStatusData questStatus = _QuestStatus.LookupByKey(quest_id);
		...
		public void RewardQuestPackage(uint questPackageId, uint onlyItemId = 0)
		{
			...
			if (questPackageItems != null)
				...
				{
					if (onlyItemId != 0 &&
					    questPackageItem.ItemID != onlyItemId)
						continue;
		...
		public void RewardQuest(Quest quest, LootItemType rewardType, uint rewardId, WorldObject questGiver, bool announce = true)
		{
			...
			switch (rewardType)
			{
				case LootItemType.Item:
					...
					break;
				case LootItemType.Currency:
					if (CliDB.CurrencyTypesStorage.HasRecord(rewardId) &&
					    quest.GetRewChoiceItemsCount() != 0)
						for (uint i = 0; i < SharedConst.QuestRewardChoicesCount; ++i)
							if (quest.RewardChoiceItemId[i] != 0 &&
			...
			if (quest.RewardSpell > 0)
			{
				...
				if (questGiver != null &&
				    questGiver.IsTypeMask(TypeMask.Unit) &&
				    !quest.HasFlag(QuestFlags.PlayerCastOnComplete) &&
				    !spellInfo.HasTargetType(Targets.UnitCaster))
				{
					Unit unit = questGiver.ToUnit();
		...
		public void FailQuest(uint questId)
		{
			...
			if (quest != null)
			{
				...
				if (qStatus != QuestStatus.Incomplete)
					...
					if (qStatus != QuestStatus.Complete ||
					    quest.LimitTime == 0 ||
		...
		public bool SatisfyQuestPreviousQuest(Quest qInfo, bool msg)
		{
			...
			if (qInfo.PrevQuestId > 0 &&
			    _RewardedQuests.Contains(prevId))
				return true;

			// If negative previous quest active, return true
			if (qInfo.PrevQuestId < 0 &&
			    GetQuestStatus(prevId) == QuestStatus.Incomplete)
				return true;
		...
		public bool SatisfyQuestReputation(Quest qInfo, bool msg)
		{
			...
			if (fIdMin != 0 &&
			    GetReputationMgr().GetReputation(fIdMin) < qInfo.RequiredMinRepValue)
			{
				if (msg)
				{
					SendCanTakeQuestResponse(QuestFailedReasons.None);
			...
			if (fIdMax != 0 &&
			    GetReputationMgr().GetReputation(fIdMax) >= qInfo.RequiredMaxRepValue)
			{
				if (msg)
				{
					SendCanTakeQuestResponse(QuestFailedReasons.None);
		...
		public bool SatisfyQuestDay(Quest qInfo, bool msg)
		{
			...
			return _activePlayerData.DailyQuestsCompleted.FindIndex(qInfo.Id) == -1;
		}
		...
		{
			if (!qInfo.IsSeasonal() ||
			    _seasonalquests.Empty())
				return true;

			var list = _seasonalquests.LookupByKey(qInfo.GetEventIdForQuest());
		...
		public bool CanShareQuest(uint quest_id)
		{
			...
			if (qInfo != null &&
			    qInfo.HasFlag(QuestFlags.Sharable))
			{
				var questStatusData = _QuestStatus.LookupByKey(quest_id);
		...
		public void SetQuestStatus(uint questId, QuestStatus status, bool update = true)
		{
			...
			if (quest != null)
			{
				...
				QuestStatus oldStatus = _QuestStatus[questId].Status;
				_QuestStatus[questId].Status = status;
		...
		{
			var questStatus = _QuestStatus.LookupByKey(questId);

			if (questStatus != null)
			{
				foreach (var objective in _questObjectiveStatus.KeyValueList)
					if (objective.Value.QuestStatusPair.Status == questStatus)
						_questObjectiveStatus.Remove(objective);

				_QuestStatus.Remove(questId);
		...
		{
			var questLogField = _values.ModifyValue(_playerData).ModifyValue(_playerData.QuestLog, slot);
		...
		public void SetQuestSlotCounter(ushort slot, byte counter, ushort count)
		{
			...
			QuestLog questLog = _values.ModifyValue(_playerData).ModifyValue(_playerData.QuestLog, slot);
		...
		{
			QuestLog questLogField = _values.ModifyValue(_playerData).ModifyValue(_playerData.QuestLog, slot);
		...
		private void SetQuestCompletedBit(uint questBit, bool completed)
		{
			...
			if (completed)
				SetUpdateFieldFlagValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.QuestCompleted, (int)fieldOffset), flag);
			else
				RemoveUpdateFieldFlagValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.QuestCompleted, (int)fieldOffset), flag);
		}
		...
		public void AreaExploredOrEventHappens(uint questId)
		{
			if (questId != 0)
			{
				QuestStatusData status = _QuestStatus.LookupByKey(questId);

				if (status != null)
					// Dont complete failed quest
					if (!status.Explored &&
					    status.Status != QuestStatus.Failed)
					{
		...
		public void GroupEventHappens(uint questId, WorldObject pEventObject)
		{
			...
			if (group)
				for (GroupReference refe = group.GetFirstMember(); refe != null; refe = refe.Next())
				{
					...
					if (player &&
					    player.IsAtGroupRewardDistance(pEventObject) &&
					    !player.GetCorpse())
		...
		public void UpdateQuestObjectiveProgress(QuestObjectiveType objectiveType, int objectId, long addCount, ObjectGuid victimGuid = default)
		{
			...
			foreach (var objectiveStatusData in _questObjectiveStatus.LookupByKey((objectiveType, objectId)))
			{
				...
				if (!QuestObjective.CanAlwaysBeProgressedInRaid(objectiveType))
					if (GetGroup() &&
					    GetGroup().IsRaidGroup() &&
					    quest.IsAllowedInRaid(GetMap().GetDifficultyID()))
						continue;
				    ...
				    addCount < 0)
				{
					...
					if (objective.IsStoringValue())
					{
						if (objectiveType == QuestObjectiveType.PlayerKills &&
						    objective.Flags.HasAnyFlag(QuestObjectiveFlags.KillPlayersSameFaction))
						{
						...
						if (addCount > 0 ? (currentProgress < objective.Amount) : (currentProgress > 0))
						{
							...
							if (addCount > 0 &&
							    !objective.Flags.HasAnyFlag(QuestObjectiveFlags.HideCreditMsg))
		...
		public bool HasQuestForItem(uint itemid)
		{
			...
			foreach (var objectiveItr in _questObjectiveStatus.LookupByKey((QuestObjectiveType.Item, (int)itemid)))
			{
				...
				if (GetGroup() &&
				    GetGroup().IsRaidGroup() &&
				    !qInfo.IsAllowedInRaid(GetMap().GetDifficultyID()))
			...
			foreach (var questStatus in _QuestStatus)
			{
				...
				for (byte j = 0; j < SharedConst.QuestItemDropCount; ++j)
				{
					...
					if (pProto.GetMaxCount() != 0 &&
					    pProto.GetMaxCount() < maxAllowedCount)
		...
		public void SetQuestObjectiveData(QuestObjective objective, int data)
		{
			...
			var status = _QuestStatus.LookupByKey(objective.QuestID);
		...
		public bool IsQuestObjectiveCompletable(ushort slot, Quest quest, QuestObjective objective)
		{
			...
			if (objective.Flags.HasAnyFlag(QuestObjectiveFlags.Sequenced))
			{
				...
				if (!previousSequencedObjectiveComplete ||
				    !objectiveSequenceSatisfied)
					return false;
			}
			else if (!previousSequencedObjectiveComplete &&
			         previousSequencedObjectiveIndex != -1)
			{
		...
		public void SendQuestReward(Quest quest, Creature questGiver, uint xp, bool hideChatMessage)
		{
			...
			if (questGiver)
			{
				...
				else if (quest.NextQuestInChain != 0 &&
		...
		public void SendQuestGiverStatusMultiple()
		{
			...
			foreach (var itr in _clientGUIDs)
				...
				else if (itr.IsGameObject())
				{
					...
					if (!questgiver ||
					    questgiver.GetGoType() != GameObjectTypes.QuestGiver)
						continue;
				...
				if (guid.IsGameObject())
				{
					...
					if (obj != null)
					{
						...
						if (_questObjectiveStatus.ContainsKey((QuestObjectiveType.GameObject, (int)obj.GetEntry())))
							objMask.MarkChanged(obj._objectData.DynamicFlags);
						...
						if (objMask.GetUpdateMask().IsAnySet() ||
						    goMask.GetUpdateMask().IsAnySet())
				...
				else if (guid.IsCreatureOrVehicle())
				{
					...
					foreach (var spellClickInfo in clickBounds)
					{
						...
						if (conds != null)
						{
							...
							unitMask.MarkChanged(_unitData.NpcFlags, 0); // NpcFlags[0] has UNIT_NPC_FLAG_SPELLCLICK
		...
		private void SetDailyQuestStatus(uint quest_id)
		{
			...
			if (qQuest != null)
			{
				if (!qQuest.IsDFQuest())
				{
					AddDynamicUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.DailyQuestsCompleted), quest_id);
		...
		{
			return _activePlayerData.DailyQuestsCompleted.FindIndex(quest_id) >= 0;
		}
		...
		private void SetSeasonalQuestStatus(uint quest_id)
		{
			...
			if (!_seasonalquests.ContainsKey(quest.GetEventIdForQuest()))
				_seasonalquests[quest.GetEventIdForQuest()] = new Dictionary<uint, long>();

			_seasonalquests[quest.GetEventIdForQuest()][quest_id] = GameTime.GetGameTime();
			_SeasonalQuestChanged                                 = true;
		}
		...
		private void PushQuests()
		{
			...
			{
				if (quest.GetQuestTag() != 0 &&
				    quest.GetQuestTag() != QuestTagType.Tag)
					continue;

				if (!quest.IsUnavailable() &&
				    CanTakeQuest(quest, false))
