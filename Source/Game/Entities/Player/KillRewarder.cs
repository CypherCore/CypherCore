...
namespace Game.Entities
{
	public class KillRewarder
	{
		...
		private void _InitGroupData(Player killer)
		{
			...
			if (group != null)
			{
				...
				for (GroupReference refe = group.GetFirstMember(); refe != null; refe = refe.Next())
				{
					...
					if (member != null)
						if (killer == member ||
						    (member.IsAtGroupRewardDistance(_victim) && member.IsAlive()))
						{
							...
							if (_victim.GetLevelForTarget(member) > grayLevel &&
		...
		private void _RewardGroup(Group group, Player killer)
		{
			if (_maxLevel != 0)
			{
				if (_maxNotGrayMember != null)
