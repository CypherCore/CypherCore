...
namespace Game.Entities
{
	...
	{
		private static Dictionary<uint, uint> FactionSpecificMounts = new();
		private BitSet _appearances;
		private Dictionary<uint, FavoriteAppearanceState> _favoriteAppearances = new();
		private Dictionary<uint, HeirloomData> _heirlooms = new();
		private Dictionary<uint, MountStatusFlags> _mounts = new();

		private WorldSession _owner;
		private MultiMap<uint, ObjectGuid> _temporaryAppearances = new();
		private Dictionary<uint, ToyFlags> _toys = new();
		...
		public static void LoadMountDefinitions()
		{
			...
			do
			{
				...
				if (otherFactionSpellId != 0 &&
				    Global.DB2Mgr.GetMount(otherFactionSpellId) == null)
				{
		...
		public bool AddMount(uint spellId, MountStatusFlags flags, bool factionMount = false, bool learned = false)
		{
			...
			if (mount.PlayerConditionID != 0)
			{
				...
				if (playerCondition != null &&
				    !ConditionManager.IsPlayerMeetingCondition(player, playerCondition))
					return false;
			}
		...
		private bool CanAddAppearance(ItemModifiedAppearanceRecord itemModifiedAppearance)
		{
			...
			if (itemModifiedAppearance.TransmogSourceTypeEnum == 6 ||
			    itemModifiedAppearance.TransmogSourceTypeEnum == 9)
				return false;
			...
			if (itemTemplate.HasFlag(ItemFlags2.NoSourceForItemVisual) ||
			    itemTemplate.GetQuality() == ItemQuality.Artifact)
				return false;
			...
			if (itemTemplate.GetQuality() < ItemQuality.Uncommon)
				if (!itemTemplate.HasFlag(ItemFlags2.IgnoreQualityForItemVisualSource) ||
				    !itemTemplate.HasFlag(ItemFlags3.ActsAsTransmogHiddenVisualOption))
					return false;

			if (itemModifiedAppearance.Id < _appearances.Count &&
			    _appearances.Get((int)itemModifiedAppearance.Id))
				return false;
