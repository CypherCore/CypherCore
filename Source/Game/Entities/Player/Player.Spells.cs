...
namespace Game.Entities
{
	public partial class Player
	{
		private void UpdateSkillsForLevel()
		{
			...
			SkillInfo skillInfoField = _activePlayerData.Skill;

			foreach (var pair in mSkillStatus)
			{
				if (pair.Value.State == SkillState.Deleted ||
				    skillInfoField.SkillRank[pair.Value.Pos] == 0)
					continue;
		...
		private void InitializeSelfResurrectionSpells()
		{
			...
			foreach (var auraEffect in dummyAuras)
				...
				if (auraEffect.GetSpellInfo().SpellFamilyName == SpellFamilyNames.Warlock &&
				    auraEffect.GetSpellInfo().SpellFamilyFlags[1].HasAnyFlag(0x1000000u))
					spells[0] = 3026;
				// Twisting Nether                                  // prio: 2 (max)
				else if (auraEffect.GetId() == 23701 &&
				         RandomHelper.randChance(10))
					spells[1] = 23700;

			// Reincarnation (passive spell)  // prio: 1
			if (HasSpell(20608) &&
			    !GetSpellHistory().HasCooldown(21169))
				spells[2] = 21169;
		...
		{
			SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.OverrideSpellsID), overrideSpellsId);
		}
		...
		private void UpdateSkillEnchantments(uint skill_id, ushort curr_value, ushort new_value)
		{
			for (byte i = 0; i < InventorySlots.BagEnd; ++i)
				if (_items[i] != null)
					...
					{
						uint ench_id = _items[i].GetEnchantmentId(slot);
						...
						if ((slot == EnchantmentSlot.Sock1 || slot == EnchantmentSlot.Sock2 || slot == EnchantmentSlot.Sock3) &&
						    _items[i].GetSocketColor((uint)(slot - EnchantmentSlot.Sock1)) == 0)
						{
							SpellItemEnchantmentRecord pPrismaticEnchant = CliDB.SpellItemEnchantmentStorage.LookupByKey(_items[i].GetEnchantmentId(EnchantmentSlot.Prismatic));

							if (pPrismaticEnchant != null &&
		...
		{
			for (var i = 0; i < _enchantDuration.Count; ++i)
			{
				var enchantDuration = _enchantDuration[i];

				if (enchantDuration.item.GetEnchantmentId(enchantDuration.slot) == 0)
				{
					_enchantDuration.Remove(enchantDuration);
				}
		...
		public void ApplyEnchantment(Item item, EnchantmentSlot slot, bool apply, bool apply_dur = true, bool ignore_condition = false)
		{
			...
			if (!ignore_condition &&
			    pEnchant.ConditionID != 0 &&
			    !EnchantmentFitsRequirements(pEnchant.ConditionID, -1))
				return;
			...
			if (pEnchant.RequiredSkillID > 0 &&
			    pEnchant.RequiredSkillRank > GetSkillValue((SkillType)pEnchant.RequiredSkillID))
				return;
			...
			if ((slot == EnchantmentSlot.Sock1 || slot == EnchantmentSlot.Sock2 || slot == EnchantmentSlot.Sock3))
			{
				if (item.GetSocketColor((uint)(slot - EnchantmentSlot.Sock1)) == 0)
				{
					...
					if (pPrismaticEnchant == null ||
					    (pPrismaticEnchant.RequiredSkillID > 0 && pPrismaticEnchant.RequiredSkillRank > GetSkillValue((SkillType)pPrismaticEnchant.RequiredSkillID)))
						return;
				}
				...
				if (gem != null)
				{
					...
					if (gemTemplate != null)
						if (gemTemplate.GetRequiredSkill() != 0 &&
						    GetSkillValue((SkillType)gemTemplate.GetRequiredSkill()) < gemTemplate.GetRequiredSkillRank())
							return;
				}
			}
			...
			if (!item.IsBroken())
				for (int s = 0; s < ItemConst.MaxItemEnchantmentEffects; ++s)
				{
					...
					switch (enchant_display_type)
					{
						...
						{
							WeaponAttackType attackType = GetAttackBySlot(item.GetSlot(), item.GetTemplate().GetInventoryType());
			...
			{
				VisibleItem visibleItem = _values.ModifyValue(_playerData).ModifyValue(_playerData.VisibleItems, item.GetSlot());
		...
		public void ModifySkillBonus(SkillType skillid, int val, bool talent)
		{
			...
			if (skillStatusData == null ||
			    skillStatusData.State == SkillState.Deleted ||
			    skillInfoField.SkillRank[skillStatusData.Pos] == 0)
				return;
		...
		public void RemoveArenaEnchantments(EnchantmentSlot slot)
		{
			...
			for (var i = 0; i < _enchantDuration.Count; ++i)
			{
				...
				{
					if (enchantDuration.item &&
					    enchantDuration.item.GetEnchantmentId(slot) != 0)
					{
		...
		public void UpdatePotionCooldown(Spell spell = null)
		{
			...
			if (!spell)
			{
				...
				ItemTemplate proto = Global.ObjectMgr.GetItemTemplate(_lastPotionId);

				if (proto != null)
					for (byte idx = 0; idx < proto.Effects.Count; ++idx)
						if (proto.Effects[idx].SpellID != 0 &&
						    proto.Effects[idx].TriggerType == ItemSpelltriggerType.OnUse)
						{
				...
				if (newVal != 0)
				{
					...
					if (skillStatusData.State == SkillState.Unchanged)
					{
						if (currVal == 0) // activated skill, mark as new to save into database
				         ...
				         newVal == 0) // Deactivate skill line
				{
					...
					if (_activePlayerData.ProfessionSkillLine[0] == id)
						SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ProfessionSkillLine, 0), 0u);
					else if (_activePlayerData.ProfessionSkillLine[1] == id)
						SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ProfessionSkillLine, 1), 0u);
				}
			}
			else
			{
				...
				for (int i = 0; i < SkillConst.MaxPlayerSkills; ++i)
					if (((SkillInfo)_activePlayerData.Skill).SkillLineID[i] == 0)
					{
						skillSlot = (byte)i;
				...
				else
				{
					...
					if (skillEntry.CategoryID == SkillCategory.Profession)
					{
						...
						if (freeProfessionSlot != -1)
							SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ProfessionSkillLine, freeProfessionSlot), id);
					}
				}
		...
		private bool EnchantmentFitsRequirements(uint enchantmentcondition, sbyte slot)
		{
			...
			for (byte i = EquipmentSlot.Start; i < EquipmentSlot.End; ++i)
			{
				...
				if (pItem2 != null &&
				    !pItem2.IsBroken())
					foreach (SocketedGem gemData in pItem2._itemData.Gems)
					{
		...
		public void LearnSkillRewardedSpells(uint skillId, uint skillValue, Race race)
		{
			...
			foreach (var ability in skillLineAbilities)
			{
				...
				if (skillId == (uint)SkillType.Riding &&
				    (ability.AcquireMethod != AbilityLearnType.OnSkillLearn || ability.NumSkillUps != 1))
					continue;

				// Check race if set
				if (ability.RaceMask != 0 &&
				    !Convert.ToBoolean(ability.RaceMask & raceMask))
					continue;

				// Check class if set
				if (ability.ClassMask != 0 &&
				    !Convert.ToBoolean(ability.ClassMask & classMask))
					continue;
				...
				if (skillValue < ability.MinSkillLineRank &&
				    ability.AcquireMethod == AbilityLearnType.OnSkillValue)
		...
		private int FindProfessionSlotFor(uint skillId)
		{
			...
			foreach (var b in _activePlayerData.ProfessionSkillLine)
			{
				if (b == 0)
					return index;
		...
		{
			PlayerSpell spell = _spells.LookupByKey(spellId);
		...
		public void RemoveSpell(uint spellId, bool disabled = false, bool learnLowRank = true, bool suppressMessaging = false)
		{
			...
			if (pSpell.State == PlayerSpellState.Removed ||
			    (disabled && pSpell.Disabled) ||
			...
			if (nextSpell != 0)
			{
				...
				if (HasSpell(nextSpell) &&
				    !spellInfo1.HasAttribute(SpellCustomAttributes.IsTalent))
			...
			if (spellInfo != null &&
			    spellInfo.IsPrimaryProfessionFirstRank())
			{
			...
			if (_canTitanGrip)
				if (spellInfo != null &&
				    spellInfo.IsPassive() &&
				    spellInfo.HasEffect(SpellEffectName.TitanGrip))
				{
			...
			if (CanDualWield())
				if (spellInfo != null &&
				    spellInfo.IsPassive() &&
				    spellInfo.HasEffect(SpellEffectName.DualWield))
		...
		{
			StoredAuraTeleportLocation storedLocation = _storedAuraTeleportLocations.LookupByKey(spellId);
		...
		{
			StoredAuraTeleportLocation auraLocation = _storedAuraTeleportLocations.LookupByKey(spellId);
		...
		private bool AddSpell(uint spellId, bool active, bool learning, bool dependent, bool disabled, bool loading = false, uint fromSkill = 0, bool favorite = false, int? traitDefinitionId = null)
		{
			...
			if (spell != null &&
			    spell.State == PlayerSpellState.Temporary)
			...
			if (spell != null)
			{
				...
				if (spell.Active != active &&
				    spell.State != PlayerSpellState.Removed &&
				    !spell.Disabled)
				{
					spell.Active = active;
					...
					if (!IsInWorld &&
					    !learning &&
					    !dependent_set) // explicitly load from DB and then exist in it already and set correctly
						spell.State = PlayerSpellState.Unchanged;
					...
					{
						if (spellInfo.IsPassive() &&
						    HandlePassiveSpellLearn(spellInfo))
			...
			if (!loading &&
			    spellInfo.HasAttribute(SpellCustomAttributes.IsTalent) &&
			    spellInfo.HasEffect(SpellEffectName.LearnSpell))
			...
			if (castSpell)
			{
				...
				if (traitDefinitionId.HasValue)
				{
					TraitConfig traitConfig = GetTraitConfig((int)(uint)_activePlayerData.ActiveCombatTraitConfigID);

					if (traitConfig != null)
					{
						int traitEntryIndex = traitConfig.Entries.FindIndexIf(traitEntry => { return CliDB.TraitNodeEntryStorage.LookupByKey(traitEntry.TraitNodeEntryID)?.TraitDefinitionID == traitDefinitionId; });
			...
			foreach (var spellNode in spell_bounds)
			{
				...
				{
					if (!IsInWorld ||
					    !spellNode.Active) // at spells loading, no output, but allow save
		...
		public void AddSpellMod(SpellModifier mod, bool apply)
		{
			...
			switch (mod.type)
			{
				...
				case SpellModType.LabelFlat:
					if (apply)
					{
						AddDynamicUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.SpellFlatModByLabel), (mod as SpellFlatModifierByLabel).value);
					}
					else
					{
						int firstIndex = _activePlayerData.SpellFlatModByLabel.FindIndex((mod as SpellFlatModifierByLabel).value);

						if (firstIndex >= 0)
							RemoveDynamicUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.SpellFlatModByLabel), firstIndex);
					}
				...
				case SpellModType.LabelPct:
					if (apply)
					{
						AddDynamicUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.SpellPctModByLabel), (mod as SpellPctModifierByLabel).value);
					}
					else
					{
						int firstIndex = _activePlayerData.SpellPctModByLabel.FindIndex((mod as SpellPctModifierByLabel).value);

						if (firstIndex >= 0)
							RemoveDynamicUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.SpellPctModByLabel), firstIndex);
					}
		...
		public void GetSpellModValues<T>(SpellInfo spellInfo, SpellModOp op, Spell spell, T baseValue, ref int flat, ref float pct) where T : IComparable
		{
			...
			if (_spellModTakingSpell)
				spell = _spellModTakingSpell;
			...
			switch (op)
			{
				...
				case SpellModOp.ChangeCastTime:
				{
					...
					foreach (SpellModifierByClassMask mod in _spellMods[(int)op][(int)SpellModType.Pct])
					{
					...
					if (modInstantSpell == null)
						foreach (SpellPctModifierByLabel mod in _spellMods[(int)op][(int)SpellModType.LabelPct])
						{
							if (!IsAffectedBySpellmod(spellInfo, mod, spell))
								continue;
				...
				case SpellModOp.CritChance:
				{
					...
					foreach (SpellModifierByClassMask mod in _spellMods[(int)op][(int)SpellModType.Flat])
					{
						if (!IsAffectedBySpellmod(spellInfo, mod, spell))
							continue;
					...
					if (modCritical == null)
						foreach (SpellFlatModifierByLabel mod in _spellMods[(int)op][(int)SpellModType.LabelFlat])
						{
							if (!IsAffectedBySpellmod(spellInfo, mod, spell))
								continue;
		...
		{
			if (apply && _spellModTakingSpell != null)
				return;

			if (!apply &&
			    (_spellModTakingSpell == null || _spellModTakingSpell != spell))
				return;

			_spellModTakingSpell = apply ? spell : null;
		}
		...
		public void UpdateEquipSpellsAtFormChange()
		{
			for (byte i = 0; i < InventorySlots.BagEnd; ++i)
				if (_items[i] &&
				    !_items[i].IsBroken() &&
				    CanUseAttackType(GetAttackBySlot(i, _items[i].GetTemplate().GetInventoryType())))
				{
		...
		private void UpdateItemSetAuras(bool formChange = false)
		{
			...
			for (int setindex = 0; setindex < ItemSetEff.Count; ++setindex)
			{
				...
				foreach (ItemSetSpellRecord itemSetSpell in eff.SetBonuses)
				{
					...
					if (itemSetSpell.ChrSpecID != 0 &&
					    itemSetSpell.ChrSpecID != GetPrimarySpecialization())
					{
		...
		public void SetRuneCooldown(byte index, uint cooldown)
		{
			...
			int activeRunes = _runes.Cooldown.Count(p => p == 0);
		...
		public bool CanNoReagentCast(SpellInfo spellInfo)
		{
			...
			noReagentMask[0] = _activePlayerData.NoReagentCostMask[0];
			noReagentMask[1] = _activePlayerData.NoReagentCostMask[1];
			noReagentMask[2] = _activePlayerData.NoReagentCostMask[2];
			noReagentMask[3] = _activePlayerData.NoReagentCostMask[3];
		...
		{
			for (byte i = 0; i < 4; ++i)
				SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.NoReagentCostMask, i), mask[i]);
		}
		...
		public void CastItemCombatSpell(DamageInfo damageInfo)
		{
			...
			for (byte i = EquipmentSlot.Start; i < EquipmentSlot.End; ++i)
			{
				...
				if (item != null)
					if (!item.IsBroken() &&
					    CanUseAttackType(damageInfo.GetAttackType()))
					{
		...
		public void CastItemCombatSpell(DamageInfo damageInfo, Item item, ItemTemplate proto)
		{
				...
				if (!item.GetTemplate().HasFlag(ItemFlags.Legacy))
					foreach (ItemEffectRecord effectData in item.GetEffects())
					{
						...
						if (RandomHelper.randChance(chance) &&
						    Global.ScriptMgr.RunScriptRet<IItemOnCastItemCombatSpell>(tmpscript => tmpscript.OnCastItemCombatSpell(this, damageInfo.GetVictim(), spellInfo, item), item.GetScriptId()))
			...
			for (byte e_slot = 0; e_slot < (byte)EnchantmentSlot.Max; ++e_slot)
			{
				...
				for (byte s = 0; s < ItemConst.MaxItemEnchantmentEffects; ++s)
				{
					...
					if (entry != null &&
					    entry.AttributesMask.HasAnyFlag(EnchantProcAttributes.WhiteHit) &&
					    damageInfo.GetSpellInfo() != null)
						continue;
					...
					if (FindCurrentSpellBySpellId(5938) != null &&
					    e_slot == (byte)EnchantmentSlot.Temp)
						chance = 100.0f;
					...
					if (RandomHelper.randChance(chance))
					{
						...
						if (entry != null &&
						    entry.AttributesMask.HasAnyFlag(EnchantProcAttributes.Limit60) &&
						    target.GetLevelForTarget(this) > 60)
						{
		...
		{
			SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.PetSpellPower), spellPower);
		}

		public void SetSkillLineId(uint pos, ushort skillLineId)
		{
			SkillInfo skillInfo = _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.Skill);
