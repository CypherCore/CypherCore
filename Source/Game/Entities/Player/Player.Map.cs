...
namespace Game.Entities
{
	public partial class Player
	{
		...
		public void SetDungeonDifficultyID(Difficulty dungeon_difficulty)
		{
			_dungeonDifficulty = dungeon_difficulty;
		}

		public void SetRaidDifficultyID(Difficulty raid_difficulty)
		{
			_raidDifficulty = raid_difficulty;
		}

		public void SetLegacyRaidDifficultyID(Difficulty raid_difficulty)
		{
			_legacyRaidDifficulty = raid_difficulty;
		}
		...
		public static Difficulty CheckLoadedRaidDifficultyID(Difficulty difficulty)
		{
			...
			if (!difficultyEntry.Flags.HasAnyFlag(DifficultyFlags.CanSelect) ||
			    difficultyEntry.Flags.HasAnyFlag(DifficultyFlags.Legacy))
				return Difficulty.NormalRaid;
		...
		public void ConfirmPendingBind()
		{
			...
			if (map == null ||
			    map.GetInstanceId() != _pendingBindId)
				return;
		...
		public bool Satisfy(AccessRequirement ar, uint target_map, TransferAbortParams abortParams = null, bool report = false)
		{
			if (!IsGameMaster())
			{
				...
				if (LevelMin != 0 ||
				    LevelMax != 0 ||
				    failedMapDifficultyXCondition != 0 ||
				    missingItem != 0 ||
				    missingQuest != 0 ||
