...
namespace Game.Entities
{
    public class Vehicle : ITransport, IDisposable
    {
        ...
        public ITransport RemovePassenger(WorldObject passenger)
        {
            ...
            if (seat.Value.SeatInfo.HasFlag(VehicleSeatFlags.DisableGravity) &&
                !seat.Value.Passenger.IsGravityDisabled)
                unit.SetDisableGravity(false);

            // Remove UNIT_FLAG_NOT_SELECTABLE if passenger did not have it before entering vehicle
            if (seat.Value.SeatInfo.HasFlag(VehicleSeatFlags.PassengerNotSelectable) &&
                !seat.Value.Passenger.IsUninteractible)
        ...
        {
            ITransport.CalculatePassengerPosition(ref x,
                                                  ref y,
                                                  ref z,
                                                  ref o,
        ...
        {
            ITransport.CalculatePassengerOffset(ref x,
                                                ref y,
                                                ref z,
                                                ref o,
                                                GetBase().GetPositionX(),
        ...
        {
            if (GetBase().IsTypeId(TypeId.Player) ||
                !evading)
            ...
            foreach (var acc in accessories)
                if (!evading ||
                    acc.IsMinion) // only install minions on evade mode
        ...
        public void Uninstall()
        {
            ...
            if (_status == Status.UnInstalling &&
                !GetBase().HasUnitTypeMask(UnitTypeMask.Minion))
            {
        ...
        public VehicleSeatAddon GetSeatAddonForSeatOfPassenger(Unit passenger)
        {
            foreach (var pair in Seats)
                if (!pair.Value.IsEmpty() &&
                    pair.Value.Passenger.Guid == passenger.GetGUID())
            ...
            if (seatId < 0) // no specific Seat requirement
            {
                foreach (var _seat in Seats)
                {
                    ...
                    if (seat.Value.IsEmpty() &&
                        !HasPendingEventForSeat(seat.Key) &&
        ...
        public byte GetAvailableSeatCount()
        {
            ...
            foreach (var pair in Seats)
                if (pair.Value.IsEmpty() &&
                    !HasPendingEventForSeat(pair.Key) &&
        ...
        {
            StringBuilder str = new("Vehicle seats:\n");
