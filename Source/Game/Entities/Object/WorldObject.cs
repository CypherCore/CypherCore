...
{
    public abstract class WorldObject : BaseEntity, IDisposable
    {
        ...
        public void BuildValuesUpdateBlockForPlayerWithFlag(UpdateData data, UpdateFieldFlag flags, Player target)
        {
            ...
            BuildValuesUpdateWithFlag(flags, tempBuffer, target);
        ...
        public void BuildEntityFragments(WorldPacket data, EntityFragment[] fragments)
        {
        ...
        public virtual void ClearValuesChangesMask()
        {
            m_values.ClearChangesMask(m_objectData);
        }

        public virtual void BuildValuesUpdateWithFlag(UpdateFieldFlag flags, WorldPacket data, Player target)
        {
            data.WriteUInt32(0);
        }
        ...
        public virtual void BuildValuesCreate(UpdateFieldFlag flags, WorldPacket data, Player target) { }

        public virtual void BuildValuesUpdate(UpdateFieldFlag flags, WorldPacket data, Player target) { }
        ...
        public int ModSpellDuration(SpellInfo spellInfo, WorldObject target, int duration, bool positive, uint effectMask)
        {
            ...
            if (!positive)
            {
                ...
                int durationMod_always = (int)unitTarget.GetTotalAuraModifier(AuraType.MechanicDurationMod, mechanicCheck);
                // Find max mod (negative bonus)
                int durationMod_not_stack = (int)unitTarget.GetMaxNegativeAuraModifier(AuraType.MechanicDurationModNotStack, mechanicCheck);
                ...
                durationMod_always = (int)unitTarget.GetTotalAuraModifierByMiscValue(AuraType.ModAuraDurationByDispel, (int)spellInfo.Dispel);
                durationMod_not_stack = (int)unitTarget.GetMaxNegativeAuraModifierByMiscValue(AuraType.ModAuraDurationByDispelNotStack, (int)spellInfo.Dispel);
        ...
        public SpellMissInfo SpellHitResult(Unit victim, SpellInfo spellInfo, bool canReflect, bool canImmune)
        {
            ...
            if (canReflect)
            {
                float reflectchance = victim.GetTotalAuraModifier(AuraType.ReflectSpells);
