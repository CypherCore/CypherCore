...
namespace Game.Entities
{
    public class GameObject : WorldObject
    {
        ...
        bool IsAtInteractDistance(Position pos, float radius)
        {
            ...
            if (displayInfo != null)
            {
                ...
                var worldSpaceBox = MathFunctions.toWorldSpace(worldRotation.ToRotationMatrix(), new Vector3(GetPositionX(), GetPositionY(), GetPositionZ()), new Box(new Vector3(minX, minY, minZ), new Vector3(maxX, maxY, maxZ)));
        ...
        public void BuildValuesUpdateForPlayerWithMask(UpdateData data, UpdateMask requestedObjectMask, UpdateMask requestedGameObjectMask, Player target, bool ignoreNestedChangesMask)
        {
        ...
        {
            m_model = GameObjectModel.Create(new GameObjectModelOwnerImpl(this), Global.WorldMgr.GetDataPath());
            if (m_model != null)
            {
    ...
    class GameObjectModelOwnerImpl : GameObjectModelOwnerBase
    {
        ...
        public override long GetPackedRotation() { return _owner.GetPackedLocalRotation(); }
            ...
            public override void Update(uint diff)
            {
                ...
                if (oldAnimation != null && newAnimation != null)
                {
                    Matrix4x4 pathRotation = new Quaternion(_owner.m_gameObjectData.ParentRotation.GetValue().X, _owner.m_gameObjectData.ParentRotation.GetValue().Y,
                        _owner.m_gameObjectData.ParentRotation.GetValue().Z, _owner.m_gameObjectData.ParentRotation.GetValue().W).ToRotationMatrix();
