...
namespace Game.Entities
{
	...
	namespace GameObjectType
	{
		...
		{
			private static TimeSpan PositionUpdateInterval = TimeSpan.FromMilliseconds(50);
			private TransportAnimation _animationInfo;
			private bool _autoCycleBetweenStopFrames;
			private List<WorldObject> _passengers = new();
			private uint _pathProgress;
			private TimeTracker _positionUpdateTimer = new();
			private uint _stateChangeProgress;
			private uint _stateChangeTime;
			private List<uint> _stopFrames = new();
			...
			public override void Update(uint diff)
			{
				...
				else
				{
					...
					if (now < _owner._gameObjectData.Level)
					{
						int   timeToStop               = (int)(_owner._gameObjectData.Level - _stateChangeTime);
				...
				if (oldAnimation != null &&
				    newAnimation != null)
				{
					Matrix4x4 pathRotation = new Quaternion(_owner._gameObjectData.ParentRotation.GetValue().X,
					                                        _owner._gameObjectData.ParentRotation.GetValue().Y,
					                                        _owner._gameObjectData.ParentRotation.GetValue().Z,
					                                        _owner._gameObjectData.ParentRotation.GetValue().W).ToMatrix();
