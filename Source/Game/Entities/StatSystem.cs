	...
	public partial class Unit
	{
		public void HandleStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, float amount, bool apply)
		{
			if (unitMod >= UnitMods.End ||
			    modifierType >= UnitModifierFlatType.End)
			{
			...
			switch (modifierType)
			{
				...
				case UnitModifierFlatType.Total:
					_auraFlatModifiersGroup[(int)unitMod][(int)modifierType] += apply ? amount : -amount;
		...
		{
			if (unitMod >= UnitMods.End ||
			    modifierType >= UnitModifierPctType.End)
			{
			...
			switch (modifierType)
			{
				...
				case UnitModifierPctType.Total:
					MathFunctions.AddPct(ref _auraPctModifiersGroup[(int)unitMod][(int)modifierType], pct);
		...
		{
			if (_auraFlatModifiersGroup[(int)unitMod][(int)modifierType] == val)
				return;

			_auraFlatModifiersGroup[(int)unitMod][(int)modifierType] = val;
			UpdateUnitMod(unitMod);
		}

		public void SetStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, float val)
		{
			if (_auraPctModifiersGroup[(int)unitMod][(int)modifierType] == val)
				return;

			_auraPctModifiersGroup[(int)unitMod][(int)modifierType] = val;
			UpdateUnitMod(unitMod);
		}
		...
		public float GetFlatModifierValue(UnitMods unitMod, UnitModifierFlatType modifierType)
		{
			...
			return _auraFlatModifiersGroup[(int)unitMod][(int)modifierType];
		}
		...
		public float GetPctModifierValue(UnitMods unitMod, UnitModifierPctType modifierType)
		{
			...
			return _auraPctModifiersGroup[(int)unitMod][(int)modifierType];
		}
		...
		public void UpdateStatBuffMod(Stats stat)
		{
			...
			modPos += GetTotalAuraModifier(AuraType.ModStat,
			                               aurEff =>
			                               {
				                               if ((aurEff.GetMiscValue() < 0 || aurEff.GetMiscValue() == (int)stat) &&
				                                   aurEff.GetAmount() > 0)
					                               return true;
			...
			factor = GetTotalAuraMultiplier(AuraType.ModPercentStat,
			                                aurEff =>
			                                {
				                                if (aurEff.GetMiscValue() == -1 ||
				                                    aurEff.GetMiscValue() == (int)stat)
					                                return true;
		...
		{
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.StatPosBuff, (int)stat), (int)_floatStatPosBuff[(int)stat]);
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.StatNegBuff, (int)stat), (int)_floatStatNegBuff[(int)stat]);
		}
		...
		public virtual void UpdateDamagePhysical(WeaponAttackType attType)
		{
			...
			switch (attType)
			{
				...
				case WeaponAttackType.OffAttack:
					SetUpdateFieldStatValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MinOffHandDamage), minDamage);
					SetUpdateFieldStatValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MaxOffHandDamage), maxDamage);

					break;
				case WeaponAttackType.RangedAttack:
					SetUpdateFieldStatValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MinRangedDamage), minDamage);
					SetUpdateFieldStatValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MaxRangedDamage), maxDamage);
		...
		{
			return _unitData.Stats[(int)stat];
		}

		public void SetStat(Stats stat, int val)
		{
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.Stats, (int)stat), val);
		}

		public uint GetCreateMana()
		{
			return _unitData.BaseMana;
		}

		public void SetCreateMana(uint val)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.BaseMana), val);
		}
		...
		{
			return _unitData.StatPosBuff[(int)stat];
		}

		public float GetNegStat(Stats stat)
		{
			return _unitData.StatNegBuff[(int)stat];
		}

		public int GetResistance(SpellSchools school)
		{
			return _unitData.Resistances[(int)school];
		}

		public int GetBonusResistanceMod(SpellSchools school)
		{
			return _unitData.BonusResistanceMods[(int)school];
		}
		...
		public int GetResistance(SpellSchoolMask mask)
		{
			...
			for (int i = (int)SpellSchools.Normal; i < (int)SpellSchools.Max; ++i)
			{
				...
				if (Convert.ToBoolean((int)mask & (1 << i)) &&
				    (!resist.HasValue || resist.Value > schoolResistance))
		...
		{
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.Resistances, (int)school), val);
		}

		public void SetBonusResistanceMod(SpellSchools school, int val)
		{
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.BonusResistanceMods, (int)school), val);
		}

		public void SetModCastingSpeed(float castingSpeed)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModCastingSpeed), castingSpeed);
		}

		public void SetModSpellHaste(float spellHaste)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModSpellHaste), spellHaste);
		}

		public void SetModHaste(float haste)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModHaste), haste);
		}

		public void SetModRangedHaste(float rangedHaste)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModRangedHaste), rangedHaste);
		}

		public void SetModHasteRegen(float hasteRegen)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModHasteRegen), hasteRegen);
		}

		public void SetModTimeRate(float timeRate)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ModTimeRate), timeRate);
		}
		...
		{
			return _unitData.BaseHealth;
		}

		public void SetCreateHealth(uint val)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.BaseHealth), val);
		}

		public ulong GetHealth()
		{
			return _unitData.Health;
		}

		public void SetHealth(ulong val)
		{
			if (GetDeathState() == DeathState.JustDied ||
			    GetDeathState() == DeathState.Corpse)
			{
				val = 0;
			}
			...
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.Health), val);
		...
		public void SetMaxHealth(ulong val)
		{
			...
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MaxHealth), val);
		...
		public bool HealthBelowPctDamaged(int pct, uint damage)
		{
		...
		public bool HealthAbovePctHealed(int pct, uint heal)
		{
			return GetHealth() + heal > CountPctFromMaxHealth(pct);
		}

		public ulong CountPctFromMaxHealth(int pct)
		{
			return MathFunctions.CalculatePct(GetMaxHealth(), pct);
		}

		public ulong CountPctFromCurHealth(int pct)
		{
		...
		{
			return (PowerType)(byte)_unitData.DisplayPower;
		}
		...
		public void SetPowerType(PowerType powerType, bool sendUpdate = true)
		{
			...
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.DisplayPower), (byte)powerType);
		...
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.OverrideDisplayPowerID), powerDisplayId);
		}
		...
		public void SetMaxPower(PowerType powerType, int val)
		{
			...
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.MaxPower, (int)powerIndex), (uint)val);
		...
		public void SetPower(PowerType powerType, int val, bool withPowerUpdate = true)
		{
			...
			int oldPower = _unitData.Power[(int)powerIndex];
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.Power, (int)powerIndex), val);
		...
		public int GetPower(PowerType powerType)
		{
			...
			return _unitData.Power[(int)powerIndex];
		}
		...
		public int GetMaxPower(PowerType powerType)
		{
			...
			return (int)(uint)_unitData.MaxPower[(int)powerIndex];
		}
		...
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.AttackPower), attackPower);
		}

		public void SetAttackPowerModPos(int attackPowerMod)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.AttackPowerModPos), attackPowerMod);
		}

		public void SetAttackPowerModNeg(int attackPowerMod)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.AttackPowerModNeg), attackPowerMod);
		}

		public void SetAttackPowerMultiplier(float attackPowerMult)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.AttackPowerMultiplier), attackPowerMult);
		}

		public void SetRangedAttackPower(int attackPower)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.RangedAttackPower), attackPower);
		}

		public void SetRangedAttackPowerModPos(int attackPowerMod)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.RangedAttackPowerModPos), attackPowerMod);
		}

		public void SetRangedAttackPowerModNeg(int attackPowerMod)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.RangedAttackPowerModNeg), attackPowerMod);
		}

		public void SetRangedAttackPowerMultiplier(float attackPowerMult)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.RangedAttackPowerMultiplier), attackPowerMult);
		}

		public void SetMainHandWeaponAttackPower(int attackPower)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.MainHandWeaponAttackPower), attackPower);
		}

		public void SetOffHandWeaponAttackPower(int attackPower)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.OffHandWeaponAttackPower), attackPower);
		}

		public void SetRangedWeaponAttackPower(int attackPower)
		{
			SetUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.RangedWeaponAttackPower), attackPower);
		}
		...
		{
			if (spellInfo != null &&
			    spellInfo.HasAttribute(SpellAttr7.NoAttackMiss))
				return 0.0f;
			...
			if (spellInfo == null &&
			    HaveOffhandWeapon() &&
			    !IsInFeralForm() &&
			    !HasAuraType(AuraType.IgnoreDualWieldHitPenalty))
				missChance += 19.0f;
		...
		private float GetUnitCriticalChanceDone(WeaponAttackType attackType)
		{
			...
			if (thisPlayer != null)
			{
				...
				{
					case WeaponAttackType.BaseAttack:
						chance = thisPlayer._activePlayerData.CritPercentage;

						break;
					case WeaponAttackType.OffAttack:
						chance = thisPlayer._activePlayerData.OffhandCritPercentage;

						break;
					case WeaponAttackType.RangedAttack:
						chance = thisPlayer._activePlayerData.RangedCritPercentage;
		...
		private float GetUnitCriticalChanceTaken(Unit attacker, WeaponAttackType attackType, float critDone)
		{
		...
		private float GetUnitCriticalChanceAgainst(WeaponAttackType attackType, Unit victim)
		{
		...
		private float GetUnitDodgeChance(WeaponAttackType attType, Unit victim)
		{
			...
			if (playerVictim)
			{
				chance = playerVictim._activePlayerData.DodgePercentage;
			}
			else
			{
				if (!victim.IsTotem())
		...
		private float GetUnitParryChance(WeaponAttackType attType, Unit victim)
		{
			int levelDiff = (int)(victim.GetLevelForTarget(this) - GetLevelForTarget(victim));
			...
			if (playerVictim)
			{
				if (playerVictim.CanParry())
				{
					...
					if (tmpitem)
						chance = playerVictim._activePlayerData.ParryPercentage;
				}
			}
			else
			{
				if (!victim.IsTotem() &&
				    !victim.ToCreature().GetCreatureTemplate().FlagsExtra.HasAnyFlag(CreatureFlagsExtra.NoParry))
				{
					chance =  6.0f;
		...
		private float GetUnitBlockChance(WeaponAttackType attType, Unit victim)
		{
			int levelDiff = (int)(victim.GetLevelForTarget(this) - GetLevelForTarget(victim));
			...
			if (playerVictim)
			{
				if (playerVictim.CanBlock())
				{
					...
					if (tmpitem &&
					    !tmpitem.IsBroken() &&
					    tmpitem.GetTemplate().GetInventoryType() == InventoryType.Shield)
						chance = playerVictim._activePlayerData.BlockPercentage;
				}
			}
			else
			{
		...
		{
			ApplyModUpdateFieldValue(_values.ModifyValue(_unitData).ModifyValue(_unitData.ManaCostMultiplier), manaCostMultiplier, apply);
		}

		public void ApplyModManaCostModifier(SpellSchools school, int mod, bool apply)
		{
			ApplyModUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.ManaCostModifier, (int)school), mod, apply);
		}
	}

	public partial class Player
	{
		private float[] dodge_cap =
		{
			65.631440f,  // Warrior            
			65.631440f,  // Paladin
			145.560408f, // Hunter
		...
		{
			ApplyModUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModTargetResistance), mod, apply);
		}

		public void ApplyModTargetPhysicalResistance(int mod, bool apply)
		{
			ApplyModUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModTargetPhysicalResistance), mod, apply);
		}
		...
		{
			ApplyModUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDonePos, (int)school), mod, apply);
		}

		public void ApplyModDamageDoneNeg(SpellSchools school, int mod, bool apply)
		{
			ApplyModUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDoneNeg, (int)school), mod, apply);
		}

		public void ApplyModDamageDonePercent(SpellSchools school, float pct, bool apply)
		{
			ApplyPercentModUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDonePercent, (int)school), pct, apply);
		}

		public void SetModDamageDonePercent(SpellSchools school, float pct)
		{
			SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDonePercent, (int)school), pct);
		}
		...
		public void UpdateManaRegen()
		{
			...
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.PowerRegenFlatModifier, (int)manaIndex), base_regen);
			SetUpdateFieldValue(ref _values.ModifyValue(_unitData).ModifyValue(_unitData.PowerRegenInterruptedFlatModifier, (int)manaIndex), base_regen);
		}
		...
		public void UpdateSpellDamageAndHealingBonus()
		{
			...
			SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModHealingDonePos), (int)SpellBaseHealingBonusDone(SpellSchoolMask.All));
			...
			{
				SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDoneNeg, i),
				                    modDamageAuras.Aggregate(0,
				                                             (negativeMod, aurEff) =>
				                                             {
					                                             if (aurEff.GetAmount() < 0 &&
					                                                 Convert.ToBoolean(aurEff.GetMiscValue() & (1 << i)))
						                                             negativeMod += aurEff.GetAmount();

					                                             return negativeMod;
				                                             }));

				SetUpdateFieldStatValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModDamageDonePos, i),
				                        (SpellBaseDamageBonusDone((SpellSchoolMask)(1 << i)) - _activePlayerData.ModDamageDoneNeg[i]));
			}
		...
		public override void UpdateAttackPowerAndDamage(bool ranged = false)
		{
			...
			if (!HasAuraType(AuraType.OverrideAttackPowerBySpPct))
			{
				if (!ranged)
				{
					...
					if (form != null &&
					    Convert.ToBoolean((uint)form.Flags & 0x20))
			...
			{
				int minSpellPower = _activePlayerData.ModHealingDonePos;

				for (var i = SpellSchools.Holy; i < SpellSchools.Max; ++i)
					minSpellPower = Math.Min(minSpellPower, _activePlayerData.ModDamageDonePos[(int)i]);

				val2 = MathFunctions.CalculatePct(minSpellPower, _activePlayerData.OverrideAPBySpellPowerPercent);
			}
			...
			if (ranged)
			{
				...
				if (pet != null &&
				    pet.IsHunterPet()) // At ranged attack change for hunter pet
			...
			else
			{
				...
				if (offhand)
					if (CanDualWield() ||
					    offhand.GetTemplate().HasFlag(ItemFlags3.AlwaysAllowDualWield))
						UpdateDamagePhysical(WeaponAttackType.OffAttack);
				...
				if (pet != null &&
				    pet.IsPetGhoul()) // At melee attack power change for DK pet
					pet.UpdateAttackPowerAndDamage();

				if (guardian != null &&
				    guardian.IsSpiritWolf()) // At melee attack power change for Shaman feral spirit
		...
		public void UpdateRating(CombatRating cr)
		{
			...
			uint oldRating = _activePlayerData.CombatRatings[(int)cr];
			SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.CombatRatings, (int)cr), (uint)amount);
		...
		public void UpdateMastery()
		{
			if (!CanUseMastery())
			{
				SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.Mastery), 0.0f);
			...
			SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.Mastery), value);
		...
		public void UpdateVersatilityDamageDone()
		{
			...
			SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.Versatility), (int)_activePlayerData.CombatRatings[(int)CombatRating.VersatilityDamageDone]);
		...
		public void UpdateHealingDonePercentMod()
		{
			...
			for (int i = 0; i < (int)SpellSchools.Max; ++i)
				SetUpdateFieldStatValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModHealingDonePercent, i), value);
		}
		...
		{
			// Store Rating Value
			SetUpdateFieldValue(ref _values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.CombatRatings, (int)CombatRating.ArmorPenetration), (uint)amount);
		}
		...
		public void UpdateParryPercentage()
		{
			...
			SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ParryPercentage), value);
		}
		...
		public void UpdateDodgePercentage()
		{
			...
			SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.DodgePercentage), value);
		}
		...
		public void UpdateBlockPercentage()
		{
			...
			SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.BlockPercentage), value);
		}
		...
		public void UpdateCritPercentage(WeaponAttackType attType)
		{
			...
			{
				case WeaponAttackType.OffAttack:
					SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.OffhandCritPercentage),
					...
					break;
				case WeaponAttackType.RangedAttack:
					SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.RangedCritPercentage),
				...
				case WeaponAttackType.BaseAttack:
				default:
					SetUpdateFieldStatValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.CritPercentage),
		...
		public void UpdateExpertise(WeaponAttackType attack)
		{
			...
			{
				case WeaponAttackType.BaseAttack:
					SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.MainhandExpertise), expertise);

					break;
				case WeaponAttackType.OffAttack:
					SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.OffhandExpertise), expertise);
		...
		public void UpdateSpellCritChance()
		{
			...
			// Store crit value
			SetUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.SpellCritPercentage), crit);
		}
		...
		private void ApplySpellPowerBonus(int amount, bool apply)
		{
			...
			ApplyModUpdateFieldValue(_values.ModifyValue(_activePlayerData).ModifyValue(_activePlayerData.ModHealingDonePos), amount, apply);
