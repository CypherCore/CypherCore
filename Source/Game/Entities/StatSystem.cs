...
namespace Game.Entities
{
    public partial class Unit
    {
        ...
        private int GetMinPower(PowerType power) { return power == PowerType.LunarPower ? -100 : 0; }
        ...
        public int GetResistance(SpellSchoolMask mask)
        {
            ...
            {
                var schoolResistance = GetResistance((SpellSchools)i) + GetBonusResistanceMod((SpellSchools)i);
        ...
        private uint GetDamageReduction(uint damage) { return GetCombatRatingDamageReduction(CombatRating.ResiliencePlayerDamage, 1.0f, 100.0f, damage); }
        ...
        //Chances
        private float MeleeSpellMissChance(Unit victim, WeaponAttackType attType, SpellInfo spellInfo)
        {
            //calculate miss chance
            var missChance = victim.GetUnitMissChance(attType);
        ...
        private float GetUnitCriticalChance(WeaponAttackType attackType, Unit victim)
        {
            var chance = 0.0f;
            var thisPlayer = ToPlayer();
            if (thisPlayer != null)
        ...
        private float GetUnitMissChance(WeaponAttackType attType)
        {
        ...
        public int GetMechanicResistChance(SpellInfo spellInfo)
        {
            ...
            foreach (var effect in spellInfo.GetEffects())
            {
                ...
                var effect_mech = (int)spellInfo.GetEffectMechanic(effect.EffectIndex);
                if (effect_mech != 0)
                {
    ...
    public partial class Player
    {
        ...
        public void UpdateManaRegen()
        {
            ...
            var base_regen = basemana / 100.0f;
        ...
        public override void UpdateArmor()
        {
            ...
            var value = GetFlatModifierValue(unitMod, UnitModifierFlatType.Base);    // base armor (from items)
            var baseValue = value;
        ...
        {
            var amount = baseRatingValue[(int)cr];
            ...
            var oldRating = m_activePlayerData.CombatRatings[(int)cr];
        ...
        public void UpdateMastery()
        {
            ...
            for (uint i = 0; i < PlayerConst.MaxMasterySpells; ++i)
            {
                ...
                if (aura != null)
                {
                    foreach (var effect in aura.GetSpellInfo().GetEffects())
                    {
                        if (effect == null)
                            continue;

                        var mult = effect.BonusCoefficient;
