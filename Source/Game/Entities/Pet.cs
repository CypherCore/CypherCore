...
namespace Game.Entities
{
	public class Pet : Guardian
	{
		...
		public new Dictionary<uint, PetSpell> _spells = new();
		...
		{
			if (_removed) // pet already removed, just wait in remove queue, no updates
				return;
		...
		{
			foreach (var pair in _spells.ToList())
			{
		...
		private bool AddSpell(uint spellId, ActiveStates active = ActiveStates.Decide, PetSpellState state = PetSpellState.New, PetSpellType type = PetSpellType.Normal)
		{
			...
			var petSpell = _spells.LookupByKey(spellId);
		...
		public void ToggleAutocast(SpellInfo spellInfo, bool apply)
		{
			...
			var petSpell = _spells.LookupByKey(spellInfo.Id);
		...
		{
			var petSpell = _spells.LookupByKey(spell);
	...
	{
		private static uint UnslottedPetIndexMask = 0x80000000;
