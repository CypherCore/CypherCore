...
namespace Game
{
	public class ReputationMgr
	{
		...
		private SortedDictionary<uint, FactionState> _factions = new();
		private Dictionary<uint, ReputationRank> _forcedReactions = new();
		...
		{
			if (renownFactionEntry == null ||
			    !IsRenownReputation(renownFactionEntry))
				return 0;
		...
		public bool SetReputation(FactionRecord factionEntry, int standing, bool incremental, bool spillOverOnly, bool noSpillover)
		{
			...
			if (!noSpillover)
			{
				...
				else
				{
					...
					if (flist == null &&
					    factionEntry.ParentFactionID != 0 &&
					    factionEntry.ParentFactionMod[1] != 0.0f)
					{
						...
						if (parent != null)
						{
							...
							if (parentState != null &&
							    parentState.Flags.HasFlag(ReputationFlags.HeaderShowsBar))
						...
						foreach (var id in flist)
						{
							...
							if (factionEntryCalc != null)
							{
								if (factionEntryCalc == factionEntry ||
								    GetRank(factionEntryCalc) > (ReputationRank)factionEntryCalc.ParentFactionMod[0])
									continue;
			...
			if (faction != null)
			{
				...
				if (incremental &&
				    standing > 0 &&
				    CanGainParagonReputationForFaction(factionEntry))
				{
		...
		public bool SetOneFactionReputation(FactionRecord factionEntry, int standing, bool incremental)
		{
			...
			if (factionState != null)
			{
				...
				if (!IsParagonReputation(factionEntry) &&
				    !IsRenownReputation(factionEntry))
				{
		...
		public void SetVisible(FactionTemplateRecord factionTemplateEntry)
		{
			...
			if (factionEntry.Id != 0)
				...
				if (!Convert.ToBoolean(factionEntry.ReputationRaceMask[1] & SharedConst.GetMaskForRace(_player.GetRace())) &&
		...
		private void SetVisible(FactionState faction)
		{
			...
			if (faction.Flags.HasFlag(ReputationFlags.Header) &&
			    !faction.Flags.HasFlag(ReputationFlags.HeaderShowsBar))
				return;
		...
		private void SetAtWar(FactionState faction, bool atWar)
		{
			...
			if (atWar &&
			    faction.Flags.HasFlag(ReputationFlags.Peaceful) &&
		...
		private bool CanGainParagonReputationForFaction(FactionRecord factionEntry)
		{
			...
			if (GetRank(factionEntry) != ReputationRank.Exalted &&
			    !HasMaximumRenownReputation(factionEntry))
				return false;
		...
		public byte GetVisibleFactionCount()
		{
			return _visibleFactionCount;
		}

		public byte GetHonoredFactionCount()
		{
			return _honoredFactionCount;
		}

		public byte GetReveredFactionCount()
		{
			return _reveredFactionCount;
		}

		public byte GetExaltedFactionCount()
		{
			return _exaltedFactionCount;
		}

		public SortedDictionary<uint, FactionState> GetStateList()
		{
			return _factions;
		}
