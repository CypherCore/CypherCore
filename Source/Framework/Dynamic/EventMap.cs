...
namespace Framework.Dynamic
{
    public class EventMap
    {
        ...
        public void CancelEventGroup(uint group)
        {
            ...
            foreach (var pair in _eventMap.KeyValueList)
            {
                if ((pair.Value & (uint)(1 << ((int)group + 15))) != 0)
                    _eventMap.Remove(pair.Key, pair.Value);
            }

            foreach (var key in _timerSeries.Keys.ToList())
            {
                if ((key & (1 << ((int)group + 15))) != 0)
                    _timerSeries.Remove(key);
            }
        }
        ...
        public void ScheduleNextFromSeries(uint eventData)
        {
        ...
        public void ScheduleEventSeries(uint eventId, byte group, byte phase, List<TimeSpan> timeSeries)
        {
            if (group != 0 && group <= 8)
                eventId |= (1u << (group + 15));
        ...
        /// <param name="eventId">eventId of the event.</param>
        /// <param name="timeSeries">timeSeries specifying the times the event should be automatically scheduled after each trigger (first value is initial schedule)</param>
        public void ScheduleEventSeries(uint eventId, List<TimeSpan> timeSeries)
        {
        ...
        /// </summary>
        Dictionary<uint /*event data*/, Queue<TimeSpan>> _timerSeries = new();
    }
}
