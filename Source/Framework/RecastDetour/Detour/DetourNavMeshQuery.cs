...
public static partial class Detour
{
    ...
    {
        public float[] _areaCost = new float[DT_MAX_AREAS]; //< Cost per area type. (Used by default implementation.)
        public ushort _excludeFlags;                        //< Flags for polygons that should not be visted. (Used by default implementation.)
        public ushort _includeFlags;                        //< Flags for polygons that can be visited. (Used by default implementation.)
        ...
        {
            return (poly.flags & _includeFlags) != 0 && (poly.flags & _excludeFlags) == 0;
        }
        ...
        {
            return dtVdist(pa, pb) * _areaCost[curPoly.getArea()];
        }
    ...
    public class dtNavMeshQuery
    {
        ...
        private dtNavMesh _nav;             //< Pointer to navmesh data.
        private dtNodePool _nodePool;       //< Pointer to node pool.
        private dtNodeQueue _openList;      //< Pointer to open list queue.
        private readonly dtQueryData _query = new(); //< Sliced query state.

        private dtNodePool _tinyNodePool; //< Pointer to small node pool.
        ...
        public uint init(dtNavMesh nav, int maxNodes)
        {
            ...
            if (_nodePool == null ||
                _nodePool.getMaxNodes() < maxNodes)
            {
                if (_nodePool != null)
                ...
                _nodePool = new dtNodePool(maxNodes, (int)dtNextPow2((uint)(maxNodes / 4))); //(dtAlloc(sizeof(dtNodePool), DT_ALLOC_PERM)) dtNodePool(maxNodes, dtNextPow2(maxNodes/4));
            ...
            {
                _tinyNodePool = new dtNodePool(64, 32); //(dtAlloc(sizeof(dtNodePool), DT_ALLOC_PERM)) dtNodePool(64, 32);
            ...
            if (_openList == null ||
                _openList.getCapacity() < maxNodes)
            {
                if (_openList != null)
                ...
                _openList = new dtNodeQueue(maxNodes); //(dtAlloc(sizeof(dtNodeQueue), DT_ALLOC_PERM)) dtNodeQueue(maxNodes);
        ...
        public uint findRandomPoint(dtQueryFilter filter, randomFloatGenerator frand, ref ulong randomRef, ref float[] randomPt)
        {
            ...
            {
                dtMeshTile curTile = _nav.getTile(i);

                if (curTile == null ||
                    curTile.header == null) continue;
            ...
            dtPoly poly = null;
            ulong polyRef = 0;
            ulong polyRefBase = _nav.getPolyRefBase(tile);
            ...
            float h = 0.0f;
            uint status = getPolyHeight(polyRef, pt, ref h);
        ...
        {
            Debug.Assert(_nav != null);
            Debug.Assert(_nodePool != null);
            Debug.Assert(_openList != null);

            // Validate input
            if (!_nav.isValidPolyRef(startRef) ||
                centerPos == null ||
            ...
            _nav.getTileAndPolyByRefUnsafe(startRef, ref startTile, ref startPoly);
            ...
            while (!_openList.empty())
            {
                ...
                ulong bestRef = bestNode.id;
                dtMeshTile bestTile = null;
                dtPoly bestPoly = null;
                _nav.getTileAndPolyByRefUnsafe(bestRef, ref bestTile, ref bestPoly);
                ...
                if (parentRef != 0)
                    _nav.getTileAndPolyByRefUnsafe(parentRef, ref parentTile, ref parentPoly);

                for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    dtLink link = bestTile.links[i];
                    ulong neighbourRef = link.polyRef;

                    // Skip invalid neighbours and do not follow back to parent.
                    if (neighbourRef == 0 ||
                        neighbourRef == parentRef)
                        continue;

                    // Expand to neighbour
                    dtMeshTile neighbourTile = null;
                    dtPoly neighbourPoly = null;
                    _nav.getTileAndPolyByRefUnsafe(neighbourRef, ref neighbourTile, ref neighbourPoly);
            ...
            float h = 0.0f;
            uint stat = getPolyHeight(randomPolyRef, pt, ref h);
        ...
        public uint closestPointOnPoly(ulong polyRef, float[] pos, float[] closest, ref bool posOverPoly)
        {
            Debug.Assert(_nav != null);

            if (!_nav.isValidPolyRef(polyRef) ||
                pos == null ||
                !dtVisfinite(pos) ||
                closest == null)
                return DT_FAILURE | DT_INVALID_PARAM;

            _nav.closestPointOnPoly(polyRef, pos, closest, ref posOverPoly);
        ...
        public uint closestPointOnPolyBoundary(ulong polyRef, float[] pos, float[] closest)
        {
            ...
            if (dtStatusFailed(_nav.getTileAndPolyByRef(polyRef, ref tile, ref poly)))
                return DT_FAILURE | DT_INVALID_PARAM;
        ...
        public uint getPolyHeight(ulong polyRef, float[] pos, ref float height)
        {
            ...
            if (dtStatusFailed(_nav.getTileAndPolyByRef(polyRef, ref tile, ref poly, ref ip)))
                return DT_FAILURE | DT_INVALID_PARAM;
            ...
            return _nav.getPolyHeight(tile, poly, pos, ref height) ? DT_SUCCESS : DT_FAILURE | DT_INVALID_PARAM;
        }
        ...
        public uint findNearestPoly(float[] center, float[] halfExtents, dtQueryFilter filter, ref ulong nearestRef, ref float[] nearestPt)
        {
            Debug.Assert(_nav != null);
            ...
            uint status = queryPolygons(center, halfExtents, filter, query);
        ...
        public void queryPolygonsInTile(dtMeshTile tile, float[] qmin, float[] qmax, dtQueryFilter filter, dtFindNearestPolyQuery query)
        {
            ...
            ulong[] polyRefs = new ulong[batchSize];
            ...
            if (tile.bvTree != null)
            {
                ...
                while (nodeIndex < endIndex)
                {
                    ...
                    {
                        ulong polyRef = polyRefBase | (uint)node.i;
            ...
            else
            {
                ...
                for (int i = 0; i < tile.header.polyCount; ++i)
                {
                    ...
                    // Must pass filter
                    ulong polyRef = polyRefBase | (uint)i;
        ...
        public uint queryPolygons(float[] center, float[] halfExtents, dtQueryFilter filter, dtFindNearestPolyQuery query)
        {
            ...
            _nav.calcTileLoc(bmin, ref minx, ref miny);
            _nav.calcTileLoc(bmax, ref maxx, ref maxy);
            ...
            for (int y = miny; y <= maxy; ++y)
            {
                ...
                {
                    int nneis = _nav.getTilesAt(x, y, neis, MAX_NEIS);
        ...
        public uint findPath(ulong startRef, ulong endRef, float[] startPos, float[] endPos, dtQueryFilter filter, ulong[] path, ref uint pathCount, int maxPath)
        {
            ...
            while (!_openList.empty())
            {
                ...
                for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    ulong neighbourRef = bestTile.links[i].polyRef;
                    ...
                    dtNode neighbourNode = _nodePool.getNode(neighbourRef, crossSide);
                    ...
                    {
                        // Cost
                        float curCost = filter.getCost(bestNode.pos,
                                                       neighbourNode.pos,
                                                       parentRef,
                                                       parentTile,
                                                       parentPoly,
                        ...
                        float endCost = filter.getCost(neighbourNode.pos,
                                                       endPos,
                                                       bestRef,
                                                       bestTile,
                                                       bestPoly,
            ...
            uint status = getPathToNode(lastBestNode, path, ref pathCount, maxPath);
        ...
        public uint initSlicedFindPath(ulong startRef, ulong endRef, float[] startPos, float[] endPos, dtQueryFilter filter, uint options)
        {
            ...
            _query.status = DT_FAILURE;
            _query.startRef = startRef;
            _query.endRef = endRef;
            dtVcopy(_query.startPos, startPos);
            dtVcopy(_query.endPos, endPos);
            _query.filter = filter;
            _query.options = options;
            _query.raycastLimitSqr = float.MaxValue;
            ...
            if ((options & (int)dtFindPathOptions.DT_FINDPATH_ANY_ANGLE) != 0)
            {
                ...
                dtMeshTile tile = _nav.getTileByRef(startRef);
                float agentRadius = tile.header.walkableRadius;
                _query.raycastLimitSqr = dtSqr(agentRadius * 50.0f); //DT_RAY_CAST_LIMIT_PROPORTIONS;
            }

            if (startRef == endRef)
            {
                _query.status = DT_SUCCESS;
        ...
        {
            if (!dtStatusInProgress(_query.status))
                return _query.status;

            // Make sure the request is still valid.
            if (!_nav.isValidPolyRef(_query.startRef) ||
                !_nav.isValidPolyRef(_query.endRef))
            {
                _query.status = DT_FAILURE;
            ...
            while (iter < maxIter && !_openList.empty())
            {
                ...
                if (bestNode.id == _query.endRef)
                {
                    ...
                    uint details = _query.status & DT_STATUS_DETAIL_MASK;
                    _query.status = DT_SUCCESS | details;
                    //if (doneIters)
                    doneIters = iter;
                ...
                if (dtStatusFailed(_nav.getTileAndPolyByRef(bestRef, ref bestTile, ref bestPoly)))
                {
                ...
                if (bestNode.pidx != 0)
                {
                    parentNode = _nodePool.getNodeAtIdx(bestNode.pidx);
                    parentRef = parentNode.id;

                    if (parentNode.pidx != 0)
                        grandpaRef = _nodePool.getNodeAtIdx(parentNode.pidx).id;
                }

                if (parentRef != 0)
                {
                    bool invalidParent = dtStatusFailed(_nav.getTileAndPolyByRef(parentRef, ref parentTile, ref parentPoly));

                    if (invalidParent || (grandpaRef != 0 && !_nav.isValidPolyRef(grandpaRef)))
                    {
                        // The polygon has disappeared during the sliced query, fail.
                        _query.status = DT_FAILURE;
                        //if (doneIters)
                ...
                if ((_query.options & (int)dtFindPathOptions.DT_FINDPATH_ANY_ANGLE) != 0)
                    if ((parentRef != 0) &&
                        (dtVdistSqr(parentNode.pos, bestNode.pos) < _query.raycastLimitSqr))
                        tryLOS = true;
                ...
                for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    ...
                    if (!_query.filter.passFilter(neighbourRef, neighbourTile, neighbourPoly))
                        continue;
                    ...
                    if (tryLOS)
                    {
                        raycast(parentRef, parentNode.pos, neighbourNode.pos, _query.filter, (int)dtRaycastOptions.DT_RAYCAST_USE_COSTS, rayHit, grandpaRef);
                    ...
                    {
                        // No shortcut found.
                        float curCost = _query.filter.getCost(bestNode.pos,
                                                              neighbourNode.pos,
                                                              parentRef,
                                                              parentTile,
                                                              parentPoly,
                    ...
                    // Special case for last node.
                    if (neighbourRef == _query.endRef)
                    {
                        float endCost = _query.filter.getCost(neighbourNode.pos,
                                                              _query.endPos,
                                                              bestRef,
                                                              bestTile,
                                                              bestPoly,
                    ...
                    {
                        heuristic = dtVdist(neighbourNode.pos, _query.endPos) * H_SCALE;
                    }
                    ...
                    // Add or update the node.
                    neighbourNode.pidx = foundShortCut ? bestNode.pidx : _nodePool.getNodeIdx(bestNode);
                    neighbourNode.id = neighbourRef;
                    ...
                    if (heuristic < _query.lastBestNodeCost)
                    {
                        ...
                        _query.lastBestNode = neighbourNode;
                    }
                }
            }
        ...
        public uint finalizeSlicedFindPath(ulong[] path, ref int pathCount, int maxPath)
        {
            ...
            if (dtStatusFailed(_query.status))
            {
                // Reset query.
                //memset(&m_query, 0, sizeof(dtQueryData));
                _query.dtcsClear();
            ...
            {
                // Reverse the path.
                Debug.Assert(_query.lastBestNode != null);

                if (_query.lastBestNode.id != _query.endRef)
                    _query.status |= DT_PARTIAL_RESULT;

                dtNode prev = null;
                dtNode node = _query.lastBestNode;
                int prevRay = 0;

                do
                {
                    dtNode next = _nodePool.getNodeAtIdx(node.pidx);
                ...
                do
                {
                    ...
                    if ((node.flags & (byte)dtNodeFlags.DT_NODE_PARENT_DETACHED) != 0)
                    {
                        ...
                        uint m = 0;
                        ulong[] temp = new ulong[path.Length];
                        status = raycast(node.id, node.pos, next.pos, _query.filter, ref t, normal, temp, ref m, maxPath - n);
                    ...
                    {
                        _query.status |= status & DT_STATUS_DETAIL_MASK;
        ...
        public uint finalizeSlicedFindPathPartial(ulong[] existing, int existingSize, ulong[] path, ref int pathCount, int maxPath)
        {
            ...
            else
            {
                ...
                {
                    node = _nodePool.findNode(existing[i]);
                ...
                do
                {
                    ...
                    if ((node.flags & (byte)dtNodeFlags.DT_NODE_PARENT_DETACHED) != 0)
                    {
                        ...
                        float[] normal = new float[3];
                        uint m = 0;
                        ulong[] temp = new ulong[path.Length - n];
                        status = raycast(node.id, node.pos, next.pos, _query.filter, ref t, normal, temp, ref m, maxPath - n);
        ...
        private uint appendVertex(float[] pos, byte flags, ulong polyRef, float[] straightPath, byte[] straightPathFlags, ulong[] straightPathRefs, ref int straightPathCount, int maxStraightPath)
        {
            if (straightPathCount > 0 &&
                dtVequal(straightPath, (straightPathCount - 1) * 3, pos, 0))
            {
        ...
        private uint appendPortals(int startIdx, int endIdx, float[] endPos, ulong[] path, float[] straightPath, byte[] straightPathFlags, ulong[] straightPathRefs, ref int straightPathCount, int maxStraightPath, int options)
        {
            ...
            for (int i = startIdx; i < endIdx; i++)
            {
                ...
                if (dtStatusFailed(_nav.getTileAndPolyByRef(from, ref fromTile, ref fromPoly)))
                    return DT_FAILURE | DT_INVALID_PARAM;
                ...
                if (dtStatusFailed(_nav.getTileAndPolyByRef(to, ref toTile, ref toPoly)))
                    return DT_FAILURE | DT_INVALID_PARAM;
        ...
        public uint findStraightPath(float[] startPos, float[] endPos, ulong[] path, int pathSize, float[] straightPath, byte[] straightPathFlags, ulong[] straightPathRefs, ref int straightPathCount, int maxStraightPath, int options)
        {
            ...
            stat = appendVertex(closestStartPos,
                                (byte)dtStraightPathFlags.DT_STRAIGHTPATH_START,
                                path[0],
                                straightPath,
                                straightPathFlags,
                                straightPathRefs,
            ...
            if (pathSize > 1)
            {
                ...
                ulong leftPolyRef = path[0];
                ulong rightPolyRef = path[0];
                ...
                for (int i = 0; i < pathSize; ++i)
                {
                    ...
                    if (i + 1 < pathSize)
                    {
                        ...
                        if (dtStatusFailed(getPortalPoints(path[i], path[i + 1], left, right, ref fromType, ref toType)))
                        {
                            ...
                            stat = appendVertex(closestEndPos,
                                                0,
                                                path[i],
                                                straightPath,
                                                straightPathFlags,
            ...
            stat = appendVertex(closestEndPos,
                                (byte)dtStraightPathFlags.DT_STRAIGHTPATH_END,
                                0,
                                straightPath,
                                straightPathFlags,
                                straightPathRefs,
        ...
        {
            Debug.Assert(_nav != null);
            Debug.Assert(_tinyNodePool != null);
            ...
            while (nstack != 0)
            {
                ...
                // Get poly and tile.
                // The API input has been cheked already, skip checking internal data.
                ulong curRef = curNode.id;
                dtMeshTile curTile = null;
                dtPoly curPoly = null;
                _nav.getTileAndPolyByRefUnsafe(curRef, ref curTile, ref curPoly);
                ...
                for (int i = 0, j = (int)curPoly.vertCount - 1; i < (int)curPoly.vertCount; j = i++)
                {
                        ...
                        for (uint k = curPoly.firstLink; k != DT_NULL_LINK; k = curTile.links[k].next)
                        {
                            ...
                            if (link.edge == j)
                                if (link.polyRef != 0)
                                {
                                    ...
                                    dtPoly neiPoly = null;
                                    _nav.getTileAndPolyByRefUnsafe(link.polyRef, ref neiTile, ref neiPoly);
                    ...
                    else if (curPoly.neis[j] != 0)
                    {
                        ...
                        ulong polyRef = _nav.getPolyRefBase(curTile) | idx;
        ...
        /// If 0
        /// < t
        /// < 1.0 then the following applies:
        /// 
        ///  @ code
        ...
        public uint raycast(ulong startRef, float[] startPos, float[] endPos, dtQueryFilter filter, ref float t, float[] hitNormal, ulong[] path, ref uint pathCount, int maxPath)
        {
            dtRaycastHit hit = new();
            hit.path = path;
            hit.maxPath = maxPath;

            uint status = raycast(startRef, startPos, endPos, filter, 0, hit);
        ...
        /// (no wall hit), meaning it reached the end position. This is one example of why
        /// this method is meant for short distance checks.
        public uint raycast(ulong startRef, float[] startPos, float[] endPos, dtQueryFilter filter, uint options, dtRaycastHit hit, ulong prevRef = 0)
        {
            Debug.Assert(_nav != null);
            ...
            if (!_nav.isValidPolyRef(startRef) ||
                ...
                !dtVisfinite(endPos) ||
                filter == null ||
                (prevRef != 0 && !_nav.isValidPolyRef(prevRef)))
                return DT_FAILURE | DT_INVALID_PARAM;
            ...
            dtMeshTile tile = new();
            dtPoly poly = new();
            _nav.getTileAndPolyByRefUnsafe(curRef, ref tile, ref poly);
            nextTile = prevTile = tile;
            nextPoly = prevPoly = poly;

            if (prevRef != 0)
                _nav.getTileAndPolyByRefUnsafe(prevRef, ref prevTile, ref prevPoly);
            ...
            while (curRef != 0)
            {
                ...
                for (uint i = poly.firstLink; i != DT_NULL_LINK; i = tile.links[i].next)
                {
                    ...
                    nextPoly = new dtPoly();
                    _nav.getTileAndPolyByRefUnsafe(link.polyRef, ref nextTile, ref nextPoly);
                ...
                for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    ...
                    else
                    {
                        if (n < maxResult)
                        {
                            ...
                            if (resultParent != null)
                                resultParent[n] = _nodePool.getNodeAtIdx(neighbourNode.pidx).id;
        ...
        private uint findLocalNeighbourhood(ulong startRef, float[] centerPos, float radius, dtQueryFilter filter, ulong[] resultRef, ulong[] resultParent, ref int resultCount, int maxResult)
        {
            ...
            while (nstack != 0)
            {
                ...
                for (uint i = curPoly.firstLink; i != DT_NULL_LINK; i = curTile.links[i].next)
                {
                    ...
                    dtNode neighbourNode = _tinyNodePool.getNode(neighbourRef);
                    ...
                    for (int j = 0; j < n; ++j)
                    {
                        ...
                        _nav.getTileAndPolyByRefUnsafe(pastRef, ref pastTile, ref pastPoly);
            ...
            for (int i = 0, j = (int)poly.vertCount - 1; i < (int)poly.vertCount; j = i++)
            {
                ...
                else
                {
                    ...
                    if (poly.neis[j] != 0)
                    {
                        ...
                        neiRef = _nav.getPolyRefBase(tile) | idx;
            ...
            while (!_openList.empty())
            {
                ...
                for (int i = 0, j = (int)bestPoly.vertCount - 1; i < (int)bestPoly.vertCount; j = i++)
                {
                    ...
                    else if (bestPoly.neis[j] != 0)
                    {
                        ...
                        ulong polyRef = _nav.getPolyRefBase(bestTile) | idx;
        ...
        private bool isValidPolyRef(ulong polyRef, dtQueryFilter filter)
        {
            dtMeshTile tile = null;
            dtPoly poly = null;
            uint status = _nav.getTileAndPolyByRef(polyRef, ref tile, ref poly);
        ...
        {
            if (_nodePool == null) return false;

            dtNode node = _nodePool.findNode(polyRef);
    ...
    public class dtFindNearestPolyQuery
    {
        ...
        private readonly float[] _nearestPoint = new float[3];
        private ulong _nearestRef;
        ...
        public void process(dtMeshTile tile, dtPoly[] polys, ulong[] refs, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                float d;
                _query.closestPointOnPoly(refe, _center, closestPtPoly, ref posOverPoly);

                // If a point is directly over a polygon and closer than
                // climb height, favor that instead of straight line nearest point.
                dtVsub(diff, _center, closestPtPoly);
                ...
                {
                    dtVcopy(_nearestPoint, closestPtPoly);
