...
namespace Framework.Database
{
    public class HotfixDatabase : MySqlBase<HotfixStatements>
    {
	    public override void PreparedStatements()
        {
            ...
            PrepareStatement(HotfixStatements.SEL_ACHIEVEMENT_LOCALE,
            ...
            PrepareStatement(HotfixStatements.SEL_ACHIEVEMENT_CATEGORY_LOCALE,
                             "SELECT ID, Name_lang FROM achievement_category_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // AdventureJournal.db2
            PrepareStatement(HotfixStatements.SEL_ADVENTURE_JOURNAL,
                             "SELECT ID, Name, Description, ButtonText, RewardDescription, ContinueDescription, Type, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ADVENTURE_JOURNAL_LOCALE,
                             "SELECT ID, Name_lang, Description_lang, ButtonText_lang, RewardDescription_lang, " +
                             "ContinueDescription_lang FROM adventure_journal_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // AdventureMapPoi.db2
            PrepareStatement(HotfixStatements.SEL_ADVENTURE_MAP_POI,
                             "SELECT ID, Title, Description, WorldPositionX, WorldPositionY, Type, PlayerConditionID, QuestID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ADVENTURE_MAP_POI_LOCALE,
                             "SELECT ID, Title_lang, Description_lang FROM adventure_map_poi_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // AnimationData.db2
            PrepareStatement(HotfixStatements.SEL_ANIMATION_DATA,
                             "SELECT ID, Fallback, BehaviorTier, BehaviorID, Flags1, Flags2 FROM animation_data" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // AnimKit.db2
            PrepareStatement(HotfixStatements.SEL_ANIM_KIT,
                             "SELECT ID, OneShotDuration, OneShotStopAnimKitID, LowDefAnimKitID FROM anim_kit" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // AreaTable.db2
            PrepareStatement(HotfixStatements.SEL_AREA_TABLE,
            ...
            // AreaTrigger.db2
            PrepareStatement(HotfixStatements.SEL_AREA_TRIGGER,
            ...
            // ArmorLocation.db2
            PrepareStatement(HotfixStatements.SEL_ARMOR_LOCATION,
                             "SELECT ID, Clothmodifier, Leathermodifier, Chainmodifier, Platemodifier, Modifier FROM armor_location" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // Artifact.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT,
                             "SELECT Name, ID, UiTextureKitID, UiNameColor, UiBarOverlayColor, UiBarBackgroundColor, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_APPEARANCE,
                             "SELECT Name, ID, ArtifactAppearanceSetID, DisplayIndex, UnlockPlayerConditionID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_APPEARANCE_LOCALE,
                             "SELECT ID, Name_lang FROM artifact_appearance_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // ArtifactAppearanceSet.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_APPEARANCE_SET,
                             "SELECT Name, Description, ID, DisplayIndex, UiCameraID, AltHandUICameraID, " +
                             "ForgeAttachmentOverride, Flags, ArtifactID FROM artifact_appearance_set WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_ARTIFACT_APPEARANCE_SET_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM artifact_appearance_set_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            // ArtifactPower.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_POWER,
                             "SELECT DisplayPosX, DisplayPosY, ID, ArtifactID, MaxPurchasableRank, Label, Flags, Tier" +
            ...
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_POWER_RANK,
                             "SELECT ID, RankIndex, SpellID, ItemBonusListID, AuraPointsOverride, ArtifactPowerID" +
                             " FROM artifact_power_rank WHERE (`VerifiedBuild` > 0) = ?");

            // ArtifactQuestXp.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_QUEST_XP,
                             "SELECT ID, Difficulty1, Difficulty2, Difficulty3, Difficulty4, Difficulty5, Difficulty6, " +
                             "Difficulty7, Difficulty8, Difficulty9, Difficulty10 FROM artifact_quest_xp WHERE (`VerifiedBuild` > 0) = ?");

            // ArtifactTier.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_TIER,
                             "SELECT ID, ArtifactTier, MaxNumTraits, MaxArtifactKnowledge, KnowledgePlayerCondition, " +
                             "MinimumEmpowerKnowledge FROM artifact_tier WHERE (`VerifiedBuild` > 0) = ?");

            // ArtifactUnlock.db2
            PrepareStatement(HotfixStatements.SEL_ARTIFACT_UNLOCK,
                             "SELECT ID, PowerID, PowerRank, ItemBonusListID, PlayerConditionID, ArtifactID FROM artifact_unlock" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // AuctionHouse.db2
            PrepareStatement(HotfixStatements.SEL_AUCTION_HOUSE,
                             "SELECT ID, Name, FactionID, DepositRate, ConsignmentRate FROM auction_house" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_AZERITE_EMPOWERED_ITEM,
                             "SELECT ID, ItemID, AzeriteTierUnlockSetID, AzeritePowerSetID FROM azerite_empowered_item" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_AZERITE_ESSENCE_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM azerite_essence_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // AzeriteEssencePower.db2
            PrepareStatement(HotfixStatements.SEL_AZERITE_ESSENCE_POWER,
                             "SELECT ID, SourceAlliance, SourceHorde, AzeriteEssenceID, Tier, MajorPowerDescription, " +
                             "MinorPowerDescription, MajorPowerActual, MinorPowerActual FROM azerite_essence_power WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_AZERITE_ESSENCE_POWER_LOCALE,
                             "SELECT ID, SourceAlliance_lang, SourceHorde_lang FROM azerite_essence_power_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_AZERITE_ITEM_MILESTONE_POWER,
                             "SELECT ID, RequiredLevel, AzeritePowerID, Type, AutoUnlock" +
            ...
            PrepareStatement(HotfixStatements.SEL_AZERITE_LEVEL_INFO,
                             "SELECT ID, BaseExperienceToNextLevel, MinimumExperienceToNextLevel, ItemLevel" +
                             " FROM azerite_level_info WHERE (`VerifiedBuild` > 0) = ?");

            // AzeritePower.db2
            PrepareStatement(HotfixStatements.SEL_AZERITE_POWER,
                             "SELECT ID, SpellID, ItemBonusListID, SpecSetID, Flags FROM azerite_power" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // AzeritePowerSetMember.db2
            PrepareStatement(HotfixStatements.SEL_AZERITE_POWER_SET_MEMBER,
                             "SELECT ID, AzeritePowerSetID, AzeritePowerID, Class, Tier, OrderIndex" +
                             " FROM azerite_power_set_member WHERE (`VerifiedBuild` > 0) = ?");

            // AzeriteTierUnlock.db2
            PrepareStatement(HotfixStatements.SEL_AZERITE_TIER_UNLOCK,
            ...
            PrepareStatement(HotfixStatements.SEL_AZERITE_UNLOCK_MAPPING,
                             "SELECT ID, ItemLevel, ItemBonusListHead, ItemBonusListShoulders, ItemBonusListChest, " +
            ...
            // BarberShopStyle.db2
            PrepareStatement(HotfixStatements.SEL_BARBER_SHOP_STYLE,
                             "SELECT ID, DisplayName, Description, Type, CostModifier, Race, Sex, Data FROM barber_shop_style" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_BARBER_SHOP_STYLE_LOCALE,
                             "SELECT ID, DisplayName_lang, Description_lang FROM barber_shop_style_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // BattlePetBreedQuality.db2
            PrepareStatement(HotfixStatements.SEL_BATTLE_PET_BREED_QUALITY,
                             "SELECT ID, MaxQualityRoll, StateMultiplier, QualityEnum FROM battle_pet_breed_quality" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // BattlePetBreedState.db2
            PrepareStatement(HotfixStatements.SEL_BATTLE_PET_BREED_STATE,
                             "SELECT ID, BattlePetStateID, Value, BattlePetBreedID FROM battle_pet_breed_state" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // BattlePetSpecies.db2
            PrepareStatement(HotfixStatements.SEL_BATTLE_PET_SPECIES,
                             "SELECT Description, SourceText, ID, CreatureID, SummonSpellID, IconFileDataID, PetTypeEnum, " +
                             "Flags, SourceTypeEnum, CardUIModelSceneID, LoadoutUIModelSceneID, CovenantID FROM battle_pet_species WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_BATTLE_PET_SPECIES_LOCALE,
                             "SELECT ID, Description_lang, SourceText_lang FROM battle_pet_species_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // BattlePetSpeciesState.db2
            PrepareStatement(HotfixStatements.SEL_BATTLE_PET_SPECIES_STATE,
                             "SELECT ID, BattlePetStateID, Value, BattlePetSpeciesID FROM battle_pet_species_state" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // BattlemasterList.db2
            PrepareStatement(HotfixStatements.SEL_BATTLEMASTER_LIST,
                             "SELECT ID, Name, GameType, ShortDescription, LongDescription, InstanceType, MinLevel, MaxLevel, " +
            ...
            PrepareStatement(HotfixStatements.SEL_BATTLEMASTER_LIST_LOCALE,
                             "SELECT ID, Name_lang, GameType_lang, ShortDescription_lang, LongDescription_lang" +
                             " FROM battlemaster_list_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // BroadcastText.db2
            PrepareStatement(HotfixStatements.SEL_BROADCAST_TEXT,
                             "SELECT `Text`, Text1, ID, LanguageID, ConditionID, EmotesID, Flags, ChatBubbleDurationMs, " +
            ...
            PrepareStatement(HotfixStatements.SEL_BROADCAST_TEXT_LOCALE,
                             "SELECT ID, Text_lang, Text1_lang FROM broadcast_text_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // BroadcastTextDuration.db2
            PrepareStatement(HotfixStatements.SEL_BROADCAST_TEXT_DURATION,
                             "SELECT ID, BroadcastTextID, Locale, Duration FROM broadcast_text_duration" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // CfgRegions.db2
            PrepareStatement(HotfixStatements.SEL_CFG_REGIONS,
                             "SELECT ID, Tag, RegionID, Raidorigin, RegionGroupMask, ChallengeOrigin FROM cfg_regions" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_CHAR_TITLES_LOCALE,
                             "SELECT ID, Name_lang, Name1_lang FROM char_titles_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // CharacterLoadout.db2
            PrepareStatement(HotfixStatements.SEL_CHARACTER_LOADOUT,
                             "SELECT ID, RaceMask, ChrClassID, Purpose, ItemContext FROM character_loadout" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // CharacterLoadoutItem.db2
            PrepareStatement(HotfixStatements.SEL_CHARACTER_LOADOUT_ITEM,
                             "SELECT ID, CharacterLoadoutID, ItemID FROM character_loadout_item" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ChatChannels.db2
            PrepareStatement(HotfixStatements.SEL_CHAT_CHANNELS,
                             "SELECT ID, Name, Shortcut, Flags, FactionGroup, Ruleset FROM chat_channels" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_CHAT_CHANNELS_LOCALE,
                             "SELECT ID, Name_lang, Shortcut_lang FROM chat_channels_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // ChrClassUiDisplay.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CLASS_UI_DISPLAY,
                             "SELECT ID, ChrClassesID, AdvGuidePlayerConditionID, SplashPlayerConditionID" +
                             " FROM chr_class_ui_display WHERE (`VerifiedBuild` > 0) = ?");

            // ChrClasses.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CLASSES,
                             "SELECT Name, Filename, NameMale, NameFemale, PetNameToken, Description, RoleInfoString, DisabledString, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CLASSES_LOCALE,
                             "SELECT ID, Name_lang, NameMale_lang, NameFemale_lang, Description_lang, RoleInfoString_lang, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CLASSES_X_POWER_TYPES,
                             "SELECT ID, PowerType, ClassID FROM chr_classes_x_power_types" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ChrCustomizationChoice.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_CHOICE,
                             "SELECT Name, ID, ChrCustomizationOptionID, ChrCustomizationReqID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_CHOICE_LOCALE,
                             "SELECT ID, Name_lang FROM chr_customization_choice_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // ChrCustomizationDisplayInfo.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_DISPLAY_INFO,
                             "SELECT ID, ShapeshiftFormID, DisplayID, BarberShopMinCameraDistance, " +
                             "BarberShopHeightOffset FROM chr_customization_display_info WHERE (`VerifiedBuild` > 0) = ?");

            // ChrCustomizationElement.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_ELEMENT,
                             "SELECT ID, ChrCustomizationChoiceID, RelatedChrCustomizationChoiceID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_OPTION,
                             "SELECT Name, ID, SecondaryID, Flags, ChrModelID, SortIndex, ChrCustomizationCategoryID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_OPTION_LOCALE,
                             "SELECT ID, Name_lang FROM chr_customization_option_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_REQ_LOCALE,
                             "SELECT ID, ReqSource_lang FROM chr_customization_req_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // ChrCustomizationReqChoice.db2
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_REQ_CHOICE,
                             "SELECT ID, ChrCustomizationChoiceID, ChrCustomizationReqID" +
                             " FROM chr_customization_req_choice WHERE (`VerifiedBuild` > 0) = ?");

            // ChrModel.db2
            PrepareStatement(HotfixStatements.SEL_CHR_MODEL,
                             "SELECT FaceCustomizationOffset1, FaceCustomizationOffset2, FaceCustomizationOffset3, CustomizeOffset1, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_RACE_X_CHR_MODEL,
                             "SELECT ID, ChrRacesID, ChrModelID, Sex, AllowedTransmogSlots FROM chr_race_x_chr_model" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ChrRaces.db2
            PrepareStatement(HotfixStatements.SEL_CHR_RACES,
                             "SELECT ID, ClientPrefix, ClientFileString, Name, NameFemale, NameLowercase, NameFemaleLowercase, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_RACES_LOCALE,
                             "SELECT ID, Name_lang, NameFemale_lang, NameLowercase_lang, NameFemaleLowercase_lang, LoreName_lang, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_SPECIALIZATION,
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_SPECIALIZATION_LOCALE,
                             "SELECT ID, Name_lang, FemaleName_lang, Description_lang FROM chr_specialization_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // CinematicCamera.db2
            PrepareStatement(HotfixStatements.SEL_CINEMATIC_CAMERA,
                             "SELECT ID, OriginX, OriginY, OriginZ, SoundID, OriginFacing, FileDataID, ConversationID" +
            ...
            PrepareStatement(HotfixStatements.SEL_CONTENT_TUNING,
                             "SELECT ID, Flags, ExpansionID, MinLevel, MaxLevel, MinLevelType, MaxLevelType, TargetLevelDelta, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CONTENT_TUNING_X_EXPECTED,
                             "SELECT ID, ExpectedStatModID, MinMythicPlusSeasonID, MaxMythicPlusSeasonID, " +
                             "ContentTuningID FROM content_tuning_x_expected WHERE (`VerifiedBuild` > 0) = ?");

            // ConversationLine.db2
            PrepareStatement(HotfixStatements.SEL_CONVERSATION_LINE,
                             "SELECT ID, BroadcastTextID, SpellVisualKitID, AdditionalDuration, NextConversationLineID, " +
                             "AnimKitID, SpeechType, StartAnimation, EndAnimation FROM conversation_line WHERE (`VerifiedBuild` > 0) = ?");

            // CorruptionEffects.db2
            PrepareStatement(HotfixStatements.SEL_CORRUPTION_EFFECTS,
                             "SELECT ID, MinCorruption, Aura, PlayerConditionID, Flags FROM corruption_effects" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // CreatureDisplayInfo.db2
            PrepareStatement(HotfixStatements.SEL_CREATURE_DISPLAY_INFO,
            ...
            PrepareStatement(HotfixStatements.SEL_CREATURE_DISPLAY_INFO_EXTRA,
                             "SELECT ID, DisplayRaceID, DisplaySexID, DisplayClassID, Flags, BakeMaterialResourcesID, " +
                             "HDBakeMaterialResourcesID FROM creature_display_info_extra WHERE (`VerifiedBuild` > 0) = ?");

            // CreatureFamily.db2
            PrepareStatement(HotfixStatements.SEL_CREATURE_FAMILY,
                             "SELECT ID, Name, MinScale, MinScaleLevel, MaxScale, MaxScaleLevel, PetFoodMask, PetTalentType, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CREATURE_MODEL_DATA,
                             "SELECT ID, GeoBox1, GeoBox2, GeoBox3, GeoBox4, GeoBox5, GeoBox6, Flags, FileDataID, BloodID, " +
            ...
            // Criteria.db2
            PrepareStatement(HotfixStatements.SEL_CRITERIA,
            ...
            // CriteriaTree.db2
            PrepareStatement(HotfixStatements.SEL_CRITERIA_TREE,
                             "SELECT ID, Description, Parent, Amount, Operator, CriteriaID, OrderIndex, Flags FROM criteria_tree" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_CRITERIA_TREE_LOCALE,
                             "SELECT ID, Description_lang FROM criteria_tree_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // CurrencyContainer.db2
            PrepareStatement(HotfixStatements.SEL_CURRENCY_CONTAINER,
                             "SELECT ID, ContainerName, ContainerDescription, MinAmount, MaxAmount, ContainerIconID, " +
                             "ContainerQuality, OnLootSpellVisualKitID, CurrencyTypesID FROM currency_container WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_CURRENCY_CONTAINER_LOCALE,
                             "SELECT ID, ContainerName_lang, ContainerDescription_lang FROM currency_container_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // CurrencyTypes.db2
            PrepareStatement(HotfixStatements.SEL_CURRENCY_TYPES,
                             "SELECT ID, Name, Description, CategoryID, InventoryIconFileID, SpellWeight, SpellCategory, MaxQty, " +
            ...
            PrepareStatement(HotfixStatements.SEL_CURRENCY_TYPES_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM currency_types_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");
            ...
            // CurvePoint.db2
            PrepareStatement(HotfixStatements.SEL_CURVE_POINT,
                             "SELECT PosX, PosY, PreSLSquishPosX, PreSLSquishPosY, ID, CurveID, OrderIndex FROM curve_point" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // DestructibleModelData.db2
            PrepareStatement(HotfixStatements.SEL_DESTRUCTIBLE_MODEL_DATA,
            ...
            // Difficulty.db2
            PrepareStatement(HotfixStatements.SEL_DIFFICULTY,
                             "SELECT ID, Name, InstanceType, OrderIndex, OldEnumValue, FallbackDifficultyID, MinPlayers, MaxPlayers, " +
            ...
            PrepareStatement(HotfixStatements.SEL_DUNGEON_ENCOUNTER,
                             "SELECT Name, ID, MapID, DifficultyID, OrderIndex, CompleteWorldStateID, Bit, Flags, " +
                             "SpellIconFileID, Faction FROM dungeon_encounter WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_DUNGEON_ENCOUNTER_LOCALE,
                             "SELECT ID, Name_lang FROM dungeon_encounter_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // DurabilityCosts.db2
            PrepareStatement(HotfixStatements.SEL_DURABILITY_COSTS,
                             "SELECT ID, WeaponSubClassCost1, WeaponSubClassCost2, WeaponSubClassCost3, WeaponSubClassCost4, " +
            ...
            // Emotes.db2
            PrepareStatement(HotfixStatements.SEL_EMOTES,
                             "SELECT ID, RaceMask, EmoteSlashCommand, AnimID, EmoteFlags, EmoteSpecProc, EmoteSpecProcParam, EventSoundID, " +
            ...
            // EmotesTextSound.db2
            PrepareStatement(HotfixStatements.SEL_EMOTES_TEXT_SOUND,
                             "SELECT ID, RaceID, ClassID, SexID, SoundID, EmotesTextID FROM emotes_text_sound" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ExpectedStat.db2
            PrepareStatement(HotfixStatements.SEL_EXPECTED_STAT,
                             "SELECT ID, ExpansionID, CreatureHealth, PlayerHealth, CreatureAutoAttackDps, CreatureArmor, " +
                             "PlayerMana, PlayerPrimaryStat, PlayerSecondaryStat, ArmorConstant, CreatureSpellDamage, Lvl FROM expected_stat" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ExpectedStatMod.db2
            PrepareStatement(HotfixStatements.SEL_EXPECTED_STAT_MOD,
                             "SELECT ID, CreatureHealthMod, PlayerHealthMod, CreatureAutoAttackDPSMod, CreatureArmorMod, " +
                             "PlayerManaMod, PlayerPrimaryStatMod, PlayerSecondaryStatMod, ArmorConstantMod, CreatureSpellDamageMod FROM expected_stat_mod" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // Faction.db2
            PrepareStatement(HotfixStatements.SEL_FACTION,
                             "SELECT ID, ReputationRaceMask1, ReputationRaceMask2, ReputationRaceMask3, ReputationRaceMask4, Name, " +
            ...
            // FactionTemplate.db2
            PrepareStatement(HotfixStatements.SEL_FACTION_TEMPLATE,
            ...
            PrepareStatement(HotfixStatements.SEL_FRIENDSHIP_REP_REACTION,
                             "SELECT ID, Reaction, FriendshipRepID, ReactionThreshold, OverrideColor" +
                             " FROM friendship_rep_reaction WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_FRIENDSHIP_REP_REACTION_LOCALE,
                             "SELECT ID, Reaction_lang FROM friendship_rep_reaction_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // FriendshipReputation.db2
            PrepareStatement(HotfixStatements.SEL_FRIENDSHIP_REPUTATION,
                             "SELECT Description, StandingModified, StandingChanged, ID, FactionID, TextureFileID, Flags" +
                             " FROM friendship_reputation WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_FRIENDSHIP_REPUTATION_LOCALE,
                             "SELECT ID, Description_lang, StandingModified_lang, StandingChanged_lang" +
                             " FROM friendship_reputation_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // GameobjectArtKit.db2
            PrepareStatement(HotfixStatements.SEL_GAMEOBJECT_ART_KIT,
                             "SELECT ID, AttachModelFileID, TextureVariationFileID1, TextureVariationFileID2, " +
            ...
            // GarrAbility.db2
            PrepareStatement(HotfixStatements.SEL_GARR_ABILITY,
                             "SELECT ID, Name, Description, GarrAbilityCategoryID, GarrFollowerTypeID, IconFileDataID, " +
                             "FactionChangeGarrAbilityID, Flags FROM garr_ability WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_GARR_ABILITY_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM garr_ability_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // GarrBuilding.db2
            PrepareStatement(HotfixStatements.SEL_GARR_BUILDING,
                             "SELECT ID, HordeName, AllianceName, Description, Tooltip, GarrTypeID, BuildingType, " +
            ...
            PrepareStatement(HotfixStatements.SEL_GARR_BUILDING_LOCALE,
                             "SELECT ID, HordeName_lang, AllianceName_lang, Description_lang, Tooltip_lang" +
                             " FROM garr_building_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // GarrBuildingPlotInst.db2
            PrepareStatement(HotfixStatements.SEL_GARR_BUILDING_PLOT_INST,
                             "SELECT MapOffsetX, MapOffsetY, ID, GarrBuildingID, GarrSiteLevelPlotInstID, " +
                             "UiTextureAtlasMemberID FROM garr_building_plot_inst WHERE (`VerifiedBuild` > 0) = ?");

            // GarrClassSpec.db2
            PrepareStatement(HotfixStatements.SEL_GARR_CLASS_SPEC,
                             "SELECT ID, ClassSpec, ClassSpecMale, ClassSpecFemale, UiTextureAtlasMemberID, GarrFollItemSetID, " +
                             "FollowerClassLimit, Flags FROM garr_class_spec WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_GARR_CLASS_SPEC_LOCALE,
                             "SELECT ID, ClassSpec_lang, ClassSpecMale_lang, ClassSpecFemale_lang FROM garr_class_spec_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // GarrFollower.db2
            PrepareStatement(HotfixStatements.SEL_GARR_FOLLOWER,
                             "SELECT ID, HordeSourceText, AllianceSourceText, TitleName, GarrTypeID, GarrFollowerTypeID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_GARR_FOLLOWER_LOCALE,
                             "SELECT ID, HordeSourceText_lang, AllianceSourceText_lang, TitleName_lang FROM garr_follower_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // GarrFollowerXAbility.db2
            PrepareStatement(HotfixStatements.SEL_GARR_FOLLOWER_X_ABILITY,
                             "SELECT ID, OrderIndex, FactionIndex, GarrAbilityID, GarrFollowerID" +
                             " FROM garr_follower_x_ability WHERE (`VerifiedBuild` > 0) = ?");

            // GarrMission.db2
            PrepareStatement(HotfixStatements.SEL_GARR_MISSION,
            ...
            PrepareStatement(HotfixStatements.SEL_GARR_MISSION_LOCALE,
                             "SELECT ID, Name_lang, Location_lang, Description_lang FROM garr_mission_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // GarrPlot.db2
            PrepareStatement(HotfixStatements.SEL_GARR_PLOT,
                             "SELECT ID, Name, PlotType, HordeConstructObjID, AllianceConstructObjID, Flags, UiCategoryID, " +
            ...
            // GarrSiteLevel.db2
            PrepareStatement(HotfixStatements.SEL_GARR_SITE_LEVEL,
                             "SELECT ID, TownHallUiPosX, TownHallUiPosY, GarrSiteID, GarrLevel, MapID, UpgradeMovieID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_GARR_SITE_LEVEL_PLOT_INST,
                             "SELECT ID, UiMarkerPosX, UiMarkerPosY, GarrSiteLevelID, GarrPlotInstanceID, UiMarkerSize" +
                             " FROM garr_site_level_plot_inst WHERE (`VerifiedBuild` > 0) = ?");

            // GarrTalentTree.db2
            PrepareStatement(HotfixStatements.SEL_GARR_TALENT_TREE,
                             "SELECT ID, Name, GarrTypeID, ClassID, MaxTiers, UiOrder, Flags, UiTextureKitID, " +
            ...
            // GlyphProperties.db2
            PrepareStatement(HotfixStatements.SEL_GLYPH_PROPERTIES,
                             "SELECT ID, SpellID, GlyphType, GlyphExclusiveCategoryID, SpellIconFileDataID FROM glyph_properties" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // GlyphRequiredSpec.db2
            PrepareStatement(HotfixStatements.SEL_GLYPH_REQUIRED_SPEC,
                             "SELECT ID, ChrSpecializationID, GlyphPropertiesID FROM glyph_required_spec" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // GossipNpcOption.db2
            PrepareStatement(HotfixStatements.SEL_GOSSIP_NPC_OPTION,
                             "SELECT ID, GossipNpcOption, LFGDungeonsID, TrainerID, GarrFollowerTypeID, CharShipmentID, " +
            ...
            // Heirloom.db2
            PrepareStatement(HotfixStatements.SEL_HEIRLOOM,
                             "SELECT SourceText, ID, ItemID, LegacyUpgradedItemID, StaticUpgradedItemID, SourceTypeEnum, Flags, " +
            ...
            // Holidays.db2
            PrepareStatement(HotfixStatements.SEL_HOLIDAYS,
                             "SELECT ID, Region, Looping, HolidayNameID, HolidayDescriptionID, Priority, CalendarFilterType, Flags, " +
            ...
            PrepareStatement(HotfixStatements.SEL_IMPORT_PRICE_ARMOR,
                             "SELECT ID, ClothModifier, LeatherModifier, ChainModifier, PlateModifier FROM import_price_armor" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // Item.db2
            PrepareStatement(HotfixStatements.SEL_ITEM,
                             "SELECT ID, ClassID, SubclassID, Material, InventoryType, SheatheType, SoundOverrideSubclassID, IconFileDataID, " +
                             "ItemGroupSoundsID, ContentTuningID, ModifiedCraftingReagentItemID, CraftingQualityID FROM item WHERE (`VerifiedBuild` > 0) = ?");

            // ItemAppearance.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_APPEARANCE,
                             "SELECT ID, DisplayType, ItemDisplayInfoID, DefaultIconFileDataID, UiOrder, PlayerConditionID" +
                             " FROM item_appearance WHERE (`VerifiedBuild` > 0) = ?");

            // ItemArmorQuality.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_ARMOR_QUALITY,
                             "SELECT ID, Qualitymod1, Qualitymod2, Qualitymod3, Qualitymod4, Qualitymod5, Qualitymod6, " +
                             "Qualitymod7 FROM item_armor_quality WHERE (`VerifiedBuild` > 0) = ?");

            // ItemArmorShield.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_ARMOR_SHIELD,
                             "SELECT ID, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, Quality7, ItemLevel" +
                             " FROM item_armor_shield WHERE (`VerifiedBuild` > 0) = ?");

            // ItemArmorTotal.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_ARMOR_TOTAL,
                             "SELECT ID, ItemLevel, Cloth, Leather, Mail, Plate FROM item_armor_total" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // ItemBonus.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_BONUS,
                             "SELECT ID, Value1, Value2, Value3, Value4, ParentItemBonusListID, Type, OrderIndex FROM item_bonus" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ItemBonusListLevelDelta.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_BONUS_LIST_LEVEL_DELTA,
                             "SELECT ItemLevelDelta, ID FROM item_bonus_list_level_delta" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ItemBonusTreeNode.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_BONUS_TREE_NODE,
                             "SELECT ID, ItemContext, ChildItemBonusTreeID, ChildItemBonusListID, ChildItemLevelSelectorID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_CHILD_EQUIPMENT,
                             "SELECT ID, ParentItemID, ChildItemID, ChildItemEquipSlot FROM item_child_equipment" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // ItemDamageAmmo.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DAMAGE_AMMO,
                             "SELECT ID, ItemLevel, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, Quality7" +
                             " FROM item_damage_ammo WHERE (`VerifiedBuild` > 0) = ?");

            // ItemDamageOneHand.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DAMAGE_ONE_HAND,
                             "SELECT ID, ItemLevel, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, Quality7" +
                             " FROM item_damage_one_hand WHERE (`VerifiedBuild` > 0) = ?");

            // ItemDamageOneHandCaster.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DAMAGE_ONE_HAND_CASTER,
                             "SELECT ID, ItemLevel, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, " +
                             "Quality7 FROM item_damage_one_hand_caster WHERE (`VerifiedBuild` > 0) = ?");

            // ItemDamageTwoHand.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DAMAGE_TWO_HAND,
                             "SELECT ID, ItemLevel, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, Quality7" +
                             " FROM item_damage_two_hand WHERE (`VerifiedBuild` > 0) = ?");

            // ItemDamageTwoHandCaster.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DAMAGE_TWO_HAND_CASTER,
                             "SELECT ID, ItemLevel, Quality1, Quality2, Quality3, Quality4, Quality5, Quality6, " +
                             "Quality7 FROM item_damage_two_hand_caster WHERE (`VerifiedBuild` > 0) = ?");

            // ItemDisenchantLoot.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_DISENCHANT_LOOT,
                             "SELECT ID, Subclass, Quality, MinLevel, MaxLevel, SkillRequired, ExpansionID, Class" +
                             " FROM item_disenchant_loot WHERE (`VerifiedBuild` > 0) = ?");

            // ItemEffect.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_EFFECT,
                             "SELECT ID, LegacySlotIndex, TriggerType, Charges, CoolDownMSec, CategoryCoolDownMSec, SpellCategoryID, " +
                             "SpellID, ChrSpecializationID FROM item_effect WHERE (`VerifiedBuild` > 0) = ?");

            // ItemExtendedCost.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_EXTENDED_COST,
                             "SELECT ID, RequiredArenaRating, ArenaBracket, Flags, MinFactionID, MinReputation, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_LEVEL_SELECTOR,
                             "SELECT ID, MinItemLevel, ItemLevelSelectorQualitySetID, AzeriteUnlockMappingSet" +
                             " FROM item_level_selector WHERE (`VerifiedBuild` > 0) = ?");

            // ItemLevelSelectorQuality.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_LEVEL_SELECTOR_QUALITY,
                             "SELECT ID, QualityItemBonusListID, Quality, ParentILSQualitySetID" +
                             " FROM item_level_selector_quality WHERE (`VerifiedBuild` > 0) = ?");

            // ItemLevelSelectorQualitySet.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_LEVEL_SELECTOR_QUALITY_SET,
                             "SELECT ID, IlvlRare, IlvlEpic FROM item_level_selector_quality_set" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_LIMIT_CATEGORY_LOCALE,
                             "SELECT ID, Name_lang FROM item_limit_category_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // ItemLimitCategoryCondition.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_LIMIT_CATEGORY_CONDITION,
                             "SELECT ID, AddQuantity, PlayerConditionID, ParentItemLimitCategoryID" +
                             " FROM item_limit_category_condition WHERE (`VerifiedBuild` > 0) = ?");

            // ItemModifiedAppearance.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_MODIFIED_APPEARANCE,
                             "SELECT ID, ItemID, ItemAppearanceModifierID, ItemAppearanceID, OrderIndex, " +
                             "TransmogSourceTypeEnum FROM item_modified_appearance WHERE (`VerifiedBuild` > 0) = ?");

            // ItemModifiedAppearanceExtra.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_MODIFIED_APPEARANCE_EXTRA,
                             "SELECT ID, IconFileDataID, UnequippedIconFileDataID, SheatheType, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_NAME_DESCRIPTION_LOCALE,
                             "SELECT ID, Description_lang FROM item_name_description_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            // ItemSearchName.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SEARCH_NAME,
                             "SELECT ID, AllowableRace, Display, OverallQualityID, ExpansionID, MinFactionID, MinReputation, " +
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_SEARCH_NAME_LOCALE,
                             "SELECT ID, Display_lang FROM item_search_name_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // ItemSet.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SET,
            ...
            // ItemSetSpell.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SET_SPELL,
                             "SELECT ID, ChrSpecID, SpellID, Threshold, ItemSetID FROM item_set_spell" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // ItemSparse.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SPARSE,
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_SPARSE_LOCALE,
            ...
            // ItemSpec.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SPEC,
                             "SELECT ID, MinLevel, MaxLevel, ItemType, PrimaryStat, SecondaryStat, SpecializationID FROM item_spec" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_JOURNAL_ENCOUNTER,
                             "SELECT Name, Description, MapX, MapY, ID, JournalInstanceID, DungeonEncounterID, OrderIndex, " +
                             "FirstSectionID, UiMapID, MapDisplayConditionID, Flags, DifficultyMask FROM journal_encounter WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_JOURNAL_ENCOUNTER_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM journal_encounter_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // JournalEncounterSection.db2
            PrepareStatement(HotfixStatements.SEL_JOURNAL_ENCOUNTER_SECTION,
                             "SELECT ID, Title, BodyText, JournalEncounterID, OrderIndex, ParentSectionID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_JOURNAL_ENCOUNTER_SECTION_LOCALE,
                             "SELECT ID, Title_lang, BodyText_lang FROM journal_encounter_section_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // JournalInstance.db2
            PrepareStatement(HotfixStatements.SEL_JOURNAL_INSTANCE,
                             "SELECT ID, Name, Description, MapID, BackgroundFileDataID, ButtonFileDataID, " +
                             "ButtonSmallFileDataID, LoreFileDataID, Flags, AreaID FROM journal_instance WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_JOURNAL_INSTANCE_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM journal_instance_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            // Keychain.db2
            PrepareStatement(HotfixStatements.SEL_KEYCHAIN,
            ...
            PrepareStatement(HotfixStatements.SEL_KEYSTONE_AFFIX_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM keystone_affix_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");
            ...
            // Languages.db2
            PrepareStatement(HotfixStatements.SEL_LANGUAGES,
                             "SELECT ID, Name, Flags, UiTextureKitID, UiTextureKitElementCount FROM languages" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // LfgDungeons.db2
            PrepareStatement(HotfixStatements.SEL_LFG_DUNGEONS,
                             "SELECT ID, Name, Description, TypeID, Subtype, Faction, IconTextureFileID, RewardsBgTextureFileID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_LFG_DUNGEONS_LOCALE,
                             "SELECT ID, Name_lang, Description_lang FROM lfg_dungeons_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // Light.db2
            PrepareStatement(HotfixStatements.SEL_LIGHT,
                             "SELECT ID, GameCoordsX, GameCoordsY, GameCoordsZ, GameFalloffStart, GameFalloffEnd, ContinentID, " +
            ...
            // LiquidType.db2
            PrepareStatement(HotfixStatements.SEL_LIQUID_TYPE,
            ...
            // Lock.db2
            PrepareStatement(HotfixStatements.SEL_LOCK,
            ...
            // Map.db2
            PrepareStatement(HotfixStatements.SEL_MAP,
                             "SELECT ID, Directory, MapName, MapDescription0, MapDescription1, PvpShortDescription, PvpLongDescription, " +
            ...
            PrepareStatement(HotfixStatements.SEL_MAP_LOCALE,
                             "SELECT ID, MapName_lang, MapDescription0_lang, MapDescription1_lang, PvpShortDescription_lang, " +
                             "PvpLongDescription_lang FROM map_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // MapChallengeMode.db2
            PrepareStatement(HotfixStatements.SEL_MAP_CHALLENGE_MODE,
                             "SELECT Name, ID, MapID, Flags, ExpansionLevel, RequiredWorldStateID, CriteriaCount1, " +
                             "CriteriaCount2, CriteriaCount3 FROM map_challenge_mode WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_MAP_CHALLENGE_MODE_LOCALE,
                             "SELECT ID, Name_lang FROM map_challenge_mode_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // MapDifficulty.db2
            PrepareStatement(HotfixStatements.SEL_MAP_DIFFICULTY,
                             "SELECT ID, Message, DifficultyID, LockID, ResetInterval, MaxPlayers, ItemContext, " +
            ...
            PrepareStatement(HotfixStatements.SEL_MAP_DIFFICULTY_X_CONDITION,
                             "SELECT ID, FailureDescription, PlayerConditionID, OrderIndex, MapDifficultyID" +
                             " FROM map_difficulty_x_condition WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_MAP_DIFFICULTY_X_CONDITION_LOCALE,
                             "SELECT ID, FailureDescription_lang FROM map_difficulty_x_condition_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            // ModifierTree.db2
            PrepareStatement(HotfixStatements.SEL_MODIFIER_TREE,
                             "SELECT ID, Parent, Operator, Amount, Type, Asset, SecondaryAsset, TertiaryAsset FROM modifier_tree" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // Mount.db2
            PrepareStatement(HotfixStatements.SEL_MOUNT,
            ...
            PrepareStatement(HotfixStatements.SEL_MOUNT_LOCALE,
                             "SELECT ID, Name_lang, SourceText_lang, Description_lang FROM mount_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // MountCapability.db2
            PrepareStatement(HotfixStatements.SEL_MOUNT_CAPABILITY,
            ...
            PrepareStatement(HotfixStatements.SEL_MOUNT_TYPE_X_CAPABILITY,
                             "SELECT ID, MountTypeID, MountCapabilityID, OrderIndex FROM mount_type_x_capability" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // MountXDisplay.db2
            PrepareStatement(HotfixStatements.SEL_MOUNT_X_DISPLAY,
                             "SELECT ID, CreatureDisplayInfoID, PlayerConditionID, MountID FROM mount_x_display" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_NUM_TALENTS_AT_LEVEL,
                             "SELECT ID, NumTalents, NumTalentsDeathKnight, NumTalentsDemonHunter FROM num_talents_at_level" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // OverrideSpellData.db2
            PrepareStatement(HotfixStatements.SEL_OVERRIDE_SPELL_DATA,
            ...
            PrepareStatement(HotfixStatements.SEL_PARAGON_REPUTATION,
                             "SELECT ID, FactionID, LevelThreshold, QuestID FROM paragon_reputation" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // PlayerCondition.db2
            PrepareStatement(HotfixStatements.SEL_PLAYER_CONDITION,
                             "SELECT ID, RaceMask, FailureDescription, ClassMask, SkillLogic, LanguageID, MinLanguage, " +
            ...
            PrepareStatement(HotfixStatements.SEL_PLAYER_CONDITION_LOCALE,
                             "SELECT ID, FailureDescription_lang FROM player_condition_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // PowerDisplay.db2
            PrepareStatement(HotfixStatements.SEL_POWER_DISPLAY,
                             "SELECT ID, GlobalStringBaseTag, ActualType, Red, Green, Blue FROM power_display" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // PowerType.db2
            PrepareStatement(HotfixStatements.SEL_POWER_TYPE,
                             "SELECT NameGlobalStringTag, CostGlobalStringTag, ID, PowerTypeEnum, MinPower, MaxBasePower, CenterPower, " +
            ...
            PrepareStatement(HotfixStatements.SEL_PRESTIGE_LEVEL_INFO,
            ...
            PrepareStatement(HotfixStatements.SEL_PRESTIGE_LEVEL_INFO_LOCALE,
                             "SELECT ID, Name_lang FROM prestige_level_info_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");
            ...
            // PvpTalent.db2
            PrepareStatement(HotfixStatements.SEL_PVP_TALENT,
                             "SELECT Description, ID, SpecID, SpellID, OverridesSpellID, Flags, ActionBarSpellID, PvpTalentCategoryID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_PVP_TALENT_SLOT_UNLOCK,
                             "SELECT ID, Slot, LevelRequired, DeathKnightLevelRequired, DemonHunterLevelRequired" +
                             " FROM pvp_talent_slot_unlock WHERE (`VerifiedBuild` > 0) = ?");

            // PvpTier.db2
            PrepareStatement(HotfixStatements.SEL_PVP_TIER,
                             "SELECT Name, ID, MinRating, MaxRating, PrevTier, NextTier, BracketID, `Rank`, RankIconFileDataID" +
            ...
            PrepareStatement(HotfixStatements.SEL_QUEST_FACTION_REWARD,
                             "SELECT ID, Difficulty1, Difficulty2, Difficulty3, Difficulty4, Difficulty5, Difficulty6, " +
            ...
            PrepareStatement(HotfixStatements.SEL_QUEST_MONEY_REWARD,
                             "SELECT ID, Difficulty1, Difficulty2, Difficulty3, Difficulty4, Difficulty5, Difficulty6, " +
                             "Difficulty7, Difficulty8, Difficulty9, Difficulty10 FROM quest_money_reward WHERE (`VerifiedBuild` > 0) = ?");

            // QuestPackageItem.db2
            PrepareStatement(HotfixStatements.SEL_QUEST_PACKAGE_ITEM,
                             "SELECT ID, PackageID, ItemID, ItemQuantity, DisplayType FROM quest_package_item" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // QuestXp.db2
            PrepareStatement(HotfixStatements.SEL_QUEST_XP,
                             "SELECT ID, Difficulty1, Difficulty2, Difficulty3, Difficulty4, Difficulty5, Difficulty6, Difficulty7, " +
                             "Difficulty8, Difficulty9, Difficulty10 FROM quest_xp WHERE (`VerifiedBuild` > 0) = ?");

            // RandPropPoints.db2
            PrepareStatement(HotfixStatements.SEL_RAND_PROP_POINTS,
                             "SELECT ID, DamageReplaceStatF, DamageSecondaryF, DamageReplaceStat, DamageSecondary, EpicF1, " +
            ...
            // RewardPack.db2
            PrepareStatement(HotfixStatements.SEL_REWARD_PACK,
                             "SELECT ID, CharTitleID, Money, ArtifactXPDifficulty, ArtifactXPMultiplier, ArtifactXPCategoryID, " +
                             "TreasurePickerID FROM reward_pack WHERE (`VerifiedBuild` > 0) = ?");

            // RewardPackXCurrencyType.db2
            PrepareStatement(HotfixStatements.SEL_REWARD_PACK_X_CURRENCY_TYPE,
                             "SELECT ID, CurrencyTypeID, Quantity, RewardPackID FROM reward_pack_x_currency_type" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // RewardPackXItem.db2
            PrepareStatement(HotfixStatements.SEL_REWARD_PACK_X_ITEM,
                             "SELECT ID, ItemID, ItemQuantity, RewardPackID FROM reward_pack_x_item" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // ScenarioStep.db2
            PrepareStatement(HotfixStatements.SEL_SCENARIO_STEP,
                             "SELECT ID, Description, Title, ScenarioID, Criteriatreeid, RewardQuestID, RelatedStep, Supersedes, " +
                             "OrderIndex, Flags, VisibilityPlayerConditionID, WidgetSetID FROM scenario_step WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_SCENARIO_STEP_LOCALE,
                             "SELECT ID, Description_lang, Title_lang FROM scenario_step_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // SceneScript.db2
            PrepareStatement(HotfixStatements.SEL_SCENE_SCRIPT,
                             "SELECT ID, FirstSceneScriptID, NextSceneScriptID, Unknown915 FROM scene_script" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // SkillLine.db2
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE,
                             "SELECT DisplayName, AlternateVerb, Description, HordeDisplayName, OverrideSourceInfoDisplayName, ID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE_LOCALE,
                             "SELECT ID, DisplayName_lang, AlternateVerb_lang, Description_lang, HordeDisplayName_lang" +
                             " FROM skill_line_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // SkillLineAbility.db2
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE_ABILITY,
                             "SELECT RaceMask, AbilityVerb, AbilityAllVerb, ID, SkillLine, Spell, MinSkillLineRank, ClassMask, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE_ABILITY_LOCALE,
                             "SELECT ID, AbilityVerb_lang, AbilityAllVerb_lang FROM skill_line_ability_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // SkillLineXTraitTree.db2
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE_X_TRAIT_TREE,
                             "SELECT ID, SkillLineID, TraitTreeID, OrderIndex FROM skill_line_x_trait_tree" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SkillRaceClassInfo.db2
            PrepareStatement(HotfixStatements.SEL_SKILL_RACE_CLASS_INFO,
                             "SELECT ID, RaceMask, SkillID, ClassMask, Flags, Availability, MinLevel, SkillTierID" +
                             " FROM skill_race_class_info WHERE (`VerifiedBuild` > 0) = ?");

            // SoulbindConduitRank.db2
            PrepareStatement(HotfixStatements.SEL_SOULBIND_CONDUIT_RANK,
                             "SELECT ID, RankIndex, SpellID, AuraPointsOverride, SoulbindConduitID" +
                             " FROM soulbind_conduit_rank WHERE (`VerifiedBuild` > 0) = ?");

            // SoundKit.db2
            PrepareStatement(HotfixStatements.SEL_SOUND_KIT,
                             "SELECT ID, SoundType, VolumeFloat, Flags, MinDistance, DistanceCutoff, EAXDef, SoundKitAdvancedID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPECIALIZATION_SPELLS,
                             "SELECT Description, ID, SpecID, SpellID, OverridesSpellID, DisplayOrder" +
                             " FROM specialization_spells WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_SPECIALIZATION_SPELLS_LOCALE,
                             "SELECT ID, Description_lang FROM specialization_spells_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_AURA_OPTIONS,
                             "SELECT ID, DifficultyID, CumulativeAura, ProcCategoryRecovery, ProcChance, ProcCharges, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_AURA_RESTRICTIONS,
                             "SELECT ID, DifficultyID, CasterAuraState, TargetAuraState, ExcludeCasterAuraState, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_CASTING_REQUIREMENTS,
                             "SELECT ID, SpellID, FacingCasterFlags, MinFactionID, MinReputation, RequiredAreasID, " +
                             "RequiredAuraVision, RequiresSpellFocus FROM spell_casting_requirements WHERE (`VerifiedBuild` > 0) = ?");

            // SpellCategories.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_CATEGORIES,
                             "SELECT ID, DifficultyID, Category, DefenseType, DispelType, Mechanic, PreventionType, " +
                             "StartRecoveryCategory, ChargeCategory, SpellID FROM spell_categories WHERE (`VerifiedBuild` > 0) = ?");

            // SpellCategory.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_CATEGORY,
                             "SELECT ID, Name, Flags, UsesPerWeek, MaxCharges, ChargeRecoveryTime, TypeMask FROM spell_category" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_CLASS_OPTIONS,
                             "SELECT ID, SpellID, ModalNextSpell, SpellClassSet, SpellClassMask1, SpellClassMask2, " +
                             "SpellClassMask3, SpellClassMask4 FROM spell_class_options WHERE (`VerifiedBuild` > 0) = ?");

            // SpellCooldowns.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_COOLDOWNS,
                             "SELECT ID, DifficultyID, CategoryRecoveryTime, RecoveryTime, StartRecoveryTime, AuraSpellID, " +
            ...
            // SpellEffect.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_EFFECT,
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_EQUIPPED_ITEMS,
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_FOCUS_OBJECT_LOCALE,
                             "SELECT ID, Name_lang FROM spell_focus_object_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // SpellInterrupts.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_INTERRUPTS,
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_ITEM_ENCHANTMENT,
                             "SELECT ID, Name, HordeName, Duration, EffectArg1, EffectArg2, EffectArg3, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_ITEM_ENCHANTMENT_LOCALE,
                             "SELECT ID, Name_lang, HordeName_lang FROM spell_item_enchantment_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // SpellItemEnchantmentCondition.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_ITEM_ENCHANTMENT_CONDITION,
                             "SELECT ID, LtOperandType1, LtOperandType2, LtOperandType3, LtOperandType4, " +
            ...
            // SpellLearnSpell.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_LEARN_SPELL,
                             "SELECT ID, SpellID, LearnSpellID, OverridesSpellID FROM spell_learn_spell" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SpellLevels.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_LEVELS,
                             "SELECT ID, DifficultyID, MaxLevel, MaxPassiveAuraLevel, BaseLevel, SpellLevel, SpellID" +
                             " FROM spell_levels WHERE (`VerifiedBuild` > 0) = ?");

            // SpellMisc.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_MISC,
                             "SELECT ID, Attributes1, Attributes2, Attributes3, Attributes4, Attributes5, Attributes6, Attributes7, " +
            ...
            // SpellPower.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_POWER,
                             "SELECT ID, OrderIndex, ManaCost, ManaCostPerLevel, ManaPerSecond, PowerDisplayID, AltPowerBarID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_POWER_DIFFICULTY,
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_PROCS_PER_MINUTE_MOD,
                             "SELECT ID, Type, Param, Coeff, SpellProcsPerMinuteID FROM spell_procs_per_minute_mod" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SpellRadius.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_RADIUS,
                             "SELECT ID, Radius, RadiusPerLevel, RadiusMin, RadiusMax FROM spell_radius" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SpellRange.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_RANGE,
                             "SELECT ID, DisplayName, DisplayNameShort, Flags, RangeMin1, RangeMin2, RangeMax1, RangeMax2" +
                             " FROM spell_range WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_SPELL_RANGE_LOCALE,
                             "SELECT ID, DisplayName_lang, DisplayNameShort_lang FROM spell_range_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // SpellReagents.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_REAGENTS,
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_REAGENTS_CURRENCY,
                             "SELECT ID, SpellID, CurrencyTypesID, CurrencyCount FROM spell_reagents_currency" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SpellScaling.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_SCALING,
                             "SELECT ID, SpellID, MinScalingLevel, MaxScalingLevel, ScalesFromItemLevel FROM spell_scaling" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // SpellShapeshift.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_SHAPESHIFT,
                             "SELECT ID, SpellID, StanceBarOrder, ShapeshiftExclude1, ShapeshiftExclude2, ShapeshiftMask1, " +
                             "ShapeshiftMask2 FROM spell_shapeshift WHERE (`VerifiedBuild` > 0) = ?");

            // SpellShapeshiftForm.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_SHAPESHIFT_FORM,
                             "SELECT ID, Name, CreatureType, Flags, AttackIconFileID, BonusActionBar, CombatRoundTime, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_SHAPESHIFT_FORM_LOCALE,
                             "SELECT ID, Name_lang FROM spell_shapeshift_form_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // SpellTargetRestrictions.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_TARGET_RESTRICTIONS,
                             "SELECT ID, DifficultyID, ConeDegrees, MaxTargets, MaxTargetLevel, TargetCreatureType, " +
                             "Targets, Width, SpellID FROM spell_target_restrictions WHERE (`VerifiedBuild` > 0) = ?");

            // SpellTotems.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_TOTEMS,
                             "SELECT ID, SpellID, RequiredTotemCategoryID1, RequiredTotemCategoryID2, Totem1, Totem2" +
                             " FROM spell_totems WHERE (`VerifiedBuild` > 0) = ?");

            // SpellVisual.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_VISUAL,
                             "SELECT ID, MissileCastOffset1, MissileCastOffset2, MissileCastOffset3, MissileImpactOffset1, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_VISUAL_EFFECT_NAME,
                             "SELECT ID, ModelFileDataID, BaseMissileSpeed, Scale, MinAllowedScale, MaxAllowedScale, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_VISUAL_MISSILE,
                             "SELECT CastOffset1, CastOffset2, CastOffset3, ImpactOffset1, ImpactOffset2, ImpactOffset3, ID, " +
            ...
            // SpellVisualKit.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_VISUAL_KIT,
                             "SELECT ID, FallbackPriority, FallbackSpellVisualKitId, DelayMin, DelayMax, Flags1, Flags2" +
                             " FROM spell_visual_kit WHERE (`VerifiedBuild` > 0) = ?");

            // SpellXSpellVisual.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_X_SPELL_VISUAL,
                             "SELECT ID, DifficultyID, SpellVisualID, Probability, Flags, Priority, SpellIconFileID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SUMMON_PROPERTIES,
                             "SELECT ID, Control, Faction, Title, Slot, Flags1, Flags2 FROM summon_properties" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TactKey.db2
            PrepareStatement(HotfixStatements.SEL_TACT_KEY,
                             "SELECT ID, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8, Key9, Key10, Key11, Key12, Key13, Key14, Key15, " +
                             "Key16 FROM tact_key WHERE (`VerifiedBuild` > 0) = ?");

            // Talent.db2
            PrepareStatement(HotfixStatements.SEL_TALENT,
                             "SELECT ID, Description, TierID, Flags, ColumnIndex, ClassID, SpecID, SpellID, OverridesSpellID, " +
            ...
            // TaxiNodes.db2
            PrepareStatement(HotfixStatements.SEL_TAXI_NODES,
            ...
            // TaxiPathNode.db2
            PrepareStatement(HotfixStatements.SEL_TAXI_PATH_NODE,
            ...
            // TotemCategory.db2
            PrepareStatement(HotfixStatements.SEL_TOTEM_CATEGORY,
                             "SELECT ID, Name, TotemCategoryType, TotemCategoryMask FROM totem_category" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // TraitCond.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_COND,
                             "SELECT ID, CondType, TraitTreeID, GrantedRanks, QuestID, AchievementID, SpecSetID, TraitNodeGroupID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_TRAIT_CURRENCY_SOURCE,
                             "SELECT Requirement, ID, TraitCurrencyID, Amount, QuestID, AchievementID, PlayerLevel, " +
                             "TraitNodeEntryID, OrderIndex FROM trait_currency_source WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_TRAIT_CURRENCY_SOURCE_LOCALE,
                             "SELECT ID, Requirement_lang FROM trait_currency_source_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // TraitDefinition.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_DEFINITION,
                             "SELECT OverrideName, OverrideSubtext, OverrideDescription, ID, SpellID, OverrideIcon, " +
                             "OverridesSpellID, VisibleSpellID FROM trait_definition WHERE (`VerifiedBuild` > 0) = ?");

            PrepareStatement(HotfixStatements.SEL_TRAIT_DEFINITION_LOCALE,
                             "SELECT ID, OverrideName_lang, OverrideSubtext_lang, OverrideDescription_lang" +
                             " FROM trait_definition_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // TraitDefinitionEffectPoints.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_DEFINITION_EFFECT_POINTS,
                             "SELECT ID, TraitDefinitionID, EffectIndex, OperationType, CurveID" +
                             " FROM trait_definition_effect_points WHERE (`VerifiedBuild` > 0) = ?");

            // TraitEdge.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_EDGE,
                             "SELECT ID, VisualStyle, LeftTraitNodeID, RightTraitNodeID, Type FROM trait_edge" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // TraitNodeEntry.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_ENTRY,
                             "SELECT ID, TraitDefinitionID, MaxRanks, NodeEntryType FROM trait_node_entry" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeEntryXTraitCond.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_ENTRY_X_TRAIT_COND,
                             "SELECT ID, TraitCondID, TraitNodeEntryID FROM trait_node_entry_x_trait_cond" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeEntryXTraitCost.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_ENTRY_X_TRAIT_COST,
                             "SELECT ID, TraitNodeEntryID, TraitCostID FROM trait_node_entry_x_trait_cost" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_GROUP_X_TRAIT_COND,
                             "SELECT ID, TraitCondID, TraitNodeGroupID FROM trait_node_group_x_trait_cond" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeGroupXTraitCost.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_GROUP_X_TRAIT_COST,
                             "SELECT ID, TraitNodeGroupID, TraitCostID FROM trait_node_group_x_trait_cost" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeGroupXTraitNode.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_GROUP_X_TRAIT_NODE,
                             "SELECT ID, TraitNodeGroupID, TraitNodeID, `Index` FROM trait_node_group_x_trait_node" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeXTraitCond.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_X_TRAIT_COND,
                             "SELECT ID, TraitCondID, TraitNodeID FROM trait_node_x_trait_cond" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeXTraitCost.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_X_TRAIT_COST,
                             "SELECT ID, TraitNodeID, TraitCostID FROM trait_node_x_trait_cost" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitNodeXTraitNodeEntry.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_NODE_X_TRAIT_NODE_ENTRY,
                             "SELECT ID, TraitNodeID, TraitNodeEntryID, `Index` FROM trait_node_x_trait_node_entry" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitTree.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_TREE,
                             "SELECT ID, TraitSystemID, Unused1000_1, FirstTraitNodeID, PlayerConditionID, Flags, Unused1000_2, " +
                             "Unused1000_3 FROM trait_tree WHERE (`VerifiedBuild` > 0) = ?");

            // TraitTreeLoadout.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_TREE_LOADOUT,
                             "SELECT ID, TraitTreeID, ChrSpecializationID FROM trait_tree_loadout" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitTreeLoadoutEntry.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_TREE_LOADOUT_ENTRY,
                             "SELECT ID, TraitTreeLoadoutID, SelectedTraitNodeID, SelectedTraitNodeEntryID, NumPoints, " +
                             "OrderIndex FROM trait_tree_loadout_entry WHERE (`VerifiedBuild` > 0) = ?");

            // TraitTreeXTraitCost.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_TREE_X_TRAIT_COST,
                             "SELECT ID, TraitTreeID, TraitCostID FROM trait_tree_x_trait_cost" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TraitTreeXTraitCurrency.db2
            PrepareStatement(HotfixStatements.SEL_TRAIT_TREE_X_TRAIT_CURRENCY,
                             "SELECT ID, `Index`, TraitTreeID, TraitCurrencyID FROM trait_tree_x_trait_currency" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TransmogIllusion.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_ILLUSION,
                             "SELECT ID, UnlockConditionID, TransmogCost, SpellItemEnchantmentID, Flags FROM transmog_illusion" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TransmogSet.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SET,
                             "SELECT Name, ID, ClassMask, TrackingQuestID, Flags, TransmogSetGroupID, ItemNameDescriptionID, " +
            ...
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SET_GROUP_LOCALE,
                             "SELECT ID, Name_lang FROM transmog_set_group_locale WHERE (`VerifiedBuild` > 0) = ?" +
                             " AND locale = ?");

            // TransmogSetItem.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SET_ITEM,
                             "SELECT ID, TransmogSetID, ItemModifiedAppearanceID, Flags FROM transmog_set_item" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TransportAnimation.db2
            PrepareStatement(HotfixStatements.SEL_TRANSPORT_ANIMATION,
                             "SELECT ID, PosX, PosY, PosZ, SequenceID, TimeIndex, TransportID FROM transport_animation" +
                             " WHERE (`VerifiedBuild` > 0) = ?");

            // TransportRotation.db2
            PrepareStatement(HotfixStatements.SEL_TRANSPORT_ROTATION,
                             "SELECT ID, Rot1, Rot2, Rot3, Rot4, TimeIndex, GameObjectsID FROM transport_rotation" +
                             " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // UiMapAssignment.db2
            PrepareStatement(HotfixStatements.SEL_UI_MAP_ASSIGNMENT,
            ...
            // UiMapLink.db2
            PrepareStatement(HotfixStatements.SEL_UI_MAP_LINK,
            ...
            // UiSplashScreen.db2
            PrepareStatement(HotfixStatements.SEL_UI_SPLASH_SCREEN,
                             "SELECT ID, Header, TopLeftFeatureTitle, TopLeftFeatureDesc, BottomLeftFeatureTitle, " +
            ...
            PrepareStatement(HotfixStatements.SEL_UI_SPLASH_SCREEN_LOCALE,
                             "SELECT ID, Header_lang, TopLeftFeatureTitle_lang, TopLeftFeatureDesc_lang, " +
            ...
            // UnitCondition.db2
            PrepareStatement(HotfixStatements.SEL_UNIT_CONDITION,
            ...
            // UnitPowerBar.db2
            PrepareStatement(HotfixStatements.SEL_UNIT_POWER_BAR,
            ...
            PrepareStatement(HotfixStatements.SEL_UNIT_POWER_BAR_LOCALE,
                             "SELECT ID, Name_lang, Cost_lang, OutOfError_lang, ToolTip_lang FROM unit_power_bar_locale" +
                             " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");

            // Vehicle.db2
            PrepareStatement(HotfixStatements.SEL_VEHICLE,
            ...
            // VehicleSeat.db2
            PrepareStatement(HotfixStatements.SEL_VEHICLE_SEAT,
            ...
            // WmoAreaTable.db2
            PrepareStatement(HotfixStatements.SEL_WMO_AREA_TABLE,
                             "SELECT AreaName, ID, WmoID, NameSetID, WmoGroupID, SoundProviderPref, SoundProviderPrefUnderwater, " +
            ...
            // WorldEffect.db2
            PrepareStatement(HotfixStatements.SEL_WORLD_EFFECT,
                             "SELECT ID, QuestFeedbackEffectID, WhenToDisplay, TargetType, TargetAsset, PlayerConditionID, " +
                             "CombatConditionID FROM world_effect WHERE (`VerifiedBuild` > 0) = ?");

            // WorldMapOverlay.db2
            PrepareStatement(HotfixStatements.SEL_WORLD_MAP_OVERLAY,
                             "SELECT ID, UiMapArtID, TextureWidth, TextureHeight, OffsetX, OffsetY, HitRectTop, HitRectBottom, " +
