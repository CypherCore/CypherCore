...
namespace Framework.Database
{
    public class HotfixDatabase : MySqlBase<HotfixStatements>
    {
        public override void PreparedStatements()
        {
            ...
            PrepareStatement(HotfixStatements.SEL_BATTLEMASTER_LIST, "SELECT ID, Name, GameType, ShortDescription, LongDescription, InstanceType, PvpType, MinLevel, " +
                "MaxLevel, RatedPlayers, MinPlayers, MaxPlayers, GroupsAllowed, MaxGroupSize, HolidayWorldState, Flags, IconFileDataID, " +
                "RequiredPlayerConditionID FROM battlemaster_list WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_CHARACTER_LOADOUT, "SELECT ID, ChrClassID, Purpose, ItemContext, RaceMask1, RaceMask2 FROM character_loadout" +
                " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_CHR_CUSTOMIZATION_REQ, "SELECT ID, ReqSource, Flags, ClassMask, RegionGroupMask, AchievementID, QuestID, " +
                "OverrideArchive, ItemModifiedAppearanceID, RaceMask1, RaceMask2 FROM chr_customization_req WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // CraftingQuality.db2
            PrepareStatement(HotfixStatements.SEL_CRAFTING_QUALITY, "SELECT ID, QualityTier, CraftingQualityAtlasSetID FROM crafting_quality" +
                " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_CURRENCY_TYPES, "SELECT ID, Name, Description, CategoryID, InventoryIconFileID, SpellWeight, SpellCategory, MaxQty, " +
                ...
                "MCRCurrencyID, Flags1, Flags2 FROM currency_types WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // Emotes.db2
            PrepareStatement(HotfixStatements.SEL_EMOTES, "SELECT ID, EmoteSlashCommand, AnimID, EmoteFlags, EmoteSpecProc, EmoteSpecProcParam, EventSoundID, " +
                "SpellVisualKitID, ClassMask, RaceMask1, RaceMask2 FROM emotes WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // Faction.db2
            PrepareStatement(HotfixStatements.SEL_FACTION, "SELECT ID, Name, Description, ReputationIndex, ParentFactionID, Expansion, FriendshipRepID, Flags, " +
                ...
                "ReputationRaceMask41, ReputationRaceMask42 FROM faction WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // Gameobjects.db2
            PrepareStatement(HotfixStatements.SEL_GAMEOBJECTS, "SELECT Name, PosX, PosY, PosZ, Rot1, Rot2, Rot3, Rot4, ID, OwnerID, DisplayID, Flags, Scale, TypeID, " +
            ...
            // GarrPlot.db2
            PrepareStatement(HotfixStatements.SEL_GARR_PLOT, "SELECT ID, Name, PlotType, HordeConstructObjID, AllianceConstructObjID, Flags, UpgradeRequirement1, " +
                "UpgradeRequirement2 FROM garr_plot WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM, "SELECT ID, ClassID, SubclassID, Material, InventoryType, SheatheType, SoundOverrideSubclassID, IconFileDataID, " +
                ...
                "RecraftReagentCountPercentage, OrderSource FROM item WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_ITEM_SCALING_CONFIG, "SELECT ID, ItemOffsetCurveID, ItemLevel, RequiredLevel, ItemSquishEraID, Flags" +
                " FROM item_scaling_config WHERE (`VerifiedBuild` > 0) = ?");

            // ItemSearchName.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SEARCH_NAME, "SELECT ID, Display, OverallQualityID, ExpansionID, MinFactionID, MinReputation, AllowableClass, " +
                ...
                "AllowableRace2 FROM item_search_name WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // ItemSparse.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SPARSE, "SELECT ID, Description, Display3, Display2, Display1, Display, ExpansionID, DmgVariance, LimitCategory, " +
            ...
            // ItemSquishEra.db2
            PrepareStatement(HotfixStatements.SEL_ITEM_SQUISH_ERA, "SELECT ID, Patch, CurveID, Flags FROM item_squish_era WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_LIQUID_TYPE, "SELECT ID, Name, Texture1, Texture2, Texture3, Texture4, Texture5, Texture6, Flags, SoundBank, SoundID, " +
                ...
                "`Int2`, `Int3`, `Int4`, Coefficient1, Coefficient2, Coefficient3, Coefficient4 FROM liquid_type WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // PlayerCondition.db2
            PrepareStatement(HotfixStatements.SEL_PLAYER_CONDITION, "SELECT ID, FailureDescription, MinLevel, MaxLevel, ClassMask, SkillLogic, LanguageID, MinLanguage, " +
            ...
            PrepareStatement(HotfixStatements.SEL_SKILL_LINE_ABILITY, "SELECT AbilityVerb, AbilityAllVerb, ID, SkillLine, Spell, MinSkillLineRank, ClassMask, " +
                ...
                "SkillupSkillLineID, RaceMask1, RaceMask2 FROM skill_line_ability WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SKILL_RACE_CLASS_INFO, "SELECT ID, SkillID, ClassMask, Flags, Availability, MinLevel, SkillTierID, RaceMask1, " +
                "RaceMask2 FROM skill_race_class_info WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // SpellCategories.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_CATEGORIES, "SELECT ID, DifficultyID, Category, DefenseType, DiminishType, DispelType, Mechanic, " +
                "PreventionType, StartRecoveryCategory, ChargeCategory, SpellID FROM spell_categories WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // SpellDuration.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_DURATION, "SELECT ID, Duration, MaxDuration, DurationPerResource FROM spell_duration" +
                " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_REAGENTS_CURRENCY, "SELECT ID, SpellID, CurrencyTypesID, CurrencyCount, OverrideRecraftCurrencyCount, " +
                "OrderSource FROM spell_reagents_currency WHERE (`VerifiedBuild` > 0) = ?");

            // SpellScaling.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_SCALING, "SELECT ID, SpellID, MinScalingLevel, MaxScalingLevel FROM spell_scaling" +
                " WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_OUTFIT_SLOT_INFO, "SELECT InventorySlotName, ID, TransmogOutfitSlotEnum, InventorySlotEnum, Flags, " +
                ...
                "IllusionCostMultiplier FROM transmog_outfit_slot_info WHERE (`VerifiedBuild` > 0) = ?");

            // TransmogOutfitSlotOption.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_OUTFIT_SLOT_OPTION, "SELECT ID, Name, OptionEnum, TransmogOutfitSlotInfoID, Flags, SecondaryOptionID, " +
                "ItemCostMultiplier, IllusionCostMultiplier FROM transmog_outfit_slot_option WHERE (`VerifiedBuild` > 0) = ?");
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_OUTFIT_SLOT_OPTION_LOCALE, "SELECT ID, Name_lang FROM transmog_outfit_slot_option_locale" +
                " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SITUATION, "SELECT Name, ID, SituationEnum, Flags, TransmogSituationGroupID, OrderIndex" +
                " FROM transmog_situation WHERE (`VerifiedBuild` > 0) = ?");
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SITUATION_LOCALE, "SELECT ID, Name_lang FROM transmog_situation_locale WHERE (`VerifiedBuild` > 0) = ?" +
                " AND locale = ?");

            // TransmogSituationGroup.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SITUATION_GROUP, "SELECT ID, TransmogSituationTriggerID, OrderIndex, Flags FROM transmog_situation_group" +
                " WHERE (`VerifiedBuild` > 0) = ?");

            // TransmogSituationTrigger.db2
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SITUATION_TRIGGER, "SELECT Name, Description, ID, TriggerEnum, Flags FROM transmog_situation_trigger" +
                " WHERE (`VerifiedBuild` > 0) = ?");
            PrepareStatement(HotfixStatements.SEL_TRANSMOG_SITUATION_TRIGGER_LOCALE, "SELECT ID, Name_lang, Description_lang FROM transmog_situation_trigger_locale" +
                " WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_UI_MAP, "SELECT Name, ID, ParentUiMapID, Flags, `System`, Type, BountySetID, BountyDisplayLocation, " +
                ...
                "MapArtZoneTextPosition FROM ui_map WHERE (`VerifiedBuild` > 0) = ?");
    ...
    public enum HotfixStatements
    {
        ...
        SEL_TRANSMOG_OUTFIT_ENTRY,
        SEL_TRANSMOG_OUTFIT_ENTRY_LOCALE,

        SEL_TRANSMOG_OUTFIT_SLOT_INFO,

        SEL_TRANSMOG_OUTFIT_SLOT_OPTION,
        SEL_TRANSMOG_OUTFIT_SLOT_OPTION_LOCALE,
        ...
        SEL_TRANSMOG_SITUATION,
        SEL_TRANSMOG_SITUATION_LOCALE,

        SEL_TRANSMOG_SITUATION_GROUP,

        SEL_TRANSMOG_SITUATION_TRIGGER,
        SEL_TRANSMOG_SITUATION_TRIGGER_LOCALE,
