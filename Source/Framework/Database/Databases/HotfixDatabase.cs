...
namespace Framework.Database
{
    public class HotfixDatabase : MySqlBase<HotfixStatements>
    {
        public override void PreparedStatements()
        {
            ...
            PrepareStatement(HotfixStatements.SEL_AREA_POI_LOCALE, "SELECT ID, Name_lang, Description_lang FROM area_poi_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_GARR_TALENT_TREE, "SELECT ID, Title, GarrTypeID, ClassID, MaxTiers, UiOrder, Flags, UiTextureKitID, " +
                ...
                " WHERE (`VerifiedBuild` > 0) = ?");
            PrepareStatement(HotfixStatements.SEL_GARR_TALENT_TREE_LOCALE, "SELECT ID, Title_lang FROM garr_talent_tree_locale WHERE (`VerifiedBuild` > 0) = ? AND locale = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_EMPOWER_STAGE, "SELECT ID, SpellEmpowerStage, OtherValue FROM spell_empower_stage  WHERE (`VerifiedBuild` > 0) = ?");
            ...
            PrepareStatement(HotfixStatements.SEL_SPELL_KEYBOUND_OVERRIDE, "SELECT `ID`, `Function`, `Type`, `Data`, `Flags` FROM spell_keybound_override WHERE (`VerifiedBuild` > 0) = ?");
            ...
            // SpellEmpowerStage.db2
            PrepareStatement(HotfixStatements.SEL_SPELL_REPLACEMENT, "SELECT ID, SpellID FROM spell_replacement  WHERE (`VerifiedBuild` > 0) = ?");
