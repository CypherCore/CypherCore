    ...
    public static partial class ZLib
    {
        ...
        public static int deflateInit2(z_stream strm, int level, int method, int windowBits, int memLevel, int strategy)
        {
            ...
            if (memLevel < 1 ||
                ...
                level > 9 ||
                strategy < 0 ||
                strategy > Z_FIXED) return Z_STREAM_ERROR;
        ...
        public static int deflateSetDictionary(z_stream strm, byte[] dictionary, uint dictLength)
        {
            ...
            if (strm == null ||
                strm.state == null ||
                dictionary == null) return Z_STREAM_ERROR;

            deflate_state s = strm.state as deflate_state;

            if (s == null ||
                s.wrap == 2 ||
                (s.wrap == 1 && s.status != INIT_STATE))
        ...
        {
            if (strm == null ||
                strm.state == null) return Z_STREAM_ERROR;
        ...
        public static int deflateParams(z_stream strm, int level, int strategy)
        {
            ...
            if ((strategy != s.strategy || func != configuration_table[level].func) &&
                strm.total_in != 0) // Flush the last buffer:
        ...
        public static uint deflateBound(z_stream strm, uint sourceLen)
        {
            ...
            if (strm == null ||
                strm.state == null) return complen + 6;
            ...
            switch (s.wrap)
            {
                ...
                default:
                    ...
                    break; // for compiler happiness
            }

            // if not default parameters, return conservative bound
            if (s.w_bits != 15 ||
                s.hash_bits != 8 + 7) return complen + wraplen;
        ...
        public static int deflate(z_stream strm, int flush)
        {
            if (strm == null ||
                strm.state == null ||
                flush > Z_BLOCK ||
                flush < 0) return Z_STREAM_ERROR;

            deflate_state s = (deflate_state)strm.state;

            if (strm.out_buf == null ||
                (strm.in_buf == null && strm.avail_in != 0) ||
            ...
            if (s.status == INIT_STATE)
            {
                if (s.wrap == 2)
                {
                    ...
                    {
                        s.pending_buf[s.pending++] = (byte)((s.gzhead.text != 0 ? 1 : 0) +
                                                            (s.gzhead.hcrc != 0 ? 2 : 0) +
            ...
            if (s.status == EXTRA_STATE)
            {
                if (s.gzhead.extra != null)
                {
                    ...
                    while (s.gzindex < (s.gzhead.extra_len & 0xffff))
                    {
                        ...
                        {
                            if (s.gzhead.hcrc != 0 &&
                                s.pending > beg) strm.adler = crc32(strm.adler, s.pending_buf, beg, s.pending - beg);
                ...
                (flush != Z_NO_FLUSH && s.status != FINISH_STATE))
            {
                ...
                if (bstate == block_state.finish_started ||
                    bstate == block_state.finish_done) s.status = FINISH_STATE;
        ...
        public static int deflateEnd(z_stream strm)
        {
            ...
            if (status != INIT_STATE &&
                ...
                status != HCRC_STATE &&
                status != BUSY_STATE &&
                status != FINISH_STATE) return Z_STREAM_ERROR;
        ...
        public static int deflateCopy(z_stream dest, z_stream source)
        {
            if (source == null ||
                dest == null ||
                source.state == null) return Z_STREAM_ERROR;
        ...
        private static uint longest_match(deflate_state s, uint cur_match)
        {
            ...
            do
            {
                ...
                if (match[match_ind + best_len] != scan_end ||
                    match[match_ind + best_len - 1] != scan_end1 ||
                    match[match_ind] != scan[scan_ind] ||
                    match[++match_ind] != scan[scan_ind + 1]) continue;
                ...
                do
                {
                } while (scan[++scan_ind] == match[++match_ind] &&
                         scan[++scan_ind] == match[++match_ind] &&
                         scan[++scan_ind] == match[++match_ind] &&
                         scan[++scan_ind] == match[++match_ind] &&
                         scan[++scan_ind] == match[++match_ind] &&
                         scan[++scan_ind] == match[++match_ind] &&
        ...
        private static uint longest_match_fast(deflate_state s, uint cur_match)
        {
            ...
            if (match[match_ind] != scan[scan_ind] ||
                match[match_ind + 1] != scan[scan_ind + 1]) return MIN_MATCH - 1;
        ...
        private static block_state deflate_stored(deflate_state s, int flush)
        {
            ...
            for (; ; )
            {
                ...
                if (s.lookahead <= 1)
                {
                    ...
                    if (s.lookahead == 0 &&
                        flush == Z_NO_FLUSH) return block_state.need_more;
                    ...
                    (uint)s.strstart >= max_start)
                {
                    ...
                    //was FLUSH_BLOCK(s, 0);
                    _tr_flush_block(s,
                                    s.block_start >= 0 ? s.window : null,
                                    s.block_start >= 0 ? s.block_start : 0,
                                    (uint)((int)s.strstart - s.block_start),
                                    0);
            ...
            for (; ; )
            {
                ...
                if (s.match_length >= MIN_MATCH)
                {
                    ...
                    if (s.match_length <= s.max_lazy_match &&
                        s.lookahead >= MIN_MATCH) // max_lazy_match was max_insert_length as #define
                    {
        ...
        private struct ct_data
        {
            ...
            public ushort Dad { get; set; } // father node in Huffman tree
        ...
        private class deflate_state //internal_state 
        {
            ...
            public deflate_state Clone()
            {
                ...
                ret.bl_count = new ushort[MAX_BITS + 1];
                bl_count.CopyTo(ret.bl_count, 0);
                ret.heap = new int[2 * L_CODES + 1];
                heap.CopyTo(ret.heap, 0);
                ret.depth = new byte[2 * L_CODES + 1];
                depth.CopyTo(ret.depth, 0);
