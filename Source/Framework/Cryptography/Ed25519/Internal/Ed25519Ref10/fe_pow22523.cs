namespace Framework.Cryptography.Ed25519.Internal.Ed25519Ref10
{
	internal static partial class FieldOperations
	{
		internal static void fe_pow22523(out FieldElement result, ref FieldElement z)
		{
			...
			fe_sq(out t1, ref t0);
			for (i = 1; i < 2; ++i) fe_sq(out t1, ref t1);
			...
			fe_sq(out t1, ref t0);
			for (i = 1; i < 5; ++i) fe_sq(out t1, ref t1);
			...
			fe_sq(out t1, ref t0);
			for (i = 1; i < 10; ++i) fe_sq(out t1, ref t1);
			...
			fe_sq(out t2, ref t1);
			for (i = 1; i < 20; ++i) fe_sq(out t2, ref t2);
			...
			fe_sq(out t1, ref t0);
			for (i = 1; i < 50; ++i) fe_sq(out t1, ref t1);
			...
			fe_sq(out t2, ref t1);
			for (i = 1; i < 100; ++i) fe_sq(out t2, ref t2);
			...
			fe_sq(out t0, ref t0);
			for (i = 1; i < 2; ++i) fe_sq(out t0, ref t0);
