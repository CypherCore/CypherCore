namespace Framework.Cryptography.Ed25519.Internal.Ed25519Ref10
{
	internal static partial class FieldOperations
	{
		internal static void fe_invert(out FieldElement result, ref FieldElement z)
		{
			...
			fe_sq(out t2, ref t1);
			for (i = 1; i < 5; ++i) fe_sq(out t2, ref t2);
			...
			fe_sq(out t2, ref t1);
			for (i = 1; i < 10; ++i) fe_sq(out t2, ref t2);
			...
			fe_sq(out t3, ref t2);
			for (i = 1; i < 20; ++i) fe_sq(out t3, ref t3);
			...
			fe_sq(out t2, ref t1);
			for (i = 1; i < 50; ++i) fe_sq(out t2, ref t2);
			...
			fe_sq(out t3, ref t2);
			for (i = 1; i < 100; ++i) fe_sq(out t3, ref t3);
