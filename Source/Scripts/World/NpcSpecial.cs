...
namespace Scripts.World.NpcSpecial
{
	...
	internal class npc_air_force_bots : NullCreatureAI
	{
		...
		private AirForceSpawn _spawn;
		private List<ObjectGuid> _toAttack = new();
	...
	{
		private List<Position> Coordinates = new();

		private bool Event;
		private uint PatientDiedCount;

		private List<ObjectGuid> Patients = new();
	...
	{
		private List<ObjectGuid> _revelerGuids = new();

		private Emote[] BrewfestRandomEmote =
		{
	...
	{
		private Dictionary<ObjectGuid, TimeSpan> _combatTimer = new();
	...
	{
		private Dictionary<uint, uint> ChairSpells = new()
		                                             {
			                                             {
	...
	internal class npc_gen_void_zone : ScriptedAI
	{
		...
		{
			_scheduler.Schedule(TimeSpan.FromSeconds(2), task => { DoCastSelf(SpellIds.Consumption); });
		}
