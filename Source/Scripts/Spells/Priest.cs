...
struct SpellIds
{
    ...
    public const uint AssuredSafety = 440766;
    ...
    public const uint Holy10_1_ClassSet2PChooser = 411097;
    public const uint Holy10_1_ClassSet4P = 405556;
    public const uint Holy10_1_ClassSet4PEffect = 409479;
    ...
    public const uint ShadowCovenant = 314867;
    public const uint ShadowCovenantEffect = 322105;
...
struct CreatureIds
{
    public const uint DivineImage = 198236;
    public const uint Mindbender = 62982;
    public const uint Shadowfiend = 19668;
}

class RaidCheck(Unit caster)
{
...
class areatrigger_pri_angelic_feather(AreaTrigger areaTrigger) : AreaTriggerAI(areaTrigger)
{
...
class spell_pri_answered_prayers : AuraScript
{
    ...
    void HandleOnProc(AuraEffect aurEff, ProcEventInfo eventInfo)
    {
        ...
        else
        {
            ...
            if (apotheosis != null)
            {
                apotheosis.SetDuration(apotheosis.GetDuration() + (int)extraDuration.TotalSeconds);
                apotheosis.SetMaxDuration(apotheosis.GetMaxDuration() + (int)extraDuration.TotalSeconds);
            }
            else
                target.CastSpell(target, SpellIds.Apotheosis,
                    ...
                    .AddSpellMod(SpellValueMod.Duration, (int)extraDuration.TotalSeconds));
        }
    }
...
class spell_pri_atonement : AuraScript
{
    ...
    public void TriggerAtonementHealOnTargets(AuraEffect atonementEffect, ProcEventInfo eventInfo)
    {
        ...
        {
            Unit target = Global.ObjAccessor.GetUnit(priest, targetGuid);
            if (target != null)
            {
    ...
    void HandleOnHitTarget()
    {
        ...
        switch (GetSpellInfo().Id)
        {
            case SpellIds.PowerWordShield:
                ...
                if (indemnity != null)
                    args.AddSpellMod(SpellValueMod.Duration,
                        (int)(TimeSpan.FromSeconds(indemnity.GetAmount())
                            + TimeSpan.FromSeconds(Aura.CalcMaxDuration(Global.SpellMgr.GetSpellInfo(_effectSpellId, GetCastDifficulty()),
                                caster, GetSpell().GetPowerCost()))).TotalSeconds);
                break;
...
class spell_pri_atonement_effect_aura : AuraScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
...
class spell_pri_crystalline_reflection : AuraScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.CrystallineReflection, SpellIds.CrystallineReflectionHeal, SpellIds.CrystallineReflectionReflect) && ValidateSpellEffect((SpellIds.CrystallineReflection, 0));
    }
...
struct DivineImageHelpers
{
    public static Unit GetSummon(Unit owner)
    {
        foreach (Unit summon in owner.m_Controlled)
            if (summon.GetEntry() == CreatureIds.DivineImage)
                return summon;
    ...
    public static void Trigger(AuraEffect aurEff, ProcEventInfo eventInfo)
    {
        ...
        uint? spellId = GetSpellToCast(eventInfo.GetSpellInfo().Id);
        if (!spellId.HasValue)
            return;

        divineImage.CastSpell(new CastSpellTargetArg(eventInfo.GetProcSpell().m_targets), spellId.Value, aurEff);
    }
}
...
class spell_pri_divine_image_spell_triggered : AuraScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        ...
        (
            SpellIds.Renew,
            SpellIds.PowerWordShield,
            SpellIds.PowerWordLife,
            SpellIds.FlashHeal,
            SpellIds.HolyWordSerenity,
            SpellIds.PrayerOfMending,
            SpellIds.PrayerOfMendingHeal,
            SpellIds.PrayerOfHealing,
            SpellIds.CircleOfHealing,
            SpellIds.HaloHoly,
            SpellIds.DivineStarHolyHeal,
            SpellIds.DivineHymnHeal,
            SpellIds.HolyWordSanctify,
            SpellIds.HolyWordSalvation,
            SpellIds.Smite,
            SpellIds.HolyFire,
            SpellIds.ShadowWordDeath,
            SpellIds.ShadowWordPain,
            SpellIds.Mindgames,
            SpellIds.MindgamesVenthyr,
            SpellIds.HolyWordChastise,
            SpellIds.HolyNova,
            SpellIds.TranquilLight,
            SpellIds.HealingLight,
            SpellIds.BlessedLight,
            SpellIds.DazzlingLight,
            SpellIds.SearingLight,
            SpellIds.LightEruption,
...
class spell_pri_divine_image_stack_timer : AuraScript
{
    void TrackStackApplicationTime(AuraEffect aurEff, AuraEffectHandleModes mode)
    {
        ...
        GetUnitOwner().m_Events.AddEventAtOffset(() => owner.RemoveAuraFromStack(spelId), TimeSpan.FromSeconds(GetMaxDuration()));
    }
...
[Script] // 122121 - Divine Star (Shadow)
class areatrigger_pri_divine_star(AreaTrigger areaTrigger) : AreaTriggerAI(areaTrigger)
{
    ...
    void ReturnToCaster()
    {
        _scheduler.Schedule(TimeSpan.FromSeconds(0), task =>
        {
            ...
            at.InitSplines([at, caster]);

            task.Repeat(TimeSpan.FromSeconds(250));
        });
    }
}
...
class spell_pri_focused_mending : SpellScript
{
    ...
    void CalculateHealingBonus(SpellEffectInfo spellEffectInfo, Unit victim, ref int healing, ref int flatMod, ref float pctMod)
    {
        ...
        {
            if (GetSpell().m_customArg is bool isEmpoweredByFocusedMending && isEmpoweredByFocusedMending)
...
// 120517 - Halo (Holy)
[Script] // 120644 - Halo (Shadow)
class areatrigger_pri_halo(AreaTrigger areaTrigger) : AreaTriggerAI(areaTrigger)
{
    public override void OnUnitEnter(Unit unit)
    {
        Unit caster = at.GetCaster();
...
class spell_pri_holy_word_salvation : SpellScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.PrayerOfMendingAura, SpellIds.Renew) && ValidateSpellEffect((SpellIds.PrayerOfMendingHeal, 0), (spellInfo.Id, 1))
    ...
    SpellEffectInfo _healEffectDummy = null;
}
...
class spell_pri_lights_wrath : SpellScript
{
    ...
    public override void OnPrecast()
    {
        ...
        {
            Unit target = Global.ObjAccessor.GetUnit(GetCaster(), atonementTarget);
            if (target != null)
            {
...
class spell_pri_mind_devourer_buff_aura : AuraScript
{
    ...
    public override void Register()
    {
        DoEffectCalcDamageAndHealing.Add(new(CalculateDamage, 1, AuraType.PeriodicLeech));
    }
}
...
class spell_pri_mind_devourer_buff : SpellScript
{
    ...
    public override void OnPrecast()
    {
        ...
        _damageIncrease = (int)mindDevourer.GetAmount();
    }
...
[Script("spell_pri_penance", SpellIds.PenanceChannelDamage, SpellIds.PenanceChannelHealing)]
[Script("spell_pri_dark_reprimand", SpellIds.DarkReprimandChannelDamage, SpellIds.DarkReprimandChannelHealing)]
class spell_pri_penance(uint damageSpellId, uint healingSpellId) : SpellScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(damageSpellId, healingSpellId);
    }
    ...
    void HandleDummy(uint effIndex)
    {
        ...
        {
            if (caster.IsFriendlyTo(target))
                caster.CastSpell(target, healingSpellId, new CastSpellExtraArgs(TriggerCastFlags.IgnoreGCD | TriggerCastFlags.IgnoreSpellAndCategoryCD)
                    .SetTriggeringSpell(GetSpell()));
            else
                caster.CastSpell(target, damageSpellId, new CastSpellExtraArgs(TriggerCastFlags.IgnoreGCD | TriggerCastFlags.IgnoreSpellAndCategoryCD)
                    .SetTriggeringSpell(GetSpell()));
        }
    }
...
class spell_pri_power_leech_passive : AuraScript
{
    ...
    void HandleOnProc(AuraEffect aurEff, ProcEventInfo eventInfo)
    {
        ...
        {
            if (target.GetEntry() == CreatureIds.Shadowfiend)
            {
                // Note: divisor is 100 because effect value is 5 and its supposed to restore 0.5%
                spellInfo = Global.SpellMgr.GetSpellInfo(SpellIds.PowerLeechShadowfiendMana, GetCastDifficulty());
                divisor = 10;
            }
            else
            {
                // Note: divisor is 100 because effect value is 20 and its supposed to restore 0.2%
                spellInfo = Global.SpellMgr.GetSpellInfo(SpellIds.PowerLeechMindbenderMana, GetCastDifficulty());
                divisor = 100;
            }
        }
        else
            spellInfo = Global.SpellMgr.GetSpellInfo(target.GetEntry() == CreatureIds.Shadowfiend
        ...
        if (summoner.HasAura(SpellIds.EssenceDevourer))
            summoner.CastSpell(null, target.GetEntry() == CreatureIds.Shadowfiend ? SpellIds.EssenceDevourerShadowfiendHeal : SpellIds.EssenceDevourerMindbenderHeal, aurEff);
    }
...
class spell_pri_power_word_radiance : SpellScript
{
    ...
    void HandleEffectHitTarget(uint effIndex)
    {
        ...
        {
            Unit target = Global.ObjAccessor.GetUnit(GetHitUnit(), guid);
            if (target != null)
...
class spell_pri_prayer_of_mending_dummy : SpellScript
{
    ...
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.PrayerOfMendingHeal, SpellIds.PrayerOfMendingAura, SpellIds.Epiphany, SpellIds.EpiphanyHighlight) && ValidateSpellEffect((SpellIds.PrayerOfMendingHeal, 0));
    }
...
class spell_pri_prayer_of_mending_aura : AuraScript
{
...
class spell_pri_prayer_of_mending : SpellScript
{
    void HandleEffectDummy(uint effIndex)
    {
        ...
        if (GetSpell().m_customArg is bool isEmpoweredByFocusedMending)
...
class spell_pri_holy_10_1_class_set_2pc : AuraScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.Holy10_1_ClassSet2PChooser)
...
class spell_pri_holy_10_1_class_set_4pc : SpellScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.Holy10_1_ClassSet4P, SpellIds.Holy10_1_ClassSet4PEffect);
    }

    void HandleEffectDummy(uint effIndex)
    {
        if (GetOriginalCaster().HasAura(SpellIds.Holy10_1_ClassSet4P))
            GetOriginalCaster().CastSpell(GetOriginalCaster(), SpellIds.Holy10_1_ClassSet4PEffect, TriggerCastFlags.IgnoreGCD);
    }
...
[Script] // 41635 - Prayer of Mending (Aura)
class spell_pri_holy_10_1_class_set_4pc_aura : AuraScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.Holy10_1_ClassSet4P, SpellIds.Holy10_1_ClassSet4PEffect);
    ...
    void HandleOnRemove(AuraEffect aurEff, AuraEffectHandleModes mode)
    {
        ...
        if (GetCaster().HasAura(SpellIds.Holy10_1_ClassSet4P))
            GetCaster().CastSpell(GetCaster(), SpellIds.Holy10_1_ClassSet4PEffect, new CastSpellExtraArgs(TriggerCastFlags.IgnoreGCD).SetTriggeringAura(aurEff));
    }
...
[Script] // 17 - Power Word: Shield
class spell_pri_assured_safety : SpellScript
{
    SpellInfo _spellInfoHeal = null;

    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.PrayerOfMendingHeal, SpellIds.PrayerOfMendingAura) && ValidateSpellEffect((SpellIds.PrayerOfMendingHeal, 0), (SpellIds.AssuredSafety, 0));
    }

    public override bool Load()
    {
        _spellInfoHeal = Global.SpellMgr.GetSpellInfo(SpellIds.PrayerOfMendingHeal, Difficulty.None);
        return GetCaster().HasAura(SpellIds.AssuredSafety);
    }

    void HandleEffectHitTarget(uint effIndex)
    {
        Unit caster = GetCaster();
        AuraEffect effect = caster.GetAuraEffect(SpellIds.AssuredSafety, 0);
        if (effect != null)
        {
            CastSpellExtraArgs args = new();
            args.TriggerFlags = TriggerCastFlags.FullMask;
            args.AddSpellMod(SpellValueMod.AuraStack, (byte)effect.GetAmount());
            ...
            caster.CastSpell(GetHitUnit(), SpellIds.PrayerOfMendingAura, args);

            // Note: the visualSender is the priest if it is first cast or the aura holder when the aura triggers.
            caster.SendPlaySpellVisual(GetHitUnit(), SpellIds.VisualPriestPrayerOfMending, 0, 0, 40.0f);
        }
    }
...
class spell_pri_purge_the_wicked_dummy : SpellScript
{
    ...
    void FilterTargets(List<WorldObject> targets)
    {
        ...
        // Note: there's no SpellEffectName.Dummy with BasePoints 1 in any of the spells related to use as reference so we hardcode the value.
        uint spreadCount = 1;

        // Note: we must sort our list of targets whose priority is 1) aura, 2) distance, and 3) duration.
        targets.Sort((WorldObject lhs, WorldObject rhs) =>
        {
...
[Script] // 451235 - Voidwrath
class spell_pri_shadow_covenant : SpellScript
{
    public override bool Validate(SpellInfo spellInfo)
    {
        return ValidateSpellInfo(SpellIds.ShadowCovenant, SpellIds.ShadowCovenantEffect);
    }

    public override bool Load()
    {
        return GetCaster().HasAura(SpellIds.ShadowCovenant);
    }

    void HandleAfterCast()
    {
        GetCaster().CastSpell(GetCaster(), SpellIds.ShadowCovenantEffect, new CastSpellExtraArgs()
        {
...
[Script] // 32379 - Shadow Word: Death
class spell_pri_shadow_word_death : SpellScript
{
    static TimeSpan BacklashDelay = TimeSpan.FromSeconds(1);
...
class spell_pri_trail_of_light : AuraScript
{
    ...
    void HandleOnProc(AuraEffect aurEff, ProcEventInfo eventInfo)
    {
        ...
        Unit oldTarget = Global.ObjAccessor.GetUnit(caster, _healQueue.Peek());
        if (oldTarget == null)
            return;
        ...
        SpellInfo healSpellInfo = Global.SpellMgr.GetSpellInfo(SpellIds.TrailOfLightHeal, Difficulty.None);
...
class spell_pri_unfurling_darkness : SpellScript
{
    ...
    public override void Register()
    {
        OnObjectTargetSelect.Add(new(PreventDirectDamage, 3, Targets.UnitTargetEnemy));
        AfterCast.Add(new(TriggerUnfurlingDarkness));
    }
}
...
class spell_pri_vampiric_touch : AuraScript
{
    ...
    void HandleApplyEffect(AuraEffect aurEff, AuraEffectHandleModes mode)
    {
        Unit caster = GetCaster();
        if (caster != null)
            if (caster.HasAura(SpellIds.Misery))
