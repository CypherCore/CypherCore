...
public class spell_dk_scourge_strike : SpellScript, IHasSpellEffects
{
	...
	private void HandleOnHit(uint UnnamedParameter)
	{
		...
		if (target != null)
		{
			Aura festeringWoundAura = target.GetAura(DeathKnightSpells.SPELL_DK_FESTERING_WOUND, GetCaster().GetGUID());
			if (festeringWoundAura != null)
			{
				...
				Aura pestilentPustulesAura = caster.GetAura(DeathKnightSpells.SPELL_DK_PESTILENT_PUSTULES);
				...
				Aura castiragorAura      = caster.GetAura(DeathKnightSpells.SPELL_DK_CASTIGATOR);
