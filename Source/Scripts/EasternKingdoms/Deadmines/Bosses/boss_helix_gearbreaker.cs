...
namespace Scripts.EasternKingdoms.Deadmines.Bosses
{
    ...
    public class boss_helix_gearbreaker : BossAI
    {
        ...
        {
            // Helix
            public const uint SPELL_OAFQUARD = 90546;
            public const uint SPELL_HELIX_RIDE = 88337;
            public const uint SPELL_THROW_BOMB = 88264;

            // Oaf spell
            public const uint SPELL_OAF_GRAB_TARGETING = 88289;
            public const uint SPELL_RIDE_OAF = 88278; // 88277;
            public const uint SPELL_RIDE_VEHICLE_HARDCODED = 46598;
            public const uint OAF_CHARGE = 88288;
            public const uint SPELL_OAF_SMASH = 88300;
            public const uint SPELL_OAF_SMASH_H = 91568;

            // BOMB
            public const uint SPELL_STICKY_BOMB_EXPLODE = 95500; //88329; // 95500 -> 88321; 88974
            public const uint SPELL_STICKY_BOMB_EXPLODE_H = 91566;
            public const uint SPELL_ARMING_VISUAL_YELLOW = 88315;
            public const uint SPELL_ARMING_VISUAL_ORANGE = 88316;
            public const uint SPELL_ARMING_VISUAL_RED = 88317;
            public const uint SPELL_BOMB_ARMED_STATE = 88319;
            public const uint SPELL_CHEST_BOMB = 88352; // Unused
        }
        ...
        public void OafSupport()
        {
            ...
            if (_oaf == null)
            {
                ...
                {
                    me.CastSpell(_oaf, eSpels.SPELL_RIDE_VEHICLE_HARDCODED);
                }
                else
                {
            ...
            while ((eventId = _events.ExecuteEvent()) != 0)
            {
                switch (eventId)
                {
                    case HelOaf_Events.EVENT_THROW_BOMB:
                        ...
                        if (target != null)
                        {
                            me.CastSpell(target, eSpels.SPELL_THROW_BOMB, new CastSpellExtraArgs(TriggerCastFlags.IgnoreCasterMountedOrOnVehicle | TriggerCastFlags.IgnoreCasterAurastate));
                        }
                        _events.ScheduleEvent(HelOaf_Events.EVENT_THROW_BOMB, TimeSpan.FromMilliseconds(3000));
                        break;
                    case HelOaf_Events.EVENT_CHEST_BOMB:
                        ...
                        if (target1 != null)
                        {
                            ...
                            me.AddAura(eSpels.SPELL_CHEST_BOMB, target1);
                        }
                    ...
                    case HelOaf_Events.EVENT_NO_OAF:
                        ...
                        me.RemoveAura(eSpels.SPELL_OAFQUARD);
                        Talk(2);
                    ...
                    case HelOaf_Events.EVENT_ACHIEVEVEMENT_BUFF:
                        ...
                        {
                            me.CastSpell(item, eSpels.SPELL_HELIX_RIDE, true);
                        }
