...
namespace Scripts.EasternKingdoms.MagistersTerrace.PriestessDelrissa
{
	...
	internal class boss_priestess_delrissa : BossAI
	{
		...
		private List<uint> LackeyEntryList = new();
		...
		private void Initialize()
		{
			...
			_scheduler.Schedule(TimeSpan.FromSeconds(15),
			                    task =>
			                    {
				                    ...
				                    {
					                    Unit pAdd = Global.ObjAccessor.GetUnit(me, _auiLackeyGUID[i]);
			...
			_scheduler.Schedule(TimeSpan.FromSeconds(10),
			                    task =>
			                    {
				                    ...
				                    {
					                    Unit pAdd = Global.ObjAccessor.GetUnit(me, _auiLackeyGUID[RandomHelper.Rand32() % _auiLackeyGUID.Length]);
		...
		private void InitializeLackeys()
		{
			...
			else
			{
				foreach (var guid in LackeyEntryList)
				{
					Unit pAdd = Global.ObjAccessor.GetUnit(me, _auiLackeyGUID[j]);
	...
	{
		private TaskScheduler _meleeScheduler = new();
